---
title: 'C++Kit de développement logiciel (SDK) Build Insights : table d’événements'
description: Référence d’événement pour le kit C++ de développement logiciel (SDK) Visual Studio Build Insights
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
ms.locfileid: "78332956"
---
# <a name="c-build-insights-sdk-event-table"></a>C++Kit de développement logiciel (SDK) Build Insights : table d’événements
...
|  | Description | Se produit au démarrage et à l’arrêt de l’étape principale du compilateur. Ce test est responsable de l’optimisation du code C/C++ source analysé et de sa conversion en code machine. |
...
|  | Description | Se produit au début et à l’arrêt d’un appel de *C1. dll* ou *c1xx. dll* . Ces dll sont le C et C++ le front-end du compilateur. Ils sont appelés uniquement par le pilote du compilateur (*CL. exe*). |
...
|  | Description | Se produit au début et à l’arrêt d’un appel *C2. dll* . Cette DLL est le back end du compilateur. Elle est appelée par le pilote du compilateur (*CL. exe*). Elle est également appelée par l’éditeur de liens (*Link. exe*) lors de l’utilisation de la génération de code durant l’édition de liens. |
...
|  | Propriétés | -Version du compilateur<br/>-Répertoire de travail<br/>-Chemin d’accès absolu au fichier *CL. exe* appelé |
...
|  | Description | Se produit au démarrage et à l’arrêt d’un appel de *CL. exe* . |
...
|  | Description | Se produit une fois pour chaque variable d’environnement existante au moment de l’appel de *CL. exe* ou de *Link. exe* . |
...
|  | Description | Se produit lorsqu’une fonction est forcée d’être inline dans une autre fonction par le biais de l’utilisation du mot clé `__forceinline`. |
...
|  | Description | Se produit au début et à la fin de l’étape frontale du compilateur. Ce test est responsable de l’analyse du codeC++ C/source et de sa conversion en langage intermédiaire. |
...
|  | Classes de capture | [Activité](cpp-event-data-types/activity.md)<br/>[Invocation](cpp-event-data-types/invocation.md)<br/>[Linker](cpp-event-data-types/linker.md) |
|  | Description | Se produit au démarrage et à l’arrêt d’un appel *Link. exe* . |
...
|  | Description | Se produit au début et à la fin d’une instanciation de modèle. Un type de modèle principal (par exemple, `vector`) est instancié, ce qui donne un type spécialisé (tel que `std::vector<int>`). Une clé est donnée pour les deux types. Utilisez l’événement [SYMBOL_NAME](#symbol-name) pour convertir une clé en nom de type. Les clés de type sont uniques au sein de la trace en cours d’analyse. Toutefois, différentes clés provenant de différentes passes front-end du compilateur peuvent pointer vers le même type. La comparaison des types entre les différentes passes frontales du compilateur requiert l’utilisation de noms de symboles. Cet événement est récursif. La récursivité se produit dans certains cas, lorsque le serveur frontal instancie des modèles imbriqués. |
...
|  | Children | [FUNCTION](#function)<br/>[THREAD](#thread) |
|  | Propriétés | None |
|  | Classes de capture | [Activité](cpp-event-data-types/activity.md)<br/>[Vers le](cpp-event-data-types/top-down.md) |
