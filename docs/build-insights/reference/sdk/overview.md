---
title: C++Kit de développement logiciel (SDK) Build Insights
description: Vue d’ensemble du kit de C++ développement logiciel (SDK) Visual Studio Build Insights.
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
ms.locfileid: "78332396"
---
# <a name="c-build-insights-sdk"></a>C++Kit de développement logiciel (SDK) Build Insights
...
Le C++ Kit de développement logiciel (SDK) Build Insights est un ensemble d’API qui vous permettent de créer des outils C++ personnalisés en plus de la plateforme Build Insights. Cette page fournit une vue d’ensemble de haut niveau pour vous aider à commencer.

## <a name="obtaining-the-sdk"></a>Obtention du kit de développement logiciel

Vous pouvez télécharger le C++ Kit de développement logiciel (SDK) Build Insights sous la forme d’un package NuGet en procédant comme suit :

1. Dans Visual Studio 2017 et versions ultérieures, créez C++ un nouveau projet.
...
Lisez la suite pour plus d’informations sur les concepts généraux entourant le kit de développement logiciel (SDK). Vous pouvez également accéder au [ C++ référentiel GitHub officiel Build Insights Samples](https://github.com/microsoft/cpp-build-insights-samples) pour voir des exemples d’applications réelles C++ qui utilisent le kit de développement logiciel (SDK).

## <a name="collecting-a-trace"></a>Collecte d’une trace

L’utilisation C++ du kit de développement logiciel (SDK) Build Insights pour analyser les événements en provenance du chaîne d’outils MSVC nécessite la première collecte d’une trace. Le kit de développement logiciel (SDK) utilise Suivi d’v nements pour Windows (ETW) comme technologie de suivi sous-jacente. La collecte d’une trace peut être effectuée de deux manières :
...
1. Exécutez la commande suivante : `vcperf /stopnoanalyze MySessionName outputTraceFile.etl`
   ...
   > [!IMPORTANT]
   > Utilisez la commande `/stopnoanalyze` lors de l’arrêt de votre trace avec vcperf. Vous ne pouvez pas C++ utiliser le kit de développement logiciel (SDK) Build Insights pour analyser les traces arrêtées par la commande `/stop` normale.

### <a name="method-2-programmatically"></a>Méthode 2 : par programmation

Utilisez l’une de C++ ces fonctions de collection de trace du SDK de build Insights pour démarrer et arrêter des traces par programmation. **Le programme qui exécute ces appels de fonction doit disposer de privilèges d’administrateur.** Seules les fonctions de suivi de démarrage et d’arrêt requièrent des privilèges d’administrateur. Toutes les autres fonctions du C++ Kit de développement logiciel (SDK) Build Insights peuvent être exécutées sans elles.
...
Une fois que vous avez une trace ETW, C++ utilisez le kit de développement logiciel (SDK) Build Insights pour la décompresser. Le kit de développement logiciel (SDK) vous donne les événements dans un format qui vous permet de développer rapidement vos outils. Nous vous déconseillons d’utiliser le suivi ETW brut sans utiliser le kit de développement logiciel (SDK). Le format d’événement utilisé par MSVC est non documenté, optimisé pour s’adapter à des builds énormes et difficile à comprendre. En outre, l' C++ API SDK de build Insights est stable, tandis que le format de trace ETW brut peut faire l’objet de modifications sans préavis.
...
|--|--|--|--|
| Configuration des rappels d’événements | [IAnalyzer](other-types/ianalyzer-class.md)<br />[IRelogger](other-types/irelogger-class.md) | [ANALYSIS_CALLBACKS](other-types/analysis-callbacks-struct.md)<br />[RELOG_CALLBACKS](other-types/relog-callbacks-struct.md) | Le C++ Kit de développement logiciel (SDK) Build Insights fournit des événements via des fonctions de rappel. Dans C++, implémentez les fonctions de rappel en créant un analyseur ou une classe de rejournalisation qui hérite de l’interface IAnalyzer ou IRelogger. En C, implémentez les rappels dans les fonctions globales et fournissez-leur des pointeurs dans la structure ANALYSIS_CALLBACKS ou RELOG_CALLBACKS. |
| Génération de groupes | [MakeStaticAnalyzerGroup](functions/make-static-analyzer-group.md)<br />[MakeStaticReloggerGroup](functions/make-static-relogger-group.md)<br />[MakeDynamicAnalyzerGroup](functions/make-dynamic-analyzer-group.md)<br />[MakeDynamicReloggerGroup](functions/make-dynamic-relogger-group.md) |  | L' C++ API fournit des fonctions d’assistance et des types pour regrouper plusieurs analyseurs et objets de journalisation. Les groupes sont un moyen pratique de diviser une analyse complexe en étapes plus simples. [vcperf](https://github.com/microsoft/vcperf) est organisé de cette manière. |
| Analyse ou rejournalisation | [Analyser](functions/analyze.md)<br />[Rejournaliser](functions/relog.md) | [Analyse](functions/analyze-a.md)<br />[AnalyzeW](functions/analyze-w.md)<br />[RelogA](functions/relog-a.md)<br />[RelogW](functions/relog-w.md) |  |
...
La reconnexion est réservée à des analyses à usage spécifique qui doivent produire un fichier de sortie ETW. À l’aide de la journalisation, C++ vous pouvez traduire les événements build Insights dans votre propre format d’événement ETW. Une utilisation appropriée de la journalisation consisterait à C++ raccorder des données de build Insights à vos outils et infrastructure ETW existants. Par exemple, [vcperf](https://github.com/microsoft/vcperf) utilise les interfaces de rejournalisation. C’est parce qu’il doit produire des données que l’analyseur de performances Windows, un outil ETW, peut comprendre. Une connaissance préalable du fonctionnement d’ETW est nécessaire si vous envisagez d’utiliser les interfaces de rejournalisation.
...
### <a name="consuming-events-provided-by-the-c-build-insights-sdk"></a>Utilisation des événements fournis par le C++ Kit de développement logiciel (SDK) Build Insights

#### <a name="the-event-stack"></a>Pile des événements

Chaque fois C++ que le kit de développement logiciel (SDK) Build Insights vous donne un événement, il est fourni sous la forme d’une pile. La dernière entrée de la pile est l’événement actuel, et les entrées avant sa hiérarchie parente. Par exemple, les événements de démarrage et d’arrêt de [LTCG](event-table.md#ltcg) se produisent pendant la passe 1 de l’éditeur de liens. Dans ce cas, la pile que vous souhaitez recevoir contient : \[[éditeur de liens](event-table.md#linker), [PASS1](event-table.md#pass1), LTCG\]. La hiérarchie parente est pratique, car vous pouvez remonter un événement à sa racine. Si l’activité LTCG mentionnée ci-dessus est lente, vous pouvez découvrir immédiatement l’appel de l’éditeur de liens qui a été impliqué.

#### <a name="matching-events-and-event-stacks"></a>Événements et piles d’événements correspondants

Le C++ Kit de développement logiciel (SDK) Build Insights vous donne tous les événements d’une trace, mais la plupart du temps, vous ne vous souciez que d’un sous-ensemble d’entre eux. Dans certains cas, vous ne vous souciez que d’un sous-ensemble de *piles d’événements*. Le kit de développement logiciel (SDK) fournit des fonctionnalités qui vous permettent d’extraire rapidement les événements ou la pile d’événements dont vous avez besoin, et de rejeter ceux dont vous n’avez pas besoin. Pour ce faire, vous disposez des fonctions de correspondance suivantes :
...
Les fonctions de mise en correspondance de la pile des événements comme `MatchEventStack` autorisent les intervalles lors de la description de la hiérarchie parente à faire correspondre. Par exemple, vous pouvez indiquer que vous êtes intéressé par l' [éditeur de liens](event-table.md#linker)\[, la pile de\] [LTCG](event-table.md#ltcg) . Cela correspond également à l’éditeur de liens \[, [PASS1](event-table.md#pass1), à la pile de\] LTCG. Le dernier type spécifié doit être le type d’événement à mettre en correspondance et ne fait pas partie de la hiérarchie parente.

#### <a name="capture-classes"></a>Classes de capture

L’utilisation des fonctions `Match*` nécessite que vous spécifiiez les types que vous souhaitez faire correspondre. Ces types sont sélectionnés dans une liste de *classes de capture*. Les classes de capture sont disponibles dans plusieurs catégories, décrites ci-dessous.
...
Une fois qu’une correspondance s’est terminée avec succès, les `Match*` fonctions construisent les objets de classe de capture et les transfèrent à la fonction spécifiée. Utilisez ces objets de classe de capture pour accéder aux propriétés des événements.
