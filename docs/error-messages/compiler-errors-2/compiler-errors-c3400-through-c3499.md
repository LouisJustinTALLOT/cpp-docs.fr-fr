...
|C3401 d’erreur du compilateur|'*spécificateur*' : spécificateur d’accès d’assembly non valide - seuls 'private' n’est autorisée sur les modèles de classe|
|C3402 d’erreur du compilateur|'*fonction*' : Impossible de résoudre la surcharge sauf dans la portée actuelle|
...
|C3411 d’erreur du compilateur|'*type*' n’est pas valide en tant que la taille d’un tableau n’est pas un type entier|
...
|[Erreur du compilateur C3417](compiler-error-c3417.md)|'*déclarateur*' : les types valeur ne peut pas contenir les fonctions membres spéciales définies par l’utilisateur|
...
|[Erreur du compilateur C3421](compiler-error-c3421.md)|'*fonction*' : vous ne peut pas appeler le finaliseur de cette classe, car il est soit inaccessible ou il n’existe pas|
|C3422 d’erreur du compilateur|'*déclaration*' : incompatibilité de types'*type*'et'*type*'|
|C3423 d’erreur du compilateur|Obsolète.|
|C3424 d’erreur du compilateur|'*type*' : un cast de style fonction vers un type tableau n’est pas autorisé.|
...
|C3427 d’erreur du compilateur|'*contexte*' : '*mot clé*' ne peut pas être utilisé avec layout_version (*numéro*)|
