...
|[Avertissement du compilateur (niveau 1) C4829](compiler-warning-level-1-c4829.md)|Paramètres potentiellement incorrects de la fonction main. Envisagez’int main (Platform :: Array\<plateforme :: String ^ > ^ argv) '|
...
| Avertissement du compilateur (niveau 4) C4845 | '\_\_declspec (no\_init\_All) 'est ignoré si'/d1initall\[0\|1\|2\|3] 'n’a pas été spécifié sur la ligne de commande |
...
| Avertissement du compilateur (niveau 4) C4847 | '\_\_declspec (no\_init\_All) 'ne peut s’appliquer qu’à une fonction, à un type de classe ou à une variable locale : ignoré |
| Avertissement du compilateur (niveau 1) C4848 | la prise en charge de l’attribut standard « no\_Address\_unique » dans C++ 17 et versions antérieures est une extension de fournisseur |
...
|Avertissement du compilateur (niveau 4) C4881|le constructeur et/ou le destructeur ne sont pas appelés pour tile_static variable'*variable*'|
|Avertissement du compilateur (niveau 1) C4882|passage de functors avec des opérateurs d’appel non-const à la concurrence ::p arallel_for_each est déconseillé|
...
|[Avertissement du compilateur (niveau 1) C4920](compiler-warning-level-1-c4920.md)|enum enum member member_1 = value_1 déjà vu dans enum enum As member_2 = value_2|
...
|[Avertissement du compilateur (niveau 4) C4932](compiler-warning-level-4-c4932.md)|__identifier (*identifier*) et \__Identifier (*identificateur*) ne peuvent pas être distingués|
...
|[Avertissement du compilateur (niveau 1) C4946](compiler-warning-level-1-c4946.md)|reinterpret_cast utilisées entre des classes connexes : '*Class1*'et'*Class2*'|
...
|Avertissement du compilateur C4954|'*fonction*' : non profilé (contient __int64 expression de commutateur)|
...
|Avertissement du compilateur (niveau 1) C4966|'*Function*'a __code_seg annotation avec un nom de segment non pris en charge, l’annotation est ignorée|
|Avertissement du compilateur C4970|constructeur délégué : objet cible ignoré, car'*type*'est statique|
|Avertissement du compilateur (niveau 1) C4971|Ordre des arguments : \<objet cible >, \<fonction cible > pour le constructeur délégué est déconseillée, utilisez \<fonction cible >, \<objet cible = "" >|
...
|Avertissement du compilateur (niveau 3) C4992|Warbird : la fonction'*Function*'marquée comme __forceinline pas Inline, car elle contient un assembly inline qui ne peut pas être protégé|
...
| Avertissement du compilateur (niveau 1) C5047 | utilisation d' \_non standard \_si\_existe avec des modules n’est pas pris en charge |
...
| Avertissement du compilateur (niveau 1) C5100 | \_\_VA\_ARGS\_\_ est réservé pour une utilisation dans les macros variadiques |
...
| Avertissement du compilateur (niveau 1) C5104 | «*chaîne1*#*Chaîne2*» trouvée dans la liste de remplacement de macro, vouliez-vous dire «*Chaîne1*» « #*Chaîne2*» ? |
