---
description: 'En savoir plus sur : avertissement des outils Éditeur de liens LNK4049'
...
[__declspec (dllimport)](../../cpp/dllexport-dllimport.md) a été spécifié pour *symbol* même si le symbole est défini dans le fichier objet *NomFichier. obj* de la même image. Supprimez le `__declspec(dllimport)` modificateur pour résoudre cet avertissement.
...
- Supprimez le `__declspec(dllimport)` modificateur de la déclaration anticipée du symbole qui a déclenché LNK4049. Vous pouvez rechercher des symboles dans une image binaire à l’aide de l’utilitaire **DUMPBIN** . Le commutateur **DUMPBIN/SYMBOLS** affiche la table de symboles COFF de l’image. Pour plus d’informations sur l’utilitaire **DUMPBIN** , consultez [Référence DUMPBIN](../../build/reference/dumpbin-reference.md).

- Désactivez temporairement la liaison incrémentielle et l’optimisation de l’ensemble du programme. Lors de la recompilation, l’application génère un avertissement LNK4217, qui comprend le nom de la fonction qui fait référence au symbole importé. Supprimez le `__declspec(dllimport)` modificateur de déclaration du symbole importé et réactivez l’édition de liens incrémentielle ou l’optimisation de la totalité du programme selon les besoins.
