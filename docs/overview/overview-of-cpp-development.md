...
- **Cmake** est un système de génération multiplateforme intégré à l’IDE de Visual Studio lorsque vous installez la charge de travail développement Desktop en C++. Vous pouvez utiliser le modèle de projet CMake pour les nouveaux projets, ou simplement ouvrir un dossier avec un fichier CMakeLists.txt. Pour plus d’informations, consultez [Projets CMake dans Visual Studio](../build/cmake-projects-in-visual-studio.md).

- Tout autre système de génération C++, y compris une collection de fichiers libre, est pris en charge via la fonctionnalité **ouvrir le dossier** . Vous créez des fichiers JSON simples pour appeler votre programme de génération et configurer des sessions de débogage. Pour plus d’informations, consultez [Projets Dossier ouvert pour C++](../build/open-folder-projects-cpp.md).
...
Pour plus d’informations sur la création d’une interface utilisateur pour une application MFC, consultez [Applications de bureau MFC](../mfc/mfc-desktop-applications.md). Pour plus d’informations sur les programmes Windows Win32, consultez [Applications de bureau Windows](../windows/desktop-applications-visual-cpp.md).
...
Visual Studio inclut des outils d’analyse statique du code qui peuvent détecter des problèmes potentiels dans votre code source. Ces outils incluent une implémentation des vérificateurs de règles [C++ Core Guidelines](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md). Pour plus d’informations, consultez [Vue d’ensemble de l’analyse de code pour C/C++](../code-quality/code-analysis-for-c-cpp-overview.md).
