...
Le compilateur émet l’avertissement [C4866](https://docs.microsoft.com/cpp/error-messages/compiler-warnings/c4866?view=vs-2017) quand il ne peut pas garantir l’évaluation de gauche à droite. Cet avertissement est généré uniquement si **/std:c++17** ou version ultérieure est spécifiée, car l’exigence d’ordre de gauche à droite de ces opérateurs a été introduite dans C++17.
...
Dans Visual Studio 2017 version 15.9, le code génère cette erreur : *Erreur C3615 : la fonction constexpr « S<int>::f » ne peut pas produire une expression constante. Remarque : échec en raison de l’appel d’une fonction indéfinie ou « constexpr » non déclarée Remarque : voir l’utilisation de « g »*. Pour éviter l’erreur, supprimez le qualificateur `constexpr` de l’instanciation explicite de la fonction f(). 
