---
title: informations de référence sur le schéma Launch. vs. JSON (C++)
ms.date: 08/20/2019
helpviewer_keywords:
...
---
# <a name="launchvsjson-schema-reference-c"></a>informations de référence sur le schéma Launch. vs. JSON (C++)
...
|`noDebug`|boolean|Spécifie s’il faut déboguer le programme lancé. La valeur par défaut de ce paramètre `false` est si elle n’est pas spécifiée.|
|`stopOnEntry`|boolean|Spécifie s’il faut arrêter une prochaine fois que le processus est lancé et que le débogueur est attaché. La valeur par défaut de ce paramètre `false`est.|
...
externalConsole|boolean|Si la valeur est true, une console est lancée pour l’élément débogué. Si `false`la condition est, aucune console n’est lancée. La valeur par défaut est `false`. Remarque : cette option est ignorée dans certains cas pour des raisons techniques.|
...
|`useForBreakpoints`|boolean|Lors de la définition de points d’arrêt, ce mappage de source doit être utilisé. Si `false`la valeur est, seuls le nom de fichier et le numéro de ligne sont utilisés pour définir des points d’arrêt. Si `true`la valeur est, les points d’arrêt sont définis avec le chemin d’accès complet au fichier et le numéro de ligne uniquement lorsque ce mappage de source est utilisé. Sinon, seul le nom de fichier et le numéro de ligne seront utilisés lors de la définition de points d’arrêt. La valeur par défaut est `true`.|
