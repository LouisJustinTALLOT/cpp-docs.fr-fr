...
|[C4532](../../error-messages/compiler-warnings/compiler-warning-level-1-c4532.md)|/we4532|L’utilisation de `continue` , `break` ou de `goto` Mots clés dans un `__finally` / `finally` bloc a un comportement indéfini au cours d’un arrêt anormal.|
...
- Effectue un assainissement de pointeur limité. Dans les expressions qui n’impliquent pas de déréférencement et dans les types qui n’ont pas de destructeur défini par l’utilisateur, les références de pointeur sont définies sur une adresse non valide après un appel à **`delete`** . Cela contribue à empêcher la réutilisation de références périmées de pointeur.

- Exécute l’initialisation du pointeur de membre de classe. Initialise automatiquement les membres de classe du type pointeur **`nullptr`** sur sur l’instanciation d’objet (avant l’exécution du constructeur). Cela permet d’éviter l’utilisation de pointeurs non initialisés que le constructeur ne initialise pas explicitement. L’initialisation du pointeur membre générée par le compilateur est appelée, à condition que :
...
Pour plus d’informations, consultez [avertissements,/SDL et amélioration de la détection des variables non initialisées](https://cloudblogs.microsoft.com/microsoftsecure/2012/06/06/warnings-sdl-and-improving-uninitialized-variable-detection/).
...
1. Sur la page **général** , sélectionnez l’option dans la liste déroulante **vérifications SDL** .
