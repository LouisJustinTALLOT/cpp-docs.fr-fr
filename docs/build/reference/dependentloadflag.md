...
Dans les deux cas, si vous spécifiez l’option `/DEPENDENTLOADFLAG:0x800` de lien (la valeur de l’indicateur `LOAD_LIBRARY_SEARCH_SYSTEM32` ), le chemin de recherche de module est limité au répertoire%Windows%\System32. Il offre une protection contre les attaques sur les autres annuaires. Pour plus d’informations, consultez [sécurité de la bibliothèque de liens dynamiques](/windows/win32/dlls/dynamic-link-library-security).
