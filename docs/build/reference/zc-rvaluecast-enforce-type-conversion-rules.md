...
ms.locfileid: "77504575"
---
# <a name="zcrvaluecast-enforce-type-conversion-rules"></a>/Zc:rvalueCast (Appliquer les règles de conversion de type)

Lorsque l’option **`/Zc:rvalueCast`** est spécifiée, le compilateur identifie correctement un type référence rvalue comme résultat d’une opération de conversion. Son comportement est conforme à la norme C++ 11. Lorsque l’option n’est pas spécifiée, le comportement du compilateur est le même que dans Visual Studio 2012.
...
Par défaut, **`/Zc:rvalueCast`** est désactivée ( **`/Zc:rvalueCast-`** ). L’option de compilateur [/permissive-](permissive-standards-conformance.md) définit implicitement cette option, mais elle peut être substituée à l’aide de **`/Zc:rvalueCast-`** .
