...
description: Guide de référence sur les propriétés de la pageC++ de propriétés de Visual Studio Microsoft C/Project.
ms.date: 02/09/2020
ms.topic: article
...
Spécifie un ou plusieurs répertoires à ajouter au chemin Include, séparés par des points-virgules s’il en existe plusieurs. Définit [/i (autres répertoires Include)](i-additional-include-directories.md).
...
Traite les avertissements du compilateur comme des erreurs. Pour un nouveau projet, il peut être préférable d’utiliser [/WX](wx-treat-linker-warnings-as-errors.md) dans chaque compilation. Résolvez tous les avertissements pour réduire les erreurs de code difficiles à trouver.
...
- **Uniquement __inline** -développe uniquement les fonctions marquées comme **inline**, `__inline`, `__forceinline`ou `__inline`. Ou, dans une C++ fonction membre, définie dans une déclaration de classe.
- **Toutes les fonctions appropriées** -étendent les fonctions marquées comme **inline** ou `__inline` et toute autre fonction choisie par le compilateur. (L’expansion se produit à la discrétion du compilateur, souvent appelée *auto-inline*.)
...
Prétraite les fichiers C C++ et source et écrit la sortie prétraitée dans un fichier. Cette option supprime la compilation et ne produit pas de fichier *`.obj`* .
...
Active la régénération minimale, qui détermine s’il C++ faut recompiler les fichiers C++ sources qui incluent les définitions de classe modifiées, stockées dans les fichiers d’en-tête *`.h`* .
...
- **Débogage multithread** -Définit _DEBUG et _MT. Cette option indique également au compilateur de placer le nom de bibliothèque *LIBCMTD. lib* dans le fichier *`.obj`* afin que l’éditeur de liens utilise *LIBCMTD. lib* pour résoudre les symboles externes.
- **Dll** multithread-fait en sorte que votre application utilise la version multithread propre à la dll de la bibliothèque Runtime. Définit _MT et _DLL et indique au compilateur de placer le nom de la bibliothèque *Msvcrt. lib* dans le fichier *`.obj`* .
- **Dll de débogage** multithread-Définit _DEBUG, _MT et _DLL et fait en sorte que votre application utilise la version de débogage multithread et la dll de la bibliothèque Runtime. Il indique également au compilateur de placer le nom de la bibliothèque *msvcrtd. lib* dans le fichier *`.obj`* .
...
Activez l’utilisation des instructions disponibles sur les processeurs qui prennent en charge les jeux d’instructions améliorés. Par exemple, les améliorations de SSE, SSE2, AVX et AVX2 apportées à IA-32. Et, les améliorations AVX et AVX2 pour x64. Actuellement, les **`/arch:SSE`** et les **`/arch:SSE2`** sont disponibles uniquement lors de la création de pour l’architecture x86. Si aucune option n’est spécifiée, le compilateur utilise les instructions trouvées sur les processeurs qui prennent en charge SSE2. L’utilisation d’instructions améliorées peut être désactivée avec **`/arch:IA32`** . Pour plus d’informations, consultez [/Arch (x86)](arch-x86.md), [/Arch (x64)](arch-x64.md) et [/Arch (ARM)](arch-arm.md).
...
- **Strict** : modèle à virgule flottante le plus strict. **`/fp:strict`** provoque la désactivation et l' **`fenv_access`** de **`fp_contract`** . **`/fp:except`** est implicite et peut être désactivée en spécifiant explicitement **`/fp:except-`** . En cas d’utilisation avec **`/fp:except-`** , **`/fp:strict`** applique la sémantique à virgule flottante stricte, mais sans respecter les événements exceptionnels.
...
Détermine la C++ norme du langage que le compilateur active. Utilisez la version la plus récente si possible. Définit [/std : c++ 14,/std : c++ 17,/std : c + + latest](std-specify-language-standard-version.md).
...
Spécifie le nom des fichiers de documentation XML générés ; peut être un nom de fichier ou de répertoire. Définit [/doc :\<nom >](doc-process-documentation-comments-c-cpp.md).
...
Spécifie le niveau des informations de consultation dans *`.bsc`* fichier. Définit [/fr](fr-fr-create-dot-sbr-file.md).

### <a name="browse-information-file"></a>Fichier d’informations de consultation

Spécifie le nom facultatif du fichier d’informations du navigateur. Définit [/fr\<nom >](fr-fr-create-dot-sbr-file.md).
...
- **__cdecl** : spécifie la Convention d’appel __cdecl pour toutes C++ les fonctions, à l’exception des fonctions membres et des fonctions marquées __stdcall ou __fastcall.
- **__fastcall** : spécifie la Convention d’appel __fastcall pour toutes C++ les fonctions, à l’exception des fonctions membres et des fonctions marquées __cdecl ou __stdcall. Toutes les fonctions __fastcall doivent avoir des prototypes.
- **__stdcall** : spécifie la Convention d’appel __stdcall pour toutes C++ les fonctions, à l’exception des fonctions membres et des fonctions marquées __cdecl ou __fastcall. Toutes les fonctions __stdcall doivent avoir des prototypes.
- **__vectorcall** : spécifie la Convention d’appel __vectorcall pour toutes C++ les fonctions, à l’exception des fonctions membres et des fonctions marquées __cdecl, __fastcall ou __stdcall. Toutes les fonctions __vectorcall doivent avoir des prototypes.

### <a name="compile-as"></a>Compiler en

Sélectionnez l’option langage de compilation pour les fichiers *`.c`* et *`.cpp`* . Définit [/TC,/TP](tc-tp-tc-tp-specify-source-file-type.md).
...
Désactive les numéros d’avertissement spécifiés. Placez les numéros d’avertissement dans une liste délimitée par des points-virgules. Définit [/wd\<num >](compiler-option-warning-level.md).

### <a name="forced-include-file"></a>Fichier include forcé

un ou plusieurs fichiers Include forcés. Définit [/fi\<nom >](fi-name-forced-include-file.md).
...
N’inclut pas les noms de bibliothèque par défaut dans les fichiers *`.obj`* . Définit [/zl](zl-omit-default-library-name.md).
