...
|[/Qspectre-load-cf](qspectre-load-cf.md)|Générez des instructions de sérialisation pour chaque instruction de workflow de contrôle qui charge de la mémoire.|
...
|[/F](v-version-number.md)|Action déconseillée. Définit la chaîne version.|
...
|[/F](v-version-number.md)|Action déconseillée. Définit la *`.obj`* chaîne de version de fichier.|
