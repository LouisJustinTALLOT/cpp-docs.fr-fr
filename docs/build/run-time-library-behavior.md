...
Dans votre DLL MFC normale liée de manière dynamique aux MFC, si vous utilisez une bibliothèque OLE MFC, une base de données MFC (ou DAO) ou une prise en charge des sockets MFC, respectivement, les dll d’extension de débogage MFC MFCO*version*d. dll, MFCD*version*d. dll et MFCN*version*d. dll ( où *version* correspond au numéro de version) est lié automatiquement. Vous devez appeler l’une des fonctions d’initialisation prédéfinies suivantes pour chacune des dll que vous utilisez dans vos DLL `CWinApp::InitInstance`MFC normales.
