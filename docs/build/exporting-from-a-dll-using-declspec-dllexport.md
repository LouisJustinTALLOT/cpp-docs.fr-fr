...
Cette pratique est plus évidente lorsque vous tentez d' C++ exporter des noms de fonctions décorés. Étant donné qu’il n’existe pas de spécification standard pour la décoration de nom, le nom d’une fonction exportée peut changer entre les versions du compilateur. Si vous utilisez **__declspec (dllexport)** , la recompilation de la dll et des fichiers. exe dépendants est nécessaire uniquement pour tenir compte des modifications de convention d’affectation de noms.
