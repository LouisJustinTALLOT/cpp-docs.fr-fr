...
ms.locfileid: "74303230"
---
# <a name="common-visual-c-arm-migration-issues"></a>Problèmes courants de migration ARM Visual C++

Lorsque vous utilisez le C++ compilateur Microsoft (MSVC), le C++ même code source peut produire des résultats différents sur l’architecture ARM que sur des architectures x86 ou x64.
...
Le reste de ce document décrit comment les différents comportements de ces C++ éléments de langage peuvent produire des résultats différents sur différentes plateformes.
...
Pour l’architecture ARM, la combinaison de saturation et de troncation signifie que la conversion en types non signés entraîne une saturation correcte des types non signés plus petits lorsqu’il sature un entier 32 bits, mais produit un résultat tronqué pour les valeurs supérieures à un type plus petit peut être représenté mais trop petit pour saturer l’entier 32 bits complet. La conversion est également saturée pour les entiers signés 32 bits, mais la troncation des entiers signés saturés donne la valeur-1 pour les valeurs saturées de manière positive et 0 pour les valeurs ayant une saturation négative. La conversion en un entier signé plus petit produit un résultat tronqué qui est imprévisible.

Pour les architectures x86 et x64, combinaison du comportement de retour à la ligne pour les conversions d’entiers non signés et l’évaluation explicite pour les conversions d’entiers signés en cas de dépassement de capacité, ainsi que la troncation, les résultats pour la plupart des décalages sont imprévisibles s’ils sont trop grand.
...
Dans l’architecture ARM, les paramètres de la liste d’arguments variables qui sont transmis sur la pile sont sujets à l’alignement. Par exemple, un paramètre 64 bits est aligné sur une limite de 64 bits. Sur x86 et x64, les arguments passés sur la pile ne sont pas soumis à un alignement et à un pack serrés. Cette différence peut entraîner une fonction variadiques comme `printf` pour lire les adresses mémoire qui étaient destinées à être remplies sur ARM si la disposition attendue de la liste d’arguments de la variable n’est pas mise en correspondance exactement, même si elle peut fonctionner pour un sous-ensemble de certaines valeurs sur les architectures x86 ou x64. Considérez cet exemple :
