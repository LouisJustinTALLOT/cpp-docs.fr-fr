...
1. Dans la boîte de dialogue **nouveau projet** , sélectionnez**modèles** >  **installés** > **Visual C++** > **Win32**. Dans le volet central, sélectionnez **Application console Win32**.
...
1. Utilisez ce fichier source pour implémenter les fonctionnalités de `Arithmetic`la classe. Le code doit ressembler à ce qui suit :
...
1. Une fois que vous avez créé une application console, un programme vide est créé à votre intention. Le nom du fichier source est identique au nom que vous avez choisi précédemment. Dans l’exemple, il s’agit `MathClient.cpp`du nom.
...
1. Pour référencer `MathLibrary.h` le fichier d’en-tête, vous devez modifier le chemin d’accès aux répertoires inclus. Dans **Explorateur de solutions**, cliquez avec le bouton droit sur **MathClient** pour ouvrir le menu contextuel. Sélectionnez **Propriétés** pour ouvrir la boîte de dialogue **pages de propriétés de MathClient** .
...
1. Sélectionnez la page de propriétés**général** des **Propriétés** > de configuration**C/C++** > . Dans la propriété **autres répertoires Include** , spécifiez le chemin d’accès du répertoire **MathLibrary** ou recherchez-le.
