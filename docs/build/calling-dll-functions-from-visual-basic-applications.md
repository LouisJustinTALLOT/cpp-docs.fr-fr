...
La `__stdcall` décoration de nom préfixe le nom du symbole avec un trait **\_** de soulignement () et ajoute le symbole avec**\@** un caractère arobase () suivi du nombre d’octets dans la liste d’arguments (espace de pile requis). En conséquence, la fonction est déclarée comme suit :
...
La Convention d’appel C`__cdecl`() décore le nom `_func`en.
...
- Si la fonction est exportée avec la Convention d'`__cdecl`appel C (), elle supprime le trait **\_** de soulignement de début () lorsque le nom est exporté.

- Si la fonction en cours d’exportation n’utilise pas la Convention d’appel C ( `__stdcall`par exemple,), elle exporte le nom décoré.

Étant donné qu’il n’existe aucun moyen de remplacer l’emplacement où le nettoyage de la `__stdcall`pile se produit, vous devez utiliser. Pour dédécorerr des `__stdcall`noms avec, vous devez les spécifier à l’aide d’alias dans la section EXPORTS du fichier. def. Cette procédure est illustrée ci-dessous pour la déclaration de fonction suivante :
...
- [Exportation à partir d’une DLL à l’aide d' __declspec (dllexport)](exporting-from-a-dll-using-declspec-dllexport.md)
