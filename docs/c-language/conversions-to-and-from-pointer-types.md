---
description: 'En savoir plus sur : conversions vers et à partir des types pointeur'
...
Un pointeur vers **`void`** peut être converti vers ou à partir d’un pointeur vers tout type, sans restriction ni perte d’informations. Si le résultat est converti vers le type d'origine, le pointeur d'origine est récupéré.
