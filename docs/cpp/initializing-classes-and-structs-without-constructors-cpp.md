...
description: Utilisez l’initialisation des accolades avec n’importe quelle classe, struct ou Union C++
ms.date: 11/19/2019
...
Si une classe a des constructeurs non définis par défaut, l’ordre dans lequel les membres de la classe apparaissent dans l’initialiseur de l’accolade est l’ordre dans lequel les paramètres correspondants apparaissent dans le constructeur, et non l’ordre dans lequel les membres sont déclarés (comme `class_a` dans l’exemple précédent). Sinon, si le type n’a pas de constructeur déclaré, l’ordre dans lequel les membres apparaissent dans l’initialiseur d’accolades est le même que l’ordre dans lequel ils sont déclarés ; dans ce cas, vous pouvez initialiser autant de membres publics que vous le souhaitez, mais vous ne pouvez pas ignorer un membre. L’exemple suivant montre l’ordre utilisé dans l’initialisation des accolades lorsqu’il n’existe aucun constructeur déclaré :
...
Vous pouvez utiliser l’initialisation des accolades partout où vous effectuez généralement l’initialisation, par exemple en tant que paramètre de fonction ou valeur de retour, ou avec le **`new`** mot clé :
...
> [!IMPORTANT]
> Pour utiliser cette classe, vous devez inclure l' [\<initializer_list>](../standard-library/initializer-list.md) en-tête.

Un `initializer_list` peut être copié. Dans ce cas, les membres de la nouvelle liste sont des références aux membres de la liste d’origine :
...
Les classes de conteneur de la bibliothèque standard, et également, `string` `wstring` et `regex` , ont des `initializer_list` constructeurs. Les exemples suivants montrent comment effectuer une initialisation d’accolade avec ces constructeurs :
