...
- pour stocker la valeur de retour d'une fonction qui retourne un type défini par l'utilisateur. Ces objets temporaires sont créés uniquement si votre programme ne copie pas la valeur de retour passée à un objet. Par exemple :
   ...
   Prenons l'exemple de l'expression `ComplexResult = Complex1 + Complex2 + Complex3`. L'expression `Complex1 + Complex2` est évaluée, et le résultat est stocké dans un objet temporaire. Ensuite, l' `+ Complex3` *temporaire* d’expression est évaluée et le résultat est copié dans `ComplexResult` (en supposant que l’opérateur d’assignation n’est pas surchargé).
...
|------------------------------|-----------------------|
|Résultat de l'évaluation de l'expression|Tous les temporaires créés à la suite de l’évaluation de l’expression sont détruits à la fin de l’instruction d’expression (autrement dit, au point-virgule) ou à la fin des expressions de contrôle pour **les instructions for**, **If**, **while**, **do**et **switch** .|
