...
L'exemple de code corrigé utilise la mémoire de la pile locale pour créer le magasin de stockage vers lequel pointe `pNumber`. Nous utilisons un type fondamental pour des raisons de simplicité. En pratique, le magasin de stockage pour les pointeurs sont plus souvent des types définis par l’utilisateur qui sont alloués dynamiquement dans une zone de mémoire appelée *tas* (ou *magasin libre*) à l’aide d’une **nouvelle** expression de mot clé (dans la programmation de style c, l’ancienne fonction de bibliothèque Runtime c `malloc()` a été utilisée). Une fois allouées, ces variables sont généralement appelées objets, surtout si elles sont basées sur une définition de classe. La mémoire allouée avec **New** doit être supprimée par une instruction **Delete** correspondante (ou, si vous avez utilisé la fonction `malloc()` pour l’allouer, la fonction C Runtime `free()`).
