...
Lorsque vous effectuez une recherche dépendante d’un argument pour des résolutions de surcharge dans l’unité de traduction d’importation, le compilateur considère les fonctions déclarées dans la même unité de traduction (y compris les interfaces de module) comme où le type des arguments de la fonction sont définis.
...
> [!NOTE]
> Cette section est à titre d’information uniquement. Les importations héritées ne sont pas encore implémentées dans le compilateur Microsoft C++ .
...
[module, importer, exporter](import-export-module.md)
