...
Pour déclencher un événement, il vous suffit d’appeler une méthode dans l’interface déclarée avec le **`__event`** mot clé dans la classe source de l’événement. Si des gestionnaires ont été raccordés à l'événement, les gestionnaires sont appelés.
...
Toutefois, dans la gestion des événements COM, lorsque vous affectez au paramètre *layout_dependent* de la valeur `event_receiver` **`true`** , le nom et la correspondance de signature sont appliqués. Cela signifie que les noms et les signatures des gestionnaires dans le récepteur d'événements doivent correspondre exactement aux noms et aux signatures des événements auxquels ils sont raccordés.
