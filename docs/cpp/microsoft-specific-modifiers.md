---
title: Modificateurs spécifiques Microsoft | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-language
ms.topic: language-reference
dev_langs:
- C++
ms.assetid: 22c7178c-f854-47fa-9de6-07d23fda58e1
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
ms.openlocfilehash: c67c1eab1be6cb1e9e1eadbaaba780bbf23b5a30
ms.sourcegitcommit: 3614b52b28c24f70d90b20d781d548ef74ef7082
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 07/11/2018
ms.locfileid: "38964897"
---
# <a name="microsoft-specific-modifiers"></a>Modificateurs Microsoft spécifiques
Cette section décrit les extensions C++ spécifiques à Microsoft dans les domaines suivants :  
  
-   [Adressage basé sur](../cpp/based-addressing.md), la pratique consistant à l’aide d’un pointeur comme base à partir de laquelle d’autres pointeurs peuvent être décalées  
  
-   [Conventions d’appel de fonction](../cpp/calling-conventions.md)  
  
-   Attributs étendus de stockage-classe déclarés avec le [__declspec](../cpp/declspec.md) mot clé  
  
-   Le [__w64](../cpp/w64.md) mot clé  
  
