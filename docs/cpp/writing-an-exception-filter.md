...
Notez l’utilisation de [GetExceptionCode](/windows/win32/Debug/getexceptioncode) pour déterminer l’exception. Vous devez appeler cette fonction à l'intérieur du filtre lui-même. `Eval_Exception` ne pouvez pas appeler `GetExceptionCode`, mais le code d’exception doit lui être passé.
