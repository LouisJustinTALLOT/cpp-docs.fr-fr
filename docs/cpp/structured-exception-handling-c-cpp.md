...
ms.locfileid: "87231102"
---
# <a name="structured-exception-handling-cc"></a>Structured Exception Handling (C/C++)

La gestion structurée des exceptions (SEH) est une extension Microsoft de C permettant de gérer certaines situations de code exceptionnelles, telles que les défaillances matérielles, en douceur. Bien que Windows et Microsoft C++ prennent en charge SEH, nous vous recommandons d’utiliser la gestion des exceptions C++ standard, car cela rend votre code plus portable et plus flexible. Néanmoins, pour conserver le code existant ou pour certains genres de programmes, vous devrez peut-être utiliser la gestion des exceptions STRUCTURÉes.
...
&nbsp;&nbsp;&nbsp;&nbsp;**__try** composée *-Statement* **`__except`** **(** *expression* **)** *Compound-Statement*
...
Si vous utilisez **/EHsc** pour compiler ce code, mais que la macro de contrôle de test local `CPPEX` n’est pas définie, il n’y a pas d’exécution du `TestClass` destructeur et la sortie ressemble à ceci :
...
Si vous utilisez **/EHa** pour compiler le code, le `TestClass` destructeur s’exécute indépendamment du fait que l’exception a été levée à l’aide de `std::throw` ou de l’utilisation de SEH pour déclencher l’exception, c’est-à-dire si elle est `CPPEX` définie ou non. Le résultat se présente ainsi :
