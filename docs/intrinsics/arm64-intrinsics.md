...
description: Liste de référence des intrinsèques ARM64 prise en charge par le compilateur Microsoft C++ dans Visual Studio.
f1_keywords:
- __break
- __addx18byte
- __addx18word
...
ms.locfileid: "90039753"
---
# <a name="arm64-intrinsics"></a>Intrinsèques ARM64

Le compilateur Microsoft C++ (MSVC) rend les intrinsèques suivantes disponibles sur l’architecture ARM64. Pour plus d’informations sur ARM, consultez les sections architecture et outils de développement logiciel du site Web de [documentation pour développeurs ARM](https://developer.arm.com/docs) .
...
## <a name="arm64-specific-intrinsics-listing"></a><a name="A"></a> Liste des intrinsèques spécifiques à ARM64
...
|__cas32|Site d’administration centrale|unsigned __int32 __cas32 (unsigned __int32 volatile * _Target, unsigned __int32 _Comp, unsigned __int32 _Value)|
|__cas64|Site d’administration centrale|unsigned __int64 __cas64 (unsigned __int64 volatile * _Target, unsigned __int64 _Comp, unsigned __int64 _Value)|
...
|__prefetch|PRFM|void __cdecl \_ _prefetch (const void \* )<br /><br /> Fournit une `PRFM` indication de mémoire avec l’opération de prérécupération `PLDL1KEEP` sur le système qui permet d’accéder bientôt à la mémoire à l’adresse spécifiée ou à proximité de celle-ci. Certains systèmes peuvent choisir d’effectuer une optimisation selon ce modèle d’accès mémoire, pour augmenter les performances d’exécution. Cependant, du point de vue du langage C++, la fonction n'a aucun effet observable et peut ne rien faire du tout.|
|__prefetch2|PRFM|void __cdecl \_ _prefetch (const void \* , uint8_t prfop)<br /><br /> Fournit un `PRFM` indicateur de mémoire avec l’opération de prérécupération fournie au système qui peut accéder à la mémoire à l’adresse spécifiée ou à proximité de celle-ci. Certains systèmes peuvent choisir d’effectuer une optimisation selon ce modèle d’accès mémoire, pour augmenter les performances d’exécution. Cependant, du point de vue du langage C++, la fonction n'a aucun effet observable et peut ne rien faire du tout.|
...
## <a name="arm64-support-for-intrinsics-from-other-architectures"></a><a name="I"></a> Prise en charge ARM64 pour les fonctions intrinsèques d’autres architectures
...
|Décrémentation|Aucun|Complète|Complète|Complète|None|None|
|Exchange|Complète|Complète|Complète|Complète|Aucun|Complète|
|ExchangeAdd|Complète|Complète|Complète|Complète|None|None|
|Incrément|Aucun|Complète|Complète|Complète|None|None|
...
|_InterlockedCompareExchange128|unsigned char _InterlockedCompareExchange128 ( \_ _int64 volatile \* _Destination, \_ _int64 _ExchangeHigh, _int64 _ExchangeLow, _int64 _ComparandResult \_ \_ \* )|
|_InterlockedCompareExchange128_acq|unsigned char _InterlockedCompareExchange128_acq ( \_ _int64 volatile \* _Destination, \_ _int64 _ExchangeHigh, _int64 _ExchangeLow, _int64 _ComparandResult \_ \_ \* )|
|_InterlockedCompareExchange128_nf|unsigned char _InterlockedCompareExchange128_nf ( \_ _int64 volatile \* _Destination, \_ _int64 _ExchangeHigh, _int64 _ExchangeLow, _int64 _ComparandResult \_ \_ \* )|
|_InterlockedCompareExchange128_rel|unsigned char _InterlockedCompareExchange128_rel ( \_ _int64 volatile \* _Destination, \_ _int64 _ExchangeHigh, _int64 _ExchangeLow, _int64 _ComparandResult \_ \_ \* )|
...
Les intrinsèques de test de bit avec verrouillage brut sont communs à toutes les plateformes. ARM64 ajoute `_acq` des `_rel` variantes, et, `_nf` qui modifient simplement la sémantique de cloisonnement d’une opération, comme décrit dans [_nf suffixe (aucune limite](#nf_suffix) de délimitation) plus haut dans cet article.
