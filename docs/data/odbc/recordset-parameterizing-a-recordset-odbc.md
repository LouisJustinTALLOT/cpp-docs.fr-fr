...
2.  Spécifier des membres de données de champ pour les colonnes du jeu d’enregistrements.  
...
5.  Dans le constructeur de votre classe de jeu d’enregistrements, incrémentez le nombre de paramètres, `m_nParams`.  
...
6.  Lorsque vous écrivez le code qui crée un objet de jeu d’enregistrements de cette classe, placez un « ? » symbole (point d’interrogation) dans chaque endroit dans l’instruction SQL où un paramètre à remplacer.  
...
2.  Préparer un ou plusieurs chaînes, comme le `m_strFilter` chaîne, qui contient l’instruction SQL ou des parties de celui-ci. Placer « ? » des espaces réservés dans lequel les informations de paramètre consiste à accéder.  
  
3.  Affecter une valeur de paramètre d’exécution à chaque membre de données de paramètre de l’objet.  
  
4.  Appelez le `Open` fonction membre (ou `Requery`, pour un jeu d’enregistrements existant).  
