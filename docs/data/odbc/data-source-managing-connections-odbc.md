...
2.  Sur le **fichier DSN** , cliquez sur **ajouter**.  
  
3.  Dans le **créer une nouvelle Source de données** boîte de dialogue, sélectionnez un pilote Excel, puis cliquez sur **suivant**.  
  
4.  Cliquez sur **Parcourir**, puis sélectionnez le nom du fichier à utiliser comme source de données.  
...
2.  Appeler son `OpenEx` ou `Open` fonction membre.  
...
2.  Plutôt que de détruire l’objet, appelez sa `OpenEx` ou `Open` fonction membre à nouveau.  
