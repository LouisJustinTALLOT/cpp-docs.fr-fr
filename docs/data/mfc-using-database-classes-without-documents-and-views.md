---
description: 'En savoir plus sur : MFC : utilisation des classes de base de données sans document ni vue'
...
Il peut arriver que vous ne souhaitiez pas utiliser l’architecture document/vue de l’infrastructure dans vos applications de base de données. Cette rubrique répond aux questions suivantes :
...
L’Assistant Application MFC a deux options qui prennent en charge les applications d’accès aux données basées sur les formulaires. Chaque option crée une `CRecordView` classe d’affichage dérivée de et un document. Ils diffèrent dans ce qu’ils quittent du document.
...
Vous pouvez parfois souhaiter écrire une application qui n’utilise pas de documents ou de vues. Sans documents, vous stockez vos données (par exemple, un `CRecordset` objet) dans votre classe de fenêtre frame ou votre classe d’application. Toutes les exigences supplémentaires varient selon que l’application présente une interface utilisateur.
...
Étant donné que l’Assistant application ne prend pas en charge la création d’applications sans documents, vous devez écrire votre propre `CWinApp` classe dérivée et, si nécessaire, créer également une `CFrameWnd` `CMDIFrameWnd` classe ou. Substituez `CWinApp::InitInstance` et déclarez un objet d’application comme suit :
...
Étant donné qu’aucun document n’est propriétaire de l' `CRecordset` objet, vous souhaiterez probablement le stocker en tant que membre de données incorporé dans votre `CWinApp` classe d’application dérivée de. Les alternatives sont les suivantes :

- Ne pas conserver un `CRecordset` objet permanent. Vous pouvez passer la valeur NULL aux constructeurs de la classe Recordset. Dans ce cas, l’infrastructure crée un `CDatabase`  objet temporaire à l’aide des informations contenues dans la fonction membre du recordset `GetDefaultConnect` . Il s’agit de l’approche alternative la plus probable.

- Définition `CRecordset` de l’objet comme variable globale. Cette variable doit être un pointeur vers un objet Recordset que vous créez dynamiquement dans votre `CWinApp::InitInstance` remplacement. Cela évite toute tentative de construction de l’objet avant l’initialisation de l’infrastructure.
