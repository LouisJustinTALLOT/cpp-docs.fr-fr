...
Toutefois, `CXMLAccessor` diffère de `CDynamicStringAccessorW` dans la mesure où elle convertit toutes les données accessibles depuis le magasin de données en tant que données au format XML (balises). Cela s’avère particulièrement utile pour la sortie vers des pages Web compatibles avec XML. Les noms de balises XML correspondent le plus fidèlement possible aux noms de colonnes de la Banque de données.

Utilisez des `CDynamicAccessor` méthodes pour obtenir des informations sur les colonnes. Vous utilisez ces informations de colonne pour créer un accesseur de manière dynamique au moment de l’exécution.
