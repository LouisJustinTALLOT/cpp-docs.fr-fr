...
Certains fournisseurs doivent établir une connexion distincte pour chaque session. Ces connexions supplémentaires doivent être inscrit séparément dans la transaction distribuée, s’il en existe. OLE DB services caches et réutilise une seule session par source de données, mais si l’application demande plus d’une session à la fois à partir d’une source de données, le fournisseur peut finir par effectuer des connexions supplémentaires, ainsi que les inscriptions de transactions supplémentaires qui ne sont pas regroupées. Il est plus efficace de créer une source de données distincte pour chaque session dans un environnement de bureaux que de créer plusieurs sessions à partir d’une source de données.
