---
title: Itération de la Collection de bibliothèque C++ Standard en utilisant for each | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
...
Le code suivant exemple utilise `for each` pour itérer un [ \<carte >](../standard-library/map.md).
...
L’exemple de code suivant utilise une référence const (`const&`) pour une variable d’itération avec des conteneurs de bibliothèque C++ Standard. Vous pouvez utiliser une référence (`&`) comme une variable d’itération sur n’importe quelle collection d’un type qui peut être déclaré comme un *T*`&`.
