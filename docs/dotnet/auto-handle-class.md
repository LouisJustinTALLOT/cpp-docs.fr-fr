...
|[auto_handle::reset](#reset)|Détruire l’objet actuel détenu et éventuellement prendre possession d’un nouvel objet.| 
|[auto_handle::swap](#swap)|Échange les objets avec un autre `auto_handle`.|  
...
|[auto_handle::operator auto_handle](#operator-auto-handle)|Opérateur de cast de type entre `auto_handle` et types compatibles.|  
...
Le destructeur destruction également de l’objet détenu.
...
L’objet de relation contenant-contenu.
...
Libère l’objet à partir de `auto_handle` gestion.
...
*_right*<br/>
Le `auto_handle` à assigner à actuel `auto_handle`.

### <a name="return-value"></a>Valeur de retour

Actuel `auto_handle`, désormais propriétaire `_right`.
...
Opérateur de cast de type entre `auto_handle` et types compatibles.
...
Actuel `auto_handle` castée en `auto_handle<_other_type>`.
