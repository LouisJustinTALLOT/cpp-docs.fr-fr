...
|[operator!= (multimap) (STL/CLR)](#op_neq)|Détermine si un objet `multimap` n’est pas égal à un autre objet `multimap`.|
|[operator< (multimap) (STL/CLR)](#op_lt)|Détermine si un objet `multimap` est inférieur à un autre objet `multimap`.|
|[operator<= (multimap) (STL/CLR)](#op_lteq)|Détermine si un objet `multimap` est inférieur ou égal à un autre objet `multimap`.|
|[operator== (multimap) (STL/CLR)](#op_eq)|Détermine si un objet `multimap` est égal à un autre objet `multimap`.|
|[operator> (multimap) (STL/CLR)](#op_gt)|Détermine si un objet `multimap` est supérieur à un autre objet `multimap`.|
|[operator>= (multimap) (STL/CLR)](#op_gteq)|Détermine si un objet `multimap` est supérieur ou égal à un autre objet `multimap`.|
...
Un tel objet délégué doit imposer un classement faible strict sur les clés de type [Multimap :: KEY_TYPE (STL/CLR)](../dotnet/multimap-key-type-stl-clr.md). Autrement dit, pour deux clés `X` et `Y`:
...
La fonction membre retourne la valeur true pour une séquence contrôlée vide. Cela équivaut à [Multimap :: Size (STL/CLR)](../dotnet/multimap-size-stl-clr.md)`() == 0`. Vous l’utilisez pour vérifier si le Multimap est vide.
...
La fonction membre détermine le premier élément `X` dans la séquence contrôlée qui a un classement équivalent à la *clé*. Si aucun élément de ce type n’existe, il retourne [Multimap :: end (STL/CLR)](../dotnet/multimap-end-stl-clr.md)`()`; Sinon, elle retourne un itérateur qui désigne `X`. Vous l’utilisez pour localiser le début d’une séquence d’éléments actuellement dans la séquence contrôlée qui correspondent à une clé spécifiée.
...
La fonction operator retourne `!(left == right)`. Vous l’utilisez pour tester si *Left* n’est pas *ordonné de la même façon que* lorsque les deux multimappages sont comparés élément par élément.
...
## <a name="operatorlt-multimap-stlclr"></a><a name="op_lt"></a>&lt; d’opérateur (Multimap) (STL/CLR)
...
La fonction operator retourne true si, pour la position la plus basse `i` pour laquelle `!(right[i] < left[i])` il est également vrai que `left[i] < right[i]`. Dans le cas contraire, elle retourne `left->size() < right->size()` vous l’utilisez pour déterminer si la *gauche* est triée *avant le* moment où les deux multimappages sont comparés élément par élément.
...
La fonction operator retourne `!(right < left)`. Vous l’utilisez pour tester si *Left* n’est pas trié après *le* moment où les deux multimappages sont comparés élément par élément.
...
## <a name="operatorgt-multimap-stlclr"></a><a name="op_gt"></a>&gt; d’opérateur (Multimap) (STL/CLR)
...
La fonction operator retourne `right` `<` `left`. Vous l’utilisez pour tester si *Left* est trié après *le* moment où les deux multimappages sont comparés élément par élément.
...
La fonction operator retourne `!(left` `<` `right)`. Vous l’utilisez pour tester si *Left* n’est pas trié *avant le moment où* les deux multimappages sont comparés élément par élément.
