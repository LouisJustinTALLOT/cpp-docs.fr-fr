...
Variable de type `To_Type` qui est la valeur convertie `input`de.
...
Lève une exception si le paramètre d’entrée a la valeur null. `System::ArgumentNullException(_EXCEPTION_NULLPTR)`
