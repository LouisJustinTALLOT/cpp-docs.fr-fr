...
De même, si vous exportez ([dllexport, DllImport](../cpp/dllexport-dllimport.md)) une fonction managée, un point d’entrée natif est généré et toute fonction qui importe et appelle cette fonction appellera via le point d’entrée natif. Pour éviter le double médiateur dans cette situation, n’utilisez pas de sémantique d’exportation/importation native. il vous suffit de référencer les métadonnées via `#using` (consultez [#using directive](../preprocessor/hash-using-directive-cpp.md)).
...
L’exemple précédent a montré l’existence d’un double médiateur. Cet exemple illustre son effet. La **`for`** boucle appelle la fonction virtuelle et le programme signale la durée d’exécution. Le temps le plus lent est signalé lorsque le programme est compilé avec **/CLR**. Les temps les plus rapides sont signalés lors de la compilation sans **/CLR** ou si la fonction virtuelle est déclarée avec `__clrcall` .
