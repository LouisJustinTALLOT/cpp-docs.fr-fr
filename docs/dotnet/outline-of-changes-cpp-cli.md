---
title: Plan des modifications (C++ / c++ / CLI) | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
...
ms.locfileid: "46415120"
---
# <a name="outline-of-changes-ccli"></a>Plan des modifications (C++/CLI)

Ce plan propose des exemples de certains des changements dans le langage d’Extensions managées pour C++ et Visual C++. Suivez le lien qui accompagne chaque élément pour plus d’informations.

## <a name="no-double-underscore-keywords"></a>Aucun mot clé de trait de soulignement Double

Double trait de soulignement devant tous les mots clés a été supprimé, à une exception près. Par conséquent, `__value` devient `value`, et `__interface` devient `interface`, et ainsi de suite. Pour éviter les conflits de nom entre les mots clés et des identificateurs dans le code utilisateur, les mots clés sont essentiellement traités comme contextuels.
...
Consultez [Types managés (C++ / c++ / CL)](../dotnet/managed-types-cpp-cl.md) pour plus d’informations.
...
Le tableau CLR a été repensé. Elle est similaire à la bibliothèque stl `vector` collection de modèles, mais correspond à sous-jacent `System::Array` classe - autrement dit, il n’est pas une implémentation de modèle.
...
Les Extensions managées de syntaxe des propriétés indexées :
...
Consultez [déclaration de propriété Index](../dotnet/property-index-declaration.md) pour plus d’informations.
...
Syntaxe d’opérateur conversion Extensions gérée :
...
Consultez [change aux opérateurs de Conversion](../dotnet/changes-to-conversion-operators.md) pour plus d’informations.
...
Consultez [remplacer explicite d’un membre d’Interface](../dotnet/explicit-override-of-an-interface-member.md) pour plus d’informations.
...
- La résolution de surcharge entre les types arithmétiques intégrés et la hiérarchie de classe d’objet a été inversée entre Extensions managées et de Visual C++. Comme un effet secondaire, les enums CLR sont ne sont plus converties implicitement en types arithmétiques.
...
## <a name="removal-of-box-keyword"></a>Suppression du mot clé __box

Le boxing de syntaxe des Extensions managées :
...
Consultez [suivi Handle à une valeur Boxed](../dotnet/a-tracking-handle-to-a-boxed-value.md) pour plus d’informations.

## <a name="pinning-pointer"></a>Pointeur épingle

Syntaxe du pointeur épingle les Extensions managées :
...
Consultez [sémantique de Type valeur](../dotnet/value-type-semantics.md) pour plus d’informations.
