...
Cet exemple implémente une classe CLR qui utilise un `com::ptr` pour encapsuler son membre privé `IXMLDOMDocument` objet. Le `WriteDocument` utilise les fonctions `operator->` pour appeler le `get_firstChild` membre de l’objet document.
