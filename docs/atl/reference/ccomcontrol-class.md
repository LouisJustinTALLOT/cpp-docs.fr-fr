---
description: 'En savoir plus sur : classe CComControl'
...
`CComControl` est un ensemble de fonctions d’assistance de contrôle utiles et de membres de données essentiels pour les contrôles ATL. Lorsque vous créez un contrôle standard ou DHTML à l’aide de l’Assistant contrôle ATL, l’Assistant dérivera automatiquement votre classe de `CComControl` . `CComControl` dérive la plupart de ses méthodes de [CComControlBase](../../atl/reference/ccomcontrolbase-class.md).
...
Appelle le constructeur [CComControlBase](ccomcontrolbase-class.md#ccomcontrolbase) , en passant le `m_hWnd` membre de données hérité par le biais de [CWindowImpl](../../atl/reference/cwindowimpl-class.md).
...
Si votre classe de contrôle dérive de [IPropertyNotifySink](/windows/win32/api/ocidl/nn-ocidl-ipropertynotifysink), cette méthode appelle [CFirePropNotifyEvent :: FireOnRequestEdit](cfirepropnotifyevent-class.md#fireonrequestedit) pour notifier toutes les interfaces connectées `IPropertyNotifySink` que la propriété de contrôle spécifiée va changer. Si votre classe de contrôle ne dérive pas de `IPropertyNotifySink` , cette méthode retourne S_OK.
