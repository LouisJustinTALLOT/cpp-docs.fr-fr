---
description: 'En savoir plus sur : classe IViewObjectExImpl'
...
|Nom|Description|
|----------|-----------------|
|[IViewObjectExImpl ::D RAW](#draw)|Dessine une représentation du contrôle dans un contexte de périphérique (Device Context).|
|[IViewObjectExImpl :: Freeze](#freeze)|Fige la représentation dessinée d’un contrôle afin qu’il ne soit pas modifié jusqu’à un `Unfreeze` . L’implémentation ATL retourne E_NOTIMPL.|
...
|[IViewObjectExImpl::QueryHitRect](#queryhitrect)|Vérifie si le rectangle d’affichage du contrôle chevauche un point dans le rectangle d’emplacement spécifié et retourne une valeur HITRESULT dans `pHitResult` .|
...
Cette méthode appelle `CComControl::OnDrawAdvanced` qui, à son tour, appelle la méthode de votre classe de contrôle `OnDraw` . Une `OnDraw` méthode est automatiquement ajoutée à votre classe de contrôle lorsque vous créez votre contrôle à l’aide de l’Assistant contrôle ATL. La valeur par défaut de l’Assistant `OnDraw` dessine un rectangle avec l’étiquette « ATL 3,0 ».
...
Fige la représentation dessinée d’un contrôle afin qu’il ne soit pas modifié jusqu’à un `Unfreeze` . L’implémentation ATL retourne E_NOTIMPL.
...
## <a name="iviewobjecteximplunfreeze"></a><a name="unfreeze"></a> IViewObjectExImpl :: libérer
