...
|[CAxDialogImpl::IsDialogMessage](#isdialogmessage)|Appelez cette méthode pour déterminer si un message est destiné à cette boîte de dialogue et, s’il s’agit, traiter le message.|  
  
### <a name="protected-data-members"></a>Membres de données protégés  
  
|Name|Description|  
|----------|-----------------|  
|[CAxDialogImpl::m_bModal](#m_bmodal)|Une variable qui existe uniquement en mode débogage génère et est définie sur true si la boîte de dialogue est modale.|  
  
## <a name="remarks"></a>Notes  
 `CAxDialogImpl` vous permet de créer une boîte de dialogue modale ou non. `CAxDialogImpl` Fournit la procédure de boîte de dialogue, qui utilise la table des messages par défaut pour diriger les messages vers les gestionnaires appropriés.  
  
 `CAxDialogImpl` dérive de `CDialogImplBaseT`, qui dérive à son tour *TBase* (par défaut, `CWindow`) et `CMessageMap`.  
 ...
 Notez que la création d’un contrôle ActiveX dans une boîte de dialogue modale avec `CAxDialogImpl` ne prendra pas en charge des touches accélérateur. Pour prendre en charge des touches d’accès sur une boîte de dialogue créée avec `CAxDialogImpl`, créer une boîte de dialogue non modale et, à l’aide de votre propre boucle de messages, utilisez [CAxDialogImpl::IsDialogMessage](#isdialogmessage) après obtention d’un message à partir de la file d’attente pour gérer un touche accélérateur.  
...
### <a name="parameters"></a>Paramètres  
 `bAdvise`  
 La valeur true si toutes les entrées de récepteur doivent être avertie ; les récepteur False si toutes les entrées doivent être cessent d’être averties.  
...
### <a name="remarks"></a>Notes  
 ...
 Le deuxième remplacement est fourni uniquement pour les boîtes de dialogue peuvent être utilisés avec [CComControl](../../atl/reference/ccomcontrol-class.md).  
...
### <a name="parameters"></a>Paramètres  
 `nRetCode`  
 [in] La valeur doit être retournée par [DoModal](#domodal).  
...
##  <a name="isdialogmessage"></a>  CAxDialogImpl::IsDialogMessage  
 Appelez cette méthode pour déterminer si un message est destiné à cette boîte de dialogue et, s’il s’agit, traiter le message.  
...
### <a name="parameters"></a>Paramètres  
 `pMsg`  
 Pointeur vers un [MSG](http://msdn.microsoft.com/library/windows/desktop/ms644958) structure qui contient le message doit être vérifiée.  
...
##  <a name="m_bmodal"></a>  CAxDialogImpl::m_bModal  
 Une variable qui existe uniquement en mode débogage génère et est définie sur true si la boîte de dialogue est modale.  
