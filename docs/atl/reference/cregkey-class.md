---
description: 'En savoir plus sur : classe CRegKey'
...
|Nom|Description|
|----------|-----------------|
|[CRegKey :: Attach](#attach)|Appelez cette méthode pour attacher un HKEY à l' `CRegKey` objet en affectant à la [m_hKey](#m_hkey) handle de membre la valeur `hKey` .|
|[CRegKey :: Close](#close)|Appelez cette méthode pour libérer le [m_hKey](#m_hkey) handle de membre et affectez-lui la valeur null.|
|[CRegKey :: Create](#create)|Appelez cette méthode pour créer la clé spécifiée, si elle n’existe pas en tant que sous-clé de `hKeyParent` .|
|[CRegKey ::D eleteSubKey](#deletesubkey)|Appelez cette méthode pour supprimer la clé spécifiée du Registre.|
|[CRegKey ::D eleteValue](#deletevalue)|Appelez cette méthode pour supprimer un champ de valeur de [m_hKey](#m_hkey).|
|[CRegKey ::D Etach](#detach)|Appelez cette méthode pour détacher le [m_hKey](#m_hkey) handle de membre de l' `CRegKey` objet et affectez-lui la valeur `m_hKey` null.|
...
|[CRegKey :: QueryValue](#queryvalue)|Appelez cette méthode pour récupérer les données du champ de valeur spécifié de [m_hKey](#m_hkey). Les versions antérieures de cette méthode ne sont plus prises en charge et sont marquées comme ATL_DEPRECATED.|
...
|Nom|Description|
|----------|-----------------|
|[CRegKey :: Operator HKEY](#operator_hkey)|Convertit un `CRegKey` objet en HKEY.|
...
|Nom|Description|
|----------|-----------------|
|[CRegKey :: m_hKey](#m_hkey)|Contient un handle de la clé de registre associée à l' `CRegKey` objet.|
|[CRegKey :: m_pTM](#m_ptm)|Pointeur vers un `CAtlTransactionManager` objet|
...
Appelez cette méthode pour attacher un HKEY à l' `CRegKey` objet en affectant à la [m_hKey](#m_hkey) handle de membre la valeur *HKEY*.
...
Appelez cette méthode pour libérer le [m_hKey](#m_hkey) handle de membre et affectez-lui la valeur null.
...
*samDesired*<br/>
Accès de sécurité pour la clé. La valeur par défaut est KEY_READ &#124; KEY_WRITE. Pour obtenir la liste des valeurs possibles et des descriptions, consultez `RegCreateKeyEx` .
...
Crée un objet `CRegKey`. L’objet peut être créé à partir d’un `CRegKey` objet existant ou d’un handle à une clé de registre.
...
Appelez cette méthode pour détacher le [m_hKey](#m_hkey) handle de membre de l' `CRegKey` objet et affectez-lui la valeur `m_hKey` null.
...
Si la méthode est réussie, la valeur de retour est ERROR_SUCCESS. Si la méthode échoue, la valeur de retour est un code d’erreur différent de zéro défini dans WINERROR. H.

### <a name="remarks"></a>Notes

Pour énumérer les sous-clés, appelez `CRegKey::EnumKey` avec un index de zéro. Incrémentez la valeur d’index et répétez l’opération jusqu’à ce que la méthode retourne ERROR_NO_MORE_ITEMS. Pour plus d’informations, consultez [RegEnumKeyEx](/windows/win32/api/winreg/nf-winreg-regenumkeyexw) dans le SDK Windows.
...
Si la méthode est réussie, la valeur de retour est ERROR_SUCCESS. Si la méthode échoue, la valeur de retour est un code d’erreur différent de zéro est défini dans WINERROR. H.
...
Contient un handle de la clé de registre associée à l' `CRegKey` objet.
...
En cas de réussite, retourne ERROR_SUCCESS ; Sinon, valeur d’erreur différente de zéro définie dans WINERROR. H.
...
Convertit un `CRegKey` objet en HKEY.
...
Si la méthode est réussie, ERROR_SUCCESS est retourné. Si la méthode ne parvient pas à lire une valeur, elle retourne un code d’erreur différent de zéro défini dans WINERROR. H. Si les données référencées ne sont pas de type REG_BINARY, ERROR_INVALID_DATA est retourné.

### <a name="remarks"></a>Notes

Cette méthode utilise `RegQueryValueEx` et confirme que le type de données correct est retourné. Pour plus d’informations, consultez [RegQueryValueEx](/windows/win32/api/winreg/nf-winreg-regqueryvalueexw) .
...
Si la méthode est réussie, ERROR_SUCCESS est retourné. Si la méthode ne parvient pas à lire une valeur, elle retourne un code d’erreur différent de zéro défini dans WINERROR. H. Si les données référencées ne sont pas de type REG_DWORD, ERROR_INVALID_DATA est retourné.
...
Si la méthode est réussie, ERROR_SUCCESS est retourné. Si la méthode ne parvient pas à lire une valeur, elle retourne un code d’erreur différent de zéro défini dans WINERROR. H. Si les données référencées ne sont pas un GUID valide, ERROR_INVALID_DATA est retourné.
...
Si la méthode est réussie, ERROR_SUCCESS est retourné. Si la méthode ne parvient pas à lire une valeur, elle retourne un code d’erreur différent de zéro défini dans WINERROR. H. Si les données référencées ne sont pas de type REG_MULTI_SZ, ERROR_INVALID_DATA est retourné.
...
Si la méthode est réussie, ERROR_SUCCESS est retourné. Si la méthode ne parvient pas à lire une valeur, elle retourne un code d’erreur différent de zéro défini dans WINERROR. H. Si les données référencées ne sont pas de type REG_QWORD, ERROR_INVALID_DATA est retourné.
...
Appelez cette méthode pour récupérer les données du champ de valeur spécifié de [m_hKey](#m_hkey). Les versions antérieures de cette méthode ne sont plus prises en charge et sont marquées comme ATL_DEPRECATED.
...
En cas de réussite, retourne ERROR_SUCCESS ; dans le cas contraire, il s’agit d’un code d’erreur différent de zéro défini dans WINERROR. H.
...
Cette méthode utilise `CRegKey::SetStringValue` et convertit le GUID en chaîne à l’aide de [StringFromGUID2](/windows/win32/api/combaseapi/nf-combaseapi-stringfromguid2).
...
*bMulti*<br/>
Si la valeur est false, indique que la chaîne est de type REG_SZ. Si la valeur est true, indique que la chaîne est une chaîne de type REG_MULTI_SZ.
...
Les deux versions d’origine de `SetValue` sont marquées comme ATL_DEPRECATED et ne doivent plus être utilisées. Le compilateur émet un avertissement si ces formulaires sont utilisés.
