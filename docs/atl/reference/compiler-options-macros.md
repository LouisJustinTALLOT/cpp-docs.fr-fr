---
description: En savoir plus sur les macros d’options du compilateur
...
|[_ATL_ENABLE_PTM_WARNING](#_atl_enable_ptm_warning)|Définissez cette macro afin d’utiliser la syntaxe compatible C++ standard, qui génère l’erreur du compilateur C4867 quand une syntaxe non standard est utilisée pour initialiser un pointeur vers une fonction membre.|
|[_ATL_FREE_THREADED](#_atl_free_threaded)|Définissez si un ou plusieurs de vos objets utilisent le Threading libre ou neutre.|
|[_ATL_MULTI_THREADED](#_atl_multi_threaded)|Symbole qui indique que le projet aura des objets marqués comme étant à la fois, libres ou neutres. Le [_ATL_FREE_THREADED](#_atl_free_threaded) de macro doit être utilisé à la place.|
...
Avant de Visual C++ .NET 2002, les ATL ont désactivé de nombreux avertissements et les ont laissés désactivés afin qu’ils ne soient jamais affichés dans le code utilisateur. Plus précisément :
...
Les bibliothèques ATL et MFC ont été modifiées pour correspondre à la conformité C++ standard améliorée du compilateur Microsoft C++. Selon la norme C++ ANSI, la syntaxe d’un pointeur vers une fonction membre de classe doit être `&CMyClass::MyFunc` .

Lorsque [_ATL_ENABLE_PTM_WARNING](#_atl_enable_ptm_warning) n’est pas défini (cas par défaut), ATL/MFC désactive l’erreur C4867 dans les cartes de macros (notamment les tables des messages) pour que le code créé dans les versions antérieures puisse continuer à être généré comme avant. Si vous définissez **_ATL_ENABLE_PTM_WARNING**, votre code doit être conforme à la norme C++.

Toutefois, le formulaire non standard est déconseillé. Vous devez déplacer le code existant vers la syntaxe compatible C++ standard. Par exemple, le code suivant :
...
Si ce symbole est défini, ATL extrait du code qui synchronisera correctement l’accès aux données globales. Le nouveau code doit utiliser la macro équivalente [_ATL_FREE_THREADED](#_atl_free_threaded) à la place.
...
Si le pointeur vtable ne peut pas être initialisé dans le constructeur et le destructeur de la classe, l’éditeur de liens peut éliminer le vtable et toutes les fonctions vers lesquelles il pointe. Développe à **`__declspec(novtable)`** .
...
Utilisez ce symbole si vous souhaitez vous assurer qu’une fonction n’est pas Inline par le compilateur, même si elle doit être déclarée comme Inline pour pouvoir être placée dans un fichier d’en-tête. Développe à **`__declspec(noinline)`** .
