---
title: CComCurrency, classe | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-atl
...
|Nom|Description|  
|----------|-----------------|  
|[CComCurrency::m_currency](#m_currency)|La variable de devise créée par votre instance de classe.|  
  
## <a name="remarks"></a>Notes  
 `CComCurrency` est un wrapper pour le type de données de devise. DEVISE est implémentée comme une valeur entière de 8 octets, MSB mis à l’échelle par 10 000. Ceci fournit un nombre à virgule fixe avec 15 chiffres à gauche du séparateur décimal et 4 chiffres à droite. Le type de données de devise est extrêmement utile pour les calculs impliquant des valeurs monétaires, ou pour tous les calculs à virgule fixe où la précision est importante.  
  
 Le `CComCurrency` wrapper implémente des opérations arithmétiques, d’affectation et de comparaison de ce type à virgule fixe. Les applications prises en charge ont été sélectionnées pour contrôler les erreurs d'arrondi qui peuvent se produire lors de calculs à virgule fixe.  
  
 L'objet `CComCurrency` permet d'accéder aux nombres figurant de chaque côté du séparateur décimal sous la forme de deux composantes : une partie entière correspondant à la valeur à gauche du séparateur décimal et une partie fractionnaire correspondant à la valeur à droite du séparateur décimal. La partie fractionnaire est stockée en interne comme un entier compris entre -9999 (CY_MIN_FRACTION) et + 9999 (CY_MAX_FRACTION). La méthode [CComCurrency::GetFraction](#getfraction) retourne une valeur d’un facteur de 10 000 (CY_SCALE).  
...
### <a name="remarks"></a>Notes  
 Le constructeur définit la valeur initiale de [CComCurrency::m_currency](#m_currency)et accepte un large éventail de types de données, y compris des entiers, chaînes, nombres à virgule flottante, les variables de devise et d’autres `CComCurrency` objets. Si aucune valeur n’est fournie, `m_currency` est définie sur 0.  
  
 En cas d’erreur, comme un dépassement de capacité, les constructeurs d’une spécification d’exception vide en l’absence (**throw()**) appeler `AtlThrow` avec un HRESULT qui décrit l’erreur.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne le composant fractionnaire de la `m_currency` membre de données.  
  
### <a name="remarks"></a>Notes  
 La partie fractionnaire est une valeur d’entier de 4 chiffres entre -9999 (CY_MIN_FRACTION) et + 9999 (CY_MAX_FRACTION). `GetFraction` Retourne cette valeur multipliée par 10 000 (CY_SCALE). Les valeurs de CY_MIN_FRACTION, CY_MAX_FRACTION et CY_SCALE sont définies dans atlcur.h.  
...
##  <a name="m_currency"></a>  CComCurrency::m_currency  
 Le membre de données de devise.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne un `CComCurrency` objet représentant le résultat de la soustraction. En cas d’erreur, comme un dépassement de capacité, cet opérateur appelle `AtlThrow` avec un HRESULT qui décrit l’erreur.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne la valeur TRUE si l’élément en cours de comparaison n’est pas égale à la `CComCurrency` de l’objet ; sinon, FALSE.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne un `CComCurrency` objet représentant le résultat de la multiplication. En cas d’erreur, comme un dépassement de capacité, cet opérateur appelle `AtlThrow` avec un HRESULT qui décrit l’erreur.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne la mise à jour `CComCurrency` objet. En cas d’erreur, comme un dépassement de capacité, cet opérateur appelle `AtlThrow` avec un HRESULT qui décrit l’erreur.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne un `CComCurrency` objet représentant le résultat de l’addition. En cas d’erreur, comme un dépassement de capacité, cet opérateur appelle `AtlThrow` avec un HRESULT qui décrit l’erreur.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne la valeur TRUE si le premier objet est inférieure à la seconde, FALSE sinon.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne la valeur TRUE si le premier objet est inférieur ou égal au second, sinon FALSE.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne la valeur TRUE si les objets sont égaux (autrement dit, le `m_currency` membres de données, les deux entiers et fractionnaires, dans les deux objets ont la même valeur), FALSE sinon.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne la valeur TRUE si le premier objet est supérieur à la seconde, FALSE sinon.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne la valeur TRUE si le premier objet est supérieur ou égal au second, FALSE sinon.  
...
### <a name="return-value"></a>Valeur de retour  
 Retourne une référence à un objet de devise.  
...
## <a name="see-also"></a>Voir aussi  
 ...
 [DEVISE](https://msdn.microsoft.com/5e81273c-7289-45c7-93c0-32c1553f708e)   
