...
> [!NOTE]
> Pour tout `CWindowImpl` objet donné, appelez `Create` ou `SubclassWindow` . N'appelez pas les deux méthodes sur le même objet.
...
`CWindowImpl` dérive de `CWindowImplBaseT` , qui dérive de `CWindowImplRoot` , qui dérive de `TBase` et de [CMessageMap](../../atl/reference/cmessagemap-class.md).
...
`Create` inscrit d’abord la classe de fenêtre si elle n’a pas encore été inscrite. La fenêtre nouvellement créée est automatiquement attachée à l' `CWindowImpl` objet.
...
Retourne le message en cours, empaqueté dans la `MSG` structure.
