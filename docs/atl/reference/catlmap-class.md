...
|[CAtlMap :: GetKeyAt](#getkeyat)|Appelez cette méthode pour récupérer la clé stockée à la position donnée dans `CAtlMap` l’objet.|
|[CAtlMap :: GetNext](#getnext)|Appelez cette méthode pour obtenir un pointeur vers la paire d’éléments suivante stockée `CAtlMap` dans l’objet.|
...
|[CAtlMap :: GetValueAt](#getvalueat)|Appelez cette méthode pour récupérer la valeur stockée à une position donnée dans `CAtlMap` l’objet.|
...
|[CAtlMap :: Lookup](#lookup)|Appelez cette méthode pour rechercher des clés ou des valeurs dans `CAtlMap` l’objet.|
|[CAtlMap :: rehash](#rehash)|Appelez cette méthode pour rehacher `CAtlMap` l’objet.|
|[CAtlMap :: RemoveAll](#removeall)|Appelez cette méthode pour supprimer tous les éléments de `CAtlMap` l’objet.|
...
|[CAtlMap :: SetValueAt](#setvalueat)|Appelez cette méthode pour modifier la valeur stockée à une position donnée dans `CAtlMap` l’objet.|
...
Le paramètre *fLoThreshold* est la valeur inférieure que le ratio de charge peut atteindre `CAtlMap` avant que ne recalcule la taille optimale de la carte.

Le paramètre *fHiThreshold* est la valeur supérieure que le ratio de charge peut atteindre avant `CAtlMap` que l’objet ne recalcule la taille optimale de la carte.
...
Appelez cette méthode pour récupérer la clé stockée à la position donnée dans `CAtlMap` l’objet.
...
Retourne une référence à la clé stockée à la position donnée dans `CAtlMap` l’objet.
...
Appelez cette méthode pour obtenir un pointeur vers la paire d’éléments suivante stockée `CAtlMap` dans l’objet.
...
Appelez cette méthode pour démarrer une itération de mappage en retournant une valeur de POSITION qui peut être `GetNextAssoc` passée à la méthode.
...
Appelez cette méthode pour récupérer la valeur stockée à une position donnée dans `CAtlMap` l’objet.
...
Retourne une référence à la valeur stockée à la position donnée dans `CAtlMap` l’objet.
...
Appelez cette méthode pour rechercher des clés ou des valeurs dans `CAtlMap` l’objet.
...
Appelez cette méthode pour rehacher `CAtlMap` l’objet.
...
Si *nBins* a la valeur `CAtlMap` 0, calcule un nombre raisonnable en fonction du nombre d’éléments dans le mappage et du paramètre de charge optimal. Normalement, le processus de rehachage est automatique, mais si [CAtlMap ::D isableautorehash](#disableautorehash) a été appelé, cette méthode effectue le redimensionnement nécessaire.

## <a name="catlmapremoveall"></a><a name="removeall"></a>CAtlMap :: RemoveAll

Appelez cette méthode pour supprimer tous les éléments de `CAtlMap` l’objet.
...
Retourne la position de la paire d’éléments clé/valeur dans `CAtlMap` l’objet.
...
Cette méthode redéfinit la valeur de chargement optimale pour `CAtlMap` l’objet. Consultez [CAtlMap :: CAtlMap](#catlmap) pour une discussion sur les différents paramètres. Si *bRehashNow* a la valeur true et que le nombre d’éléments est en dehors des valeurs minimales et maximales, la table de hachage est recalculée.

## <a name="catlmapsetvalueat"></a><a name="setvalueat"></a>CAtlMap :: SetValueAt

Appelez cette méthode pour modifier la valeur stockée à une position donnée dans `CAtlMap` l’objet.
...
Modifie l’élément de valeur stocké à la position donnée `CAtlMap` dans l’objet.
