...
- **Modèle de thread**
   ...
   |Option|Description|
   |------------|-----------------|
   |**Single**|Spécifie que l’objet s’exécute toujours dans le thread COM principal. Pour plus d’informations, consultez Apartments et [InprocServer32](/windows/win32/com/inprocserver32) à [thread unique](/windows/win32/com/single-threaded-apartments) .|
...
- **Interface**
   ...
   |Option|Description|
   |------------|-----------------|
   |**Double**|Spécifie que l’objet prend en charge une interface double (son vtable a des fonctions d’interface `IDispatch` personnalisées et des méthodes de liaison tardive). Permet aux clients COM et aux [contrôleurs Automation](../../mfc/automation-clients.md) d’accéder à l’objet. Valeur par défaut.|
