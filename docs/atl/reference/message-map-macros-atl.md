...
Lorsqu’un `CMyExtWindow` objet reçoit un message de WM_PAINT, le message est dirigé vers `CMyExtWindow::OnPaint` pour le traitement réel. Si `OnPaint` indique que le message nécessite un traitement supplémentaire, le message est ensuite dirigé vers la table des messages par défaut dans `CMyBaseWindow` .
...
- Si une procédure de fenêtre utilise la `CMyClass` table des messages par défaut de et `OnPaint` ne gère pas un message, le message est dirigé vers la `CMyBaseClass` table des messages par défaut de pour le traitement.

- Si une procédure de fenêtre utilise la première table des messages de remplacement dans `CMyClass` , tous les messages sont dirigés vers la `CMyBaseClass` table des messages par défaut de.
...
- Si une procédure de fenêtre utilise la `CMyClass` table des messages par défaut de et `OnPaint` ne gère pas un message, le message est dirigé vers la `m_obj` table des messages par défaut de pour le traitement.

- Si une procédure de fenêtre utilise la première table des messages de remplacement dans `CMyClass` , tous les messages sont dirigés vers la `m_obj` table des messages par défaut de.
...
La table des messages `bHandled` est définie sur true avant l' `CommandHandler` appel de. Si `CommandHandler` ne gère pas complètement le message, il doit avoir la valeur `bHandled` false pour indiquer que le message doit être traité ultérieurement.
...
La table des messages `bHandled` est définie sur true avant l' `MessageHandler` appel de. Si `MessageHandler` ne gère pas complètement le message, il doit avoir la valeur `bHandled` false pour indiquer que le message doit être traité ultérieurement.
...
La table des messages `bHandled` est définie sur true avant l' `NotifyHandler` appel de. Si `NotifyHandler` ne gère pas complètement le message, il doit avoir la valeur `bHandled` false pour indiquer que le message doit être traité ultérieurement.
