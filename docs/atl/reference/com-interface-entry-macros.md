...
ms.locfileid: "69496721"
---
# <a name="com_interface_entry-macros"></a>Macros COM_INTERFACE_ENTRY

Ces macros entrent les interfaces d’un objet dans sa carte COM afin qu’elles soient accessibles par `QueryInterface`. L’ordre des entrées dans la table COM est l’ordre dans lequel les interfaces sont vérifiées pour obtenir `QueryInterface`un IID correspondant pendant.
...
|[COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate)|Lorsque l’interface identifiée par *IID* est interrogée, `COM_INTERFACE_ENTRY_AGGREGATE` est transféré à `punk`.|
|[COM_INTERFACE_ENTRY_AGGREGATE_BLIND](#com_interface_entry_aggregate_blind)|Identique à [COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate), à ceci près que l’interrogation de tous les IID entraîne le transfert de la requête vers *punk*.|
|[COM_INTERFACE_ENTRY_AUTOAGGREGATE](#com_interface_entry_autoaggregate)|Identique à [COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate), sauf si *punk* a la valeur null, il crée automatiquement l’agrégat décrit par le *CLSID*.|
|[COM_INTERFACE_ENTRY_AUTOAGGREGATE_BLIND](#com_interface_entry_autoaggregate_blind)|Identique à [COM_INTERFACE_ENTRY_AUTOAGGREGATE](#com_interface_entry_autoaggregate), à ceci près que l’interrogation de tout IID entraîne le transfert de la requête vers *punk*, et si *punk* a la valeur null, créant automatiquement l’agrégat décrit par le *CLSID*.|
...
|[COM_INTERFACE_ENTRY_FUNC](#com_interface_entry_func)|Mécanisme général de raccordement à la logique d' `QueryInterface` ATL.|
|[COM_INTERFACE_ENTRY_FUNC_BLIND](#com_interface_entry_func_blind)|Identique à [COM_INTERFACE_ENTRY_FUNC](#com_interface_entry_func), à ceci près que l’interrogation de tout IID entraîne un appel à *Func*.|
...
Par exemple, si vous dérivez votre objet de classe de deux interfaces doubles `IDispatch` , vous exposez à l’aide de COM_INTERFACE_ENTRY2, car `IDispatch` peut être obtenu à partir de l’une des interfaces.
...
Le paramètre *punk* est supposé pointer vers l’élément interne inconnu d’un agrégat ou vers la valeur null, auquel cas l’entrée est ignorée. En général, il `CoCreate` s’agit de `FinalConstruct`l’agrégat dans.
...
Identique à [COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate), à ceci près que l’interrogation de tous les IID entraîne le transfert de la requête vers *punk*.
...
Identique à [COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate), sauf si *punk* a la valeur null, il crée automatiquement l’agrégat décrit par le *CLSID*.
...
Identique à [COM_INTERFACE_ENTRY_AUTOAGGREGATE](#com_interface_entry_autoaggregate), à ceci près que l’interrogation de tout IID entraîne le transfert de la requête vers *punk*, et si *punk* a la valeur null, créant automatiquement l’agrégat décrit par le *CLSID*.
...
L’IID d’interface est construit en ajoutant *x* à `IID_`. Par exemple, si *x* est `IPersistStorage`, `IID_IPersistStorage`l’IID sera.
