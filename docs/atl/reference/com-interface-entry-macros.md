...
ms.locfileid: "78855664"
---
# <a name="com_interface_entry-macros"></a>Macros COM_INTERFACE_ENTRY

Ces macros entrent les interfaces d’un objet dans sa carte COM afin qu’elles soient accessibles par `QueryInterface`. L’ordre des entrées dans la table COM est l’ordre dans lequel les interfaces sont vérifiées pour obtenir un IID correspondant lors de la `QueryInterface`.
...
|[COM_INTERFACE_ENTRY_AGGREGATE](#com_interface_entry_aggregate)|Lorsque l’interface identifiée par l' *IID* est interrogée, `COM_INTERFACE_ENTRY_AGGREGATE` est transféré à `punk`.|
...
|[COM_INTERFACE_ENTRY_FUNC](#com_interface_entry_func)|Mécanisme général de raccordement à la logique de `QueryInterface` d’ATL.|
...
Par exemple, si vous dérivez votre objet de classe de deux interfaces doubles, vous exposez `IDispatch` à l’aide de COM_INTERFACE_ENTRY2, car `IDispatch` peut être obtenue à partir de l’une des interfaces.
...
*Punk*<br/>
dans Nom d’un pointeur de `IUnknown`.

### <a name="remarks"></a>Notes

Le paramètre *punk* est supposé pointer vers l’élément interne inconnu d’un agrégat ou vers la valeur null, auquel cas l’entrée est ignorée. En règle générale, vous `CoCreate` l’agrégat dans `FinalConstruct`.
...
*Punk*<br/>
dans Nom d’un pointeur de `IUnknown`. Doit être un membre de la classe qui contient le mappage COM.
...
*Punk*<br/>
dans Nom d’un pointeur de `IUnknown`. Doit être un membre de la classe qui contient le mappage COM. Doit être initialisé avec la valeur NULL dans le constructeur de l’objet de classe.
...
Mécanisme général de raccordement à la logique de `QueryInterface` d’ATL.
