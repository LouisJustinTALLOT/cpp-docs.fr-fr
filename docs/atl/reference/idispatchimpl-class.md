---
description: 'En savoir plus sur : IDispatchImpl, classe'
...
*tihclass*<br/>
dans Classe utilisée pour gérer les informations de type pour *T*. Par défaut, la valeur est `CComTypeInfoHolder` .
...
`IDispatchImpl` contient un membre statique de type `CComTypeInfoHolder` qui gère les informations de type de l’interface double. Si vous avez plusieurs objets qui implémentent la même interface double, une seule instance de `CComTypeInfoHolder` est utilisée.
