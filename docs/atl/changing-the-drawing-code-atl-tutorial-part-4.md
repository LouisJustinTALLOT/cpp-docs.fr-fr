...
2. Une fois que les points de polygone sont calculées, ils sont stockés dans un tableau de type `POINT`, donc ajouter le tableau après la définition de `m_nSides` dans PolyCtl.h :
...
1. Dans le conteneur de Test, cliquez sur **appeler les méthodes** sur le **contrôle** menu.
...
3. Type `5` dans le **valeur du paramètre** , cliquez sur **définir la valeur**, puis cliquez sur **Invoke**.

Notez que le contrôle ne change pas. Bien que vous avez modifié le nombre de côtés en interne en définissant le `m_nSides` variable, cela n’a pas entraîné le contrôle à redessiner. Si vous basculez vers une autre application et revenez à un conteneur de Test, vous trouverez que le contrôle a été redessiné et a le nombre correct de côtés.
