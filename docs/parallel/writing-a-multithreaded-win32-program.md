...
> [!NOTE]
>  Pour une description similaire du point de vue MFC, consultez [Multithreading : conseils de programmation](../parallel/multithreading-programming-tips.md) et [Multithreading : quand utiliser les Classes de synchronisation](../parallel/multithreading-when-to-use-the-synchronization-classes.md).  
...
Win32 fournit également un stockage Local des threads (TLS) pour stocker des données par thread. Pour plus d’informations, consultez [stockage Local des threads (TLS)](../parallel/thread-local-storage-tls.md).  
