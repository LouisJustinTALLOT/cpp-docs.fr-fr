...
3.  Dans le `run` (méthode), après l’initialisation des variables `negative_count` et `positive_count`, ajoutez un `countdown_event` objet qui assure le suivi du nombre d’opérations actives.  
  
 [!code-cpp[concrt-dataflow-agent#6](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-a-dataflow-agent_3.cpp)]  
  
     Le `countdown_event` classe est illustrée plus loin dans cette rubrique.  
  
4.  Créer des objets de mémoire tampon qui fera partie le message dans le réseau de flux de données.  
  
 [!code-cpp[concrt-dataflow-agent#3](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-a-dataflow-agent_4.cpp)]  
  
5.  Connectez les mémoires tampons de messages pour former un réseau.  
  
 [!code-cpp[concrt-dataflow-agent#4](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-a-dataflow-agent_5.cpp)]  
  
6.  Attendez que le `event` et `countdown event` objets à définir. Ces événements indiquent que l’agent a reçu la valeur de sentinelle et que toutes les opérations terminées.  
