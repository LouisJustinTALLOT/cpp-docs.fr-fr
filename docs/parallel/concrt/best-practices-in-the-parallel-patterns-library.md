...
- [Utiliser l’annulation ou des exceptions pour rompre une boucle parallèle](#breaking-loops)  
...
- [N’effectuez pas les opérations de blocage quand vous annulez un travail parallèle](#blocking)  
  
- [Ne pas écrire de données partagées dans une boucle parallèle](#shared-writes)  
  
- [Lorsque cela est Possible, éviter le faux partage](#false-sharing)  
...
##  <a name="highest"></a> Exprimer le parallélisme au niveau plus élevé  
 ...
 Étant donné que chaque itération de boucle est indépendante, vous pouvez paralléliser une grande partie du travail, comme indiqué dans l'exemple suivant. Cet exemple utilise le [concurrency::parallel_for](reference/concurrency-namespace-functions.md#parallel_for) algorithme pour paralléliser la boucle externe.  
 ...
 Pour réduire la quantité de surcharge de planification dans cet exemple, vous pouvez paralléliser les boucles externes avant de paralléliser les boucles internes ou utiliser une autre construction parallèle comme le traitement « pipeline ». L’exemple suivant modifie le `ProcessImages` fonction à utiliser le [concurrency::parallel_for_each](reference/concurrency-namespace-functions.md#parallel_for_each) algorithme pour paralléliser la boucle externe.  
...
##  <a name="breaking-loops"></a> Utiliser l’annulation ou des exceptions pour rompre une boucle parallèle  
 ...
 Pour plus d’informations sur l’annulation et les mécanismes de gestion des exceptions qui sont fournis par la bibliothèque de modèles parallèles, consultez [l’annulation dans la bibliothèque PPL](cancellation-in-the-ppl.md) et [la gestion des exceptions](../../parallel/concrt/exception-handling-in-the-concurrency-runtime.md).  
...
-   L’annulation de la tâche parente amène la tâche enfant, l’appel à [concurrency::parallel_invoke](reference/concurrency-namespace-functions.md#parallel_invoke), également. Ainsi, ces deux ressources ne sont pas libérées.  
...
##  <a name="repeated-blocking"></a> Ne pas bloquer plusieurs fois dans une boucle parallèle  
 ...
 Prenons l’exemple suivant appelle la [concurrency::send](reference/concurrency-namespace-functions.md#send) fonction dans chaque itération d’un `parallel_for` boucle. Étant donné que `send` se bloque de manière coopérative, le runtime crée un thread pour exécuter le travail supplémentaire chaque fois que `send` est appelé.  
...
##  <a name="blocking"></a> N’effectuez pas les opérations de blocage quand vous annulez un travail parallèle  
 ...
 L'opérateur `new` effectue une allocation de tas, qui peut se bloquer. Le runtime effectue un autre travail uniquement lorsque la tâche effectue un appel, tel qu’un appel à de blocage coopératif [Concurrency::critical_section :: lock](reference/critical-section-class.md#lock).  
...
##  <a name="lifetime"></a> Assurez-vous que les Variables sont valides pendant la durée de vie d’une tâche  
 ...
 L’exemple suivant utilise le [Concurrency::task_group :: wait](reference/task-group-class.md#wait) méthode pour s’assurer que la tâche se termine avant la `perform_action` fonction renvoie.  
