---
title: Meilleures pratiques dans la bibliothèque de modèles parallèles | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-concrt
...
- [Utiliser parallel_invoke pour résoudre des problèmes de division pour mieux régner](#divide-and-conquer)
...
- [Ne bloquent pas à plusieurs reprises dans une boucle parallèle](#repeated-blocking)
...
- [Dans la mesure du possible, éviter le faux partage](#false-sharing)
...
##  <a name="repeated-blocking"></a> Ne bloquent pas à plusieurs reprises dans une boucle parallèle
...
##  <a name="false-sharing"></a> Dans la mesure du possible, éviter le faux partage

*Faux partage* se produit lorsque plusieurs tâches simultanées qui sont en cours d’exécution sur des processeurs distincts écrivent des variables qui sont trouvent sur la même ligne de cache. Quand une seule tâche écrit dans l’une des variables, la ligne de cache des deux variables est invalidée. Chaque processeur doit recharger la ligne de cache à chaque fois que la ligne de cache est invalidée. Ainsi, le faux partage peut diminuer les performances dans votre application.
...
L’exemple suivant utilise le [Concurrency::task_group :: wait](reference/task-group-class.md#wait) méthode pour vous assurer que la tâche se termine avant le `perform_action` fonction renvoie.
