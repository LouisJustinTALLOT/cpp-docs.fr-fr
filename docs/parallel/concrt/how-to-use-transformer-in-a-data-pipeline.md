...
Il est courant qu’une étape d’un pipeline de données génère une valeur dont le type diffère de sa valeur d’entrée. Dans cet exemple, la deuxième étape prend une valeur de type `int` comme entrée et génère la racine carrée de cette valeur (un `double`) comme sortie.
