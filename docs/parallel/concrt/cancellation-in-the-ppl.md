...
![Une arborescence de travail parallèle](../../parallel/concrt/media/parallelwork_trees.png "Arborescence de travail parallèle")

L’exemple suivant montre le code requis pour créer l’arborescence dans l’illustration. Dans cet exemple, `tg1` et `tg2` sont des objets [Concurrency :: structured_task_group](../../parallel/concrt/reference/structured-task-group-class.md) ; `t1`, `t2`, `t3`, `t4`et `t5` sont des objets [Concurrency :: task_handle](../../parallel/concrt/reference/task-handle-class.md) .
...
- Pour les objets `task`, utilisez la fonction [Concurrency :: cancel_current_task](reference/concurrency-namespace-functions.md#cancel_current_task) . `cancel_current_task` annule la tâche en cours et toutes ses continuations basées sur des valeurs. (Cela n’annule pas le *jeton* d’annulation associé à la tâche ou ses continuations.)

- Pour les groupes de tâches et les algorithmes parallèles, utilisez la fonction [Concurrency :: is_current_task_group_canceling](reference/concurrency-namespace-functions.md#is_current_task_group_canceling) pour détecter l’annulation et retourner dès que possible du corps de la tâche lorsque cette fonction retourne la **valeur true**. (N’appelez pas `cancel_current_task` à partir d’un groupe de tâches.)
...
> [!TIP]
> Vous pouvez également appeler la fonction [Concurrency :: interruption_point](reference/concurrency-namespace-functions.md#interruption_point) au lieu de `cancel_current_task`.
...
> [!TIP]
> Utilisez la méthode [Concurrency :: cancellation_token :: None](reference/cancellation-token-class.md#none) quand vous appelez un constructeur ou une fonction qui accepte un objet `cancellation_token` et que vous ne souhaitez pas que l’opération soit annulable.
...
Quand vous fournissez un jeton d’annulation à la fonction `when_all` et `when_any`, cette fonction annule uniquement quand ce jeton d’annulation est annulé ou lorsque l’une des tâches participantes se termine dans un état annulé ou lève une exception.

La fonction `when_all` hérite du jeton d’annulation de chaque tâche qui compose l’opération globale quand vous ne lui fournissez pas de jeton d’annulation. La tâche retournée à partir de `when_all` est annulée quand l’un de ces jetons est annulé et qu’au moins une des tâches participantes n’a pas encore démarré ou est en cours d’exécution. Un comportement similaire se produit lorsque l’une des tâches lève une exception : la tâche retournée à partir de `when_all` est immédiatement annulée avec cette exception.
...
La classe `structured_task_group` n'est pas thread-safe. Par conséquent, une tâche enfant qui appelle une méthode de son objet `structured_task_group` parent entraîne un comportement non spécifié. Les exceptions à cette règle sont les méthodes `structured_task_group::cancel` et [Concurrency :: structured_task_group :: is_canceling](reference/structured-task-group-class.md#is_canceling) . Une tâche enfant peut appeler ces méthodes pour annuler le groupe de tâches parent et vérifier l'annulation.
...
L’exemple suivant utilise la méthode [Concurrency :: structured_task_group :: run_and_wait](reference/structured-task-group-class.md#run_and_wait) pour appeler l’algorithme `parallel_for`. La méthode `structured_task_group::run_and_wait` attend que la tâche fournie se termine. L’objet `structured_task_group` permet à la fonction de travail d’annuler la tâche.
