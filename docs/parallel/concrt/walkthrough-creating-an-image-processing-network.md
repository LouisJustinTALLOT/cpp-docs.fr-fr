---
description: 'En savoir plus sur : procédure pas à pas : création d’un réseau Image-Processing'
...
## <a name="defining-image-processing-functionality"></a><a name="functionality"></a> Définition de la fonctionnalité de traitement d’image
...
Les fonctions suivantes,,, `Grayscale` `Sepiatone` `ColorMask` et `Darken` appellent la `ProcessImage` fonction pour transformer la valeur de couleur de chaque pixel dans un `Bitmap` objet. Chacune de ces fonctions utilise une expression lambda pour définir la transformation de couleur d’un pixel.
...
## <a name="creating-the-image-processing-network"></a><a name="network"></a> Création du réseau de traitement des images
...
1. Crée un objet [std :: Map](../../standard-library/map-class.md) qui associe un `Bitmap` objet avec son nom de fichier d’origine.
...
|Membre|Description|
|------------|-----------------|
|`load_bitmap`|Objet [Concurrency :: transformer](../../parallel/concrt/reference/transformer-class.md) qui charge un `Bitmap` objet à partir du disque et ajoute une entrée à l' `map` objet pour associer l’image à son nom de fichier d’origine.|
...
|`sepiatone`|`transformer`Objet qui applique le ton sépia aux images qui ne sont pas créées par Tom et qui ne sont pas principalement en rouge.|
|`save_bitmap`|`transformer`Objet qui enregistre le traité `image` sur le disque en tant que bitmap. `save_bitmap` Récupère le nom de fichier d’origine de l' `map` objet et remplace son extension de nom de fichier par. bmp.|
...
La `loaded_bitmaps` mémoire tampon des messages est importante car, en tant qu' `unbounded_buffer` objet, elle offre des `Bitmap` objets à plusieurs destinataires. Quand un bloc cible accepte un `Bitmap` objet, l' `unbounded_buffer` objet n’offre pas cet `Bitmap` objet à d’autres cibles. Par conséquent, l’ordre dans lequel vous liez des objets à un `unbounded_buffer` objet est important. Les `grayscale` `colormask` blocs de message,, et `sepiatone` utilisent chacun un filtre pour accepter uniquement certains `Bitmap` objets. La `decrement` mémoire tampon des messages est une cible importante de la `loaded_bitmaps` mémoire tampon des messages, car elle accepte tous les `Bitmap` objets rejetés par les autres tampons de messages. Un `unbounded_buffer` objet est requis pour propager les messages dans l’ordre. Par conséquent, un `unbounded_buffer` objet se bloque jusqu’à ce qu’un nouveau bloc cible lui soit lié et accepte le message si aucun bloc cible actuel n’accepte ce message.

Si votre application nécessite que plusieurs blocs de messages traitent le message, au lieu d’un seul bloc de message qui accepte le message, vous pouvez utiliser un autre type de bloc de message, tel que `overwrite_buffer` . La `overwrite_buffer` classe contient un message à la fois, mais propage ce message à chacune de ses cibles.

L’illustration suivante montre le réseau de traitement des images :

![Réseau de traitement d'images](../../parallel/concrt/media/concrt_imageproc.png "Réseau de traitement d'images")

L' `countdown_event` objet de cet exemple permet au réseau de traitement des images d’informer l’application principale lorsque toutes les images ont été traitées. La `countdown_event` classe utilise un objet [Concurrency :: Event](../../parallel/concrt/reference/event-class.md) pour signaler quand une valeur de compteur atteint zéro. L’application principale incrémente le compteur chaque fois qu’il envoie un nom de fichier au réseau. Le nœud terminal du réseau décrémente le compteur après que chaque image a été traitée. Une fois que l’application principale a parcouru le répertoire spécifié, elle attend `countdown_event` que l’objet signale que son compteur a atteint zéro.
...
![Sortie de l'exemple](../../parallel/concrt/media/concrt_imageout.png "Sortie de l'exemple")

`Lighthouse` est créée par Tom Alphin et est donc convertie en nuances de gris. `Chrysanthemum`, `Desert` , `Koala` et `Tulips` ont rouge comme couleur dominante et les composants de couleur bleu et vert sont donc supprimés et sont obscurcis. `Hydrangeas`, `Jellyfish` et `Penguins` correspondent aux critères par défaut ; par conséquent, il s’agit de sépia toned.
