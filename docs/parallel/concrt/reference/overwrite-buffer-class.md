---
title: overwrite_buffer, classe | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-concrt
...
|Nom|Description|
|----------|-----------------|
|[accept_message](#accept_message)|Accepte un message qui a été proposé par ce `overwrite_buffer` bloc de messagerie, qui renvoie une copie du message à l’appelant.|
|[consume_message](#consume_message)|Consomme un message précédemment proposé par le `overwrite_buffer` bloc de messagerie et réservé par la cible, en retournant une copie du message à l’appelant.|
...
|[propagate_message](#propagate_message)|Passe de façon asynchrone un message à partir d’un `ISource` à ce bloc `overwrite_buffer` bloc de messagerie. Elle est appelée par le `propagate` (méthode), lorsqu’elle est appelée par un bloc source.|
...
|[reserve_message](#reserve_message)|Réserve un message précédemment proposé par ce `overwrite_buffer` bloc de messagerie. (Substitue [source_block::reserve_message](source-block-class.md#reserve_message).)|
...
|[send_message](#send_message)|Passe de façon synchrone un message à partir d’un `ISource` à ce bloc `overwrite_buffer` bloc de messagerie. Elle est appelée par le `send` (méthode), lorsqu’elle est appelée par un bloc source.|
...
Accepte un message qui a été proposé par ce `overwrite_buffer` bloc de messagerie, qui renvoie une copie du message à l’appelant.
...
Consomme un message précédemment proposé par le `overwrite_buffer` bloc de messagerie et réservé par la cible, en retournant une copie du message à l’appelant.
...
Le type `filter_method` est un functor avec la signature `bool (T const &)` qui est appelé par ce `overwrite_buffer` bloc de messagerie pour déterminer s’il doit accepter un message proposé.

##  <a name="propagate_message"></a> propagate_message

Passe de façon asynchrone un message à partir d’un `ISource` à ce bloc `overwrite_buffer` bloc de messagerie. Elle est appelée par le `propagate` (méthode), lorsqu’elle est appelée par un bloc source.
...
*_PMessage*<br/>
Un pointeur vers un `message` objet que ce `overwrite_buffer` a pris la propriété.

### <a name="remarks"></a>Notes

Cette méthode remplace le message actuel dans le `overwrite_buffer` avec le message qui vient d’être accepté `_PMessage`.

##  <a name="send_message"></a> send_message

Passe de façon synchrone un message à partir d’un `ISource` à ce bloc `overwrite_buffer` bloc de messagerie. Elle est appelée par le `send` (méthode), lorsqu’elle est appelée par un bloc source.
...
La valeur stockée dans le `overwrite_buffer` peut changer immédiatement après que cette méthode est retournée. Cette méthode attend un message arrive si aucun message n’est actuellement stocké dans le `overwrite_buffer`.
