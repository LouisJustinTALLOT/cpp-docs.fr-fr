...
-   Lire l’élément en appelant le [lire](../mfc/reference/canimatectrl-class.md#play) fonction membre. Si votre contrôle animation se trouve dans une boîte de dialogue, un bon emplacement pour ce faire est la classe de boîte de dialogue `OnInitDialog` (fonction). Appel de `Play` n’est pas nécessaire si le style ACS_AUTOPLAY est définie par le contrôle de l’animation.  
