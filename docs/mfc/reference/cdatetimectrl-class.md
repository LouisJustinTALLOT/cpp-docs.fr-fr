...
|[CDateTimeCtrl::Create](#create)|Crée le contrôle de sélecteur de date et d’heure et l’attache `CDateTimeCtrl` à l’objet.|
...
|[CDateTimeCtrl::GetTime](#gettime)|Récupère l’heure actuellement sélectionnée dans un contrôle de sélecteur de dates et d’heures et la place dans `SYSTEMTIME` une structure spécifiée.|
...
Crée le contrôle de sélecteur de date et d’heure et l’attache `CDateTimeCtrl` à l’objet.
...
1. Appelez cette fonction membre, qui crée le contrôle de sélecteur de date et d’heure Windows et l’attache `CDateTimeCtrl` à l’objet.
...
Cette fonction membre implémente le comportement du [DTM_GETMCCOLOR](/windows/win32/Controls/dtm-getmccolor)de message Win32, comme décrit dans la SDK Windows.
...
*pMaxRange*<br/>
Pointeur vers un `COleDateTime` objet ou un `CTime` objet contenant l’heure la plus récente autorisée dans `CDateTimeCtrl` l’objet.
...
Cette fonction membre implémente le comportement du [DTM_GETRANGE](/windows/win32/Controls/dtm-getrange)de message Win32, comme décrit dans la SDK Windows. Dans l’implémentation de MFC, vous pouvez spécifier `COleDateTime` des `CTime` utilisations ou.
...
Récupère l’heure actuellement sélectionnée dans un contrôle de sélecteur de dates et d’heures et la place dans `SYSTEMTIME` une structure spécifiée.
...
Cette fonction membre implémente le comportement du [DTM_SETFORMAT](/windows/win32/Controls/dtm-setformat)de message Win32, comme décrit dans la SDK Windows.
...
Cette fonction membre implémente le comportement du [DTM_SETMCCOLOR](/windows/win32/Controls/dtm-setmccolor)de message Win32, comme décrit dans la SDK Windows.
...
Cette fonction membre implémente le comportement du [DTM_SETMCFONT](/windows/win32/Controls/dtm-setmcfont)de message Win32, comme décrit dans la SDK Windows.
...
> [!NOTE]
>  Si vous utilisez ce code, vous souhaiterez créer un membre de votre `CDialog`classe dérivée de, appelé *m_MonthFont* de `CFont`type.
...
Cette fonction membre implémente le comportement du [DTM_SETSYSTEMTIME](/windows/win32/Controls/dtm-setsystemtime)de message Win32, comme décrit dans la SDK Windows. Dans l’implémentation MFC de `SetTime`, vous pouvez utiliser les `COleDateTime` classes `CTime` ou, ou vous pouvez utiliser une `SYSTEMTIME` structure pour définir les informations d’heure.
