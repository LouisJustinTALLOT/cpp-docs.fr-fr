...
Ce contrôle (et par conséquent la `CSliderCtrl` classe) est uniquement disponible pour les programmes qui s’exécutent sous windows 95/98 et Windows NT version 3,51 et versions ultérieures.
...
Vous créez un curseur à l’aide du constructeur et `Create` de la fonction membre de `CSliderCtrl` . Une fois que vous avez créé un contrôle Slider, vous pouvez utiliser des fonctions membres dans `CSliderCtrl` pour modifier un grand nombre de ses propriétés. Les modifications que vous pouvez apporter incluent la définition des positions minimale et maximale pour le curseur, le dessin des graduations, la définition d’une plage de sélection et le repositionnement du curseur.

Pour plus d’informations sur l’utilisation de `CSliderCtrl` , consultez [contrôles](../../mfc/controls-mfc.md) et [utilisation de CSliderCtrl](../../mfc/using-csliderctrl.md).
...
*rectangulaire*<br/>
Spécifie la taille et la position du contrôle Slider. Il peut s’agir d’un objet [CRect](../../atl-mfc-shared/reference/crect-class.md) ou d’une structure [Rect](/windows/win32/api/windef/ns-windef-rect) .
...
Vous construisez un `CSliderCtrl` en deux étapes. Tout d’abord, appelez le constructeur, puis appelez `Create` , qui crée le contrôle Slider et l’attache à l' `CSliderCtrl` objet.
...
Pour appliquer des styles de fenêtre étendus au contrôle Slider, appelez [CreateEx](#createex) au lieu de `Create` .

## <a name="csliderctrlcreateex"></a><a name="createex"></a> CSliderCtrl :: CreateEx

Crée un contrôle (une fenêtre enfant) et l’associe à l' `CSliderCtrl` objet.
...
*fLocation*<br/>
Valeur booléenne qui indique le ou les deux handles de fenêtre associée à récupérer. Il peut s'agir de l'une des valeurs suivantes :
...
La taille de ligne affecte le déplacement du curseur pour les notifications de TB_LINEUP et de TB_LINEDOWN. La valeur par défaut de la taille de ligne est 1.
...
La taille de la page affecte le déplacement du curseur pour les notifications d’TB_PAGEUP et de TB_PAGEDOWN.
...
Cette fonction membre implémente le comportement de la [TBM_GETTOOLTIPS](/windows/win32/Controls/tbm-gettooltips)de message Win32, comme décrit dans le SDK Windows. Notez que cette fonction membre retourne un `CToolTipCtrl` objet au lieu d’un handle vers un contrôle.
...
- TRUE l’ami apparaît à gauche du TrackBar si le contrôle TrackBar utilise le style TBS_HORZ. Si le TrackBar utilise le style TBS_VERT, l’ami s’affiche au-dessus du contrôle TrackBar.
...
Cette fonction membre implémente le comportement de la [TBM_SETBUDDY](/windows/win32/Controls/tbm-setbuddy)de message Win32, comme décrit dans le SDK Windows. Notez que cette fonction membre utilise des pointeurs vers des `CWnd` objets plutôt que des handles de fenêtre pour la valeur de retour et le paramètre.
...
La taille de ligne affecte le déplacement du curseur pour les notifications de TB_LINEUP et de TB_LINEDOWN.
...
*nLength*\
dans Longueur du curseur, en pixels.

### <a name="remarks"></a>Notes

Cette méthode exige que le contrôle TrackBar ait la valeur [TBS_FIXEDLENGTH](/windows/win32/Controls/trackbar-control-styles) style.
...
Cette fonction membre implémente le comportement de la TBM_SETTIPSIDE de message Win32, comme décrit dans le SDK Windows. Les contrôles Slider qui utilisent le style TBS_TOOLTIPS affichent les info-bulles. Pour obtenir une description des styles de contrôle Slider, consultez la rubrique [styles de contrôle TrackBar](/windows/win32/Controls/trackbar-control-styles) dans le SDK Windows.
...
Cette fonction membre implémente le comportement de la [TBM_SETTOOLTIPS](/windows/win32/Controls/tbm-settooltips)de message Win32, comme décrit dans le SDK Windows. Lorsqu’un contrôle Slider est créé avec le style TBS_TOOLTIPS, il crée un contrôle ToolTip par défaut qui apparaît à côté du curseur, en affichant la position actuelle du curseur. Pour obtenir une description des styles de contrôle Slider, consultez la rubrique [styles de contrôle TrackBar](/windows/win32/Controls/trackbar-control-styles) dans le SDK Windows.
