---
title: Macros de table (MFC) du message | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
|[BEGIN_TEMPLATE_MESSAGE_MAP](#begin_template_interface_map)|Commence la définition d’une table des messages sur un type de classe contenant un seul argument de modèle. |
...
|[ON_UPDATE_COMMAND_UI_RANGE](#on_update_command_ui_range)|Indique quel gestionnaire de mise à jour gère la plage d’ID de commande spécifié dans les deux premières pa] paramè dans la macro.|
...
> [!NOTE]
>  En plus des messages définis par l’utilisateur, ON_MESSAGE gère les messages Windows moins courantes. Pour plus d’informations, consultez l’article de la Base de connaissances [99848 : INFO : utilisation de Macro ON_MESSAGE() aux Messages moins courantes de mappage](http://go.microsoft.com/fwlink/p/?linkid=192022).
