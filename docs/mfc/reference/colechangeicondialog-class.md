...
|[COleChangeIconDialog::DoModal](#domodal)|Affiche la boîte de dialogue Modifier l’icône OLE 2.|
...
- CIF_SELECTCURRENT spécifie que la case d’option actuelle sera sélectionnée initialement lorsque la boîte de dialogue sera appelée. Il s'agit de la valeur par défaut.
...
- CIF_SELECTFROMFILE spécifie que la case d’option from file est sélectionnée initialement lorsque la boîte de dialogue est appelée.

- CIF_SHOWHELP spécifie que le bouton aide s’affiche lorsque la boîte de dialogue est appelée.

- CIF_USEICONEXE spécifie que l’icône doit être extraite de l’exécutable spécifié dans `szIconExe` le champ de [m_ci](#m_ci) au lieu d’être récupérée à partir du type. Cela est utile pour l’incorporation ou la liaison à des fichiers non-OLE.
...
Si vous souhaitez initialiser les différents contrôles de boîte de dialogue en définissant les membres de la structure [m_ci](#m_ci) , vous devez le `DoModal`faire avant d’appeler, mais après la construction de l’objet de boîte de dialogue.
