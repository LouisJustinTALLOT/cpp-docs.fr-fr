---
description: 'En savoir plus sur : classe CMiniFrameWnd'
...
Pour plus d’informations sur l’utilisation des `CMiniFrameWnd` objets, consultez l’article sur les [barres d’outils ancrées et flottantes](../../mfc/docking-and-floating-toolbars.md).
...
- Définit le CS_DBLCLKS de type bit, qui envoie des messages de double-clic à la procédure de fenêtre lorsque l’utilisateur double-clique sur la souris.
...
Si le style de WS_VISIBLE est donné, Windows envoie tous les messages nécessaires à l’activation et à l’affichage de la fenêtre. Si le style de fenêtre spécifie une barre de titre, le titre de la fenêtre pointé par le paramètre *lpszWindowName* est affiché dans la barre de titre.
...
Les anciennes fenêtres de la boîte à outils de la palette de style ne sont plus prises en charge. L’ancien style, qui n’avait pas de bouton de fermeture « X », était pris en charge lors de l’exécution d’une application MFC sur des versions antérieures de Windows, mais n’est plus pris en charge dans Visual C++ .NET. Seul le nouveau style de WS_EX_TOOLWINDOW est désormais pris en charge ; pour obtenir une description de ce style, consultez [styles de fenêtre étendus](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles).
