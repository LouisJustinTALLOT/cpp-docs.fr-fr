...
ms.locfileid: "79416871"
---
# <a name="coblist-class"></a>CObList (classe)

fSupports des listes ordonnées de pointeurs `CObject` non uniques accessibles séquentiellement ou par valeur de pointeur.
...
|Name|Description|
|----------|-----------------|
|[CObList :: CObList](#coblist)|Construit une liste vide pour les pointeurs de `CObject`.|
...
les listes de `CObList` se comportent comme des listes doublement liées.
...
`CObList` incorpore la macro IMPLEMENT_SERIAL pour prendre en charge la sérialisation et le vidage de ses éléments. Si une liste de pointeurs de `CObject` est stockée dans une archive, soit avec un opérateur d’insertion surchargé, soit avec la fonction membre `Serialize`, chaque élément `CObject` est sérialisé à son tour.

Si vous avez besoin d’un vidage d’éléments `CObject` individuels dans la liste, vous devez définir la profondeur du contexte de vidage sur une valeur supérieure ou égale à 1.

Quand un objet `CObList` est supprimé ou que ses éléments sont supprimés, seuls les pointeurs de `CObject` sont supprimés, et non les objets qu’ils référencent.

Vous pouvez dériver vos propres classes à partir de `CObList`. Votre nouvelle classe de liste, conçue pour contenir des pointeurs vers des objets dérivés de `CObject`, ajoute de nouveaux membres de données et de nouvelles fonctions membres. Notez que la liste résultante n’est pas strictement de type sécurisé, car elle permet l’insertion de n’importe quel pointeur `CObject`.
...
*pNewList*<br/>
Pointeur vers une autre liste de `CObList`. Les éléments de *pNewList* seront ajoutés à cette liste.
...
### <a name="example"></a>Exemple
  ...
  Consultez [CObList :: CObList](#coblist) pour obtenir une liste de la classe `CAge`.
...
Construit une liste de pointeurs de `CObject` vide.
...
À mesure que la liste croît, la mémoire est allouée en unités d’entrées *nBlockSize* . En cas d’échec d’une allocation de mémoire, une `CMemoryException` est levée.
...
### <a name="example"></a>Exemple
  ...
  Vous trouverez ci-dessous une liste de la classe dérivée de `CObject``CAge` utilisée dans tous les exemples de collection :

[!code-cpp[NVC_MFCCollections#91](../../mfc/codesnippet/cpp/coblist-class_3.h)]

Voici un exemple d’utilisation de `CObList` constructeur :

[!code-cpp[NVC_MFCCollections#92](../../mfc/codesnippet/cpp/coblist-class_4.cpp)]

##  <a name="find"></a>CObList :: find

Recherche séquentiellement la liste pour trouver le premier pointeur de `CObject` correspondant au pointeur de `CObject` spécifié.
...
|Classe|Fonction membre|
|-----------|---------------------|
|[CPtrList](../../mfc/reference/cptrlist-class.md)|**Position trouve (void** <strong>\*</strong> `searchValue` **, position** `startAfter` **= null) const ;**|
...
*position*<br/>
Valeur de POSITION retournée par un appel de fonction membre `GetHeadPosition` ou `Find` précédent.
...
Ce n’est pas le même qu’un index, et vous ne pouvez pas utiliser une valeur POSITION vous-même. `GetAt` récupère le pointeur d' `CObject` associé à une position donnée.
...
Obtient le pointeur de `CObject` qui représente l’élément head de cette liste.
...
Si la liste est accessible via un pointeur vers un `const CObList`, `GetHead` retourne un pointeur `CObject`. Cela permet d’utiliser la fonction uniquement sur le côté droit d’une instruction d’assignation et de protéger ainsi la liste contre toute modification.

Si la liste est accessible directement ou par le biais d’un pointeur vers un `CObList`, `GetHead` retourne une référence à un pointeur `CObject`. Cela permet d’utiliser la fonction de chaque côté d’une instruction d’assignation, ce qui permet de modifier les entrées de liste.
...
Obtient l’élément de liste identifié par *rPosition*, puis affecte à *rPosition* la valeur `POSITION` de l’entrée suivante dans la liste.
...
*rPosition*<br/>
Référence à une valeur de POSITION retournée par une `GetNext`précédente, `GetHeadPosition`ou un autre appel de fonction membre.
...
Vous pouvez utiliser `GetNext` dans une boucle d’itération directe si vous établissez la position initiale avec un appel à `GetHeadPosition` ou `Find`.
...
> [!NOTE]
>  Depuis MFC 8,0, la version const de cette méthode a changé pour retourner `const CObject*` au lieu de `const CObject*&`.  Cette modification a été apportée pour mettre le compilateur en conformité avec C++ la norme.
...
*rPosition*<br/>
Référence à une valeur de POSITION retournée par une `GetPrev` précédente ou un autre appel de fonction membre.
...
Obtient le pointeur de `CObject` qui représente l’élément de fin de cette liste.
...
*position*<br/>
Valeur de POSITION retournée par un appel de fonction membre `GetNext`, `GetPrev`ou `Find` précédent.
...
Supprime tous les éléments de cette liste et libère la mémoire `CObList` associée.
...
Pointeur de `CObject` précédemment situé à l’en-tête de la liste.
...
Une variable de type POSITION est une clé pour la liste. Ce n’est pas le même qu’un index, et vous ne pouvez pas utiliser une valeur POSITION vous-même. `SetAt` écrit le pointeur `CObject` à la position spécifiée dans la liste.
