...
|Name|Description|
|----------|-----------------|
|[CPalette :: CPalette](#cpalette)|Construit un objet `CPalette` sans palette Windows attachée. Vous devez initialiser l’objet `CPalette` avec l’une des fonctions membres d’initialisation avant de pouvoir l’utiliser.|

### <a name="public-methods"></a>M&#233;thodes publiques

|Name|Description|
|----------|-----------------|
|[CPalette :: AnimatePalette](#animatepalette)|Remplace les entrées de la palette logique identifiée par l’objet `CPalette`. L’application n’a pas besoin de mettre à jour sa zone client, car Windows mappe immédiatement les nouvelles entrées dans la palette système.|
|[CPalette :: CreateHalftonePalette](#createhalftonepalette)|Crée une palette de demi-teintes pour le contexte de périphérique et l’attache à l’objet `CPalette`.|
|[CPalette :: CreatePalette](#createpalette)|Crée une palette de couleurs Windows et l’attache à l’objet `CPalette`.|
|[CPalette :: FromHandle](#fromhandle)|Retourne un pointeur vers un objet `CPalette` en fonction d’un handle vers un objet palette Windows.|
...
|Name|Description|
|----------|-----------------|
|[CPalette :: Operator HPALETTE](#operator_hpalette)|Retourne le HPALETTE attaché au `CPalette`.|
...
Un objet `CPalette` fournit des fonctions membres pour manipuler la palette référencée par l’objet. Construisez un objet `CPalette` et utilisez ses fonctions membres pour créer la palette réelle, un objet GDI (Graphics Device Interface) et manipuler ses entrées et d’autres propriétés.
...
La fonction `AnimatePalette` modifie uniquement les entrées avec l’indicateur PC_RESERVED défini dans le membre `palPaletteEntry` correspondant de la structure [LOGPALETTE](/windows/win32/api/wingdi/ns-wingdi-logpalette) jointe à l’objet `CPalette`. Pour plus d’informations sur cette structure, consultez LOGPALETTE dans le SDK Windows.
...
L’objet n’a pas de palette attachée tant que vous n’avez pas appelé `CreatePalette` pour en attacher un.
...
Initialise un objet `CPalette` en créant une palette de couleurs logiques Windows et en l’attachant à l’objet `CPalette`.
...
Pour plus d’informations sur la structure de `LOGPALETTE`, consultez la SDK Windows.

##  <a name="fromhandle"></a>CPalette :: FromHandle

Retourne un pointeur vers un objet `CPalette` en fonction d’un handle vers un objet palette Windows.
...
Utilisez cet opérateur pour récupérer le handle Windows GDI attaché de l’objet `CPalette`.
...
En cas de réussite, un handle vers l’objet Windows GDI représenté par l’objet `CPalette` ; Sinon, NULL.
...
Remplace la taille de la palette logique attachée à l’objet `CPalette` par le nombre d’entrées spécifié par *nNumEntries*.
...
Si la palette logique est sélectionnée dans un contexte de périphérique quand l’application appelle `SetPaletteEntries`, les modifications ne prendront pas effet tant que l’application n’aura pas appelé [CDC :: RealizePalette](../../mfc/reference/cdc-class.md#realizepalette).
