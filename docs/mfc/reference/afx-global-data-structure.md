---
description: 'En savoir plus sur : structure AFX_GLOBAL_DATA'
...
## <a name="afx_global_datagetitaskbarlist"></a><a name="getitaskbarlist"></a> AFX_GLOBAL_DATA :: les

Crée et stocke dans les données globales un pointeur vers l' `ITaskBarList` interface.
...
Pointeur vers l' `ITaskbarList` interface si la création d’un objet de liste de barre de tâches a échoué ; NULL si la création échoue ou si le système d’exploitation actuel est inférieur à Windows 7.

## <a name="afx_global_datagetitaskbarlist3"></a><a name="getitaskbarlist3"></a> AFX_GLOBAL_DATA :: GetITaskbarList3

Crée et stocke dans les données globales un pointeur vers l' `ITaskBarList3` interface.
...
Pointeur vers l' `ITaskbarList3` interface si la création d’un objet de liste de barre de tâches a échoué ; NULL si la création échoue ou si le système d’exploitation actuel est inférieur à Windows 7.
...
*ppv*<br/>
à Lorsque cette fonction est retournée, contient le pointeur d’interface demandé dans *riid*. Il s’agit généralement `IShellItem` de ou `IShellItem2` .
...
[Gestionnaire de fenêtres du Bureau](/windows/win32/dwm/dwm-overview)<br/>
