...
Si `DoModal` retourne IDOK, vous pouvez utiliser l’une `CColorDialog`des fonctions membres de pour récupérer les informations entrées par l’utilisateur.
...
`CColorDialog`s’appuie sur le COMMDLG. Fichier DLL fourni avec Windows versions 3,1 et ultérieures.

Pour personnaliser la boîte de dialogue, dérivez `CColorDialog`une classe de, fournissez un modèle de boîte de dialogue personnalisé et ajoutez une table des messages pour traiter les messages de notification des contrôles étendus. Tous les messages non traités doivent être passés à la classe de base.

La personnalisation de la fonction de raccordement n’est pas obligatoire.

> [!NOTE]
>  Sur certaines installations, `CColorDialog` l’objet ne s’affiche pas avec un arrière-plan gris si vous avez utilisé l' `CDialog` infrastructure pour rendre d’autres objets en grisé.

Pour plus d’informations sur `CColorDialog`l’utilisation de, consultez [classes de boîtes de dialogue communes](../../mfc/common-dialog-classes.md)
...
Si vous souhaitez initialiser les différentes options de boîte de dialogue de couleur en définissant les membres de la structure [m_cc](#m_cc) , vous devez effectuer `DoModal` cette opération avant d’appeler, mais après la construction de l’objet de boîte de dialogue.
...
Appelez cette fonction après avoir `DoModal` appelé pour forcer la sélection de couleur actuelle à la valeur de couleur spécifiée dans *CLR*.
