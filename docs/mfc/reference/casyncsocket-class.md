...
|[CAsyncSocket::Detach](#detach)|Détache un handle de socket d’un `CAsyncSocket` objet.|
...
|Nom|Description|
|----------|-----------------|
|[CAsyncSocket::OnAccept](#onaccept)|Avertit un socket d’écoute qu’il peut accepter les demandes de connexion `Accept`en attente en appelant.|
...
|[CAsyncSocket :: OnReceive](#onreceive)|Avertit un socket d’écoute qu’il y a des données à récupérer `Receive`en appelant.|
|[CAsyncSocket::OnSend](#onsend)|Avertit un socket qu’il peut envoyer des données en `Send`appelant.|
...
|[CAsyncSocket :: Operator, SOCKET](#operator_socket)|Utilisez cet opérateur pour récupérer le handle de socket de `CAsyncSocket` l’objet.|

### <a name="public-data-members"></a>Membres de données publics

|Nom|Description|
|----------|-----------------|
|[CAsyncSocket::m_hSocket](#m_hsocket)|Indique le handle de socket attaché à `CAsyncSocket` cet objet.|

## <a name="remarks"></a>Notes

La `CAsyncSocket` classe encapsule l’API des fonctions de socket Windows, en fournissant une abstraction orientée objet pour les programmeurs qui souhaitent utiliser Windows Sockets conjointement avec MFC.
...
Pour utiliser un `CAsyncSocket` objet, appelez son constructeur, puis appelez la fonction [Create](#create) pour créer le handle de Socket sous `SOCKET`-jacent (type), à l’exception des sockets acceptés. Pour un socket serveur, appelez la fonction membre [Listen](#listen) , et pour un socket client, appelez la fonction membre [Connect](#connect) . Le socket serveur doit appeler la fonction [Accept](#accept) lors de la réception d’une demande de connexion. Utilisez les fonctions `CAsyncSocket` restantes pour effectuer des communications entre les sockets. Une fois l’opération terminée `CAsyncSocket` , détruisez l’objet s’il a été créé sur le tas ; le destructeur appelle automatiquement la fonction [Close](#close) . Le type de données Socket est décrit dans l' [article Windows Sockets : Arrière](../../mfc/windows-sockets-background.md)-plan.
...
Pour plus d’informations, [consultez Windows Sockets : Utilisation de la](../../mfc/windows-sockets-using-class-casyncsocket.md) classe CAsyncSocket et des Articles connexes, ainsi que de l' [API Windows Sockets 2](/windows/win32/WinSock/windows-sockets-start-page-2).
...
Cette fonction est utilisée pour spécifier les fonctions de notification de rappel MFC qui seront appelées pour le Socket. `AsyncSelect`définit automatiquement ce socket en mode non bloquant. Pour plus d’informations, consultez l' [article Windows Sockets : Notifications](../../mfc/windows-sockets-socket-notifications.md)de Socket.

##  <a name="attach"></a>  CAsyncSocket::Attach

Appelez cette fonction membre pour attacher le handle *hSocket* à un `CAsyncSocket` objet.
...
L’adresse réseau, un nombre en pointillés tel que « 128.56.22.8 ». Le passage de la chaîne NULL pour ce paramètre `CAsyncSocket` indique que l’instance doit écouter l’activité des clients sur toutes les interfaces réseau.
...
- WSAEADDRINUSE l’adresse spécifiée est déjà utilisée. (Consultez l’option de socket SO_REUSEADDR sous [SetSockOpt](#setsockopt).)
...
Cette routine est utilisée sur un socket de datagramme ou de flux non connecté, avant `Connect` ou `Listen` après les appels suivants. Avant de pouvoir accepter les demandes de connexion, un socket de serveur d’écoute doit sélectionner un numéro de port et le rendre connu aux `Bind`sockets Windows en appelant. `Bind`établit l’association locale (adresse d’hôte/numéro de port) du socket en affectant un nom local à un socket sans nom.
...
Cette fonction libère le descripteur de socket afin que d’autres références à ce dernier échouent avec l’erreur WSAENOTSOCK. S’il s’agit de la dernière référence au Socket sous-jacent, les informations de nom et les données mises en file d’attente associées sont ignorées. Le destructeur de l’objet Socket vous `Close` appelle pour vous.

Pour `CAsyncSocket`, mais pas pour `CSocket`, la sémantique de est `Close` affectée par les options de socket SO_LINGER et SO_DONTLINGER. Pour plus d’informations, consultez fonction `GetSockOpt`membre.
...
Si le socket est indépendant, les valeurs uniques sont affectées à l’association locale par le système et le socket est marqué comme étant lié. Notez que si le champ d’adresse de la structure de nom ne comporte que `Connect` des zéros, retourne la valeur zéro. Pour afficher les informations d’erreur étendues `GetLastError` , appelez la fonction membre.
...
Pour un socket datagramme (type SOCK_DGRAM), une destination par défaut est définie, qui sera utilisée lors des `Send` appels `Receive` suivants et.

##  <a name="create"></a>  CAsyncSocket::Create

Appelez la `Create` fonction membre après avoir construit un objet Socket pour créer le socket Windows et l’attacher.
...
*lpszSockAddress*<br/>
Pointeur vers une chaîne contenant l’adresse réseau du socket connecté, un nombre en pointillés tel que « 128.56.22.8 ». Le passage de la chaîne NULL pour ce paramètre `CAsyncSocket` indique que l’instance doit écouter l’activité des clients sur toutes les interfaces réseau.
...
- SOCK_DGRAM prend en charge les datagrammes, qui sont des paquets non fiables, sans connexion, d’une longueur maximale fixe (généralement petite). Utilise le protocole UDP (User Datagram Protocol) pour la famille d’adresses Internet.
    ...
    > [!NOTE]
    >  La `Accept` fonction membre accepte une référence à un nouvel objet vide `CSocket` comme paramètre. Vous devez construire cet objet avant d’appeler `Accept`. Gardez à l’esprit que si cet objet Socket est hors de portée, la connexion se ferme. N’appelez `Create` pas pour ce nouvel objet Socket.
...
Pour plus d’informations sur les sockets de flux et de datagrammes, consultez les articles [Windows Sockets : Arrière](../../mfc/windows-sockets-background.md) - [plan et Windows Sockets : Les ports et les](../../mfc/windows-sockets-ports-and-socket-addresses.md) adresses de sockets et l' [API Windows Sockets 2](/windows/win32/WinSock/windows-sockets-start-page-2).

##  <a name="detach"></a>  CAsyncSocket::Detach

Appelez cette fonction membre pour détacher le handle de Socket dans le membre de `CAsyncSocket` données m_hSocket de l’objet et affectez à *m_hSocket* la valeur null.
...
Pointeur vers un `CAsyncSocket` objet, ou null s’il n’y a `CAsyncSocket` aucun objet attaché à *hSocket*.

### <a name="remarks"></a>Notes

Lorsqu’un handle de socket est fourni, `CAsyncSocket` si un objet n’est pas attaché au descripteur, la fonction membre retourne la valeur null.
...
- WSAEINVAL le socket n’a pas été lié à une adresse `Bind`avec.

### <a name="remarks"></a>Notes

Cet appel est particulièrement utile lorsqu’un `Connect` appel a été effectué sans passer par `Bind` un premier ; cet appel offre le seul moyen par lequel vous pouvez déterminer l’association locale qui a été définie par le système.
...
Différent de zéro si la fonction réussit ; Sinon, il peut s’agir de la valeur 0, et un code d’erreur spécifique peut être récupéré en appelant [GetLastError](#getlasterror). Si une option n’a jamais été `SetSockOpt`définie avec `GetSockOpt` , retourne la valeur par défaut de l’option. Les erreurs suivantes s’appliquent à cette fonction membre :
...
Les options suivantes sont prises en `GetSockOpt`charge pour. Le type identifie le type de données adressé par *lpOptionValue*. L’option TCP_NODELAY utilise le niveau IPPROTO_TCP ; toutes les autres options utilisent le niveau SOL_SOCKET.
...
Les options BSD (Berkeley Software Distribution) ne sont `GetSockOpt` pas prises en charge pour :
...
L' `GetSockOpt` appel de avec une option non prise en charge entraîne le renvoi d’un code d’erreur `GetLastError`WSAENOPROTOOPT à partir de.
...
- FIONBIO active ou désactive le mode non bloquant sur le Socket. Le paramètre *lpArgument* pointe vers un `DWORD`, qui est différent de zéro si le mode non bloquant est activé et zéro s’il doit être désactivé. Si `AsyncSelect` a été émis sur un socket, toute tentative d’utilisation `IOCtl` de pour redéfinir le socket en mode blocage échoue avec WSAEINVAL. Pour rétablir le socket en mode blocage et empêcher l’erreur WSAEINVAL, une application doit d’abord se `AsyncSelect` désactiver en appelant `AsyncSelect` avec le paramètre *lEvent* égal à 0 `IOCtl`, puis appeler.

- FIONREAD Déterminez le nombre maximal d’octets qui peuvent être lus à `Receive` l’aide d’un appel à partir de ce Socket. Le paramètre *lpArgument* pointe vers un `DWORD` dans lequel `IOCtl` stocke le résultat. Si ce Socket est de type SOCK_STREAM, FIONREAD retourne la quantité totale de données pouvant être lues dans un unique `Receive`; il s’agit normalement du même que la quantité totale de données en file d’attente sur le Socket. Si ce Socket est de type SOCK_DGRAM, FIONREAD retourne la taille du premier datagramme mis en file d’attente sur le Socket.

- SIOCATMARK déterminer si toutes les données hors bande ont été lues. Cela s’applique uniquement à un socket de type SOCK_STREAM qui a été configuré pour la réception en ligne de toutes les données hors bande (SO_OOBINLINE). Si aucune donnée hors bande n’est en attente de lecture, l’opération retourne une valeur différente de zéro. Dans le cas contraire, elle retourne 0 `Receive` , `ReceiveFrom` et le suivant ou effectué sur le socket récupère une partie ou la totalité des données précédant la « marque »; l’application doit utiliser l’opération SIOCATMARK pour déterminer si des données sont conservées. Si des données normales précèdent les données « urgentes » (hors bande), elles seront reçues dans l’ordre. (Notez qu’un `Receive` ou `ReceiveFrom` ne ne mélangera jamais de données hors bande et normales dans le même appel.) Le paramètre *lpArgument* pointe vers un `DWORD` dans lequel `IOCtl` stocke le résultat.

Cette fonction est un sous- `ioctl()` ensemble de utilisé dans les sockets Berkeley. En particulier, il n’y a aucune commande qui est équivalente à FIOASYNC, tandis que SIOCATMARK est la seule commande au niveau du socket qui est prise en charge.
...
- WSAEINVAL le socket n’a pas été lié `Bind` à ou est déjà connecté.
...
`Listen`tente de continuer à fonctionner de façon rationnelle lorsqu’il n’existe aucun port disponible (descripteurs). Il acceptera les connexions jusqu’à ce que la file d’attente soit vidée. Si les ports deviennent disponibles, un appel ultérieur `Listen` à `Accept` ou rechargera la file d’attente dans le « backlog » actuel ou le plus récent, si possible, et reprendre l’écoute des connexions entrantes.

##  <a name="m_hsocket"></a>  CAsyncSocket::m_hSocket

Contient le handle de socket pour le socket encapsulé par cet `CAsyncSocket` objet.
...
Pour plus d’informations, [consultez Windows Sockets : Notifications](../../mfc/windows-sockets-socket-notifications.md)de Socket.
...
> [!NOTE]
>  Dans [CSocket](../../mfc/reference/csocket-class.md), la `OnConnect` fonction de notification n’est jamais appelée. Pour les connexions, il vous `Connect`suffit d’appeler, qui retournera lorsque la connexion sera terminée (avec succès ou par erreur). La façon dont les notifications de connexion sont gérées est un détail d’implémentation MFC.
...
MFC prend en charge les données hors bande, mais les utilisateurs de `CAsyncSocket` la classe sont déconseillés de l’utiliser. Le moyen le plus simple consiste à créer un deuxième Socket pour transmettre ces données. Pour plus d’informations sur les données hors bande, consultez [Windows Sockets : Notifications](../../mfc/windows-sockets-socket-notifications.md)de Socket.

##  <a name="onreceive"></a>  CAsyncSocket::OnReceive

Appelé par l’infrastructure pour notifier à ce socket que des données dans la mémoire tampon peuvent être récupérées en `Receive` appelant la fonction membre.
...
Appelée par l’infrastructure pour informer le socket qu’il peut désormais envoyer des données en appelant `Send` la fonction membre.
...
Appelez cette fonction pour copier un objet `CAsyncSocket` existant dans un `CAsyncSocket` autre objet.

##  <a name="operator_socket"></a>CAsyncSocket :: Operator, SOCKET

Utilisez cet opérateur pour récupérer le handle de socket de `CAsyncSocket` l’objet.
...
- MSG_PEEK aperçu des données entrantes. Les données sont copiées dans la mémoire tampon, mais ne sont pas supprimées de la file d’attente d’entrée.

- MSG_OOB traite les données hors bande.

### <a name="return-value"></a>Valeur de retour

Si aucune erreur ne se `Receive` produit, retourne le nombre d’octets reçus. Si la connexion a été fermée, elle retourne 0. Sinon, une valeur de SOCKET_ERROR est retournée, et un code d’erreur spécifique peut être récupéré en appelant [GetLastError](#getlasterror). Les erreurs suivantes s’appliquent à cette fonction membre :
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d' `Receive` appeler sur un socket `ShutDown` après que a été appelé avec *nHow* défini sur 0 ou 2.

- WSAEWOULDBLOCK le socket est marqué comme étant non bloquant et l' `Receive` opération est bloquée.

- WSAEMSGSIZE le datagramme était trop grand pour tenir dans la mémoire tampon spécifiée et a été tronqué.

- WSAEINVAL le socket n’a pas été lié `Bind`.
...
Pour les sockets de type SOCK_STREAM, toutes les informations actuellement disponibles jusqu’à la taille de la mémoire tampon fournie sont retournées. Si le socket a été configuré pour la réception en ligne de données hors bande (option de socket SO_OOBINLINE) et si les données hors bande ne sont pas lues, seules les données hors bande sont retournées. L’application peut utiliser l' `IOCtlSIOCATMARK` option ou [OnOutOfBandData](#onoutofbanddata) pour déterminer si d’autres données hors bande restent à lire.

Pour les sockets de datagrammes, les données sont extraites du premier datagramme mis en file d’attente, jusqu’à la taille de la mémoire tampon fournie. Si le datagramme est plus grand que la mémoire tampon fournie, la mémoire tampon est remplie avec la première partie du datagramme, les données excédentaires sont `Receive` perdues et retourne la valeur SOCKET_ERROR avec le code d’erreur défini sur WSAEMSGSIZE. Si aucune donnée entrante n’est disponible au niveau du socket, la valeur SOCKET_ERROR est retournée avec le code d’erreur défini sur WSAEWOULDBLOCK. La fonction de rappel [OnReceive](#onreceive) peut être utilisée pour déterminer quand des données supplémentaires arrivent.

Si le socket est de type SOCK_STREAM et que le côté distant a arrêté la connexion normalement, un `Receive` se termine immédiatement avec 0 octets reçus. Si la connexion a été réinitialisée `Receive` , un échoue avec l’erreur WSAECONNRESET.

`Receive`doit être appelé une seule fois pour chaque fois que [CAsyncSocket :: OnReceive](#onreceive) est appelé.
...
Si aucune erreur ne se `ReceiveFrom` produit, retourne le nombre d’octets reçus. Si la connexion a été fermée, elle retourne 0. Sinon, une valeur de SOCKET_ERROR est retournée, et un code d’erreur spécifique peut être récupéré `GetLastError`en appelant. Les erreurs suivantes s’appliquent à cette fonction membre :
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d' `ReceiveFrom` appeler sur un socket `ShutDown` après que a été appelé avec *nHow* défini sur 0 ou 2.

- WSAEWOULDBLOCK le socket est marqué comme étant non bloquant et l' `ReceiveFrom` opération est bloquée.
...
Pour les sockets de type SOCK_STREAM, toutes les informations actuellement disponibles jusqu’à la taille de la mémoire tampon fournie sont retournées. Si le socket a été configuré pour la réception en ligne de données hors bande (option de socket SO_OOBINLINE) et si les données hors bande ne sont pas lues, seules les données hors bande sont retournées. L’application peut utiliser l' `IOCtlSIOCATMARK` option ou `OnOutOfBandData` pour déterminer si d’autres données hors bande restent à lire. Les paramètres *lpSockAddr* et *lpSockAddrLen* sont ignorés pour les sockets SOCK_STREAM.

Pour les sockets de datagrammes, les données sont extraites du premier datagramme mis en file d’attente, jusqu’à la taille de la mémoire tampon fournie. Si le datagramme est plus grand que la mémoire tampon fournie, la mémoire tampon est remplie avec la première partie du message, les données excédentaires sont `ReceiveFrom` perdues et retourne la valeur SOCKET_ERROR avec le code d’erreur défini sur WSAEMSGSIZE.

Si *lpSockAddr* est différent de zéro et que le socket est de type SOCK_DGRAM, l’adresse réseau du socket qui a envoyé les données est copiée dans la structure [sockaddr](/windows/win32/winsock/sockaddr-2) correspondante. La valeur pointée par *lpSockAddrLen* est initialisée à la taille de cette structure et est modifiée au retour pour indiquer la taille réelle de l’adresse qui y est stockée. Si aucune donnée entrante n’est disponible au niveau du `ReceiveFrom` Socket, l’appel attend que les données arrivent, sauf si le socket n’est pas bloqué. Dans ce cas, la valeur SOCKET_ERROR est retournée avec le code d’erreur défini sur WSAEWOULDBLOCK. Le `OnReceive` rappel peut être utilisé pour déterminer quand des données supplémentaires arrivent.
...
Si aucune erreur ne se `ReceiveFromEx` produit, retourne le nombre d’octets reçus. Si la connexion a été fermée, elle retourne 0. Sinon, une valeur de SOCKET_ERROR est retournée, et un code d’erreur spécifique peut être récupéré `GetLastError`en appelant. Les erreurs suivantes s’appliquent à cette fonction membre :
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d' `ReceiveFromEx` appeler sur un socket `ShutDown` après que a été appelé avec *nHow* défini sur 0 ou 2.

- WSAEWOULDBLOCK le socket est marqué comme étant non bloquant et l' `ReceiveFromEx` opération est bloquée.
...
Pour les sockets de datagrammes, les données sont extraites du premier datagramme mis en file d’attente, jusqu’à la taille de la mémoire tampon fournie. Si le datagramme est plus grand que la mémoire tampon fournie, la mémoire tampon est remplie avec la première partie du message, les données excédentaires sont `ReceiveFromEx` perdues et retourne la valeur SOCKET_ERROR avec le code d’erreur défini sur WSAEMSGSIZE.

Si *lpSockAddr* est différent de zéro et que le socket est de type SOCK_DGRAM, l’adresse réseau du socket qui a envoyé les données est copiée dans la structure [sockaddr](/windows/win32/winsock/sockaddr-2) correspondante. La valeur pointée par *lpSockAddrLen* est initialisée à la taille de cette structure et est modifiée au retour pour indiquer la taille réelle de l’adresse qui y est stockée. Si aucune donnée entrante n’est disponible au niveau du `ReceiveFromEx` Socket, l’appel attend que les données arrivent, sauf si le socket n’est pas bloqué. Dans ce cas, la valeur SOCKET_ERROR est retournée avec le code d’erreur défini sur WSAEWOULDBLOCK. Le `OnReceive` rappel peut être utilisé pour déterminer quand des données supplémentaires arrivent.
...
- MSG_OOB envoie des données hors bande (SOCK_STREAM uniquement).

### <a name="return-value"></a>Valeur de retour

Si aucune erreur ne se `Send` produit, retourne le nombre total de caractères envoyés. (Notez que ce nombre peut être inférieur au nombre indiqué par *nBufLen*.) Sinon, une valeur de SOCKET_ERROR est retournée, et un code d’erreur spécifique peut être récupéré en appelant [GetLastError](#getlasterror). Les erreurs suivantes s’appliquent à cette fonction membre :
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d' `Send` appeler sur un socket `ShutDown` après que a été appelé avec *nHow* défini sur 1 ou 2.
...
`Send`est utilisé pour écrire des données sortantes sur des sockets de flux ou de datagrammes connectés. Pour les sockets datagrammes, il faut veiller à ne pas dépasser la taille maximale des paquets IP des sous-réseaux sous-jacents, `iMaxUdpDg` qui est donnée par l’élément de `AfxSocketInit`la structure [wsadata,](/windows/win32/api/winsock2/ns-winsock2-wsadata) retournée par. Si les données sont trop longues pour être transmises atomiquement par le biais du protocole sous-jacent, l' `GetLastError`erreur WSAEMSGSIZE est retournée via et aucune donnée n’est transmise.

Notez que, pour un socket datagramme, l’exécution réussie `Send` d’un n’indique pas que les données ont été correctement remises.
...
Si aucune erreur ne se `SendTo` produit, retourne le nombre total de caractères envoyés. (Notez que ce nombre peut être inférieur au nombre indiqué par *nBufLen*.) Sinon, une valeur de SOCKET_ERROR est retournée, et un code d’erreur spécifique peut être récupéré en appelant [GetLastError](#getlasterror). Les erreurs suivantes s’appliquent à cette fonction membre :
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d' `SendTo` appeler sur un socket `ShutDown` après que a été appelé avec *nHow* défini sur 1 ou 2.
...
Si aucune erreur ne se `SendToEx` produit, retourne le nombre total de caractères envoyés. (Notez que ce nombre peut être inférieur au nombre indiqué par *nBufLen*.) Sinon, une valeur de SOCKET_ERROR est retournée, et un code d’erreur spécifique peut être récupéré en appelant [GetLastError](#getlasterror). Les erreurs suivantes s’appliquent à cette fonction membre :
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d' `SendToEx` appeler sur un socket `ShutDown` après que a été appelé avec *nHow* défini sur 1 ou 2.
...
Notez que l’exécution réussie d’un `SendToEx` n’indique pas que les données ont été correctement remises.
...
- La connexion WSAENETRESET a expiré lorsque SO_KEEPALIVE est défini.
...
- La connexion WSAENOTCONN a été réinitialisée lorsque SO_KEEPALIVE est défini.
...
Les options suivantes sont prises en `SetSockOpt`charge pour. Le type identifie le type de données adressé par *lpOptionValue*.
...
|SO_DONTLINGER|BOOL|Ne pas `Close` bloquer l’envoi des données non envoyées. La définition de cette option revient à affecter à `l_onoff` SO_LINGER la valeur zéro.|
|SO_DONTROUTE|BOOL|Ne pas Router : envoyer directement à l’interface.|
|SO_KEEPALIVE|BOOL|Envoyer des connexions persistantes.|
|SO_LINGER|`struct LINGER`|En attente de `Close` la présence de données non envoyées.|
...
Les options BSD (Berkeley Software Distribution) ne sont `SetSockOpt` pas prises en charge pour :
...
Notez que `ShutDown` ne ferme pas le socket et que les ressources attachées au socket ne sont pas libérées `Close` tant que n’est pas appelé. Une application ne doit pas s’appuyer sur la possibilité de réutiliser un socket une fois qu’il a été arrêté. En particulier, une implémentation de Windows Sockets n’est pas requise pour prendre en `Connect` charge l’utilisation de sur ce type de Socket.
...
*nSocketType*<br/>
`SOCK_STREAM` Spécifie `SOCK_DGRAM`ou.
...
Retourne `TRUE` en cas de `FALSE` réussite, en cas d’échec.
