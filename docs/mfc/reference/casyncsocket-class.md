---
description: 'En savoir plus sur : CAsyncSocket, classe'
...
|[CAsyncSocket :: CreateEx](#createex)|Crée un socket avec des options avancées.|
|[CAsyncSocket ::D Etach](#detach)|Détache un handle de socket d’un `CAsyncSocket` objet.|
...
|[CAsyncSocket :: Operator, SOCKET](#operator_socket)|Utilisez cet opérateur pour récupérer le handle de SOCKET de l' `CAsyncSocket` objet.|
...
Pour utiliser un `CAsyncSocket` objet, appelez son constructeur, puis appelez la fonction [Create](#create) pour créer le handle de Socket sous-jacent (type `SOCKET` ), à l’exception des sockets acceptés. Pour un socket serveur, appelez la fonction membre [Listen](#listen) , et pour un socket client, appelez la fonction membre [Connect](#connect) . Le socket serveur doit appeler la fonction [Accept](#accept) lors de la réception d’une demande de connexion. Utilisez les fonctions restantes `CAsyncSocket` pour effectuer des communications entre les sockets. Une fois l’opération terminée, détruisez l' `CAsyncSocket` objet s’il a été créé sur le tas ; le destructeur appelle automatiquement la fonction [Close](#close) . Le type de données SOCKET est décrit dans l’article [Windows Sockets : Background](../../mfc/windows-sockets-background.md).
...
L’adresse réseau, un nombre en pointillés tel que « 128.56.22.8 ». Le passage de la chaîne NULL pour ce paramètre indique que l' `CAsyncSocket` instance doit écouter l’activité des clients sur toutes les interfaces réseau.
...
Pour `CAsyncSocket` , mais pas pour `CSocket` , la sémantique de `Close` est affectée par les options de socket SO_LINGER et SO_DONTLINGER. Pour plus d’informations, consultez fonction membre `GetSockOpt` .
...
Pour un socket datagramme (type SOCK_DGRAM), une destination par défaut est définie, qui sera utilisée lors des `Send` appels suivants et `Receive` .
...
Appelez la `CreateEx` fonction membre après avoir construit un objet Socket pour créer le socket Windows et l’attacher.
...
*pAI*<br/>
Pointeur vers un [ADDRINFOT](/windows/win32/api/ws2def/ns-ws2def-addrinfoa) pour contenir les informations de socket telles que la famille et le type de Socket.
...
Appelez cette fonction membre pour détacher le handle de SOCKET dans le *m_hSocket* membre de données de l' `CAsyncSocket` objet et affectez à *m_hSocket* la valeur null.
...
Les options suivantes sont prises en charge pour `GetSockOpt` . Le type identifie le type de données adressé par *lpOptionValue*. L’option TCP_NODELAY utilise le niveau IPPROTO_TCP ; toutes les autres options utilisent le niveau SOL_SOCKET.
...
Appelé par l’infrastructure pour notifier à ce socket que des données dans la mémoire tampon peuvent être récupérées en appelant la `Receive` fonction membre.
...
Appelée par l’infrastructure pour informer le socket qu’il peut désormais envoyer des données en appelant la `Send` fonction membre.
...
Utilisez cet opérateur pour récupérer le handle de SOCKET de l' `CAsyncSocket` objet.
...
*nFlags*<br/>
Spécifie la façon dont l’appel est effectué. La sémantique de cette fonction est déterminée par les options de socket et le paramètre *nFlags* . Ce dernier est construit en combinant l’une des valeurs suivantes avec l’opérateur C++ **or** :
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d’appeler `Receive` sur un socket après que `ShutDown` a été appelé avec *nHow* défini sur 0 ou 2.
...
Si le socket est de type SOCK_STREAM et que le côté distant a arrêté la connexion normalement, un `Receive` se termine immédiatement avec 0 octets reçus. Si la connexion a été réinitialisée, un `Receive` échoue avec l’erreur WSAECONNRESET.
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d’appeler `ReceiveFrom` sur un socket après que `ShutDown` a été appelé avec *nHow* défini sur 0 ou 2.
...
Pour les sockets de datagrammes, les données sont extraites du premier datagramme mis en file d’attente, jusqu’à la taille de la mémoire tampon fournie. Si le datagramme est plus grand que la mémoire tampon fournie, la mémoire tampon est remplie avec la première partie du message, les données excédentaires sont perdues et `ReceiveFrom` retourne la valeur SOCKET_ERROR avec le code d’erreur défini sur WSAEMSGSIZE.

Si *lpSockAddr* est différent de zéro et que le socket est de type SOCK_DGRAM, l’adresse réseau du socket qui a envoyé les données est copiée dans la structure [sockaddr](/windows/win32/winsock/sockaddr-2) correspondante. La valeur pointée par *lpSockAddrLen* est initialisée à la taille de cette structure et est modifiée au retour pour indiquer la taille réelle de l’adresse qui y est stockée. Si aucune donnée entrante n’est disponible au niveau du socket, l' `ReceiveFrom` appel attend que les données arrivent, sauf si le socket n’est pas bloqué. Dans ce cas, la valeur SOCKET_ERROR est retournée avec le code d’erreur défini sur WSAEWOULDBLOCK. Le `OnReceive` rappel peut être utilisé pour déterminer quand des données supplémentaires arrivent.
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d’appeler `ReceiveFromEx` sur un socket après que `ShutDown` a été appelé avec *nHow* défini sur 0 ou 2.
...
Pour les sockets de datagrammes, les données sont extraites du premier datagramme mis en file d’attente, jusqu’à la taille de la mémoire tampon fournie. Si le datagramme est plus grand que la mémoire tampon fournie, la mémoire tampon est remplie avec la première partie du message, les données excédentaires sont perdues et `ReceiveFromEx` retourne la valeur SOCKET_ERROR avec le code d’erreur défini sur WSAEMSGSIZE.

Si *lpSockAddr* est différent de zéro et que le socket est de type SOCK_DGRAM, l’adresse réseau du socket qui a envoyé les données est copiée dans la structure [sockaddr](/windows/win32/winsock/sockaddr-2) correspondante. La valeur pointée par *lpSockAddrLen* est initialisée à la taille de cette structure et est modifiée au retour pour indiquer la taille réelle de l’adresse qui y est stockée. Si aucune donnée entrante n’est disponible au niveau du socket, l' `ReceiveFromEx` appel attend que les données arrivent, sauf si le socket n’est pas bloqué. Dans ce cas, la valeur SOCKET_ERROR est retournée avec le code d’erreur défini sur WSAEWOULDBLOCK. Le `OnReceive` rappel peut être utilisé pour déterminer quand des données supplémentaires arrivent.
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d’appeler `Send` sur un socket après que `ShutDown` a été appelé avec *nHow* défini sur 1 ou 2.
...
Notez que, pour un socket datagramme, l’exécution réussie d’un `Send` n’indique pas que les données ont été correctement remises.

Sur les `CAsyncSocket` objets de type SOCK_STREAM, le nombre d’octets écrits peut être compris entre 1 et la longueur demandée, en fonction de la disponibilité de la mémoire tampon sur les hôtes locaux et étrangers.
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d’appeler `SendTo` sur un socket après que `ShutDown` a été appelé avec *nHow* défini sur 1 ou 2.
...
- WSAESHUTDOWN le socket a été arrêté ; Il n’est pas possible d’appeler `SendToEx` sur un socket après que `ShutDown` a été appelé avec *nHow* défini sur 1 ou 2.
...
SO_LINGER contrôle l’action entreprise lorsque les données non envoyées sont mises en file d’attente sur un socket et que la `Close` fonction est appelée pour fermer le Socket.
...
Pour informer l’implémentation de Windows Sockets qu’un `Bind` appel sur un socket ne doit pas être interdit parce que l’adresse souhaitée est déjà utilisée par un autre Socket, l’application doit définir l’option de socket SO_REUSEADDR pour le socket avant d’émettre l' `Bind` appel. Notez que l’option est interprétée uniquement au moment de l' `Bind` appel : il est donc inutile (mais inoffensif) de définir l’option sur un socket qui ne doit pas être lié à une adresse existante, et la définition ou la réinitialisation de l’option après l' `Bind` appel n’a aucun effet sur ce Socket ou sur un autre.
...
Les options suivantes sont prises en charge pour `SetSockOpt` . Le type identifie le type de données adressé par *lpOptionValue*.
...
|SO_DONTLINGER|BOOL|Ne pas bloquer l' `Close` envoi des données non envoyées. La définition de cette option équivaut à définir SO_LINGER avec la `l_onoff` valeur zéro.|
...
- `FD_CONNECT`: Voulez recevoir une notification de la connexion terminée.
