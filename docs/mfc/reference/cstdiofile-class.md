---
description: 'En savoir plus sur : CStdioFile, classe'
...
Le mode texte fournit un traitement spécial pour les paires retour chariot-saut de ligne. Lorsque vous écrivez un caractère de saut de ligne (nouvelle ligne) (0x0A) sur un objet en mode texte `CStdioFile` , la paire d’octets (0x0D, 0x0A) est envoyée au fichier. Lorsque vous lisez, la paire d’octets (0x0D, 0x0A) est traduite en un seul octet 0x0A.

Les [](../../mfc/reference/cfile-class.md) fonctions CFile [duplicate](../../mfc/reference/cfile-class.md#duplicate), [LockRange](../../mfc/reference/cfile-class.md#lockrange)et [UnlockRange](../../mfc/reference/cfile-class.md#unlockrange) ne sont pas prises en charge pour `CStdioFile` .
...
Pour plus d’informations sur l’utilisation de `CStdioFile` , consultez les articles [fichiers dans MFC](../../mfc/files-in-mfc.md) et [gestion des fichiers](../../c-runtime-library/file-handling.md) dans la référence de la *bibliothèque Runtime*.
...
Le constructeur par défaut n’attache pas de fichier à l' `CStdioFile` objet. Lorsque vous utilisez ce constructeur, vous devez utiliser la `CStdioFile::Open` méthode pour ouvrir un fichier et l’attacher à l' `CStdioFile` objet.

Le constructeur à un seul paramètre attache un flux de fichier ouvert à l' `CStdioFile` objet. Les valeurs de pointeur autorisées incluent les pointeurs de fichier d’entrée/sortie ( *stdin*, *stdout* ou *stderr*) prédéfinis.
...
Si vous transmettez la valeur NULL pour *pOpenStream* ou *lpszFileName*, le constructeur lève une exception `CInvalidArgException*` .

Si le fichier ne peut pas être ouvert ou créé, le constructeur lève une exception `CFileException*` .
...
Le `m_pStream` membre de données est le pointeur vers un fichier ouvert tel qu’il est retourné par la fonction Runtime C `fopen` .
...
Lit les données de texte dans une mémoire tampon, jusqu’à une limite de *nmax*-1 caractères, à partir du fichier associé à l' `CStdioFile` objet.
...
Si la position demandée est conforme, `Seek` retourne le nouvel offset d’octet à partir du début du fichier. Dans le cas contraire, la valeur de retour n’est pas définie et un `CFileException` objet est levé.
...
Cette implémentation de `Seek` est basée sur la fonction de bibliothèque de Run-Time (CRT) `fseek` . Il existe plusieurs limites sur l’utilisation de `Seek` sur des flux ouverts en mode texte. Pour plus d’informations, consultez [fseek, _fseeki64](../../c-runtime-library/reference/fseek-fseeki64.md).

### <a name="example"></a>Exemple

L’exemple suivant montre comment utiliser `Seek` pour déplacer le pointeur 1000 octets à partir du début du `cfile` fichier. Notez que `Seek` ne lit pas les données. vous devez donc appeler [CStdioFile :: ReadString](#readstring) pour lire les données.

[!code-cpp[NVC_MFCFiles#39](../../atl-mfc-shared/reference/codesnippet/cpp/cstdiofile-class_3.cpp)]

## <a name="cstdiofilewritestring"></a><a name="writestring"></a> CStdioFile :: WriteString

Écrit des données à partir d’une mémoire tampon dans le fichier associé à l' `CStdioFile` objet.
...
Si vous souhaitez écrire des données qui ne se terminent pas par un caractère NULL dans un fichier, utilisez `CStdioFile::Write` ou [CFile :: Write](../../mfc/reference/cfile-class.md#write).

Cette méthode lève une exception `CInvalidArgException*` si vous spécifiez NULL pour le paramètre *lpsz* .

Cette méthode lève une exception `CFileException*` en réponse à des erreurs du système de fichiers.
...
[CFile, classe](../../mfc/reference/cfile-class.md)<br/>
[CFile ::D quée](../../mfc/reference/cfile-class.md#duplicate)<br/>
