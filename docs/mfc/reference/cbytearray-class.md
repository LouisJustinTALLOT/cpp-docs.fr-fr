...
`CByteArray` incorpore la macro IMPLEMENT_SERIAL pour prendre en charge la sérialisation et le vidage de ses éléments. Si un tableau d’octets est stocké dans une archive, soit avec l’opérateur d’insertion surchargé ( **<<** ), soit avec la fonction membre `Serialize`, chaque élément est, à son tour, sérialisé.
