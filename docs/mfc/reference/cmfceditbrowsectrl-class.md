...
|`CMFCEditBrowseCtrl::PreTranslateMessage`|Traduit les messages de fenêtre avant qu’ils soient distribués à le [TranslateMessage](https://msdn.microsoft.com/library/windows/desktop/ms644955) et [DispatchMessage](https://msdn.microsoft.com/library/windows/desktop/ms644934) des fonctions de Windows. Pour la syntaxe et plus d’informations, consultez [CWnd::PreTranslateMessage](../../mfc/reference/cwnd-class.md#pretranslatemessage).|  
...
2.  Soit incorporer la `CMFCEditBrowseCtrl` objet ou l’objet de contrôle de recherche de modification dérivée dans l’objet de fenêtre parent.  
...
4.  Si vous ajoutez un contrôle de navigation d’édition à une boîte de dialogue, utilisez le **ClassWizard** outil pour configurer l’échange de données.  
...
7.  Pour supprimer le bouton Parcourir à partir du contrôle de navigation de modification, appelez le [EnableBrowseButton](#enablebrowsebutton) méthode avec le *bActivez* paramètre défini sur FALSE.  
