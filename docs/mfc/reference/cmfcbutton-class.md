---
description: 'En savoir plus sur : classe CMFCButton'
...
|[CMFCButton :: m_bHighlightChecked](#m_bhighlightchecked)|Indique s’il faut mettre en surbrillance un bouton de style BS_CHECKBOX lorsque le curseur pointe dessus.|
...
D’autres types de boutons sont dérivés de la `CMFCButton` classe, tels que la classe [CMFCURLLinkButton](../../mfc/reference/cmfclinkctrl-class.md) , qui prend en charge les liens hypertexte et la `CMFCColorButton` classe, qui prend en charge une boîte de dialogue Sélecteur de couleurs.
...
Créez un contrôle Button soit directement dans votre code, soit à l’aide de l’outil de l' **Assistant classe MFC** et d’un modèle de boîte de dialogue. Si vous créez un contrôle Button directement, ajoutez une `CMFCButton` variable à votre application, puis appelez le constructeur et les `Create` méthodes de l' `CMFCButton` objet. Si vous utilisez l' **Assistant classe MFC**, ajoutez une `CButton` variable à votre application, puis remplacez le type de la variable par `CButton` `CMFCButton` .

Pour gérer les messages de notification dans une application de boîte de dialogue, ajoutez une entrée de la table des messages et un gestionnaire d’événements pour chaque notification. Les notifications envoyées par un `CMFCButton` objet sont les mêmes que celles envoyées par un `CButton` objet.
...
Cette méthode affecte tous les boutons de votre application qui sont dérivés de la `CMFCButton` classe.
...
TRUE si le bouton a un style BS_CHECKBOX ou BS_AUTOCHECKBOX ; Sinon, FALSe.
...
Affectez la `m_bDrawFocus` valeur true au membre pour spécifier que le Framework dessinera un rectangle de focus autour du texte et de l’image du bouton si le bouton reçoit le focus.
...
Affectez la `m_bHighlightChecked` valeur true au membre pour spécifier que le Framework met en surbrillance un bouton de style BS_CHECKBOX lorsque la souris pointe sur lui.
...
Affectez la `m_bRightImage` valeur true au membre pour spécifier que le Framework affichera l’image du bouton à droite de l’étiquette de texte du bouton.
...
Affectez la `m_bTransparent` valeur true au membre pour spécifier que le Framework rendra le bouton transparent. Le `CMFCButton` constructeur initialise ce membre à false.
...
Utilisez l’une des `CMFCButton::AlignStyle` valeurs d’énumération suivantes pour spécifier l’alignement du texte du bouton :
...
*uiState*<br/>
dans État actuel du bouton. Pour plus d’informations, consultez le `itemState` membre de la rubrique de la [structure drawitemstruct,](/windows/win32/api/winuser/ns-winuser-drawitemstruct) .
...
*bMap3dColors*<br/>
dans Spécifie une couleur transparente pour l’arrière-plan du bouton ; autrement dit, la face du bouton. TRUE pour utiliser la valeur de couleur RVB (192, 192, 192); FALSe pour utiliser la valeur de couleur définie par `AFX_GLOBAL_DATA::clrBtnFace` .
...
*id*<br/>
dans Un des identificateurs d’image de bouton qui est défini dans l' `CMenuImage::IMAGES_IDS` énumération. Les valeurs de l’image spécifient des images telles que des flèches, des broches et des cases d’option.

*state*<br/>
dans Un des identificateurs d’état d’image de bouton qui est défini dans l' `CMenuImages::IMAGE_STATE` énumération. Les États d’image spécifient des couleurs de bouton telles que le noir, le gris, le gris clair, le blanc et le gris foncé. La valeur par défaut est `CMenuImages::ImageBlack`.

*idDisabled*<br/>
dans Un des identificateurs d’image de bouton qui est défini dans l' `CMenuImage::IMAGES_IDS` énumération. L’image indique que le bouton est désactivé. La valeur par défaut est la première image de bouton ( `CMenuImages::IdArrowDown` ).
