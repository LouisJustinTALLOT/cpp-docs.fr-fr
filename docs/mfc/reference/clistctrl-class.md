...
*pParentWnd*<br/>
Spécifie la fenêtre parente du contrôle de liste, généralement `CDialog` . Il ne doit pas être NULL.
...
Crée un contrôle (une fenêtre enfant) et l’associe à l' `CListCtrl` objet.
...
`CreateEx` crée le contrôle avec les styles Windows étendus spécifiés par *dwExStyle*. Pour définir des styles étendus spécifiques à un contrôle, appelez [SetExtendedStyle](#setextendedstyle). Par exemple, utilisez `CreateEx` pour définir des styles de ce type comme WS_EX_CONTEXTHELP, mais utilisez `SetExtendedStyle` pour définir des styles de ce type comme LVS_EX_FULLROWSELECT. Pour plus d’informations, consultez les styles décrits dans l’article [styles de vue de liste étendue](/windows/win32/Controls/extended-list-view-styles) dans le SDK Windows.
...
Par défaut, cette fonction membre ne fait rien. Substituez cette fonction membre pour implémenter le dessin pour un objet owner-draw `CListCtrl` .
...
*iIndex*\
dans Index de base zéro d’un groupe.
...
L’exemple de code suivant illustre la `GetGroupInfoByIndex` méthode. Dans une section précédente de cet exemple de code, nous avons créé un contrôle List-View qui affiche deux colonnes intitulées « ClientID » et « grade » dans une vue rapport. L’exemple de code suivant récupère des informations sur le groupe dont l’index est 0, si ce groupe existe.
...
*iGroupId*\
dans Spécifie un groupe.

*lpRect*\
[in, out] Pointeur vers une structure [Rect](/windows/win32/api/windef/ns-windef-rect) . Si cette méthode réussit, la structure reçoit les coordonnées de rectangle du groupe spécifié par *iGroupId*.

*iCoords*\
dans Spécifie les coordonnées de rectangle à récupérer. Utilisez l’une des valeurs suivantes :

- Coordonnées LVGGR_GROUP (par défaut) de l’ensemble du groupe développé.

- LVGGR_HEADER-coordonnées de l’en-tête (groupe réduit).
...
L’exemple de code suivant illustre la `GetGroupRect` méthode. Dans une section précédente de cet exemple de code, nous avons créé un contrôle List-View qui affiche deux colonnes intitulées « ClientID » et « grade » dans une vue rapport. L’exemple de code suivant dessine un rectangle 3D autour du groupe dont l’index est 0, si ce groupe existe.
...
*pItemIndex*\
dans Pointeur vers une structure [LVITEMINDEX](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) pour l’élément parent du sous-élément. L’appelant est chargé d’allouer et de définir les membres de la structure [LVITEMINDEX](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) . Ce paramètre ne peut pas être NULL.

*iColumn*\
dans Index de base zéro d’une colonne dans le contrôle.

*rectType*\
dans Partie du sous-élément de vue de liste pour lequel le rectangle englobant est récupéré. Spécifiez l'une des valeurs suivantes :
...
- LVIR_LABEL-retourne le rectangle englobant du texte du sous-élément.
...
L’exemple de code suivant illustre la `GetGroupRect` méthode. Avant d’entrer dans cet exemple de code, nous avons créé un contrôle List-View qui affiche deux colonnes intitulées « ClientID » et « grade » dans une vue rapport. L’exemple de code suivant dessine un rectangle 3D autour du deuxième sous-élément dans les deux colonnes.
...
*fSmall*\
dans Vue pour laquelle récupérer l’espacement des éléments. Spécifiez TRUE pour la vue petite icône ou FALSe pour la vue icône.

*pnHorzSpacing*\
à Contient l’espacement horizontal entre les éléments.
...
*pItemIndex*\
[in, out] Pointeur vers la structure [LVITEMINDEX](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) qui décrit l’élément où la recherche commence, ou-1 pour rechercher le premier élément qui correspond aux indicateurs dans le paramètre *nFlags* . Si cette méthode réussit, la `LVITEMINDEX` structure décrit l’élément trouvé par la recherche.
...
Vous pouvez tester la valeur LVHT_ONITEM du membre de la structure `flag` pour déterminer si une position donnée se trouve sur un élément de la vue liste. Cette valeur est une opération or au niveau du bit sur le LVHT_ONITEMICON, LVHT_ONITEMLABEL et LVHT_ONITEMSTATEICON valeurs du membre de la structure `flag` .
...
*pPoint*<br/>
Pointeur vers une structure de [points](/windows/win32/api/windef/ns-windef-point) qui contient les coordonnées de test de positionnement par rapport à la zone cliente du contrôle de liste.
...
*pItem*<br/>
Adresse d’une structure [LVITEM](/windows/win32/api/commctrl/ns-commctrl-lvitemw) qui contient les nouveaux attributs d’élément, comme décrit dans la SDK Windows. Les membres et de la structure `iItem` `iSubItem` identifient l’élément ou le sous-élément, et le membre de la structure `mask` spécifie les attributs à définir. Pour plus d’informations sur le `mask` membre, consultez les **Notes**.
...
- LVIF_TEXT le `pszText` membre ou le paramètre *lpszItem* est l’adresse d’une chaîne terminée par le caractère null ; le `cchTextMax` membre est ignoré.

- LVIF_STATE le `stateMask` paramètre member ou *nStateMask* spécifie l’état des éléments à modifier et le `state` paramètre member ou *nState* contient les valeurs de ces États.
...
*pItemIndex*\
dans Pointeur vers une structure [LVITEMINDEX](/windows/win32/api/commctrl/ns-commctrl-lvitemindex) qui décrit un élément. L’appelant est chargé d’allouer cette structure et de définir ses membres.

*dwState*\
dans État pour définir l’élément, qui est une combinaison d’opérations de bits d' [États d’élément d’affichage de liste](/windows/win32/Controls/list-view-item-states). Spécifiez zéro pour réinitialiser, ou un État à définir.
...
Pour ne pas utiliser les info-bulles, indiquez le style de LVS_NOTOOLTIPS lorsque vous créez l' `CListCtrl` objet.
...
L’exemple de code suivant illustre la `SortItemEx` méthode. Dans une section précédente de cet exemple de code, nous avons créé un contrôle List-View qui affiche deux colonnes intitulées « ClientID » et « grade » dans une vue rapport. L’exemple de code suivant trie la table à l’aide des valeurs de la colonne « grade ».
