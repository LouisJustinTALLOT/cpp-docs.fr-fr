...
- Vues de liste virtuelle
   ...
   Lorsque l’option est activée, permet au contrôle prendre en charge les éléments DWORD au maximum. Cela est possible en plaçant la surcharge de la gestion des données de l’élément sur l’application. À l’exception de la sélection d’éléments et les informations de focus, toutes les informations de l’élément doivent être gérées par l’application. Pour plus d’informations, consultez [CListCtrl à l’aide de : Contrôles de liste virtuels](../../mfc/virtual-list-controls.md).
...
Pour plus d’informations sur l’utilisation des nouveaux des styles étendus, consultez [CListCtrl à l’aide de : Modification des Styles de contrôle de liste](../../mfc/changing-list-control-styles.md).
...
Par défaut, le contrôle list view est responsable du stockage des attributs d’icône et le texte d’un élément. Toutefois, en plus de ces types d’éléments, la classe `CListCtrl` prend en charge les « éléments de rappel ». Un élément de « rappel » est un élément de liste pour lequel l’application, plutôt que le contrôle, stocke le texte, icône ou les deux. Un masque de rappel est utilisé pour spécifier les attributs de l’élément (texte et/ou l’icône) sont fournies par l’application. Si une application utilise des éléments de rappel, il doit être en mesure de fournir les attributs de texte et/ou l’icône à la demande. Éléments de rappel sont utiles lorsque votre application a déjà gère certaines de ces informations. Pour plus d’informations, consultez [CListCtrl à l’aide de : Éléments de rappel et masque de rappel](../callback-items-and-the-callback-mask.md).
...
Par défaut, un contrôle list view détruit les listes d’images qui lui est assignées lorsqu’il est détruit ; Toutefois, le développeur peut personnaliser ce comportement en détruisant chaque liste d’images lorsqu’il n’est plus utilisé, tel que déterminé par l’application. Pour plus d’informations, consultez [CListCtrl à l’aide de : Éléments de liste et listes d’images](../list-items-and-image-lists.md).
...
Une combinaison des styles étendus en cours d’utilisation par la liste de contrôle view. Pour obtenir une liste descriptive des ces styles étendus, consultez la [Styles étendus de la vue liste](/windows/desktop/Controls/extended-list-view-styles) article dans le SDK Windows.
...
Cette méthode envoie le [LVM_GETGROUPINFOBYINDEX](/windows/desktop/controls/lvm-getgroupinfobyindex) message, qui est décrite dans le SDK Windows-->.
...
L’exemple de code suivant montre le `GetGroupInfoByIndex` (méthode). Dans la section précédente de cet exemple de code, nous avons créé un contrôle d’affichage de liste qui affiche deux colonnes intitulées « ClientID » et « Grade » dans une vue de rapport. L’exemple de code suivant récupère des informations sur le groupe dont l’index est 0, si un tel groupe existe.
```cpp
    // GetGroupInfoByIndex
...
|Paramètre|Description|
|---------------|-----------------|
|*pItemIndex*|[in] Pointeur vers un [LVITEMINDEX](/windows/desktop/api/commctrl/ns-commctrl-lvitemindex) structure pour l’élément parent du sous-élément.<br /><br /> L’appelant est chargé d’allouer et définissant les membres de la [LVITEMINDEX](/windows/desktop/api/commctrl/ns-commctrl-lvitemindex) structure. Ce paramètre ne peut pas être NULL.|
...
Cette méthode envoie le [LVM_GETNEXTITEMINDEX](/windows/desktop/controls/lvm-getnextitemindex) message, qui est décrite dans le SDK Windows.
...
*nWorkAreas*<br/>
Le nombre de `RECT` structures contenues dans le *pRect* tableau.
...
*nState*<br/>
Indique l’état de l’élément, l’image de l’état et image de superposition. Pour plus d’informations, consultez les rubriques du Kit de développement logiciel Windows [LVITEM Structure](/windows/desktop/api/commctrl/ns-commctrl-taglvitema) et [États d’affichage de liste élément](/windows/desktop/Controls/list-view-item-states) pour obtenir la liste d’indicateurs valides.

*nStateMask*<br/>
Indique les bits du membre état seront récupérées ou modifiées. Pour plus d’informations, consultez [LVITEM Structure](/windows/desktop/api/commctrl/ns-commctrl-taglvitema) dans le SDK Windows.
...
Cette méthode envoie le [LVM_MAPIDTOINDEX](/windows/desktop/controls/lvm-mapidtoindex) message, qui est décrite dans le SDK Windows.
...
|Paramètre|Description|
|---------------|-----------------|
|*pItemIndex*|[in] Pointeur vers un [LVITEMINDEX](/windows/desktop/api/commctrl/ns-commctrl-lvitemindex) structure qui décrit un élément. L’appelant est chargé d’allouer cette structure et de définition de ses membres.|
