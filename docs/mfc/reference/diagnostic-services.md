...
|[CONFIRMATION](#verify)|Semblable à ASSERT, mais évalue l’expression dans la versions Release et Debug de la bibliothèque.|

### <a name="mfc-general-diagnostic-variables-and-functions"></a>Fonctions et variables de diagnostic général MFC

|||
|-|-|
|[afxDump](#afxdump)|Variable globale qui envoie des informations [CDumpContext](../../mfc/reference/cdumpcontext-class.md) à la fenêtre sortie du débogueur ou au terminal de débogage.|
...
|[AfxDoForAllObjects](#afxdoforallobjects)|Exécute une fonction spécifiée sur tous les `CObject` objets dérivés de qui ont été alloués avec **`new`** .|
...
## <a name="afxdebugbreak"></a><a name="afxdebugbreak"></a>Spéciale AfxDebugBreak,
...
Dans votre programme, vous pouvez utiliser DEBUG_NEW partout où vous utiliseriez généralement l' **`new`** opérateur pour allouer le stockage du tas.
...
Une fois que vous avez inséré cette directive, le préprocesseur insère des DEBUG_NEW où que vous utilisiez **`new`** , et MFC fait le reste. Quand vous compilez une version Release de votre programme, DEBUG_NEW se résout en une **`new`** opération simple, et les informations de nom de fichier et de numéro de ligne ne sont pas générées.
...
### <a name="ensure-and-ensure_valid"></a><a name="ensure"></a>Vérifiez et ENSURE_VALID
...
## <a name="verify"></a><a name="verify"></a>CONFIRMATION
...
`AfxDump`appelle la fonction membre d’un objet `Dump` et envoie les informations à l’emplacement spécifié par la `afxDump` variable. `AfxDump`est disponible uniquement dans la version Debug de MFC.
...
Tous les blocs de mémoire actuellement alloués sur le tas sont vérifiés, y compris ceux alloués par **`new`** mais pas ceux alloués par les appels directs aux allocateurs de mémoire sous-jacents, tels que la fonction **malloc** ou la `GlobalAlloc` fonction Windows. Si un bloc est endommagé, un message est affiché dans la sortie du débogueur.

Si vous incluez la ligne

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

dans un module de programme, les appels suivants pour `AfxCheckMemory` afficher le nom de fichier et le numéro de ligne où la mémoire a été allouée.

> [!NOTE]
> Si votre module contient une ou plusieurs implémentations de classes sérialisables, vous devez placer la `#define` ligne après la dernière IMPLEMENT_SERIAL appel de macro.
...
L’exemple ci-dessous reflète une seule ligne de la sortie générée par l’appel `AfxDumpStack` d’un gestionnaire de bouton dans une application de boîte de dialogue MFC :
...
|`+ 181 bytes`|Offset, en octets, de l’adresse du prototype de fonction (dans ce cas, `void AfxDumpStack(unsigned long)` ) à l’adresse de retour (dans ce cas, `00427D55` ).|
...
- Le fichier IMAGEHLP.DLL doit se trouver sur votre chemin d’accès. Si vous n’avez pas cette DLL, la fonction affichera un message d’erreur. Pour plus d’informations sur l’ensemble de fonctions fourni par IMAGEHLP, consultez [bibliothèque d’aide](/windows/win32/Debug/image-help-library) sur les images.
...
*plRequestNumber*<br/>
Pointe vers un **`long`** entier qui sera renseigné avec le numéro de séquence d’allocation du bloc de mémoire, ou zéro s’il ne représente pas un bloc de mémoire actuellement actif.
...
`CObject`Les classes dérivées de Serializable sont des classes dérivées à l’aide de la macro DECLARE_SERIAL. Le pointeur passé à `AfxDoForAllClasses` dans *pContext* est passé à la fonction d’itération spécifiée chaque fois qu’il est appelé.
