...
Ces macros et fonctions sont disponibles pour toutes les classes dérivées de `CObject` dans les versions Debug et Release de MFC. Toutefois, tous sauf DEBUG_NEW et vérifier ne rien faire dans la version Release.
...
|||
|-|-|
|[ASSERT](#assert)|Imprime un message et interrompt le programme si l’expression spécifiée a la valeur FALSE dans la version Debug de la bibliothèque.|
...
|[DEBUG_ONLY](#debug_only)|Similaire à ASSERT, mais ne teste ne pas la valeur de l’expression. utile pour le code qui doit s’exécuter uniquement en mode débogage.|
|[Vérifiez et ENSURE_VALID](#ensure)|Utiliser pour valider l’exactitude des données.|
|[THIS_FILE](#this_file)|Se développe pour le nom du fichier qui est en cours de compilation.|
|[TRACE](#trace)|Fournit des fonctionnalités semblables à `printf`dans la version Debug de la bibliothèque.|
|[VERIFY](#verify)|Similaire à ASSERT évalue, mais l’expression dans la version Release de la bibliothèque, ainsi que dans la version de débogage.|
...
|[AfxCheckError](#afxcheckerror)|Variable globale qui sert à tester le SCODE passé pour voir s’il s’agit d’une erreur et, le cas échéant, génère l’erreur appropriée.|
...
|[AfxDump](#afxdump)|Fonction interne qui envoie l’état d’un objet pendant le débogage.|
...
Si le résultat est 0, la macro imprime un message de diagnostic et interrompt le programme. Si la condition est différent de zéro, elle ne fait rien.

Le message de diagnostic se présente sous la forme

`assertion failed in file <name> in line <num>`

où *nom* est le nom du fichier source, et *num* est le numéro de ligne de l’assertion ayant échoué dans le fichier source.

Dans la version Release de MFC, ASSERT n’évalue pas l’expression et par conséquent n’interrompt pas le programme. Si l’expression doit être évaluée, quel que soit l’environnement, utilisez la macro VERIFY à la place d’ASSERT.
...
Cette macro déclare que l’objet pointé est un objet de la classe spécifiée ou est un objet d’une classe dérivé à partir de la classe spécifiée.
...
À l’aide de la `ASSERT_KINDOF` macro est exactement le même que de codage :

[!code-cpp[NVC_MFCDocView#195](../../mfc/codesnippet/cpp/diagnostic-services_4.cpp)]

Cette fonction fonctionne uniquement pour les classes déclarées avec la [DECLARE_DYNAMIC] (exécution-heure-objet-modèle-Services.MD #declare_dynamic ou [DECLARE_SERIAL](run-time-object-model-services.md#declare_serial) (macro).
...
*pObject*<br/>
Spécifie un objet d’une classe dérivée de `CObject` qui a une version de remplacement de la `AssertValid` fonction membre.

### <a name="remarks"></a>Notes

Les appels ASSERT_VALID le `AssertValid` fonction membre de l’objet passé comme argument.

Dans la version Release de MFC, ASSERT_VALID ne fait rien. Dans la version Debug, il valide le pointeur vérifie par rapport à NULL et appelle l’objet propre `AssertValid` fonctions membres. Si aucune de ces tests échoue, un message d’alerte s’affiche dans la même manière que [ASSERT](#assert).

> [!NOTE]
>  Cette fonction est disponible uniquement dans la version Debug des MFC.

Pour plus d’informations et des exemples, consultez [débogage des Applications MFC](/visualstudio/debugger/mfc-debugging-techniques).
...
Aide à rechercher les fuites de mémoire.
...
En mode débogage (lorsque le **_DEBUG** symbole est défini), DEBUG_NEW assure le suivi du nom de fichier et numéro de ligne pour chaque objet alloué. Ensuite, lorsque vous utilisez le [CMemoryState::DumpAllObjectsSince](cmemorystate-structure.md#dumpallobjectssince) fonction membre, chaque objet alloué avec DEBUG_NEW s’affiche avec le nom de fichier et numéro de ligne où il a été alloué.

Pour utiliser DEBUG_NEW, insérez la directive suivante dans vos fichiers sources :

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

Une fois que vous insérez cette directive, le préprocesseur insérera DEBUG_NEW partout où vous utilisez **nouveau**, et MFC s’occupe du reste. Lorsque vous compilez une version release de votre programme, DEBUG_NEW est traduit en une simple **nouveau** opération et les informations de numéro de ligne et de nom de fichier ne sont pas générés.
...
Le debug_only (macro) est équivalent à entourant *expression* avec `#ifdef _DEBUG` et `#endif`.
...
Utiliser pour valider l’exactitude des données.
...
L’objectif de ces macros consiste à améliorer la validation des paramètres. Les macros empêchent la poursuite du traitement de paramètres incorrects dans votre code. Contrairement aux macros ASSERT, les macros Vérifiez une exception en plus de générer une assertion.

Les macros se comportent de deux manières, selon la configuration de projet. Les macros appellent ASSERT et une exception si l’assertion échoue. Par conséquent, dans les configurations de débogage (autrement dit, où _DEBUG est défini) les macros génèrent une assertion et exception lors dans les configurations Release, alimentaires macros uniquement l’exception (ASSERT n’évalue pas l’expression dans les configurations Release).

La macro ENSURE_ARG agit comme la macro Vérifiez.
...
Se développe pour le nom du fichier qui est en cours de compilation.
...
Les informations sont utilisées par les macros ASSERT et à vérifier. Les Assistants Application Assistant et code placer la macro dans les fichiers de code source qu’ils créent.
...
Consultez [ATLTRACE2](../../atl/reference/debugging-and-error-reporting-macros.md#atltrace2) pour obtenir une description de la TRACE. TRACE et ATLTRACE2 ont le même comportement.

Dans la version debug des MFC, cette macro envoie la chaîne spécifiée pour le débogueur de l’application actuelle. Dans une version Release, cette macro est compilé en rien (aucun code généré du tout).
...
Dans la version Release de MFC, vérifiez évalue l’expression, mais ne pas imprimer ou interrompre le programme. Par exemple, si l’expression est un appel de fonction, l’appel est effectué.
...
Fournit la fonctionnalité base dump d’objets dans votre application.
...
`afxDump` est prédéfini [CDumpContext](../../mfc/reference/cdumpcontext-class.md) objet qui vous permet d’envoyer `CDumpContext` informations sur la fenêtre de sortie du débogueur ou dans un terminal de débogage. En règle générale, vous fournissez `afxDump` en tant que paramètre à `CObject::Dump`.
...
Cette variable est accessible à partir d’un débogueur ou votre programme et vous permet de paramétrer les diagnostics d’allocation.
...
- `allocMemDF` Active le débogage allocateur (paramètre par défaut dans la bibliothèque de débogage).

- `delayFreeMemDF` Retards de la libération de mémoire. Pendant que votre programme libère un bloc de mémoire, l’allocateur ne retourne pas que la mémoire pour le système d’exploitation sous-jacent. Contrainte de mémoire maximale sera placé sur votre programme.

- `checkAlwaysMemDF` Appels `AfxCheckMemory` chaque fois la mémoire est allouée ou libérée. Cela ralentit considérablement désallocations et les allocations de mémoire.
...
Cette fonction teste le SCODE passé pour voir si c’est une erreur.
...
S’il existe une erreur, la fonction lève une exception. Si le SCODE passé est E_OUTOFMEMORY, la fonction lève un [CMemoryException](../../mfc/reference/cmemoryexception-class.md) en appelant [AfxThrowMemoryException](exception-processing.md#afxthrowmemoryexception). Sinon, la fonction lève un [COleException](../../mfc/reference/coleexception-class.md) en appelant [AfxThrowOleException](exception-processing.md#afxthrowoleexception).

Cette fonction peut être utilisée pour vérifier les valeurs de retour d’appels de fonctions OLE dans votre application. En testant la valeur de retour avec cette fonction dans votre application, vous pouvez réagir correctement aux conditions d’erreur avec un minimum de code.
...
Cette fonction valide le pool de mémoire libre et imprime les messages d’erreur en fonction des besoins.
...
Tous les blocs de mémoire actuellement allouées sur le tas sont vérifiées, y compris ceux allouée par **nouveau** mais pas celles allouée par des appels directs à des allocateurs de mémoire sous-jacente, comme le **malloc** (fonction) ou le `GlobalAlloc` (fonction) Windows. Si n’importe quel bloc n’est pas endommagé, un message est écrit dans la sortie du débogueur.

Si vous incluez la ligne

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

dans un module de programme, les appels suivants puis à `AfxCheckMemory` afficher le nom de fichier et numéro de ligne où la mémoire a été allouée.

> [!NOTE]
>  Si votre module contient une ou plusieurs implémentations des classes sérialisables, vous devez placer le `#define` ligne après le dernier appel de la macro IMPLEMENT_SERIAL.
...
*dwTarget*<br/>
Indique la cible de la sortie du dump. Les valeurs possibles qui peuvent être combinées à l’aide de l’opération de bits OR ( **&#124;**) (opérateur), sont les suivantes :

- AFX_STACK_DUMP_TARGET_TRACE envoie la sortie par le biais de la [TRACE](#trace) macro. La macro TRACE génère une sortie dans les versions debug uniquement ; Il ne génère aucune sortie dans les versions release. En outre, la TRACE peut être redirigée vers d’autres cibles en dehors du débogueur.

- AFX_STACK_DUMP_TARGET_DEFAULT envoie la sortie du dump à la cible par défaut. Pour une version debug, la sortie est envoyée à la macro TRACE. Dans une version Release, la sortie est placé dans le Presse-papiers.

- AFX_STACK_DUMP_TARGET_CLIPBOARD envoie la sortie dans le Presse-papiers uniquement. Les données sont placées dans le Presse-papiers sous forme de texte brut en utilisant le format de Presse-papiers de CF_TEXT.

- AFX_STACK_DUMP_TARGET_BOTH envoie la sortie dans le Presse-papiers, puis dans la macro TRACE, simultanément.

- AFX_STACK_DUMP_TARGET_ODS envoie la sortie directement dans le débogueur au moyen de la fonction Win32 `OutputDebugString()`. Cette option génère la sortie du débogueur dans les versions debug et versions release lorsqu’un débogueur est attaché au processus. AFX_STACK_DUMP_TARGET_ODS atteint toujours le débogueur (si elle est attachée) et ne peuvent pas être redirigées.
...
|`DUMP2\DEBUG\DUMP2.EXE!`|Le nom de chemin d’accès complet du module qui contienne l’appel de fonction.|
...
`AfxDumpStack` est disponible dans les versions debug ou non Debug des bibliothèques MFC ; Toutefois, la fonction est toujours liée de manière statique, même si votre fichier exécutable utilise MFC dans une DLL partagée. Dans les implémentations de bibliothèque partagée, la fonction se trouve dans le MFCS42. Bibliothèque LIB (et ses variantes).
...
Active et désactive le dump de fuite de mémoire dans le destructeur AFX_DEBUG_STATE.
...
Quand une application décharge la bibliothèque MFC, cette bibliothèque recherche les fuites de mémoire. À ce stade, les fuites de mémoire sont signalées à l’utilisateur via le **déboguer** fenêtre de Visual Studio.
...
*bTrack*<br/>
La valeur TRUE active sur l’utilisation de la mémoire ; FALSE le désactive.
...
Cette fonction permet de désactiver le suivi sur les sections de votre code que vous connaissez sont allouer les blocs correctement.
...
Teste une adresse de mémoire pour vous assurer qu’il représente un bloc de mémoire actuellement actif qui a été alloué par la version de diagnostique de **nouveau**.
...
*plRequestNumber*<br/>
Pointe vers un **long** entier qui est rempli avec le numéro de séquence du bloc de mémoire d’allocation, ou zéro si elle ne représente pas un bloc de mémoire actuellement active.
...
*bReadWrite*<br/>
Spécifie si la mémoire est à la fois pour la lecture et écriture (TRUE) ou simple lecture (FALSE).
...
L’adresse n’est pas limité aux blocs alloués par **nouveau**.
...
*nLength*<br/>
Spécifie la longueur de la chaîne à tester, en octets. La valeur -1 indique que la chaîne est nul.
...
*pContext*<br/>
Points de données facultatives qui l’appelant peut fournir à la fonction de l’itération. Ce pointeur peut être NULL.

### <a name="remarks"></a>Notes

Sérialisable `CObject`-classes dérivées sont des classes dérivées à l’aide de la macro DECLARE_SERIAL. Le pointeur est passé à `AfxDoForAllClasses` dans *pContext* est passé à la fonction de l’itération spécifiée chaque fois qu’elle est appelée.
...
Pile, global, ou des objets incorporés ne sont pas énumérées. Le pointeur passé à `AfxDoForAllObjects` dans *pContext* est passé à la fonction de l’itération spécifiée chaque fois qu’elle est appelée.
