...
|[CStatusBarCtrl :: SetSimple](#setsimple)|Spécifie si un contrôle de barre d’état affiche du texte simple ou toutes les parties de contrôle définies par un appel précédent à `SetParts` .|
...
Ce contrôle (et par conséquent la `CStatusBarCtrl` classe) est uniquement disponible pour les programmes qui s’exécutent sous windows 95/98 et Windows NT version 3,51 et versions ultérieures.
...
*rectangulaire*<br/>
Spécifie la taille et la position du contrôle de barre d’État. Il peut s’agir d’un objet [CRect](../../atl-mfc-shared/reference/crect-class.md) ou d’une structure [Rect](/windows/win32/api/windef/ns-windef-rect) .

*pParentWnd*<br/>
Spécifie la fenêtre parente du contrôle de barre d’État, généralement `CDialog` . Il ne doit pas être NULL.
...
Vous construisez un `CStatusBarCtrl` en deux étapes. Tout d’abord, appelez le constructeur, puis appelez `Create` , qui crée le contrôle de barre d’État et l’attache à l' `CStatusBarCtrl` objet.
...
Crée un contrôle (une fenêtre enfant) et l’associe à l' `CStatusBarCtrl` objet.
...
Par défaut, cette fonction membre ne fait rien. Substituez cette fonction membre pour implémenter le dessin pour un objet owner-draw `CStatusBarCtrl` .
