---
description: 'En savoir plus sur : classe COleInsertDialog'
...
Vous devez allouer l' `COleClientItem` objet avant de pouvoir appeler cette fonction.
...
Chemin d’accès complet au fichier sélectionné dans la boîte de dialogue. Si le type de sélection est `createNewItem` , cette fonction retourne un sens `CString` non significatif en mode release ou provoque une assertion en mode débogage.
