---
description: 'En savoir plus sur : classe CRgn'
title: CRgn, classe
ms.date: 11/04/2016
f1_keywords:
- CRgn
...
|Nom|Description|
|----------|-----------------|
|[CRgn::CombineRgn](#combinergn)|Définit un `CRgn` objet de manière à ce qu’il soit équivalent à l’Union de deux objets spécifiés `CRgn` .|
|[CRgn::CopyRgn](#copyrgn)|Définit un `CRgn` objet afin qu’il s’agit d’une copie d’un `CRgn` objet spécifié.|
...
Une région est une zone elliptique ou polygonale dans une fenêtre. Pour utiliser des régions, vous utilisez les fonctions membres de la classe `CRgn` avec les fonctions de découpage définies en tant que membres de la classe `CDC` .
...
Spécifie le type de la zone résultante. Ce peut être l’une des valeurs suivantes :
...
Les deux régions spécifiées sont combinées et le handle de la région qui en résulte est stocké dans l' `CRgn` objet. Ainsi, quelle que soit la région stockée dans l' `CRgn` objet est remplacée par la région combinée.
...
La nouvelle région remplace la région précédemment stockée dans l' `CRgn` objet. Cette fonction est un cas particulier de la fonction membre [CombineRgn](#combinergn) .
...
La région est définie par le rectangle englobant spécifié par *x1*, *Y1*, *x2* et *Y2*. La région est stockée dans l' `CRgn` objet.

La taille d’une région est limitée à 32 767 par 32 767 unités logiques ou 64 Ko de mémoire, la valeur la plus petite étant retenue.

Lorsqu’il a fini d’utiliser une région créée avec la `CreateEllipticRgn` fonction, une application doit sélectionner la région hors du contexte de périphérique et utiliser la `DeleteObject` fonction pour la supprimer.
...
La région est définie par la structure ou l’objet désigné par *lpRect* et est stockée dans l' `CRgn` objet.

La taille d’une région est limitée à 32 767 par 32 767 unités logiques ou 64 Ko de mémoire, la valeur la plus petite étant retenue.

Lorsqu’il a fini d’utiliser une région créée avec la `CreateEllipticRgnIndirect` fonction, une application doit sélectionner la région hors du contexte de périphérique et utiliser la `DeleteObject` fonction pour la supprimer.
...
Le contexte de périphérique identifié par le paramètre *PDC* doit contenir un chemin d’accès fermé. Après avoir `CreateFromPath` converti un chemin d’accès en une région, Windows ignore le chemin d’accès fermé du contexte de périphérique.
...
*nCount*<br/>
Spécifie le nombre de `POINT` structures ou d' `CPoint` objets dans le tableau pointé par *lpPoints*.
...
Le système ferme automatiquement le polygone, si nécessaire, en dessinant une ligne du dernier vertex jusqu’au premier. La région obtenue est stockée dans l' `CRgn` objet.
...
Lorsqu’une application a fini d’utiliser une région créée à l’aide de la `CreatePolygonRgn` fonction, elle doit sélectionner la région hors du contexte de périphérique et utiliser la `DeleteObject` fonction pour la supprimer.
...
La région obtenue est stockée dans l' `CRgn` objet.
...
Lorsqu’une application a fini d’utiliser une région créée à l’aide de la `CreatePolyPolygonRgn` fonction, elle doit sélectionner la région hors du contexte de périphérique et utiliser la fonction membre [CGDIObject ::D eleteobject](../../mfc/reference/cgdiobject-class.md#deleteobject) pour la supprimer.

## <a name="crgncreaterectrgn"></a><a name="createrectrgn"></a> CRgn::CreateRectRgn

Crée une zone rectangulaire qui est stockée dans l' `CRgn` objet.
...
Lorsqu’il a fini d’utiliser une région créée par `CreateRectRgn` , une application doit utiliser la fonction membre [CGDIObject ::D eleteobject](../../mfc/reference/cgdiobject-class.md#deleteobject) pour supprimer la région.
...
*lpRect*<br/>
Pointe vers une `RECT` structure ou un `CRect` objet qui contient les coordonnées logiques des angles supérieur gauche et inférieur droit de la zone. La `RECT` structure se présente sous la forme suivante :
...
Lorsqu’il a fini d’utiliser une région créée par `CreateRectRgnIndirect` , une application doit utiliser la fonction membre [CGDIObject ::D eleteobject](../../mfc/reference/cgdiobject-class.md#deleteobject) pour supprimer la région.
...
Lorsqu’une application a fini d’utiliser une région créée à l’aide de la `CreateRoundRectRgn` fonction, elle doit sélectionner la région hors du contexte de périphérique et utiliser la fonction membre [CGDIObject ::D eleteobject](../../mfc/reference/cgdiobject-class.md#deleteobject) pour la supprimer.
...
Le `m_hObject` membre de données ne contient pas de région GDI Windows valide tant que l’objet n’est pas initialisé avec une ou plusieurs autres `CRgn` fonctions membres.
...
*lpRect*<br/>
Pointe vers une `RECT` structure ou un `CRect` objet pour recevoir les coordonnées du rectangle englobant. La `RECT` structure se présente sous la forme suivante :
...
Déplace la région stockée dans l' `CRgn` objet selon les décalages spécifiés.
...
En cas de réussite, handle vers l’objet Windows GDI représenté par l' `CRgn` objet ; sinon, null.
...
Vérifie si le point donné par *x* et *y* se trouve dans la région stockée dans l' `CRgn` objet.
...
Détermine si une partie du rectangle spécifié par *lpRect* se trouve dans les limites de la région stockée dans l' `CRgn` objet.
...
*lpRect*<br/>
Pointe vers une `RECT` structure ou un `CRect` objet. La `RECT` structure se présente sous la forme suivante :
...
*lpRect*<br/>
Spécifie la zone rectangulaire. Il peut s’agir d’un pointeur vers une `RECT` structure ou un `CRect` objet.

### <a name="remarks"></a>Notes

Toutefois, contrairement à [CreateRectRgn](#createrectrgn), il n’alloue pas de mémoire supplémentaire à partir du tas de l’application Windows locale. Au lieu de cela, il utilise l’espace alloué pour la région stockée dans l' `CRgn` objet. Cela signifie que l' `CRgn` objet doit déjà avoir été initialisé avec une région Windows valide avant d’appeler `SetRectRgn` . Les points donnés par *x1*, *Y1*, *x2* et *Y2* spécifient la taille minimale de l’espace alloué.

Utilisez cette fonction à la place de la `CreateRectRgn` fonction membre pour éviter les appels au gestionnaire de mémoire local.
