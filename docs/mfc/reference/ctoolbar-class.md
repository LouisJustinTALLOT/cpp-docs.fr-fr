---
description: 'En savoir plus sur : CToolBar, classe'
...
|[CToolBar :: Create](#create)|Crée la barre d’outils Windows et l’attache à l' `CToolBar` objet.|
...
1. Construisez l' `CToolBar` objet.

1. Appelez la fonction [Create](#create) (ou [CreateEx](#createex)) pour créer la barre d’outils Windows et l’attacher à l' `CToolBar` objet.
...
Pour créer un bouton de case à cocher, assignez-lui le style TBBS_CHECKBOX ou utilisez la `CCmdUI` fonction membre d’un objet `SetCheck` dans un gestionnaire de ON_UPDATE_COMMAND_UI. L’appel `SetCheck` de transforme un bouton de contrôle en case à cocher. Transmettez `SetCheck` un argument égal à 0 pour non coché, 1 pour activé ou 2 pour indéterminé.

Pour créer une case d’option, appelez la fonction membre [SetRadio](../../mfc/reference/ccmdui-class.md#setradio) d’un objet [CCmdUI](../../mfc/reference/ccmdui-class.md) à partir d’un gestionnaire de ON_UPDATE_COMMAND_UI. Transmettez `SetRadio` un argument de 0 pour non vérifié ou différent de zéro. Pour fournir le comportement mutuellement exclusif d’un groupe de cases d’option, vous devez avoir des gestionnaires de ON_UPDATE_COMMAND_UI pour tous les boutons du groupe.
...
Cette fonction membre retourne l’index du premier bouton de la barre d’outils, en commençant à la position 0, dont l’ID de commande correspond à `nIDFind` .
...
Appelez cette fonction pour créer une barre d’outils Windows (une fenêtre enfant) et l’associer à l' `CToolBar` objet.
...
> [!NOTE]
> Vous pouvez également définir des États de bouton à l’aide du paramètre *nStyle* ; Toutefois, étant donné que les États de bouton sont contrôlés par le gestionnaire de [ON_UPDATE_COMMAND_UI](message-map-macros-mfc.md#on_update_command_ui) , tout état que vous définissez à l’aide de `SetButtonInfo` sera perdu pendant le prochain traitement inactif. Pour plus d’informations, voir [How to Update User-Interface Objects](../../mfc/how-to-update-user-interface-objects.md) and [TN031 : Control bar](../../mfc/tn031-control-bars.md) .
...
Avant `SetButtonStyle` d’appeler, appelez la fonction membre [GetButtonStyle](#getbuttonstyle) pour récupérer le style du bouton ou du séparateur.

> [!NOTE]
> Vous pouvez également définir des États de bouton à l’aide du paramètre *nStyle* ; Toutefois, étant donné que les États de bouton sont contrôlés par le gestionnaire de [ON_UPDATE_COMMAND_UI](message-map-macros-mfc.md#on_update_command_ui) , tout état que vous définissez à l’aide de `SetButtonStyle` sera perdu pendant le prochain traitement inactif. Pour plus d’informations, voir [How to Update User-Interface Objects](../../mfc/how-to-update-user-interface-objects.md) and [TN031 : Control bar](../../mfc/tn031-control-bars.md) .
