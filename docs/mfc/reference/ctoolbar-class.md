...
|[CToolBar::Create](#create)|Crée la barre d’outils Windows et l’attache à `CToolBar` l’objet.|
|[CToolBar::CreateEx](#createex)|Crée un `CToolBar` objet avec des styles supplémentaires pour l' `CToolBarCtrl` objet incorporé.|
...
Les boutons peuvent agir comme des boutons de bouton, des boutons de case à cocher ou des cases d’option. `CToolBar`les objets sont généralement des membres incorporés des objets de fenêtre frame dérivés de la classe [CFrameWnd](../../mfc/reference/cframewnd-class.md) ou [CMDIFrameWnd](../../mfc/reference/cmdiframewnd-class.md).

[CToolBar :: GetToolBarCtrl](#gettoolbarctrl), une fonction membre qui est une nouveauté de MFC 4,0, vous permet de tirer parti de la prise en charge par le contrôle commun Windows de la personnalisation de la barre d’outils et des fonctionnalités supplémentaires. `CToolBar`les fonctions membres vous offrent la plupart des fonctionnalités des contrôles communs Windows. Toutefois, lorsque vous appelez `GetToolBarCtrl`, vous pouvez attribuer à vos barres d’outils des caractéristiques des barres d’outils Windows 95/98. Quand vous appelez `GetToolBarCtrl`, une référence à un `CToolBarCtrl` objet est retournée. Pour plus d’informations sur la conception de barres d’outils à l’aide des contrôles communs Windows, consultez [CToolBarCtrl](../../mfc/reference/ctoolbarctrl-class.md) . Pour plus d’informations générales sur les contrôles communs, consultez [contrôles communs](/windows/win32/Controls/common-controls-intro) dans le SDK Windows.
...
1. Appelez la fonction [Create](#create) (ou [CreateEx](#createex)) pour créer la barre d’outils Windows et l' `CToolBar` attacher à l’objet.
...
Pour créer un bouton de case à cocher, assignez-lui le `CCmdUI` style TBBS_CHECKBOX `SetCheck` ou utilisez la fonction membre d’un objet dans un gestionnaire ON_UPDATE_COMMAND_UI. L' `SetCheck` appel de transforme un bouton de contrôle en case à cocher. Transmettez `SetCheck` un argument égal à 0 pour non coché, 1 pour activé ou 2 pour indéterminé.

Pour créer une case d’option, appelez la fonction membre [SetRadio](../../mfc/reference/ccmdui-class.md#setradio) d’un objet [CCmdUI](../../mfc/reference/ccmdui-class.md) à partir d’un gestionnaire ON_UPDATE_COMMAND_UI. Transmettez `SetRadio` un argument de 0 pour non vérifié ou différent de zéro. Pour fournir le comportement mutuellement exclusif d’un groupe de cases d’option, vous devez avoir des gestionnaires ON_UPDATE_COMMAND_UI pour tous les boutons du groupe.

Pour plus d’informations sur `CToolBar`l’utilisation de, consultez l’article implémentation [de la [barre d’outils MFC](../../mfc/mfc-toolbar-implementation.md) et Technical Note 31 : Barres](../../mfc/tn031-control-bars.md)de contrôles.
...
Cette fonction membre retourne l’index du premier bouton de la barre d’outils, en commençant à la position 0 `nIDFind`, dont l’ID de commande correspond à.
...
Cette fonction membre crée une barre d’outils Windows (une fenêtre enfant) et l’associe à l' `CToolBar` objet.
...
- La barre de contrôle CBRS_TOP est en haut de la fenêtre frame.

- La barre de contrôle CBRS_BOTTOM est en bas de la fenêtre frame.

- La barre de contrôle CBRS_NOALIGN n’est pas repositionnée lorsque le parent est redimensionné.

- La barre de contrôle CBRS_TOOLTIPS affiche des info-bulles.

- La barre de contrôle CBRS_SIZE_DYNAMIC est dynamique.

- La barre de contrôle CBRS_SIZE_FIXED est fixe.

- La barre de contrôle CBRS_FLOATING est flottante.

- La barre d’État CBRS_FLYBY affiche des informations sur le bouton.

- La barre de contrôle CBRS_HIDE_INPLACE n’est pas affichée à l’utilisateur.
...
Appelez cette fonction pour créer une barre d’outils Windows (une fenêtre enfant) et l’associer `CToolBar` à l’objet.
...
Utilisez `CreateEx`, au lieu de [Create](#create), lorsque certains styles doivent être présents pendant la création du contrôle de barre d’outils incorporé. Par exemple, affectez à *dwCtrlStyle* la valeur TBSTYLE_FLAT &#124; TBSTYLE_TRANSPARENT pour créer une barre d’outils qui ressemble aux barres d’outils d’Internet Explorer 4.
...
Cette fonction membre construit un `CToolBar` objet et définit les tailles par défaut.
...
`CString` Objet contenant le texte du bouton.
...
Appelez cette fonction membre pour charger l’image bitmap spécifiée `lpszResourceName` par `nIDResource`ou.
...
Par exemple, appelez `SetBitmap` pour modifier l’image bitmap après que l’utilisateur a effectué une action sur un document qui modifie l’action d’un bouton.
...
- Bouton de case à cocher TBBS_CHECKBOX auto
...
> [!NOTE]
>  Vous pouvez également définir des États de bouton à l’aide du paramètre *nStyle* ; Toutefois, étant donné que les États de bouton sont contrôlés par le gestionnaire [ON_UPDATE_COMMAND_UI](message-map-macros-mfc.md#on_update_command_ui) , `SetButtonInfo` tout état que vous définissez à l’aide de sera perdu lors du prochain traitement inactif. Consultez [Comment mettre à jour des objets d’interface utilisateur](../../mfc/how-to-update-user-interface-objects.md) et [TN031 : Barres](../../mfc/tn031-control-bars.md) de contrôles pour plus d’informations.
...
Avant d' `SetButtonStyle`appeler, appelez la fonction membre [GetButtonStyle](#getbuttonstyle) pour récupérer le style du bouton ou du séparateur.

> [!NOTE]
>  Vous pouvez également définir des États de bouton à l’aide du paramètre *nStyle* ; Toutefois, étant donné que les États de bouton sont contrôlés par le gestionnaire [ON_UPDATE_COMMAND_UI](message-map-macros-mfc.md#on_update_command_ui) , `SetButtonStyle` tout état que vous définissez à l’aide de sera perdu lors du prochain traitement inactif. Consultez [Comment mettre à jour des objets d’interface utilisateur](../../mfc/how-to-update-user-interface-objects.md) et [TN031 : Barres](../../mfc/tn031-control-bars.md) de contrôles pour plus d’informations.
