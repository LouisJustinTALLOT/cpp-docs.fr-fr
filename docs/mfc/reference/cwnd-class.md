...
|[CWnd::GetSafeHwnd](#getsafehwnd)|Retourne `m_hWnd` , ou null si le **`this`** pointeur est null.|
...
|[CWnd::PreSubclassWindow](#presubclasswindow)|Permet d’effectuer d’autres sous-classes nécessaires avant l’appel de [SubclassWindow](#subclasswindow) .|
...
|[CWnd::GetCurrentMessage](#getcurrentmessage)|Retourne un pointeur vers le message actuellement traité par cette fenêtre. Doit être appelé uniquement lorsque dans une `On` fonction membre du gestionnaire de messages de *message* .|
...
|[CWnd::OnVKeyToItem](#onvkeytoitem)|Appelé par une zone de liste appartenant à `CWnd` en réponse à un message de [WM_KEYDOWN](#onkeydown) .|
...
|[CWnd::PostNcDestroy](#postncdestroy)|Cette fonction virtuelle est appelée par la fonction [OnNcDestroy](#onncdestroy) par défaut après la destruction de la fenêtre.|
...
Un objet `CWnd` est distinct d'une fenêtre Windows, mais les deux sont étroitement liés. Un objet `CWnd` est créé ou détruit par le constructeur et destructeur `CWnd`. La fenêtre Windows, en revanche, est une structure de données interne à Windows qui est créée par une `Create` fonction membre et détruite par le `CWnd` destructeur virtuel. La fonction [DestroyWindow](#destroywindow) détruit la fenêtre Windows sans détruire l’objet.

La `CWnd` classe et le mécanisme de la table des messages masquent la `WndProc` fonction. Les messages de notification Windows entrants sont acheminés automatiquement via la table des messages vers les fonctions membres **de**<em>message</em> appropriées `CWnd` . Vous substituez une fonction membre **de**<em>message</em> pour gérer le message particulier d’un membre dans vos classes dérivées.
...
Une fenêtre enfant se crée en deux étapes. Tout d’abord, appelez le constructeur `CWnd` pour construire l' `CWnd` objet, puis appelez la fonction membre [Create](#create) pour créer la fenêtre enfant et l’attacher à l' `CWnd` objet.
...
Pour plus d’informations, consultez `IAccessible::accLocation` dans la SDK Windows.
...
Prépare `CWnd` la peinture et remplit une structure de `PAINTSTRUCT` données avec des informations sur la peinture.
...
La région de mise à jour est définie par les fonctions membres [Invalidate](#invalidate), [InvalidateRect](#invalidaterect)ou [InvalidateRgn](#invalidatergn) et par le système après la taille, le déplacement, la création, le défilement ou l’exécution d’une autre opération qui affecte la zone cliente. Si la région de mise à jour est marquée pour l’effacement, `BeginPaint` envoie un message de [WM_ONERASEBKGND](#onerasebkgnd) .

N’appelez pas la `BeginPaint` fonction membre sauf en réponse à un message de [WM_PAINT](#onpaint) . Chaque appel à la `BeginPaint` fonction membre doit avoir un appel correspondant à la fonction membre [EndPaint](#endpaint) . Si le signe insertion se trouve dans la zone à peindre, la `BeginPaint` fonction membre masque automatiquement le signe insertion pour empêcher son effacement.
...
*nAdjustType*<br/>
dans Utilisez `CWnd::adjustBorder` pour calculer des coordonnées de fenêtre sans le style WS_EX_CLIENTEDGE ; sinon, utilisez `CWnd::adjustOutside` .
...
Supprime `CWnd` de la chaîne des visionneuses de presse-papiers et rend la fenêtre spécifiée par *hWndNext* le descendant de l' `CWnd` ancêtre dans la chaîne.
...
*nConsultez*<br/>
Spécifie l’action à entreprendre. Si *nConsultez* est différent de zéro, la `CheckDlgButton` fonction membre place une coche en regard du bouton ; si la valeur est 0, la coche est supprimée. Pour les boutons à trois États, si *nConsultez* est 2, l’état du bouton est indéterminé.
...
La `CheckRadioButton` fonction envoie un message [BM_SETCHECK](/windows/win32/Controls/bm-setcheck) à la case d’option spécifiée.
...
Détermine le, le cas échéant, des fenêtres enfants appartenant à qui `CWnd` contient le point spécifié.
...
*lpPoint*<br/>
Pointe vers une [structure point](/windows/win32/api/windef/ns-windef-point) ou un `CPoint` objet qui contient les coordonnées clientes à convertir.

*lpRect*<br/>
Pointe vers une [structure Rect](/windows/win32/api/windef/ns-windef-rect) ou un `CRect` objet qui contient les coordonnées clientes à convertir.

### <a name="remarks"></a>Notes

La `ClientToScreen` fonction membre utilise les coordonnées clientes dans `POINT` la `RECT` structure ou, ou l' `CPoint` `CRect` objet ou pointé par *lpPoint* ou *lpRect* pour calculer de nouvelles coordonnées d’écran, puis remplace les coordonnées de la structure par les nouvelles coordonnées. Les nouvelles coordonnées d’écran sont relatives au coin supérieur gauche de l’affichage système.

La `ClientToScreen` fonction membre part du principe que le point ou le rectangle spécifié est exprimé en coordonnées clientes.
...
Par défaut, elle retourne une valeur différente de zéro jusqu’à ce que `EndModalLoop` soit appelé.
...
> [!WARNING]
> `CWnd::PreCreateWindow` affecte à présent le membre hMenu de son `CREATESTRUCT` paramètre au **`this`** pointeur si le menu a la valeur null et que le style contient WS_CHILD. Pour une fonctionnalité appropriée, vérifiez que votre contrôle de boîte de dialogue a un ID qui n’est pas NULL.
>
...
La méthode [CWnd :: OnCreate](#oncreate) est appelée avant que la `Create` méthode ne soit retournée, et avant que la fenêtre ne soit visible.
...
Le signe insertion système est une ressource partagée. `CWnd` doit créer un signe insertion uniquement lorsqu’il a le focus d’entrée ou qu’il est actif. Elle doit détruire le signe insertion avant de perdre le focus d’entrée ou devient inactive.
...
*formations*<br/>
Pointe vers une [structure point](/windows/win32/api/windef/ns-windef-point) ou un `CPoint` objet qui contient le coin supérieur gauche du contrôle.

*pSize*<br/>
Pointe vers une structure ou un objet de [taille](/windows/win32/api/windef/ns-windef-size) `CSize` qui contient la taille du contrôle
...
`CreateControl` est une analogie directe de la fonction [CWnd :: Create](#create) , qui crée la fenêtre pour un `CWnd` . `CreateControl` crée un contrôle ActiveX au lieu d’une fenêtre ordinaire.
...
Crée la fenêtre spécifiée et l’attache à l' `CWnd` objet.
...
> `CWnd::PreCreateWindow` affecte à présent le membre hMenu de son `CREATESTRUCT` paramètre au **`this`** pointeur si le menu a la valeur null et que le style contient WS_CHILD. Pour une fonctionnalité appropriée, vérifiez que votre contrôle de boîte de dialogue a un ID qui n’est pas NULL.
>
> Cette modification résout un incident dans les scénarios d’interopérabilité managés/natifs. Une `TRACE` instruction dans `CWnd::Create` alerte le développeur du problème.
...
La méthode [CWnd :: OnCreate](#oncreate) est appelée avant que la `CreateEx` méthode ne soit retournée, et avant que la fenêtre ne soit visible.
...
La `CreateGrayCaret` fonction membre détruit automatiquement la forme de signe insertion précédente, le cas échéant, quelle que soit la fenêtre qui possède le signe insertion. Une fois créé, le signe insertion est masqué initialement. Pour afficher le signe insertion, la fonction membre [ShowCaret](#showcaret) doit être appelée.
...
La `CreateSolidCaret` fonction membre détruit automatiquement la forme de signe insertion précédente, le cas échéant, quelle que soit la fenêtre qui possède le signe insertion. Une fois créé, le signe insertion est masqué initialement. Pour afficher le signe insertion, la fonction membre [ShowCaret](#showcaret) doit être appelée.
...
Appelée automatiquement par le gestionnaire de temps d’inactivité de l' `CWinApp` objet.
...
Supprime tous les `CWnd` objets temporaires créés par la `FromHandle` fonction membre.
...
Détruit la fenêtre Windows attachée à l' `CWnd` objet.
...
La `DestroyWindow` fonction membre envoie des messages appropriés à la fenêtre pour la désactiver et supprimer le focus d’entrée. Il détruit également le menu de la fenêtre, vide la file d’attente de l’application, détruit les minuteurs en suspens, supprime la propriété du presse-papiers et interrompt la chaîne du presse-papiers si `CWnd` se trouve en haut de la chaîne de la visionneuse. Elle envoie des messages [WM_DESTROY](#ondestroy) et [WM_NCDESTROY](#onncdestroy) à la fenêtre. Elle ne détruit pas l' `CWnd` objet.

`DestroyWindow` est un espace réservé pour effectuer un nettoyage. Étant donné que `DestroyWindow` est une fonction virtuelle, elle est affichée dans toute `CWnd` classe dérivée de affichage de classes. Toutefois, même si vous remplacez cette fonction dans votre `CWnd` classe dérivée de, `DestroyWindow` n’est pas nécessairement appelé. Si `DestroyWindow` n’est pas appelé dans le code MFC, vous devez l’appeler explicitement dans votre propre code si vous souhaitez qu’il soit appelé.

Supposons, par exemple, que vous avez substitué `DestroyWindow` dans une `CView` classe dérivée de. Étant donné que le code source MFC n’appelle pas `DestroyWindow` dans l’une de ses `CFrameWnd` classes dérivées de, votre substitution `DestroyWindow` ne sera pas appelée, sauf si vous l’appelez explicitement.
...
Si le `CWnd` en cours de destruction est une fenêtre enfant et que le style de [WS_EX_NOPARENTNOTIFY](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles) n’est pas défini, le message [WM_PARENTNOTIFY](/windows/win32/inputmsg/wm-parentnotify) est envoyé au parent.
...
*nIDListBox*<br/>
Spécifie l’identificateur d’une zone de liste. Si *nIDListBox* a la valeur 0, `DlgDirList` suppose qu’il n’existe aucune zone de liste et qu’elle n’essaie pas de la remplir.
...
- Indicateur de LB_DIR DDL_POSTMSGS. Si l’indicateur de LB_DIR est défini, Windows place les messages générés par `DlgDirList` dans la file d’attente de l’application ; sinon, ils sont envoyés directement à la procédure de boîte de dialogue.
...
- Indicateur de CB_DIR DDL_POSTMSGS. Si l’indicateur de CB_DIR est défini, Windows place les messages générés par `DlgDirListComboBox` dans la file d’attente de l’application ; sinon, ils sont envoyés directement à la procédure de boîte de dialogue.
...
Dans cet exemple, le *lecteur* est une lettre de lecteur, le *répertoire* est un nom de répertoire valide et le nom *de fichier est* un nom de fichier valide qui doit contenir au moins un caractère générique. Les caractères génériques sont un point d’interrogation (**?**), qui correspond à n’importe quel caractère et un astérisque ( <strong>\*</strong> ), ce qui signifie qu’ils correspondent à n’importe quel nombre de caractères.
...
La `DlgDirSelect` fonction membre copie la sélection dans la mémoire tampon donnée par *lpString*. Si aucune sélection n’est effectuée, *lpString* ne change pas.
...
La `DlgDirSelectComboBox` fonction membre copie la sélection dans la mémoire tampon spécifiée. Si aucune sélection n’est effectuée, le contenu de la mémoire tampon n’est pas modifié.
...
Seule la fenêtre qui appelle `DragAcceptFiles` avec le paramètre *bAccept* défini sur true s’est identifiée comme capable de traiter le WM_DROPFILES de message Windows. Par exemple, dans une application MDI, si le `CMDIFrameWnd` pointeur de fenêtre est utilisé dans l' `DragAcceptFiles` appel de fonction, seule la `CMDIFrameWnd` fenêtre obtient le message WM_DROPFILES. Ce message n’est pas envoyé à toutes les `CMDIChildWnd` fenêtres ouvertes. Pour `CMDIChildWnd` qu’une fenêtre reçoive ce message, vous devez appeler `DragAcceptFiles` avec le pointeur de la `CMDIChildWnd` fenêtre.
...
*lprcFrom*<br/>
Pointeur vers une structure [Rect](/windows/win32/api/windef/ns-windef-rect) spécifiant l’emplacement et la taille de l’icône ou de la fenêtre réduite.
...
La prise en charge des Active Accessibility par défaut de MFC est suffisante pour les fenêtres et les contrôles standard, y compris les contrôles ActiveX. Toutefois, si votre `CWnd` classe dérivée de contient des éléments d’interface utilisateur sans fenêtre, les MFC n’ont aucun moyen de les connaître. Dans ce cas, vous devez substituer les [fonctions membres Active Accessibility](/windows/win32/winauto/sdk-components) appropriées dans votre classe, et vous devez appeler `EnableActiveAccessibility` dans le constructeur de la classe.
...
Le simple `EnableToolTips` fait d’appeler n’est pas suffisant pour afficher des info-bulles pour vos contrôles enfants, sauf si la fenêtre parente est dérivée de `CFrameWnd` . Cela est dû `CFrameWnd` au fait que fournit un gestionnaire par défaut pour la notification de TTN_NEEDTEXT. Si votre fenêtre parente n’est pas dérivée de `CFrameWnd` , autrement dit, s’il s’agit d’une boîte de dialogue ou d’un mode formulaire, les info-bulles de vos contrôles enfants ne s’affichent pas correctement, sauf si vous fournissez un gestionnaire pour la notification d’info-bulle TTN_NEEDTEXT. Consultez [info-bulles](../../mfc/tool-tips-in-windows-not-derived-from-cframewnd.md).

Les info-bulles par défaut fournies pour votre Windows par `EnableToolTips` n’ont pas de texte associé. Pour récupérer le texte de l’info-bulle à afficher, la notification TTN_NEEDTEXT est envoyée à la fenêtre parente du contrôle d’info-bulle juste avant l’affichage de la fenêtre d’info-bulle. S’il n’existe aucun gestionnaire pour ce message pour assigner une valeur au membre *pszText* de la `TOOLTIPTEXT` structure, aucun texte n’est affiché pour l’info-bulle.
...
La `EndPaint` fonction membre est requise pour chaque appel à la `BeginPaint` fonction membre, mais uniquement une fois que la peinture est terminée.
...
`ExecuteDlgInit` utilise des ressources liées au module en cours d’exécution, ou des ressources provenant d’autres sources. Pour ce faire, `ExecuteDlgInit` recherche un handle de ressource en appelant `AfxFindResourceHandle` . Si votre application MFC n’utilise pas la DLL partagée (MFCx0 [U] [D]. DLL), `AfxFindResourceHandle` appelle [AfxGetResourceHandle](application-information-and-management.md#afxgetresourcehandle), qui retourne le descripteur de ressource actuel pour l’exécutable. Si votre application MFC utilise MFCx0 [U] [D]. DLL, `AfxFindResourceHandle` parcourt la `CDynLinkLibrary` liste d’objets des dll d’extension partagées et MFC en recherchant le descripteur de ressource correct.
...
Retourne le niveau supérieur `CWnd` dont la classe de fenêtre est donnée par *lpszClassName* et dont le nom de fenêtre, ou titre, est donné par *lpszWindowName*.
...
*lpszClassName*<br/>
Pointe vers une chaîne se terminant par un caractère null qui spécifie le nom de classe de la fenêtre (une `WNDCLASS` structure). Si *lpClassName* a la valeur null, tous les noms de classe correspondent.
...
Pour les clignotements successifs, créez une horloge système et appelez à plusieurs reprises `FlashWindow` . Le clignotement `CWnd` signifie la modification de l’apparence de sa barre de titre comme si le `CWnd` passage de l’état inactif à l’état actif, ou vice versa. (Une barre de titre inactive devient une barre de titre active ; une barre de titre active devient une barre de titre inactive.)
...
Cette fonction retourne toujours une valeur différente de zéro pour les fenêtres réduites. Si la fenêtre est réduite, `FlashWindow` fait simplement clignoter l’icône de la fenêtre. *bInvert* est ignoré pour les fenêtres réduites.
...
*varChild*<br/>
Identifie l’enfant dont l' `IDispatch` interface doit être récupérée.
...
*pszHelpFile*<br/>
Adresse d’un BSTR qui reçoit le chemin d’accès complet du `WinHelp` fichier associé à l’objet spécifié, le cas échéant.
...
*lpRect*<br/>
Pointe vers une [structure Rect](/windows/win32/api/windef/ns-windef-rect) ou un `CRect` objet pour recevoir les coordonnées clientes. Les `left` `top` membres et seront 0. Les `right` `bottom` membres et contiendront la largeur et la hauteur de la fenêtre.

### <a name="remarks"></a>Notes

Les coordonnées clientes spécifient les angles supérieur gauche et inférieur droit de la zone cliente. Étant donné que les coordonnées clientes sont relatives aux angles supérieurs gauches de la `CWnd` zone cliente, les coordonnées du coin supérieur gauche sont (0,0).
...
Vous ne devez pas libérer ce `IUnknown` pointeur. En général, vous utilisez pour obtenir une interface spécifique du contrôle.
...
Récupère un pointeur vers un contexte de périphérique, de classe ou privé commun pour la zone cliente selon le style de classe spécifié pour `CWnd` .
...
Pour les contextes de périphérique courants, `GetDC` assigne des attributs par défaut au contexte chaque fois qu’il est récupéré. Pour les contextes de classe et privés, `GetDC` laisse les attributs précédemment assignés inchangés. Le contexte de périphérique peut être utilisé dans les fonctions GDI (Graphics Device Interface) suivantes pour dessiner dans la zone cliente.
...
Un contexte de périphérique appartenant à la `CWnd` classe est retourné par la `GetDC` fonction membre si CS_CLASSDC, CS_OWNDC ou CS_PARENTDC a été spécifié en tant que style dans la `WNDCLASS` structure lorsque la classe a été inscrite.
...
- DCX_CLIPCHILDREN exclut les zones visibles de toutes les fenêtres enfants situées sous la `CWnd` fenêtre.

- DCX_CLIPSIBLINGS exclut les zones visibles de toutes les fenêtres sœurs au-dessus de la `CWnd` fenêtre.
...
- DCX_LOCKWINDOWUPDATE permet le dessin, même s’il existe un `LockWindowUpdate` appel en vigueur qui exclut cette fenêtre. Cette valeur est utilisée pour le dessin pendant le suivi.

- DCX_PARENTCLIP utilise la région visible de la fenêtre parente et ignore le WS_CLIPCHILDREN et les bits de style WS_PARENTDC de la fenêtre parente. Cette valeur définit l’origine du contexte de périphérique dans l’angle supérieur gauche de la `CWnd` fenêtre.
...
Pour plus d’informations sur ces caractéristiques, consultez la description de la `WNDCLASS` structure dans la SDK Windows.
...
Étant donné que les fenêtres de niveau supérieur n’ont pas de valeur d’ID, la valeur de retour de cette fonction n’est pas valide si `CWnd` est une fenêtre de niveau supérieur.
...
Lorsque des erreurs se produisent, telles que l’apparition de caractères non numériques et le dépassement de la valeur maximale ci-dessus, `GetDlgItemInt` copie 0 vers l’emplacement désigné par *lpTrans*. S’il n’y a aucune erreur, *lpTrans* reçoit une valeur différente de zéro. Si *lpTrans* a la valeur null, `GetDlgItemInt` n’avertit pas les erreurs.
...
La `GetDlgItemText` fonction membre copie le texte à l’emplacement désigné par *lpStr* et retourne le nombre d’octets qu’il copie.
...
Récupère un pointeur vers `CWnd` qui a actuellement le focus d’entrée.
...
Par défaut, la `GetNextDlgGroupItem` fonction membre retourne un pointeur vers le contrôle suivant dans le groupe. Si *pWndCtl* identifie le premier contrôle dans le groupe et que *bPrevious* a la valeur true, `GetNextDlgGroupItem` retourne un pointeur vers le dernier contrôle du groupe.
...
|----------------------------|-----------------------------|
|Contrôle ActiveX avec fenêtres|Contient un HWND et associe un objet [COleControlSite](../../mfc/reference/colecontrolsite-class.md) à celui-ci. Le `m_hWnd` membre de `COleControlSiteOrWnd` est défini sur le HWND du contrôle et le `m_pSite` membre pointe sur le du contrôle `COleControlSite` .|
|Contrôle ActiveX sans fenêtre|Ne contient aucun HWND. Le `m_pSite` membre de `COleControlSiteOrWnd` pointe vers le du contrôle `COleControlSite` et le `m_hWnd` membre a la valeur null.|
|Fenêtre standard|Contient uniquement un HWND. Le `m_hWnd` membre de `COleControlSiteOrWnd` est défini sur le HWND de la fenêtre et le `m_pSite` membre a la valeur null.|
...
La `GetParent` fonction retourne un pointeur vers le parent immédiat (s’il existe). En revanche, la fonction [GetParentOwner](#getparentowner) retourne un pointeur vers la fenêtre parente ou propriétaire la plus immédiate qui n’est pas une fenêtre enfant (n’a pas le style WS_CHILD). Si vous avez une fenêtre enfant dans une fenêtre enfant `GetParent` et `GetParentOwner` retournent des résultats différents.
...
En revanche, la fonction [GetParent](#getparent) retourne un pointeur vers le parent immédiat, qu’il s’agisse d’une fenêtre enfant ou non. Si vous avez une fenêtre enfant dans une fenêtre enfant `GetParent` et `GetParentOwner` retournent des résultats différents.
...
*pParent*<br/>
Pointeur vers une fenêtre parente `CWnd` . Peut être NULL.
...
Cette fonction membre ne fonctionne pas sur les barres de défilement créées lorsque les bits WS_HSCROLL ou WS_VSCROLL sont définis lors de la création d’une fenêtre. L' `CWnd` implémentation de cette fonction retourne simplement la valeur null. Les classes dérivées, telles que `CView` , implémentent la fonctionnalité décrite.
...
Appelez cette fonction membre pour récupérer les informations que la `SCROLLINFO` structure gère à propos d’une barre de défilement.
...
Les gestionnaires de messages Windows MFC qui indiquent la position de la barre de défilement, [CWnd :: OnHScroll](#onhscroll) et [CWnd :: OnVScroll](#onvscroll), fournissent uniquement 16 bits de données de position. `GetScrollInfo` et `SetScrollInfo` fournissent 32 bits de données de position de la barre de défilement. Ainsi, une application peut appeler `GetScrollInfo` lors du traitement `CWnd::OnHScroll` `CWnd::OnVScroll` de ou pour obtenir des données de position de la barre de défilement 32 bits.
...
Si `CWnd` n’a pas de barre de défilement, la `GetScrollRange` fonction membre copie 0 vers *lpMinPos* et *lpMaxPos*.
...
Windows peut automatiquement rendre les éléments indisponibles dans le menu de contrôle standard. `CWnd` peut effectuer sa propre sélection ou indisponibilité en répondant aux messages [WM_INITMENU](#oninitmenu) , qui sont envoyés avant l’affichage d’un menu.
...
Si n' `CWnd` a pas de fenêtre jointe ou si son parent de niveau supérieur n’est pas un objet dérivé de [CFrameWnd](../../mfc/reference/cframewnd-class.md), cette fonction retourne la valeur null.
...
La fenêtre de niveau supérieur est la fenêtre qui est un enfant du bureau. Si n' `CWnd` a pas de fenêtre jointe, cette fonction retourne la valeur null.
...
Si `CWnd` a été créé avec le style CS_OWNDC et que le mode de mappage n’est pas MM_TEXT, la `GetUpdateRect` fonction membre donne au rectangle des coordonnées logiques. Sinon, `GetUpdateRect` donne le rectangle en coordonnées clientes. S’il n’existe aucune région de mise à jour, `GetUpdateRect` définit le rectangle comme étant vide (affecte à toutes les coordonnées la valeur 0).

Le paramètre *bErase* spécifie si `GetUpdateRect` doit effacer l’arrière-plan de la zone de mise à jour. Si *bErase* a la valeur true et que la région de mise à jour n’est pas vide, l’arrière-plan est effacé. Pour effacer l’arrière-plan, `GetUpdateRect` envoie le message de [WM_ERASEBKGND](#onerasebkgnd) .

Le rectangle de mise à jour récupéré par la fonction membre [BeginPaint](#beginpaint) est identique à celui récupéré par la `GetUpdateRect` fonction membre.

La `BeginPaint` fonction membre valide automatiquement la région de mise à jour, donc tout appel à `GetUpdateRect` effectué immédiatement après un appel à `BeginPaint` récupère une zone de mise à jour vide.
...
La fonction membre [BeginPaint](#beginpaint) valide automatiquement la région de mise à jour, donc tout appel à `GetUpdateRgn` effectué immédiatement après un appel à `BeginPaint` récupère une zone de mise à jour vide.
...
`GetWindowDC` est destiné à être utilisé pour des effets spéciaux de peinture dans la `CWnd` zone non cliente. La peinture dans les zones non clientes d’une fenêtre n’est pas recommandée.
...
Le `flags` membre de la structure [WINDOWPLACEMENT](/windows/win32/api/winuser/ns-winuser-windowplacement) récupérée par cette fonction est toujours 0. Si `CWnd` est agrandi, le `showCmd` membre de `WINDOWPLACEMENT` est SW_SHOWMAXIMIZED. Si la fenêtre est réduite, elle est SW_SHOWMINIMIZED. Sinon, il est SW_SHOWNORMAL.

## <a name="cwndgetwindowrect"></a><a name="getwindowrect"></a> CWnd :: GetWindowRect

Copie les dimensions du rectangle englobant de l' `CWnd` objet dans la structure vers laquelle pointe *lpRect*.
...
*lpRect*<br/>
Pointe vers un `CRect` objet ou une [structure Rect](/windows/win32/api/windef/ns-windef-rect) qui reçoit les coordonnées d’écran des angles supérieur gauche et inférieur droit.
...
Copie le `CWnd` titre de la légende (le cas échéant) dans la mémoire tampon vers laquelle pointe *lpszStringBuf* ou dans la chaîne de destination *rString*.
...
Si l' `CWnd` objet est un contrôle, la `GetWindowText` fonction membre copie le texte dans le contrôle au lieu de copier la légende.

Cette fonction membre provoque l’envoi du message [WM_GETTEXT](/windows/win32/winmsg/wm-gettext) à l' `CWnd` objet.
...
Retourne la longueur du titre de la légende de l' `CWnd` objet.
...
Si `CWnd` est un contrôle, la `GetWindowTextLength` fonction membre retourne la longueur du texte dans le contrôle au lieu de la légende.

Cette fonction membre provoque l’envoi du message [WM_GETTEXTLENGTH](/windows/win32/winmsg/wm-gettextlength) à l' `CWnd` objet.
...
Le masquage est cumulatif. Si `HideCaret` a été appelé cinq fois dans une ligne, la `ShowCaret` fonction membre doit être appelée cinq fois avant que le signe insertion ne soit affiché.
...
Invalide la totalité de la zone cliente de `CWnd` .
...
Windows envoie un message [WM_PAINT](#onpaint) chaque fois que la `CWnd` région de mise à jour n’est pas vide et qu’il n’y a pas d’autres messages dans la file d’attente d’application pour cette fenêtre.
...
Invalide la zone cliente dans le rectangle donné en ajoutant ce rectangle à la `CWnd` zone de mise à jour.
...
*lpRect*<br/>
Pointe vers un `CRect` objet ou une [structure Rect](/windows/win32/api/windef/ns-windef-rect) qui contient le rectangle (dans les coordonnées clientes) à ajouter à la zone de mise à jour. Si *lpRect* a la valeur null, la zone cliente entière est ajoutée à la zone.
...
Une fenêtre enfant est la descendante directe de `CWnd` si l' `CWnd` objet se trouve dans la chaîne de fenêtres parentes qui amène la fenêtre indépendante d’origine à la fenêtre enfant.
...
La `LockWindowUpdate` fonction membre ne rend pas la fenêtre donnée invisible et n’efface pas le bit de style [WS_VISIBLE](styles-used-by-mfc.md#window-styles) .
...
|![Icône de &#40;&#33;&#41; importante](../../mfc/reference/media/vc364f3.gif "Icône de &#40;&#33;&#41; importante")|MB_ICONEXCLAMATION et MB_ICONWARNING|
...
*y*<br/>
Spécifie la nouvelle position du haut de `CWnd` .

*nWidth*<br/>
Spécifie la nouvelle largeur de `CWnd` .

*nHeight*<br/>
Spécifie la nouvelle hauteur de `CWnd` .

*bRepaint*<br/>
Spécifie si `CWnd` doit être redessiné. Si la valeur est TRUE, `CWnd` reçoit un message de [WM_PAINT](/windows/win32/gdi/wm-paint) dans son gestionnaire de messages [OnPaint](#onpaint) comme d’habitude. Si ce paramètre a la valeur FALSe, aucun type n’est redessiné. Cela s’applique à la zone cliente, à la zone non cliente (y compris le titre et les barres de défilement) et à n’importe quelle partie de la fenêtre parente non couverte suite au `CWnd` déplacement de. Lorsque ce paramètre a la valeur FALSe, l’application doit explicitement invalider ou redessiner les parties de `CWnd` et la fenêtre parente qui doivent être redessinées.
...
Pour un objet de niveau supérieur `CWnd` , les paramètres *x* et *y* sont relatifs à l’angle supérieur gauche de l’écran. Pour un `CWnd` objet enfant, elles sont relatives au coin supérieur gauche de la zone cliente de la fenêtre parente.

La `MoveWindow` fonction envoie le message [WM_GETMINMAXINFO](#ongetminmaxinfo) . Le traitement de ce message donne `CWnd` la possibilité de modifier les valeurs par défaut pour les plus grandes et les plus petites fenêtres possibles. Si les paramètres de la `MoveWindow` fonction membre dépassent ces valeurs, les valeurs peuvent être remplacées par les valeurs minimales ou maximales dans le gestionnaire de WM_GETMINMAXINFO.
...
L’infrastructure appelle cette fonction membre lorsqu’un `CWnd` objet est en cours d’activation ou de désactivation.
...
*nState*<br/>
Spécifie si `CWnd` est en cours d’activation ou de désactivation. Ce peut être l’une des valeurs suivantes :
...
Si la valeur est TRUE, le `CWnd` est activé ; sinon, désactivé.

### <a name="remarks"></a>Notes

Si l' `CWnd` objet est activé à l’aide d’un clic de souris, il reçoit également un appel de fonction membre [OnMouseActivate](#onmouseactivate) .
...
*bActive*<br/>
Spécifie si `CWnd` est en cours d’activation ou de désactivation. TRUE signifie que `CWnd` est activé. FALSe signifie que `CWnd` est en cours de désactivation.

*dwThreadID*<br/>
Spécifie la valeur de l’ID de thread. Si *bActive* a la valeur true, *dwThreadID* identifie le thread qui détient le `CWnd` en cours de désactivation. Si *bActive* a la valeur false, *dwThreadID* identifie le thread qui détient le `CWnd` en cours d’activation.
...
*pWnd*\
dans Pointeur vers un `CWnd` objet qui représente la fenêtre dans laquelle l’utilisateur a cliqué sur le bouton de commande ou a appuyé sur la touche de commande. Cette fenêtre peut être une fenêtre enfant de la fenêtre qui reçoit le message.

*nCmd*\
dans Indique la commande de l’application. Pour obtenir la liste des valeurs possibles, consultez les commandes sous la section *cmd* du paramètre *lParam* de [WM_APPCOMMAND](/windows/win32/inputdev/wm-appcommand).

*nDevice*\
dans Périphérique d’entrée qui a généré l’événement d’entrée. Pour obtenir la liste des valeurs possibles, consultez les appareils sous la section *uDevice* du paramètre *lParam* de [WM_APPCOMMAND](/windows/win32/inputdev/wm-appcommand).

*nKey*\
dans Indique les touches virtuelles qui sont inactives, telles que la touche CTRL ou le bouton gauche de la souris. Pour obtenir la liste des valeurs possibles, consultez les clés sous la section *dwKeys* du paramètre *lParam* de [WM_APPCOMMAND](/windows/win32/inputdev/wm-appcommand). Pour plus d’informations, consultez le sous-en-tête « paramètres de message » dans [à propos](/windows/win32/inputdev/about-mouse-input)de l’entrée de souris.
...
L’infrastructure appelle cette fonction membre pour indiquer `CWnd` à d’annuler n’importe quel mode interne.
...
Si l' `CWnd` objet a le focus, sa `OnCancelMode` fonction membre est appelée lorsqu’une boîte de dialogue ou une boîte de message s’affiche. Cela donne la `CWnd` possibilité d’annuler des modes tels que la capture de la souris.
...
Pour plus d’informations sur la `ReleaseCapture` fonction Windows, consultez le SDK Windows.
...
Chaque `CWnd` objet qui reçoit un `OnChangeCbChain` appel doit utiliser la fonction Windows [SendMessage](/windows/win32/api/winuser/nf-winuser-sendmessage) pour envoyer le [WM_CHANGECBCHAIN](/windows/win32/dataxchg/wm-changecbchain) message à la fenêtre suivante dans la chaîne de la visionneuse du presse-papiers (le handle retourné par `SetClipboardViewer` ). Si *hWndRemove* est la fenêtre suivante dans la chaîne, la fenêtre spécifiée par *hWndAfter* devient la fenêtre suivante, et les messages du presse-papiers y sont transmis.
...
Si l' `CWnd` objet est une fenêtre enfant d’interface multidocument (MDI), `OnChildActivate` est appelé par le Framework lorsque l’utilisateur clique sur la barre de titre de la fenêtre ou lorsque la fenêtre est activée, déplacée ou dimensionnée.
...
L’infrastructure appelle cette fonction membre comme signal indiquant que l' `CWnd` application ou doit se terminer.
...
*dwColorizationColor*\
dans Spécifie la nouvelle couleur de colorisation. Le format de couleur est un nombre hexadécimal sous la forme 0xAARRGGBB, où chacun des quatre composants est compris entre 0x00 et 0xFF. Le composant AA est la valeur alpha, RR est la couleur rouge, GG est vert et BB est bleu.

*bOpacity*\
dans TRUE si la nouvelle couleur est mélangée avec l’opacité ; FALSe dans le cas contraire.
...
Lorsqu’un `CWnd` objet reçoit cet appel, il doit libérer autant de mémoire que possible, en tenant compte du niveau actuel d’activité de l’application et du nombre total d’applications qui s’exécutent dans Windows. L’application peut appeler la fonction Windows pour déterminer le nombre d’applications en cours d’exécution.
...
Deux éléments de la zone de liste ou de liste déroulante sont reformés dans une `COMPAREITEMSTRUCT` structure vers laquelle pointe *lpCompareItemStruct*. `OnCompareItem` doit retourner une valeur qui indique quels éléments doivent apparaître avant l’autre. En général, Windows effectue cet appel plusieurs fois jusqu’à ce qu’il détermine la position exacte du nouvel élément.

Si le `hwndItem` membre de la `COMPAREITEMSTRUCT` structure appartient à un objet [CListBox](../../mfc/reference/clistbox-class.md) ou [CComboBox](../../mfc/reference/ccombobox-class.md) , la `CompareItem` fonction virtuelle de la classe appropriée est appelée. Substituez `CComboBox::CompareItem` ou `CListBox::CompareItem` dans votre `CListBox` classe ou dérivée `CComboBox` pour effectuer la comparaison d’éléments.
...
L' `CWnd` objet reçoit cet appel après la création de la fenêtre mais avant qu’il ne soit visible. `OnCreate` est appelé avant que `Create` la `CreateEx` fonction membre ou soit retournée.
...
Pour modifier la couleur du texte, appelez la `SetTextColor` fonction membre avec les valeurs de rouge, vert et bleu (RVB) souhaitées.
...
`OnCtlColor` n’est pas appelé pour la zone de liste d’une zone de liste déroulante déroulante, car la zone de liste déroulante est en fait un enfant de la zone de liste déroulante et non un enfant de la fenêtre. Pour modifier la couleur de la zone de liste déroulante, créez un `CComboBox` avec une substitution de `OnCtlColor` qui vérifie CTLCOLOR_LISTBOX dans le `nCtlColor` paramètre. Dans ce gestionnaire, la `SetBkColor` fonction membre doit être utilisée pour définir la couleur d’arrière-plan du texte.
...
Une application utilise généralement `OnDeadChar` pour fournir à l’utilisateur des commentaires sur chaque touche enfoncée. Par exemple, une application peut afficher l’accent à la position actuelle du caractère sans déplacer le signe insertion.
...
Si le `hwndItem` membre de la `DELETEITEMSTRUCT` structure appartient à une zone de liste déroulante ou à une zone de liste, la `DeleteItem` fonction virtuelle de la classe appropriée est appelée. Substituez la `DeleteItem` fonction membre de la classe du contrôle approprié pour supprimer les données spécifiques à l’élément.
...
L’infrastructure appelle cette fonction membre pour informer l' `CWnd` objet qu’elle est en cours de destruction.
...
`OnDestroy` est appelé après que l' `CWnd` objet a été supprimé de l’écran.

`OnDestroy` est appelé en premier pour le `CWnd` en cours de destruction, puis pour les fenêtres enfants de `CWnd` telles qu’elles sont détruites. On peut supposer que toutes les fenêtres enfants existent toujours pendant l' `OnDestroy` exécution.

Si l' `CWnd` objet détruit fait partie de la chaîne de la visionneuse du presse-papiers (définie en appelant la fonction membre [SetClipboardViewer](#setclipboardviewer) ), le `CWnd` doit se supprimer de la chaîne du presse-papiers en appelant la fonction membre [ChangeClipboardChain](#changeclipboardchain) avant de retourner à partir de la `OnDestroy` fonction.
...
L’infrastructure appelle cette fonction membre pour tous les objets de niveau supérieur `CWnd` lorsque l’utilisateur modifie les paramètres du mode appareil.
...
*lpDeviceName*<br/>
Pointe vers le nom d’appareil spécifié dans le fichier d’initialisation de Windows, WIN.INI.

### <a name="remarks"></a>Notes

Les applications qui gèrent le message de WM_DEVMODECHANGE peuvent réinitialiser leurs paramètres de mode d’appareil. Les applications qui utilisent la `ExtDeviceMode` fonction Windows pour enregistrer et restaurer les paramètres de l’appareil ne traitent généralement pas cette fonction.

Cette fonction n’est pas appelée lorsque l’utilisateur modifie l’imprimante par défaut à partir du panneau de configuration. Dans ce cas, la `OnWinIniChange` fonction est appelée.
...
Chaque fenêtre qui reçoit un `OnDrawClipboard` appel doit appeler la fonction Windows [SendMessage](/windows/win32/api/winuser/nf-winuser-sendmessage) pour passer un [WM_DRAWCLIPBOARD](/windows/win32/dataxchg/wm-drawclipboard) message à la fenêtre suivante dans la chaîne du presse-papiers. Le handle de la fenêtre suivante est retourné par la fonction membre [SetClipboardViewer](#setclipboardviewer) ; Il peut être modifié en réponse à un appel de fonction membre [OnChangeCbChain](#onchangecbchain) .
...
Le `itemAction` membre de la structure [drawitemstruct,](/windows/win32/api/winuser/ns-winuser-drawitemstruct) définit l’opération de dessin à effectuer. Les données de ce membre permettent au propriétaire du contrôle de déterminer l’action de dessin requise.

Avant de revenir au traitement de ce message, une application doit s’assurer que le contexte de périphérique identifié par le membre *HDC* de la `DRAWITEMSTRUCT` structure est rétabli à l’État par défaut.

Si le `hwndItem` membre appartient à un objet [CButton](../../mfc/reference/cbutton-class.md), [CMenu](../../mfc/reference/cmenu-class.md), [CListBox](../../mfc/reference/clistbox-class.md)ou [CComboBox](../../mfc/reference/ccombobox-class.md) , la `DrawItem` fonction virtuelle de la classe appropriée est appelée. Substituez la `DrawItem` fonction membre de la classe du contrôle approprié pour dessiner l’élément.
...
*hDropInfo*<br/>
Pointeur vers une structure de données interne qui décrit les fichiers supprimés. Ce handle est utilisé par les `DragFinish` `DragQueryFile` `DragQueryPoint` fonctions Windows, et pour récupérer des informations sur les fichiers déplacés.
...
L’infrastructure appelle cette fonction membre lorsqu’une application modifie l’état activé de l' `CWnd` objet.
...
*bEnable*<br/>
Spécifie si l' `CWnd` objet a été activé ou désactivé. Ce paramètre a la valeur TRUE si `CWnd` a été activé ; il a la valeur false si `CWnd` a été désactivé.

### <a name="remarks"></a>Notes

`OnEnable` est appelé avant que la fonction membre [EnableWindow](#enablewindow) retourne, mais une fois que l’état activé pour la fenêtre ( [WS_DISABLED](styles-used-by-mfc.md#window-styles) style bit) a changé.
...
L’infrastructure appelle cette fonction membre une fois que l' `CWnd` objet a retourné une valeur différente de zéro d’un appel de fonction membre [OnQueryEndSession](#onqueryendsession) .
...
L' `OnEndSession` appel informe l' `CWnd` objet si la session se termine réellement.
...
L’infrastructure appelle cette fonction membre lorsque l' `CWnd` arrière-plan de l’objet a besoin d’être effacé (par exemple, en cas de redimensionnement).
...
L’implémentation par défaut efface l’arrière-plan à l’aide du pinceau d’arrière-plan de la classe de fenêtre spécifié par le `hbrBackground` membre de la structure de la classe de fenêtre.

Si le `hbrBackground` membre a la valeur null, votre version substituée de `OnEraseBkgnd` doit effacer la couleur d’arrière-plan. Votre version doit également aligner l’origine du pinceau prévu avec les `CWnd` coordonnées en appelant d’abord [UnrealizeObject](/windows/win32/api/wingdi/nf-wingdi-unrealizeobject) pour le pinceau, puis en sélectionnant le pinceau.
...
Toutes les fenêtres de niveau supérieur du système reçoivent un `OnFontChange` appel de l’infrastructure après que l’application a modifié le pool des ressources de police.
...
Normalement, Windows gère toutes les entrées de touche de direction et de touche TAB dans un `CWnd` contrôle. En substituant `OnGetDlgCode` , un `CWnd` contrôle peut choisir un type particulier d’entrée pour se traiter lui-même.

Les fonctions par défaut `OnGetDlgCode` pour les classes de contrôle prédéfinies retournent un code approprié pour chaque classe.
...
*lpMMI*<br/>
Pointe vers une `MINMAXINFO` structure qui contient des informations sur la taille et la position agrandies d’une fenêtre, ainsi que sur sa taille de suivi minimale et maximale. Pour plus d’informations sur cette structure, consultez la structure [minmaxinfo,](/windows/win32/api/winuser/ns-winuser-minmaxinfo) .
...
*nHotKeyId*\
dans Identificateur de la touche d’accès rapide qui a généré le message. Si le message a été généré par une touche d’accès rapide définie par le système, ce paramètre aura l’une des valeurs suivantes :

- `IDHOT_SNAPDESKTOP` -La touche d’accès rapide du Bureau d’extension a été enfoncée.
- `IDHOT_SNAPWINDOW` -La touche d’accès rapide de la fenêtre Snap a été enfoncée.

*nKey1*\
dans Combinaison d’opérations de bits d’indicateurs qui indiquent les touches qui ont été enfoncées en combinaison avec la clé spécifiée par le paramètre *nKey2* . Les valeurs possibles sont les suivantes :

- `MOD_ALT` -La touche ALT était maintenue enfoncée.
- `MOD_CONTROL` -La touche CTRL était maintenue enfoncée.
- `MOD_SHIFT` -La touche Maj était maintenue enfoncée.
- `MOD_WIN` -La touche WINDOWS était maintenue enfoncée. Ces clés sont étiquetées avec le logo Microsoft Windows.

*nKey2*\
dans Code de la touche virtuelle de la touche d’accès rapide.
...
La fonction membre du propriétaire du presse-papiers `OnHScrollClipboard` est appelée par la visionneuse du presse-papiers lorsque les données du presse-papiers ont le `CF_OWNERDISPLAY` format et qu’il y a un événement dans la barre de défilement horizontale de la visionneuse du presse-papiers.
...
L’infrastructure appelle cette fonction membre pour un objet (sous forme) réduit `CWnd` quand l’arrière-plan de l’icône doit être rempli avant de peindre l’icône.
...
*uFlag*\
dans Cet indicateur peut contenir les valeurs suivantes :
...
*nCharSet*\
dans Jeu de caractères des nouveaux paramètres régionaux. Pour plus d’informations, consultez le paramètre *lfCharSet* de la structure [LOGFONT](/windows/win32/api/wingdi/ns-wingdi-logfontw) .

*nLocaleId*\
dans Identificateur de paramètres régionaux d’entrée. Pour plus d’informations, consultez [constantes et chaînes](/windows/win32/Intl/language-identifier-constants-and-strings)de l’identificateur de langue.
...
*nFlags*\
dans Combinaison de bits (OR) d’indicateurs qui indiquent que les nouveaux paramètres régionaux ont été sélectionnés à partir des paramètres régionaux précédents ou suivants dans la liste des paramètres régionaux installés, ou que la nouvelle disposition de clavier de paramètres régionaux d’entrée peut être utilisée avec le jeu de caractères système. Les valeurs possibles sont INPUTLANGCHANGE_BACKWARD, INPUTLANGCHANGE_FORWARD et INPUTLANGCHANGE_SYSCHARSET.
...
En raison de la répétition automatique, plusieurs `OnKeyDown` appels peuvent être effectués avant l’appel d’une fonction membre [onkeyup](#onkeyup) . Le bit qui indique l’état de la clé précédente peut être utilisé pour déterminer si l' `OnKeyDown` appel est la première transition vers le haut ou une transition répétée.
...
Une touche non-système est une touche du clavier qui est enfoncée lorsque la touche ALT n’est pas enfoncée ou une touche du clavier qui est enfoncée lorsque le `CWnd` a le focus d’entrée.
...
Si l' `CWnd` objet affiche un signe insertion, le signe insertion doit être détruit à ce stade.
...
Une fenêtre enfant MDI est activée indépendamment de la fenêtre frame MDI. Lorsque le frame devient actif, la fenêtre enfant qui a été activée pour la dernière fois avec un `OnMDIActivate` appel reçoit un message de [WM_NCACTIVATE](#onncactivate) pour dessiner un cadre de fenêtre et une barre de légende active, mais il ne reçoit pas d’autre `OnMDIActivate` appel.
...
Windows lance l’appel à `OnMeasureItem` pour le propriétaire des zones de liste modifiable et des zones de liste créées avec le style OWNERDRAWFIXED avant l’envoi du message [WM_INITDIALOG](/windows/win32/dlgbox/wm-initdialog) . Par conséquent, lorsque le propriétaire reçoit cet appel, Windows n’a pas encore déterminé la hauteur et la largeur de la police utilisée dans le contrôle ; les appels de fonction et les calculs qui nécessitent ces valeurs doivent se produire dans la fonction principale de l’application ou de la bibliothèque.

Si l’élément mesuré est un `CMenu` objet, `CListBox` ou `CComboBox` , la `MeasureItem` fonction virtuelle de la classe appropriée est appelée. Substituez la `MeasureItem` fonction membre de la classe du contrôle approprié pour calculer et définir la taille de chaque élément.

`OnMeasureItem` est appelé uniquement si la classe du contrôle est créée au moment de l’exécution, ou si elle est créée avec le style LBS_OWNERDRAWVARIABLE ou CBS_OWNERDRAWVARIABLE. Si le contrôle est créé par l’éditeur de boîtes de dialogue, ne sera `OnMeasureItem` pas appelé. Cela est dû au fait que le [WM_MEASUREITEM](/windows/win32/Controls/wm-measureitem) message est envoyé tôt dans le processus de création du contrôle. Si vous sous-classez à l’aide de `DDX_Control` , `SubclassDlgItem` ou `SubclassWindow` , le sous-classement se produit généralement après le processus de création. Par conséquent, il n’existe aucun moyen de gérer le message d' [WM_MEASUREITEM](/windows/win32/Controls/wm-measureitem) dans la fonction du contrôle `OnChildNotify` , qui est le mécanisme utilisé par MFC pour implémenter ON_WM_MEASUREITEM_REFLECT.
...
Elle est envoyée au `CWnd` qui possède le menu. `OnMenuChar` est également appelé lorsque l’utilisateur appuie sur ALT et toute autre touche, même si la clé ne correspond pas à un caractère mnémonique. Dans ce cas, *pMenu* pointe vers le menu possédé par `CWnd` , et *nFlags* est égal à 0.
...
*nPos*\
dans Position d’index de l’élément de menu au début de l’opération glisser.
...
*nPos*\
dans Position d’index de l’élément de menu lorsque le bouton droit de la souris a été relâché.
...
Si l' `CWnd` objet est associé à un menu, `OnMenuSelect` est appelé par le Framework lorsque l’utilisateur sélectionne un élément de menu.
...
- MA_ACTIVATE l’activation de l' `CWnd` objet.

- MA_NOACTIVATE n’activez pas l' `CWnd` objet.

- MA_ACTIVATEANDEAT activer `CWnd` l’objet et ignorer l’événement de souris.

- MA_NOACTIVATEANDEAT n’activez pas `CWnd` l’objet et ignorez l’événement de souris.
...
*nFlags*\
dans Combinaison de bits (OR) d’indicateurs qui indiquent les touches de modification qui sont enfoncées. Par exemple, l’indicateur de MK_CONTROL indique que la touche CTRL est enfoncée.

*point*\
dans Objet [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) qui spécifie les coordonnées *x* et *y* du curseur par rapport au coin supérieur gauche de la zone cliente.
...
*nFlags*\
dans Combinaison de bits (OR) d’indicateurs qui indiquent les touches de modification qui sont enfoncées. Par exemple, l’indicateur de MK_CONTROL indique que la touche CTRL est enfoncée. Pour obtenir la liste des indicateurs, consultez le sous-titre « paramètres du message » dans [à propos de l’entrée de la souris](/windows/win32/inputdev/about-mouse-input).

*zDelta*\
dans Indique la distance de rotation de la roue, exprimée en multiples ou divisions de WHEEL_DELTA, soit 120. Une valeur positive indique que la roulette a été pivotée vers la droite ; une valeur négative indique que la roulette a été pivotée vers la gauche.
...
Si la souris n’est pas capturée, le message de WM_MOUSEMOVE est reçu par l' `CWnd` objet situé sous le curseur de la souris ; sinon, le message est envoyé à la fenêtre qui a capturé la souris.
...
L’infrastructure appelle cette fonction membre après le `CWnd` déplacement de l’objet.
...
L’infrastructure appelle cette fonction membre pendant qu’un utilisateur déplace un `CWnd` objet.
...
*lpncsp*<br/>
Pointe vers une structure de données de [NCCALCSIZE_PARAMS](/windows/win32/api/winuser/ns-winuser-nccalcsize_params) qui contient des informations qu’une application peut utiliser pour calculer la nouvelle taille et la position du `CWnd` Rectangle (y compris la zone cliente, les bordures, la légende, les barres de défilement, etc.).
...
Les `rgrc[1]` `rgrc[2]` rectangles et ne sont valides que si *bCalcValidRects* a la valeur true. Dans ce cas, le `rgrc[1]` rectangle contient les coordonnées de la fenêtre avant qu’elle ait été déplacée ou redimensionnée. Le `rgrc[2]` rectangle contient les coordonnées de la zone cliente de la fenêtre avant le déplacement de la fenêtre. Toutes les coordonnées sont relatives à la fenêtre ou à l’écran parent.
...
L’infrastructure appelle cette fonction membre avant le message [WM_CREATE](#oncreate) lorsque l' `CWnd` objet est créé pour la première fois.
...
L’infrastructure appelle cette fonction membre pour l' `CWnd` objet qui contient le curseur (ou l' `CWnd` objet qui a utilisé la fonction membre [SetCapture](#setcapture) pour capturer l’entrée de la souris) chaque fois que la souris est déplacée.
...
*point*<br/>
Spécifie un `CPoint` objet qui contient les coordonnées d’écran x et y de la position du curseur. Ces coordonnées sont toujours relatives à l’angle supérieur gauche de l’écran.
...
L’infrastructure appelle cette fonction membre lorsque l’utilisateur appuie sur le bouton gauche de la souris lorsque le curseur se trouve dans une zone non cliente de l' `CWnd` objet.
...
*nHitTest*\
dans Valeur du test de positionnement retournée par la fonction [CWnd ::D efwindowproc](#defwindowproc) suite au traitement du message [WM_NCHITTEST](/windows/win32/inputdev/wm-nchittest) .

*point*\
dans Objet [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) qui spécifie les coordonnées *x* et *y* du curseur par rapport au coin supérieur gauche de l’écran.
...
*bIsRendering*\
dans TRUE si le rendu de Gestionnaire de fenêtrage (DWM) est activé pour la zone non cliente de la fenêtre ; FALSe si le rendu est désactivé.
...
*Nbouton*\
dans Valeur de le bouton XButton1 si le premier bouton Microsoft IntelliMouse X est double-cliqué, ou XBUTTON2 si l’utilisateur double-clique sur le deuxième bouton X.
...
*Nbouton*\
dans Valeur de le bouton XButton1 si le bouton de la première souris X est enfoncé ou XBUTTON2 si le deuxième bouton X est enfoncé.
...
*Nbouton*\
dans Valeur de le bouton XButton1 si le premier bouton X de la souris est relâché ou XBUTTON2 si le deuxième bouton X est relâché.
...
*lParam*<br/>
Pointeur vers une structure de message de notification ( `NMHDR` ) qui contient le code de notification et des informations supplémentaires. Pour certains messages de notification, ce paramètre pointe vers une structure plus grande qui a la `NMHDR` structure comme premier membre.
...
*pWnd*\
dans Pointeur vers un `CWnd` objet qui représente la fenêtre qui envoie le message de [WM_NOTIFY](/windows/win32/controls/wm-notify) . Ce paramètre est le pointeur vers un contrôle si le paramètre *nLigne* est `NF_QUERY` ou le pointeur vers la fenêtre parente d’un contrôle si *nLigne* est `NF_REQUERY` .

*nLigne*\
dans Valeur de commande qui spécialise le message WM_NOTIFY. Les valeurs possibles sont les suivantes :

- `NF_QUERY` -Le message est une requête qui détermine si les structures ANSI ou Unicode doivent être utilisées dans les messages WM_NOTIFY. Ce message est envoyé à partir d’un contrôle à sa fenêtre parente pendant la création d’un contrôle, et en réponse au `NF_REQUERY` formulaire de ce message.
- `NF_REQUERY` -Le message est une demande pour qu’un contrôle envoie le `NF_QUERY` formulaire de ce message à sa fenêtre parente. Cette requête est envoyée à partir de la fenêtre parente et demande au contrôle de redemander au parent le type de structure à utiliser dans WM_NOTIFY messages. Si le paramètre *nLigne* est `NF_REQUERY` , la valeur de retour est le résultat de l’opération de rerequête.
...
Une fenêtre peut recevoir des messages de peinture internes suite à l’appel de la `RedrawWindow` fonction membre avec l’indicateur RDW_INTERNALPAINT défini. Dans ce cas, la fenêtre peut ne pas avoir de zone de mise à jour. Une application doit appeler la fonction membre [GetUpdateRect](#getupdaterect) pour déterminer si la fenêtre a une région de mise à jour. Si `GetUpdateRect` retourne 0, l’application ne doit pas appeler les fonctions membres [BeginPaint](#beginpaint) et [EndPaint](#endpaint) .

Il est de la responsabilité de l’application de vérifier si une mise à jour ou une mise à jour interne est nécessaire en examinant ses structures de données internes pour chaque message de WM_PAINT, car un message WM_PAINT peut avoir été provoqué par une zone non valide et un appel à la `RedrawWindow` fonction membre avec l’indicateur RDW_INTERNALPAINT défini.

Un message de WM_PAINT interne n’est envoyé qu’une seule fois par Windows. Après qu’un message de WM_PAINT interne a été envoyé à une fenêtre par la `UpdateWindow` fonction membre, aucun autre message de WM_PAINT n’est envoyé ou publié tant que la fenêtre n’est pas invalidée ou jusqu’à ce que la `RedrawWindow` fonction membre soit appelée à nouveau avec l’indicateur RDW_INTERNALPAINT défini.
...
La fonction membre d’un propriétaire du presse-papiers `OnPaintClipboard` est appelée par un presse-papiers quand le propriétaire du presse-papiers a placé des données dans le presse-papiers au format CF_OWNERDISPLAY et que la zone cliente de la visionneuse du presse-papiers doit être redessinée.
...
Pour déterminer si la totalité de la zone cliente ou seulement une partie de celle-ci doit être redessinée, le propriétaire du presse-papiers doit comparer les dimensions de la zone de dessin donnée dans le `rcpaint` membre de la `PAINTSTRUCT` structure aux dimensions indiquées dans l’appel de fonction membre [OnSizeClipboard](#onsizeclipboard) le plus récent.

`OnPaintClipboard` doit utiliser la fonction Windows [GlobalLock](/windows/win32/api/winbase/nf-winbase-globallock) pour verrouiller la mémoire qui contient la `PAINTSTRUCT` structure de données et déverrouiller cette mémoire avec la fonction Windows [GlobalUnlock](/windows/win32/api/winbase/nf-winbase-globalunlock) avant de quitter.
...
La `OnPaletteChanged` fonction membre est appelée pour toutes les fenêtres de niveau supérieur et Overlapped, y compris celle qui a modifié la palette système et a provoqué l’envoi du message de WM_PALETTECHANGED. Si une fenêtre enfant utilise une palette de couleurs, ce message doit lui être transmis.
...
`OnParentNotify`La fonction membre d’un parent est appelée par l’infrastructure lorsque sa fenêtre enfant est créée ou détruite, ou lorsque l’utilisateur clique sur un bouton de la souris alors que le curseur se trouve sur la fenêtre enfant.
...
Lorsque la fenêtre enfant est créée, le système appelle `OnParentNotify` juste avant que la fonction membre [Create](#create) qui crée la fenêtre retourne. Lorsque la fenêtre enfant est détruite, le système appelle `OnParentNotify` avant de détruire la fenêtre.
...
*nPowerEvent*\
dans Événement de gestion de l’alimentation.
...
Si une application retourne 0, la session Windows n’est pas terminée. Windows arrête `OnQueryEndSession` l’appel dès qu’une application retourne 0 et envoie le message de [WM_ENDSESSION](#onendsession) avec une valeur de paramètre false pour toute application qui a déjà retourné une valeur différente de zéro.

## <a name="cwndonquerynewpalette"></a><a name="onquerynewpalette"></a> CWnd :: OnQueryNewPalette

L’infrastructure appelle cette fonction membre lorsque l' `CWnd` objet est sur le point de recevoir le focus d’entrée, ce qui donne la `CWnd` possibilité de réaliser sa palette logique lorsqu’elle reçoit le focus.
...
L’infrastructure appelle cette fonction membre lorsque l' `CWnd` objet est réduit et l’utilisateur demande que le `CWnd` soit restauré à sa taille et à sa position préréduites.
...
Dans `OnQueryOpen` , ne `CWnd` doit effectuer aucune action qui entraînerait une modification d’activation ou de focus (par exemple, la création d’une boîte de dialogue).
...
*nInputCode*\
dans Code d’entrée qui indique si l’entrée s’est produite pendant que l’application était au premier plan ou non. Dans les deux cas, l’application doit appeler [CWnd ::D efwindowproc](#defwindowproc) pour que le système puisse effectuer un nettoyage. Ce paramètre peut prendre l’une des valeurs suivantes :

- `RIM_INPUT` -Une entrée s’est produite pendant que l’application était au premier plan.
- `RIM_INPUTSINK` -Une entrée s’est produite alors que l’application ne se trouvait pas au premier plan.

*hRawInput*\
dans Handle vers une structure [RAWINPUT](/windows/win32/api/winuser/ns-winuser-rawinput) qui contient l’entrée brute de l’appareil.
...
Seules les fenêtres qui ont le style CS_DBLCLKS [WNDCLASS](/windows/win32/api/winuser/ns-winuser-wndclassw) peuvent recevoir des `OnRButtonDblClk` appels. Il s’agit de la valeur par défaut pour Windows dans le bibliothèque MFC (Microsoft Foundation Class). Windows appelle `OnRButtonDblClk` lorsque l’utilisateur appuie sur, relâche, puis appuie à nouveau sur le bouton droit de la souris dans la limite de temps du double-clic du système. Le fait de double-cliquer sur le bouton droit de la souris génère en réalité quatre événements : [WM_RBUTTONDOWN](#onrbuttondown) et [WM_RBUTTONUP](#onrbuttonup) messages, l' `OnRButtonDblClk` appel et un autre WM_RBUTTONUP message lorsque le bouton est relâché.
...
À moins d’être substitué, `OnRegisteredMouseWheel` achemine le message vers la fenêtre appropriée (la fenêtre parente avec le focus) et appelle le gestionnaire de [WM_MOUSEWHEEL](/windows/win32/inputdev/wm-mousewheel) pour cette fenêtre.
...
La fonction membre du propriétaire du presse-papiers `OnRenderAllFormats` est appelée par l’infrastructure lorsque l’application propriétaire est détruite.
...
La fonction membre du propriétaire du presse-papiers `OnRenderFormat` est appelée par l’infrastructure quand un format particulier avec un rendu retardé doit être rendu.
...
*nSessionState*\
dans Un code d’état décrit la modification de l’état de la session.

*nId*\
dans Identificateur de session.
...
L’infrastructure appelle cette fonction membre si l’entrée de la souris n’est pas capturée et la souris provoque le déplacement du curseur dans l' `CWnd` objet.
...
L’implémentation par défaut appelle la fenêtre parente `OnSetCursor` avant le traitement. Si la fenêtre parente retourne la valeur TRUE, le traitement supplémentaire est interrompu. L’appel de la fenêtre parente donne le contrôle de fenêtre parente sur le paramètre du curseur dans une fenêtre enfant.
...
Si *nHitTest* est HTERROR et que *message* est un message de bouton de la souris enfoncé, la `MessageBeep` fonction membre est appelée.

Le paramètre de *message* est 0 lorsque `CWnd` passe en mode de menu.
...
*pOldWnd*<br/>
Contient l' `CWnd` objet qui perd le focus d’entrée (peut être null). Le pointeur peut être temporaire et ne doit pas être stocké pour une utilisation ultérieure.
...
Le Framework appelle `OnSettingChange` toutes les fenêtres de niveau supérieur lorsque la fonction SystemParametersInfo Win32 modifie un paramètre à l’échelle du système.
...
*uFlags*<br/>
Lorsque le système envoie le message à la suite d’un `SystemParametersInfo` appel, ce paramètre est un indicateur qui indique le paramètre système qui a été modifié. Pour obtenir la liste des valeurs, consultez [SystemParametersInfo](/windows/win32/api/winuser/nf-winuser-systemparametersinfow) dans le SDK Windows. Quand une application envoie le message, ce paramètre doit avoir la valeur 0.
...
*nStatus*<br/>
Spécifie l’état de la fenêtre affichée. Elle est égale à 0 si le message est envoyé en raison d’un `ShowWindow` appel de fonction membre ; sinon, *nStatus* est l’un des éléments suivants :
...
Si la fonction membre [SetScrollPos](#setscrollpos) ou [MoveWindow](#movewindow) est appelée pour une fenêtre enfant à partir de `OnSize` , le `bRedraw` paramètre de `SetScrollPos` ou `MoveWindow` doit être différent de zéro pour provoquer le `CWnd` redessin de.
...
La fonction membre du propriétaire du presse-papiers `OnSizeClipboard` est appelée par la visionneuse du presse-papiers lorsque le presse-papiers contient des données avec l' `CF_OWNERDISPLAY` attribut et que la taille de la zone cliente de la fenêtre de la visionneuse du presse-papiers a changé.
...
La `OnSizeClipboard` fonction membre est appelée avec un rectangle null (0, 0, 0, 0) comme nouvelle taille lorsque l’application du presse-papiers est sur le point d’être détruite ou réduite. Cela permet au propriétaire du presse-papiers de libérer ses ressources d’affichage.
...
L’infrastructure appelle cette fonction membre si `CWnd` a le focus d’entrée et si les messages [WM_SYSKEYUP](#onsyskeyup) et [WM_SYSKEYDOWN](#onsyskeydown) sont traduits.
...
Windows appelle une `OnSysColorChange` fenêtre qui est affectée par une modification de la couleur système.
...
- SC_HOTKEY activer l' `CWnd` objet associé à la touche d’accès rapide spécifiée par l’application. Le mot de poids faible de *lParam* identifie le HWND de la fenêtre à activer.

- SC_HSCROLL faire défiler horizontalement.

- SC_KEYMENU récupérer un menu à l’aide d’une séquence de touches.

- SC_MAXIMIZE (ou SC_ZOOM) maximise l' `CWnd` objet.

- SC_MINIMIZE (ou SC_ICON) réduisent l' `CWnd` objet.

- SC_MOUSEMENU récupérer un menu à l’aide d’un clic de souris.

- SC_MOVE déplacer l' `CWnd` objet.
...
- SC_SCREENSAVE exécute l’application d’économiseur d’écran spécifiée dans la section [boot] du fichier SYSTEM.INI.

- SC_SIZE taille de l' `CWnd` objet.
...
Les éléments de menu d’un menu de contrôle peuvent être modifiés à l’aide des `GetSystemMenu` `AppendMenu` fonctions membres,, `InsertMenu` et `ModifyMenu` . Les applications qui modifient le menu de contrôle doivent traiter les messages WM_SYSCOMMAND, et les messages WM_SYSCOMMAND non gérés par l’application doivent être transmis à `OnSysCommand` . Toutes les valeurs de commande ajoutées par une application doivent être traitées par l’application et ne peuvent pas être passées à `OnSysCommand` .

Une application peut exécuter n’importe quelle commande système à tout moment en passant une WM_SYSCOMMAND message à `OnSysCommand` .

Les séquences de touches d’accélérateur (raccourci) qui sont définies pour sélectionner des éléments dans le menu de contrôle sont traduites en `OnSysCommand` appels ; toutes les autres séquences de touches d’accélérateur sont converties en messages de [WM_COMMAND](#oncommand) .
...
L’infrastructure appelle cette fonction membre si l' `CWnd` objet a le focus d’entrée lorsque la fonction membre [OnSysKeyUp](#onsyskeyup) ou [OnSysKeyDown](#onsyskeydown) est appelée.
...
Si l' `CWnd` objet a le focus d’entrée, la `OnSysKeyDown` fonction membre est appelée par l’infrastructure quand l’utilisateur maintient la touche Alt enfoncée, puis appuie sur une autre touche.
...
Si aucune fenêtre n’a actuellement le focus d’entrée, la fonction membre de la fenêtre active `OnSysKeyDown` est appelée. L' `CWnd` objet qui reçoit le message peut faire la distinction entre ces deux contextes en vérifiant le code de contexte dans `nFlags` .

Quand le code de contexte est 0, le message WM_SYSKEYDOWN reçu par `OnSysKeyDown` peut être passé à la fonction Windows [TranslateAccelerator](/windows/win32/api/winuser/nf-winuser-translateacceleratorw) , qui le gère comme s’il s’agissait d’un message de clé normal au lieu d’un message de clé système. Cela permet d’utiliser les touches d’accès rapide avec la fenêtre active, même si la fenêtre active n’a pas le focus d’entrée.

En raison de la répétition automatique, plusieurs `OnSysKeyDown` appels peuvent se produire avant la réception du message de [WM_SYSKEYUP](#onsyskeyup) . L’état de clé précédent (bit 14) peut être utilisé pour déterminer si l' `OnSysKeyDown` appel indique la première transition vers le haut ou une transition répétée.
...
Si l' `CWnd` objet a le focus, la `OnSysKeyUp` fonction membre est appelée par l’infrastructure quand l’utilisateur relâche une touche qui a été enfoncée alors que la touche Alt était maintenue enfoncée.
...
Si aucune fenêtre n’a actuellement le focus d’entrée, la fonction membre de la fenêtre active `OnSysKeyUp` est appelée. L' `CWnd` objet qui reçoit l’appel peut faire la distinction entre ces deux contextes en vérifiant le code de contexte dans *nFlags*.

Quand le code de contexte est 0, le message WM_SYSKEYUP reçu par `OnSysKeyUp` peut être passé à la fonction Windows [TranslateAccelerator](/windows/win32/api/winuser/nf-winuser-translateacceleratorw) , qui le gère comme s’il s’agissait d’un message de clé normal au lieu d’un message de clé système. Cela permet d’utiliser les touches accélérateur (raccourci) avec la fenêtre active, même si la fenêtre active n’a pas le focus d’entrée.
...
*nChar*\
dans Spécifie le code de caractère de la touche enfoncée.

*nRepCnt*\
dans Spécifie le nombre de répétitions pour le message actuel. La valeur est le nombre de fois que la frappe est répétée de manière répétée à la suite de l’utilisateur qui maintient la touche enfoncée. Si la frappe de touche est suffisamment longue, plusieurs messages sont envoyés. Toutefois, le nombre de répétitions n’est pas cumulatif.

*nFlags*\
dans Indicateurs qui spécifient le code d’analyse, la clé étendue, le code de contexte, l’état de la touche précédente et l’état de transition, comme indiqué dans le tableau suivant :

| Bits d’indicateur | Description |
|--|--|
| **0-7** | Spécifie le code d’analyse. La valeur dépend du fabricant d’ordinateurs OEM (Original Equipment Manufacturer). |
| **8** | Spécifie une clé étendue, telle que les touches ALT et CTRL de droite qui s’affichent sur un clavier amélioré 101 ou 102 touches. L’indicateur est 1 si la clé est une clé étendue ; Sinon, la valeur est 0. |
| **9-12** | Utilisé en interne par Windows. |
| **13** | Spécifie le code de contexte. L’indicateur est 1 si la touche ALT est maintenue enfoncée pendant que la touche est enfoncée ; dans le cas contraire, la valeur est 0. |
| **14**   spécifie l’état de la clé précédente. L’indicateur a la valeur 1 si la touche est enfoncée avant l’envoi du message, ou 0 si la touche est active. |
| **15** | Spécifie l’état de transition. L’indicateur est 1 si la touche est relâchée, ou 0 si la touche est enfoncée. |
...
*pMenu*\
dans Pointeur vers l’objet [CMenu](../../mfc/reference/cmenu-class.md) qui représente le menu ou le sous-menu.

*nFlags*\
dans Menu qui a été détruit. Actuellement, il ne peut s’agir que du menu fenêtre, MF_SYSMENU.
...
Si l' `CWnd` objet possède une zone de liste avec le style [LBS_WANTKEYBOARDINPUT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) , la zone de liste envoie le message WM_VKEYTOITEM en réponse à un message WM_KEYDOWN.
...
*nPos*<br/>
Contient la position actuelle de la zone de défilement si le code de la barre de défilement est SB_THUMBPOSITION ou SB_THUMBTRACK ; Sinon, non utilisé. En fonction de la plage de défilement initiale, *nPos* peut être négatif et doit être converti en un **`int`** si nécessaire.
...
`OnVScroll` en général, est utilisé par les applications qui fournissent des commentaires lorsque la case de défilement est glissée.

Si `OnVScroll` fait défiler le contenu de l' `CWnd` objet, il doit également réinitialiser la position de la case de défilement avec la fonction membre [SetScrollPos](#setscrollpos) .
...
La fonction membre du propriétaire du presse-papiers `OnVScrollClipboard` est appelée par la visionneuse du presse-papiers lorsque le format des données du presse-papiers est CF_OWNERDISPLAY et qu’il y a un événement dans la barre de défilement verticale de la visionneuse du presse-papiers.
...
*bIsMaximized*\
dans TRUE si la fenêtre active est agrandie, et FALSe si ce n’est pas le cas.
...
*lpwndpos*<br/>
Pointe vers une `WINDOWPOS` structure de données qui contient des informations sur les nouvelles taille et position de la fenêtre.
...
La fonction Windows [SystemParametersInfo](/windows/win32/api/winuser/nf-winuser-systemparametersinfow) appelle `OnWinIniChange` une fois qu’une application a utilisé la fonction pour modifier un paramètre dans le fichier WIN.INI.
...
Si une application modifie plusieurs sections différentes dans WIN.INI en même temps, l’application doit envoyer un message WM_WININICHANGE avec *lpszSection* défini sur null. Dans le cas contraire, une application doit envoyer des WM_WININICHANGE chaque fois qu’une modification est apportée à WIN.INI.

Si une application reçoit un `OnWinIniChange` appel avec *lpszSection* défini sur null, l’application doit vérifier toutes les sections de WIN.INI qui affectent l’application.
...
Cette fonction membre est appelée par `WindowProc` , ou est appelée pendant la réflexion de message.
...
*Nbouton*\
dans Valeur de le bouton XButton1 si l’utilisateur a cliqué sur le premier bouton Microsoft IntelliMouse X ou XBUTTON2 si l’utilisateur a cliqué sur le deuxième bouton X.
...
*Nbouton*\
dans La valeur le bouton XButton1 si le premier bouton Microsoft IntelliMouse X a été double-cliqué, ou XBUTTON2 si l’utilisateur a double-cliqué sur le deuxième bouton X.
...
L' `CWnd` objet actuel ne devient pas le propriétaire du presse-papiers tant que la fonction Windows [EmptyClipboard](/windows/win32/api/winuser/nf-winuser-emptyclipboard) n’est pas appelée.
...
Les classes dérivées peuvent utiliser cette fonction pour un nettoyage personnalisé tel que la suppression du **`this`** pointeur.

## <a name="cwndprecreatewindow"></a><a name="precreatewindow"></a> CWnd ::P reCreateWindow

Appelé par le Framework avant la création de la fenêtre Windows attachée à cet `CWnd` objet.
...
> [!WARNING]
> `CWnd::PreCreateWindow` affecte à présent le membre hMenu de *CS* au **`this`** pointeur si le menu a la valeur null et que le style contient WS_CHILD. Pour une fonctionnalité appropriée, vérifiez que votre contrôle de boîte de dialogue a un ID qui n’est pas NULL.
>
> Cette modification résout un incident dans les scénarios d’interopérabilité managés/natifs. Une instruction de TRACE dans `CWnd::Create` alerte le développeur du problème.
...
Les indicateurs suivants contrôlent le moment où un redessin se produit. La peinture n’est pas effectuée par la `RedrawWindow` fonction, sauf si l’un de ces bits est spécifié.
...
Par défaut, les fenêtres affectées par la `RedrawWindow` fonction varient selon que la fenêtre spécifiée a le style de WS_CLIPCHILDREN. Les fenêtres enfants de WS_CLIPCHILDREN fenêtres ne sont pas affectées. Toutefois, les fenêtres qui ne sont pas WS_CLIPCHILDREN Windows sont validées ou invalidées de manière récursive jusqu’à ce qu’une fenêtre WS_CLIPCHILDREN soit rencontrée. Les indicateurs suivants contrôlent les fenêtres affectées par la `RedrawWindow` fonction :
...
L’effet de la `ReleaseDC` fonction membre dépend du type de contexte de périphérique.

L’application doit appeler la `ReleaseDC` fonction membre pour chaque appel à la fonction membre [GetWindowDC](#getwindowdc) et pour chaque appel à la fonction membre [GetDC](#getdc) .
...
Par défaut, `ContinueModal` retourne false après l' `EndModalLoop` appel de. Retourne la valeur fournie en tant que *nrésultat* à `EndModalLoop` .
...
La `ScreenToClient` fonction membre remplace les coordonnées d’écran fournies dans *LpPoint* ou *lpRect* par les coordonnées clientes. Les nouvelles coordonnées sont relatives au coin supérieur gauche de la `CWnd` zone cliente.
...
Fait défiler le contenu de la zone cliente de l' `CWnd` objet actuel.
...
La zone non couverte par la `ScrollWindow` fonction membre n’est pas repeinte, mais elle est combinée dans la `CWnd` région de mise à jour de l’objet actuel. L’application recevra finalement un message [WM_PAINT](/windows/win32/gdi/wm-paint) l’informant que la région doit être redessinée. Pour repeindre la zone non couverte en même temps que le défilement, appelez la fonction membre [UpdateWindow](#updatewindow) immédiatement après avoir appelé `ScrollWindow` .

Si *lpRect* a la valeur null, les positions des fenêtres enfants dans la fenêtre sont décalées par rapport à la quantité spécifiée par *xAmount* et *yAmount*, et toutes les zones non valides (non peintes) dans le `CWnd` sont également décalées. `ScrollWindow` est plus rapide lorsque *lpRect* a la valeur null.

Si *lpRect* n’a pas la valeur null, les positions des fenêtres enfants ne sont pas modifiées, et les zones non valides dans ne `CWnd` sont pas compensées. Pour éviter les problèmes de mise à jour lorsque *lpRect* n’est pas null, appelez la `UpdateWindow` fonction membre pour repeindre `CWnd` avant d’appeler `ScrollWindow` .
...
*lpRectUpdate*<br/>
Pointe vers une `RECT` structure qui reçoit les limites du rectangle invalidées en faisant défiler. Ce paramètre peut être NULL.
...
`SendChildNotifyLastMsg` envoie le message actuel à la source s’il s’agit d’un message qui est reflété.
...
La `SendDlgItemMessage` fonction membre ne retourne pas de réponse tant que le message n’a pas été traité.

L’utilisation de `SendDlgItemMessage` est identique à l’obtention d’un `CWnd` * vers le contrôle donné et à l’appel de la fonction membre [SendMessage](#sendmessage) .
...
La `SendMessage` fonction membre appelle directement la procédure de fenêtre et n’est pas retournée tant que cette procédure de fenêtre n’a pas traité le message. Cela diffère de la fonction membre [PostMessage](#postmessage) , qui place le message dans la file d’attente de messages de la fenêtre et est retourné immédiatement.
...
La `SetActiveWindow` fonction membre doit être utilisée avec précaution, car elle permet à une application de prendre arbitrairement la fenêtre active et le focus d’entrée. Normalement, Windows prend en charge toutes les activations.

## <a name="cwndsetcapture"></a><a name="setcapture"></a> CWnd :: SetCapture

Entraîne l’envoi de toutes les entrées de souris suivantes à l' `CWnd` objet actuel, quelle que soit la position du curseur.
...
Lorsque `CWnd` ne requiert plus l’entrée de la souris, l’application doit appeler la fonction [ReleaseCapture](/windows/win32/api/winuser/nf-winuser-releasecapture) afin que d’autres fenêtres puissent recevoir l’entrée de la souris.
...
La `SetCaretPos` fonction membre déplace le signe insertion uniquement si elle appartient à une fenêtre de la tâche actuelle. `SetCaretPos` déplace le signe insertion si le signe insertion est masqué ou non.
...
Cette fonction membre envoie un message WM_DRAWCLIPBOARD à la fenêtre. Étant donné que le handle vers la fenêtre suivante dans la chaîne du presse-papiers n’a pas encore été retourné, l’application ne doit pas passer sur le WM_DRAWCLIPBOARD message qu’elle reçoit pendant l’appel à `SetClipboardViewer` .
...
`SetDlgItemInt` envoie un message d' [WM_SETTEXT](/windows/win32/winmsg/wm-settext) au contrôle donné.
...
`SetDlgItemText` envoie un message d' [WM_SETTEXT](/windows/win32/winmsg/wm-settext) au contrôle donné.
...
La `SetFocus` fonction membre envoie un message [WM_KILLFOCUS](/windows/win32/inputdev/wm-killfocus) à la fenêtre qui perd le focus d’entrée et un message [WM_SETFOCUS](/windows/win32/inputdev/wm-setfocus) à la fenêtre qui reçoit le focus d’entrée. Il active également la fenêtre ou son parent.
...
*bAlpha*<br/>
Valeur alpha utilisée pour décrire l’opacité de la fenêtre superposée. Pour plus d’informations, consultez le `SourceConstantAlpha` membre de la structure [BLENDFUNCTION](/windows/win32/api/wingdi/ns-wingdi-blendfunction) . Quand *bAlpha* a la valeur 0, la fenêtre est complètement transparente. Quand *bAlpha* est 255, la fenêtre est opaque.
...
Appelez cette fonction membre pour définir les informations que la `SCROLLINFO` structure gère à propos d’une barre de défilement.
...
Les gestionnaires de messages Windows MFC qui indiquent la position de la barre de défilement, [CWnd :: OnHScroll](#onhscroll) et [CWnd :: OnVScroll](#onvscroll), fournissent uniquement 16 bits de données de position. [GetScrollInfo](#getscrollinfo) et `SetScrollInfo` fournissent 32 bits de données de position de la barre de défilement. Ainsi, une application peut appeler `GetScrollInfo` lors du traitement `CWnd::OnHScroll` `CWnd::OnVScroll` de ou pour obtenir des données de position de la barre de défilement 32 bits.
...
Si l’appel à `SetScrollRange` suit immédiatement un appel à la fonction membre [SetScrollPos](#setscrollpos) , le paramètre *bRedraw* de la `SetScrollPos` fonction membre doit avoir la valeur 0 pour empêcher la barre de défilement d’être dessinée deux fois.
...
Une valeur d’intervalle est spécifiée, et chaque fois que l’intervalle est écoulé, le système publie un message d’WM_TIMER dans la file d’attente des messages d’installation de l’application d’installation ou le transmet à une fonction de rappel définie par l’application `TimerProc` .
...
Cet exemple utilise `CWnd::SetTimer` , `CWnd::OnTimer` et `CWnd::KillTimer` pour gérer les messages de WM_TIMER. La première minuterie est configurée pour envoyer un message WM_TIMER à la fenêtre frame principale toutes les 2 secondes dans `OnStartTimer` . Le `OnTimer` Gestionnaire d’événements gère les messages de WM_TIMER pour la fenêtre frame principale. Cette méthode fait en sorte que le haut-parleur du PC émette un bip sonore toutes les 2 secondes. La deuxième minuterie envoie un message à la fonction de rappel toutes les 3,75 secondes. `OnStopTimer` arrêtera les deux minuteries en appelant `CWnd::KillTimer` pour chaque ID de minuteur.
...
Pour créer une fenêtre de niveau supérieur, appelez `SetWindowPos` avec le paramètre *pWndInsertAfter* égal à **&wndTopMost**, ou définissez le style WS_EX_TOPMOST lors de la création de la fenêtre.
...
Si `SetWindowPos` est appelé lorsque le paramètre *pWndInsertAfter* est **&wndBottom** et qu' `CWnd` il s’agit d’une fenêtre de premier plan, la fenêtre perd son état le plus haut (WS_EX_TOPMOST est désactivée) et le système place la fenêtre au bas de l’ordre de plan.
...
La `ShowCaret` fonction membre affiche le signe insertion uniquement si elle a une forme actuelle et n’a pas été masquée deux fois ou plus consécutivement. Si le signe insertion n’appartient pas à cette fenêtre, le signe insertion n’est pas affiché.

Le masquage du signe insertion est cumulatif. Si la fonction membre [HideCaret](#hidecaret) a été appelée cinq fois de suite, `ShowCaret` doit être appelée cinq fois pour afficher le signe insertion.
...
*nCmdShow*<br/>
Spécifie comment doit `CWnd` être affiché. Il doit s’agir de l’une des valeurs suivantes :
...
`ShowWindow` doit être appelé une seule fois par application pour la fenêtre principale avec [CWinApp :: m_nCmdShow](../../mfc/reference/cwinapp-class.md#m_ncmdshow). Les appels suivants à `ShowWindow` doivent utiliser l’une des valeurs listées ci-dessus au lieu de celui spécifié par `CWinApp::m_nCmdShow` .
...
Appelez cette fonction membre pour « sous-classe dynamiquement » un contrôle créé à partir d’un modèle de boîte de dialogue et attachez-le à cet `CWnd` objet.
...
Lorsqu’un contrôle est sous-classé de manière dynamique, les messages Windows sont acheminés via la `CWnd` table des messages de et les gestionnaires de messages de l’appel de la `CWnd` classe de. Les messages passés à la classe de base sont passés au gestionnaire de messages par défaut dans le contrôle.

Cette fonction membre attache le contrôle Windows à un `CWnd` objet et remplace les `WndProc` fonctions et du contrôle `AfxWndProc` . La fonction stocke l’ancien `WndProc` à l’emplacement retourné par la `GetSuperWndProcAddr` fonction membre.
...
Appelez cette fonction membre pour « sous-classe dynamiquement » une fenêtre et attachez-la à cet `CWnd` objet.
...
Quand une fenêtre est sous-classée dynamiquement, les messages Windows sont acheminés via la `CWnd` table des messages de et les gestionnaires de messages de l’appel de la `CWnd` classe de la classe de. Les messages passés à la classe de base sont passés au gestionnaire de messages par défaut dans la fenêtre.

Cette fonction membre attache le contrôle Windows à un `CWnd` objet et remplace les fonctions et de la fenêtre `WndProc` `AfxWndProc` . La fonction stocke un pointeur vers l’ancien `WndProc` dans l' `CWnd` objet.
...
Appelez cette fonction membre pour `WndProc` rétablir sa valeur d’origine et détacher la fenêtre identifiée par HWND de l' `CWnd` objet.
...
*pptDst*<br/>
Pointeur vers une `POINT` structure qui spécifie la nouvelle position à l’écran de la fenêtre superposée. Si la position actuelle n’est pas modifiée, *pptDst* peut avoir la valeur null.

*psize*<br/>
Pointeur vers une `SIZE` structure qui spécifie la nouvelle taille de la fenêtre superposée. Si la taille de la fenêtre n’est pas modifiée, *psize* peut avoir la valeur null.
...
*pptSrc*<br/>
Pointeur vers une `POINT` structure qui spécifie l’emplacement de la couche dans le contexte de périphérique.
...
La `UpdateWindow` fonction membre envoie un message de WM_PAINT directement, en ignorant la file d’attente de l’application. Si la région de mise à jour est vide, WM_PAINT n’est pas envoyé.
...
La fonction membre [BeginPaint](#beginpaint) valide automatiquement la totalité de la zone client. Ni [ValidateRect](#validaterect) ni la `ValidateRgn` fonction membre ne doivent être appelés si une partie de la région de mise à jour doit être validée avant la génération du message d' [WM_PAINT](/windows/win32/gdi/wm-paint) suivant.
...
`WindowFromPoint` ne récupère pas une fenêtre masquée ou désactivée, même si le point se trouve dans la fenêtre. Une application doit utiliser la fonction membre [ChildWindowFromPoint](#childwindowfrompoint) pour une recherche non restrictive.
