...
|[CTaskDialog::ClickCommandControl](#clickcommandcontrol)|Clique sur un bouton de commande ou courantes par programme.|
|[CTaskDialog::ClickRadioButton](#clickradiobutton)|Clique sur un bouton radio par programmation.|
...
|[CTaskDialog::GetCommonButtonFlag](#getcommonbuttonflag)|Convertit un standard associé à un bouton de Windows pour le type de bouton commun le `CTaskDialog` classe.|
|[CTaskDialog::GetCommonButtonId](#getcommonbuttonid)|Convertit un des types de bouton courantes liées à la `CTaskDialog` classe à un bouton Windows standard.|
...
|[CTaskDialog::GetSelectedRadioButtonID](#getselectedradiobuttonid)|Retourne le bouton de case d’option sélectionnée.|
...
|[CTaskDialog::IsCommandControlEnabled](#iscommandcontrolenabled)|Détermine si un bouton de commande ou d’un bouton courants est activé.|
...
|[CTaskDialog::RemoveAllCommandControls](#removeallcommandcontrols)|Supprime tous les contrôles de commande à partir de la `CTaskDialog`.|
...
|[CTaskDialog::SetCommonButtonOptions](#setcommonbuttonoptions)|Met à jour un sous-ensemble des boutons courants pour être activé et requièrent une élévation de UAC.|
|[CTaskDialog::SetCommonButtons](#setcommonbuttons)|Ajoute les boutons courants pour la `CTaskDialog`.|
...
|[CTaskDialog::SetDefaultRadioButton](#setdefaultradiobutton)|Spécifie le bouton de case d’option par défaut.|
|[CTaskDialog::SetDialogWidth](#setdialogwidth)|Ajuste la largeur de la `CTaskDialog`.|
|[CTaskDialog::SetExpansionArea](#setexpansionarea)|Met à jour de la zone d’expansion de le `CTaskDialog`.|
|[CTaskDialog::SetFooterIcon](#setfootericon)|Met à jour de l’icône de pied de page pour le `CTaskDialog`.|
|[CTaskDialog::SetFooterText](#setfootertext)|Met à jour le texte sur le pied de page le `CTaskDialog`.|
|[CTaskDialog::SetMainIcon](#setmainicon)|Met à jour de l’icône principale de la `CTaskDialog`.|
|[CTaskDialog::SetMainInstruction](#setmaininstruction)|Met à jour de l’instruction principale de la `CTaskDialog`.|
|[CTaskDialog::SetOptions](#setoptions)|Configure les options pour le `CTaskDialog`.|
|[CTaskDialog::SetProgressBarMarquee](#setprogressbarmarquee)|Configure une barre de type texte défilant pour le `CTaskDialog` et l’ajoute à la boîte de dialogue.|
...
|[CTaskDialog::TaskDialogCallback](#taskdialogcallback)|Le framework appelle en réponse à différents messages de Windows.|
...
|`m_footerIcon`|L’icône dans le pied de page le `CTaskDialog`.|
|`m_hWnd`|Un handle vers la fenêtre pour le `CTaskDialog`.|
...
|`m_nButtonElevation`|Un masque qui indique les boutons courants nécessite une élévation de UAC.|
|`m_nButtonId`|L’ID du contrôle de bouton de commande sélectionnée.|
|`m_nCommonButton`|Un masque qui indique les boutons courants sont affichés sur le `CTaskDialog`.|
|`m_nDefaultCommandControl`|Contrôle de l’ID du bouton de commande qui est sélectionné lorsque la `CTaskDialog` s’affiche.|
|`m_nDefaultRadioButton`|Contrôle de l’ID de la case d’option qui est sélectionnée lorsque le `CTaskDialog` s’affiche.|
...
|`m_strContent`|La chaîne de contenu de la `CTaskDialog`.|
...
|`m_strFooter`|Le pied de page le `CTaskDialog`.|
|`m_strInformation`|Les informations étendues pour le `CTaskDialog`.|
|`m_strMainInstruction`|L’instruction principale de la `CTaskDialog`.|
|`m_strTitle`|Le titre de la `CTaskDialog`.|
|`m_strVerification`|La chaîne qui le `CTaskDialog` affiche à droite de la case à cocher de vérification.|

## <a name="remarks"></a>Notes

Le `CTaskDialog` classe remplace la boîte de message Windows standard et possède des fonctionnalités supplémentaires telles que de nouveaux contrôles pour rassembler des informations à partir de l’utilisateur. Cette classe se trouve dans la bibliothèque MFC dans Visual Studio 2010 et versions ultérieures. Le `CTaskDialog` est disponible à partir de Windows Vista. Les versions antérieures de Windows ne peut pas afficher le `CTaskDialog` objet. Utilisez `CTaskDialog::IsSupported` pour déterminer lors de l’exécution si l’utilisateur actuel peut afficher la boîte de dialogue de tâche. La boîte de message Windows standard est toujours pris en charge.

Le `CTaskDialog` est disponible uniquement lorsque vous générez votre application à l’aide de la bibliothèque Unicode.

Le `CTaskDialog` possède deux constructeurs différents. Un constructeur vous permet de spécifier deux boutons de commande et un maximum de six contrôles bouton normal. Vous pouvez ajouter des boutons de commande plus après avoir créé le `CTaskDialog`. Le deuxième constructeur ne prend pas en charge des boutons de commande, mais vous pouvez ajouter un nombre illimité de contrôles de bouton normal. Pour plus d’informations sur les constructeurs, consultez [CTaskDialog::CTaskDialog](#ctaskdialog).

L’illustration suivante montre un exemple `CTaskDialog` pour illustrer l’emplacement de certains contrôles.

![Exemple de CTaskDialog](../../mfc/reference/media/ctaskdialogsample.png "ctaskdialogsample") exemple de CTaskDialog
...
[in] Numéro d’identification de contrôle de commande.
...
[in] Un paramètre booléen qui indique si une commande nécessite une élévation.

### <a name="remarks"></a>Notes

Le `CTaskDialog Class` peut afficher un nombre illimité de contrôles de bouton de commande. Toutefois, si un `CTaskDialog` affiche un bouton de commande contrôle, il peut afficher un maximum de six boutons. Si un `CTaskDialog` dispose d’aucun contrôle de bouton de commande, il peut afficher un nombre illimité de boutons.
...
*nRadioButtonID*<br/>
[in] Le numéro d’identification de la case.
...
Boutons de la case d’option pour le [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md) vous permettent de rassembler des informations de l’utilisateur. Utilisez la fonction [CTaskDialog::GetSelectedRadioButtonID](#getselectedradiobuttonid) pour déterminer quel bouton de case d’option est sélectionnée.

Le `CTaskDialog` ne nécessite pas que le *nRadioButtonID* paramètres sont uniques pour chaque case d’option. Toutefois, vous risquez de rencontrer un comportement inattendu si vous n’utilisez pas un identificateur distinct pour chaque bouton radio.
...
Clique sur un bouton de commande ou courantes par programme.
...
*nCommandControlID*<br/>
[in] L’ID de commande du contrôle de cliquer sur.
...
Clique sur un bouton radio par programmation.
...
*nRadioButtonID*<br/>
[in] L’ID de la case de cliquer sur.
...
*strContent*<br/>
[in] La chaîne à utiliser pour le contenu de la `CTaskDialog`.

*strMainInstruction*<br/>
[in] L’instruction principale de la `CTaskDialog`.

*strTitle*<br/>
[in] Le titre de la `CTaskDialog`.

*nCommonButtons*<br/>
[in] Masque les boutons courants à ajouter à la `CTaskDialog`.

*nTaskDialogOptions*<br/>
[in] Le jeu d’options à utiliser pour le `CTaskDialog`.

*strFooter*<br/>
[in] La chaîne à utiliser en tant que le pied de page.

*nIDCommandControlsFirst*<br/>
[in] L’ID de chaîne de la première commande.

*nIDCommandControlsLast*<br/>
[in] L’ID de chaîne de la dernière commande.
...
Le deuxième constructeur crée les contrôles de bouton de commande à l’aide des données à partir du fichier de ressources de votre application. La table de chaînes dans le fichier de ressources a plusieurs chaînes avec l’ID de chaîne associée. Cette méthode ajoute un contrôle de bouton de commande pour chaque entrée valide dans la table de chaînes entre *nIDCommandControlsFirst* et *nCommandControlsLast*, inclusif. Pour ces contrôles de bouton de commande, la chaîne dans la table de chaînes est la légende du contrôle et l’ID de chaîne est l’ID. du contrôle
...
*hParent*<br/>
[in] La fenêtre parente pour le `CTaskDialog`.

### <a name="return-value"></a>Valeur de retour

Entier qui correspond à la sélection faite par l’utilisateur.

### <a name="remarks"></a>Notes

Affiche cette instance de la [CTaskDialog](../../mfc/reference/ctaskdialog-class.md). L’application puis attend que l’utilisateur fermer la boîte de dialogue.
...
*nButtonId*<br/>
[in] La valeur de bouton Windows standard.

### <a name="return-value"></a>Valeur de retour

La valeur correspondante `CTaskDialog` bouton courants. S’il n’existe aucun bouton courantes correspondant, cette méthode retourne 0.

##  <a name="getcommonbuttonid"></a>  CTaskDialog::GetCommonButtonId

Convertit un des types de bouton courantes liées à la [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md) à un bouton Windows standard.
...
*nFlag*<br/>
[in] Le type de bouton courantes liées à la `CTaskDialog` classe.
...
Les indicateurs pour la `CTaskDialog`.
...
Il est inutile d’utiliser cette méthode pour récupérer l’ID du bouton de commande sélectionné par l’utilisateur. Cet ID est retourné par [CTaskDialog::DoModal](#domodal) ou [CTaskDialog::ShowDialog](#showdialog).
...
Retourne le bouton de case d’option sélectionnée.
...
*nCommandControlID*<br/>
[in] ID de contrôle de bouton de commande ou du bouton à tester.
...
Vous pouvez utiliser cette méthode pour déterminer la disponibilité de deux contrôles de bouton de commande et les boutons courants de la `CTaskDialog` classe *.

Si *nCommandControlID* ne concerne pas un identificateur valide soit un commun `CTaskDialog` bouton ou un contrôle de bouton de commande, cette méthode lève une exception.
...
TRUE si l’ordinateur prend en charge le `CTaskDialog`; FALSE sinon.

### <a name="remarks"></a>Notes

Utilisez cette fonction pour déterminer lors de l’exécution si l’ordinateur qui exécute votre application prend en charge la `CTaskDialog` classe. Si l’ordinateur ne prend pas en charge la `CTaskDialog`, vous devez fournir une autre méthode pour communiquer des informations à l’utilisateur. Votre application se bloque si elle tente d’utiliser un `CTaskDialog` sur un ordinateur qui ne prend pas en charge la `CTaskDialog` classe.
...
Cette méthode crée les contrôles de bouton de commande à l’aide des données à partir du fichier de ressources de votre application. La table de chaînes dans le fichier de ressources a plusieurs chaînes avec l’ID de chaîne associée. Nouveaux contrôles de bouton de commande ajoutées à l’aide de cette méthode utilisent la chaîne pour la légende du contrôle et l’ID de chaîne pour l’ID. du contrôle La plage de chaînes sélectionné est fournie par *nIDCommandControlsFirst* et *nCommandControlsLast*, inclusif. S’il existe une entrée de vide dans la plage, la méthode n’ajoute pas d’un contrôle de bouton de commande pour cette entrée.
...
Ajoute des contrôles de bouton de case d’option à l’aide des données à partir de la table de chaînes.
...
[in] L’ID de chaîne du premier bouton radio.

*nIDRadioButtonsLast*<br/>
[in] L’ID de chaîne du dernier bouton radio.

### <a name="remarks"></a>Notes

Cette méthode crée des cases d’option à l’aide des données à partir du fichier de ressources de votre application. La table de chaînes dans le fichier de ressources a plusieurs chaînes avec l’ID de chaîne associée. Nouveaux boutons radio ajoutés à l’aide de cette méthode destiné à la chaîne pour la légende du bouton de case d’option et l’ID de chaîne ID. du bouton radio La plage de chaînes sélectionné est fournie par *nIDRadioButtonsFirst* et *nRadioButtonsLast*, inclusif. S’il existe une entrée de vide dans la plage, la méthode n’ajoute pas d’un bouton radio pour cette entrée.

Par défaut, les nouveaux boutons radio sont activés.
...
*oTaskDialog*<br/>
[in] Le `CTaskDialog` qui reçoit le focus.

### <a name="remarks"></a>Notes

Cette méthode masque actuel `CTaskDialog` lorsqu’il affiche le *oTaskDialog*. Le *oTaskDialog* s’affiche dans le même emplacement que l’actuel `CTaskDialog`.
...
*nCommandControlID*<br/>
[in] L’ID du contrôle de bouton de commande sélectionné par l’utilisateur.
...
*bExpanded*<br/>
[in] Une valeur différente de zéro indique que les informations supplémentaires sont affichées ; 0 indique que les informations supplémentaires sont masquées.
...
*strHref*<br/>
[in] Chaîne qui représente le lien hypertexte.
...
Cette méthode appelle [ShellExecute](/windows/desktop/api/shellapi/nf-shellapi-shellexecutea) avant retourne S_OK.
...
*nRadioButtonID*<br/>
[in] L’ID du contrôle de bouton de case d’option l’utilisateur a cliqué.
...
*lTime*<br/>
[in] Durée en millisecondes depuis le `CTaskDialog` a été créé ou la minuterie a été réinitialisée.
...
*bChecked*<br/>
[in] TRUE indique que la case à cocher de vérification est sélectionnée ; La valeur FALSE indique qu’il n’est pas.
...
*bRequiresElevation*<br/>
[in] Un paramètre booléen qui indique si le contrôle de bouton de commande spécifiée requiert une élévation.

### <a name="remarks"></a>Notes

Utilisez cette méthode pour modifier un contrôle de bouton de commande est activé ou nécessite une élévation après qu’il a été ajouté à la `CTaskDialog` classe.
...
Met à jour un sous-ensemble des boutons courants pour être activé et requièrent une élévation de UAC.
...
[in] Un masque pour les boutons courants à désactiver.

*nElevationButtonMask*<br/>
[in] Un masque pour les boutons courants qui requièrent une élévation.

### <a name="remarks"></a>Notes

Vous pouvez définir les boutons courants disponibles sur une instance de la [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md) en utilisant le constructeur [CTaskDialog::CTaskDialog](#ctaskdialog) et la méthode [CTaskDialog::SetCommonButtons ](#setcommonbuttons). `CTaskDialog::SetCommonButtonOptions` ne prend pas en charge l’ajout de nouveaux boutons courants.

Si vous utilisez cette méthode pour désactiver ou élever un bouton courants qui n’est pas disponible pour cet `CTaskDialog`, cette méthode lève une exception à l’aide de la [Vérifiez](diagnostic-services.md#ensure) macro.

Cette méthode permet à n’importe quel bouton est disponible pour le `CTaskDialog` mais n’est pas dans le *nDisabledButtonMask*, même si elle a été désactivée. Cette méthode traite une élévation d’une manière similaire : il enregistre les boutons courants comme ne nécessitant ne pas d’élévation si le bouton courantes est disponible mais non inclus dans *nElevationButtonMask*.
...
Ajoute les boutons courants pour la `CTaskDialog`.
...
*nButtonMask*<br/>
[in] Un masque des boutons pour ajouter à la `CTaskDialog`.

*nDisabledButtonMask*<br/>
[in] Masque des boutons de la désactiver.

*nElevationButtonMask*<br/>
[in] Masque des boutons qui requièrent une élévation.

### <a name="remarks"></a>Notes

Vous ne pouvez pas appeler cette méthode après la fenêtre d’affichage pour cette instance de la `CTaskDialog` classe est créée. Si vous le faites, cette méthode lève une exception.

Les boutons indiqué par *nButtonMask* remplacer des boutons courants précédemment ajoutés à la `CTaskDialog`. Seuls les boutons figurant *nButtonMask* sont disponibles.

Si *nDisabledButtonMask* ou *nElevationButtonMask* contiennent un bouton qui n’est pas *nButtonMask*, cette méthode lève une exception à l’aide de la [Vérifiez](diagnostic-services.md#ensure) macro.
...
*strContent*<br/>
[in] Chaîne à afficher à l’utilisateur.
...
*nCommandControlID*<br/>
[in] ID du contrôle de bouton de commande à la valeur par défaut.

### <a name="remarks"></a>Notes

Le contrôle de bouton de commande par défaut est le contrôle est sélectionné lorsque le `CTaskDialog` est d’abord affiché à l’utilisateur.

Cette méthode lève une exception s’il ne trouve pas le contrôle de bouton de commande spécifié par *nCommandControlID*.
...
Spécifie le bouton de case d’option par défaut.
...
*nRadioButtonID*<br/>
[in] ID de la case d’option à la valeur par défaut.

### <a name="remarks"></a>Notes

La case d’option par défaut est le bouton est sélectionné lorsque le `CTaskDialog` est d’abord affiché à l’utilisateur.

Cette méthode lève une exception s’il ne trouve pas le bouton radio spécifié par *nRadioButtonID*.
...
*nWidth*<br/>
[in] Largeur de la boîte de dialogue, en pixels.
...
Met à jour de la zone d’expansion de le `CTaskDialog`.
...
[in] La chaîne qui le `CTaskDialog` affiche dans le corps principal de la boîte de dialogue lorsque l’utilisateur clique sur le bouton de développement.

*strCollapsedLabel*<br/>
[in] La chaîne qui le `CTaskDialog` affiche en regard du bouton d’extension lorsque la zone développée est réduite.

*strExpandedLabel*<br/>
[in] La chaîne qui le `CTaskDialog` affiche en regard du bouton d’extension lorsque la zone développée est affichée.

### <a name="remarks"></a>Notes

La zone d’expansion de la `CTaskDialog` classe vous permet de fournir des informations supplémentaires à l’utilisateur. La zone d’expansion est dans la partie principale de la `CTaskDialog`, qui se trouve immédiatement en dessous de la chaîne de titre et le contenu.
...
Met à jour de l’icône de pied de page de la `CTaskDialog`.
...
*hFooterIcon*<br/>
[in] L’icône Nouveau pour le `CTaskDialog`.
...
L’icône de pied de page est affichée en bas de la [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md). Il peut être associés à texte de pied de page. Vous pouvez modifier le texte de pied de page avec [CTaskDialog::SetFooterText](#setfootertext).

Cette méthode lève une exception avec le [Vérifiez](diagnostic-services.md#ensure) macro si la `CTaskDialog` est affiché ou le paramètre d’entrée est NULL.

Un `CTaskDialog` accepte uniquement un `HICON` ou `LPCWSTR` sous forme d’une icône de pied de page. Ceci est configuré en définissant l’option TDF_USE_HICON_FOOTER dans le constructeur ou [CTaskDialog::SetOptions](#setoptions). Par défaut, le `CTaskDialog` est configuré pour utiliser `LPCWSTR` comme type d’entrée pour l’icône de pied de page. Cette méthode génère une exception si vous essayez de définir l’icône de l’aide du type inapproprié.
...
Met à jour le texte sur le pied de page le `CTaskDialog`.
...
*strFooterText*<br/>
[in] Le nouveau texte du pied de page.
...
Met à jour de l’icône principale de la `CTaskDialog`.
...
Met à jour de l’instruction principale de la `CTaskDialog`.
...
*strInstructions*<br/>
[in] La nouvelle instruction principale.

### <a name="remarks"></a>Notes

L’instruction principale de la `CTaskDialog` classe est le texte affiché à l’utilisateur dans une grande taille de police en gras. Il se trouve dans la boîte de dialogue en dessous de la barre de titre.
...
*nOptionFlag*<br/>
[in] Le jeu d’indicateurs à utiliser pour le `CTaskDialog`.
...
|||
|-|-|
|TDF_ENABLE_HYPERLINKS|Permet des liens hypertexte dans le `CTaskDialog`.|
|TDF_USE_HICON_MAIN|Configure le `CTaskDialog` à utiliser un `HICON` pour l’icône principale. L’alternative consiste à utiliser un `LPCWSTR`.|
|TDF_USE_HICON_FOOTER|Configure le `CTaskDialog` à utiliser un `HICON` pour l’icône de pied de page. L’alternative consiste à utiliser un `LPCWSTR`.|
|TDF_ALLOW_DIALOG_CANCELLATION|Permet à l’utilisateur fermer la `CTaskDialog` à l’aide du clavier ou à l’aide de l’icône dans le coin supérieur droit de la boîte de dialogue, même si le **Annuler** bouton n’est pas activé. Si cet indicateur n’est pas défini et le **Annuler** bouton n’est pas activé, l’utilisateur ne peut pas fermer la boîte de dialogue à l’aide de Alt + F4, la touche ÉCHAP, ou bouton ferme de la barre de titre.|
|TDF_USE_COMMAND_LINKS|Configure le `CTaskDialog` à utiliser les contrôles de bouton de commande.|
|TDF_USE_COMMAND_LINKS_NO_ICON|Configure le `CTaskDialog` à utiliser les contrôles de bouton de commande sans affichage d’une icône à côté du contrôle. TDF_USE_COMMAND_LINKS remplace TDF_USE_COMMAND_LINKS_NO_ICON.|
|TDF_EXPAND_FOOTER_AREA|Indique la zone d’expansion est actuellement développée.|
|TDF_EXPANDED_BY_DEFAULT|Détermine si la zone d’expansion est développée par défaut.|
|TDF_VERIFICATION_FLAG_CHECKED|Indique la case à cocher de vérification est actuellement sélectionnée.|
|TDF_SHOW_PROGRESS_BAR|Configure le `CTaskDialog` pour afficher une barre de progression.|
|TDF_SHOW_MARQUEE_PROGRESS_BAR|Configure la barre de progression à une barre de progression de type texte défilant. Si vous activez cette option, vous devez définir TDF_SHOW_PROGRESS_BAR pour que le comportement attendu.|
|TDF_CALLBACK_TIMER|Indique que le `CTaskDialog` intervalle de rappel est défini à environ 200 millisecondes.|
|TDF_POSITION_RELATIVE_TO_WINDOW|Configure le `CTaskDialog` sera centré par rapport à la fenêtre parente. Si cet indicateur n’est pas activé, le `CTaskDialog` est centrée par rapport à l’analyse.|
|TDF_RTL_LAYOUT|Configure le `CTaskDialog` pour une mise en page de lecture de droite à gauche.|
|TDF_NO_DEFAULT_RADIO_BUTTON|Indique qu’aucune case d’option n’est sélectionnée lorsque le `CTaskDialog` s’affiche.|
|TDF_CAN_BE_MINIMIZED|Permet à l’utilisateur afin de minimiser la `CTaskDialog`. Pour prendre en charge cette option, le `CTaskDialog` ne peut pas être modale. MFC ne prend pas en charge cette option, car MFC ne prend pas en charge un non modal `CTaskDialog`.|
...
Configure une barre de type texte défilant pour le `CTaskDialog` et l’ajoute à la boîte de dialogue.
...
*nMarqueeSpeed*<br/>
[in] Entier qui indique la vitesse de la barre de texte défilant.

### <a name="remarks"></a>Notes

La barre de sélection apparaît en dessous du texte principal de la `CTaskDialog` classe.

Utilisez *nMarqueeSpeed* pour définir la vitesse de la barre de texte défilant ; plus grandes valeurs indiquent une vitesse inférieure. La valeur 0 pour *nMarqueeSpeed* déplace la barre de type texte défilant à la vitesse par défaut pour Windows.
...
*nProgressPos*<br/>
[in] La position de la barre de progression.
...
*nRangeMin*<br/>
[in] La limite inférieure de la barre de progression.

*nRangeMax*<br/>
[in] La limite supérieure de la barre de progression.

### <a name="remarks"></a>Notes

La position de la barre de progression est relatif à *nRangeMin* et *nRangeMax*. Par exemple, si *nRangeMin* est 50 et *nRangeMax* est 100, une position de 75 se trouve à mi-chemin entre la barre de progression. Utilisez [CTaskDialog::SetProgressBarPosition](#setprogressbarposition) pour définir la position de la barre de progression.

Pour afficher la barre de progression, l’option que tdf_show_progress_bar doit être activé et TDF_SHOW_MARQUEE_PROGRESS_BAR ne doivent pas être activé. Cette méthode définit TDF_SHOW_PROGRESS_BAR et efface TDF_SHOW_MARQUEE_PROGRESS_BAR automatiquement. Utilisez [CTaskDialog::SetOptions](#setoptions) pour modifier manuellement les options pour cette instance de la [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md).

Cette méthode lève une exception avec le [Vérifiez](diagnostic-services.md#ensure) macro si *nRangeMin* est n’est pas inférieure *nRangeMax*. Cette méthode lève également une exception si la `CTaskDialog` est déjà affichée et a une barre de progression de type texte défilant.
...
*nState*<br/>
[in] L’état de la barre de progression. Consultez la section Notes pour les valeurs possibles.

### <a name="remarks"></a>Notes

Cette méthode lève une exception avec le [Vérifiez](diagnostic-services.md#ensure) macro si la `CTaskDialog` est déjà affichée et a une barre de progression de type texte défilant.

Le tableau suivant répertorie les valeurs possibles pour *nState*. Dans tous ces cas, la barre de progression remplira avec la couleur standard jusqu'à ce qu’il atteigne la position d’arrêt désigné. À ce stade il change de couleur en fonction de l’état.

|||
|-|-|
|PBST_NORMAL|Après la progression barre soit remplie, le `CTaskDialog` ne modifie pas la couleur de la barre. Par défaut, la couleur normale est verte.|
|PBST_ERROR|Après la progression barre soit remplie, le `CTaskDialog` modifie la couleur de la barre à la couleur de l’erreur. Par défaut, il s’agit rouge.|
|PBST_PAUSED|Après la progression barre soit remplie, le `CTaskDialog` modifie la couleur de la barre à la couleur en pause. Par défaut, il s’agit jaune.|
...
Cette méthode lève une exception avec le [Vérifiez](diagnostic-services.md#ensure) macro si *nRadioButtonID* n’est pas un ID valid pour un bouton radio.
...
*bChecked*<br/>
[in] True pour avoir la case à cocher de vérification sélectionné lorsque la `CTaskDialog` est affichée ; FALSE pour que la case à cocher Vérification désélectionné lorsque la `CTaskDialog` s’affiche.
...
[in] Le texte que cette méthode s’affiche en regard de la case à cocher de vérification.

### <a name="remarks"></a>Notes

Cette méthode lève une exception avec le [Vérifiez](diagnostic-services.md#ensure) macro si cette instance de la `CTaskDialog` classe est déjà affichée.
...
*strWindowTitle*<br/>
[in] Le nouveau titre de la `CTaskDialog`.
...
*uNotification*<br/>
[in] Le code de notification qui spécifie le message généré.

*wParam*<br/>
[in] Plus d’informations sur le message.

*lParam*<br/>
[in] Plus d’informations sur le message.

*dwRefData*<br/>
[in] Un pointeur vers le `CTaskDialog` objet auquel s’applique le message de rappel.
...
L’implémentation par défaut de `TaskDialogCallback` gère le message spécifique, puis appelle approprié sur la méthode de la [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md). Par exemple, en réponse au message TDN_BUTTON_CLICKED, `TaskDialogCallback` appels [CTaskDialog::OnCommandControlClick](#oncommandcontrolclick).
...
|TDN_HYPERLINK_CLICKED|Non utilisé.|Un [LPCWSTR](/windows/desktop/WinProg/windows-data-types) structure qui contient le lien.|
|TDN_TIMER|Durée en millisecondes depuis le `CTaskDialog` a été créé ou la minuterie a été réinitialisée.|Non utilisé.|
...
|TDN_VERIFICATION_CLICKED|1 si la case à cocher est activée, 0 si ce n’est pas.|Non utilisé.|
|TDN_HELP|Non utilisé.|Non utilisé.|
|TDN_EXPANDO_BUTTON_CLICKED|0 si la zone d’expansion est réduite ; différent de zéro si le texte d’expansion est affiché.|Non utilisé.|
