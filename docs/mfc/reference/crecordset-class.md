...
|[CRecordset :: CanAppend](#canappend)|Retourne une valeur différente de zéro si de nouveaux enregistrements peuvent être ajoutés au recordset via la `AddNew` fonction membre.|
...
|[CRecordset :: CancelUpdate](#cancelupdate)|Annule toutes les mises à jour en attente en raison d’une `AddNew` `Edit` opération ou.|
|[CRecordset :: CanRestart](#canrestart)|Retourne une valeur différente `Requery` de zéro si peut être appelé pour réexécuter la requête du Recordset.|
...
|[CRecordset :: IsBOF](#isbof)|Retourne une valeur différente de zéro si le jeu d’enregistrements a été placé avant le premier enregistrement. Aucun enregistrement actif.|
...
|[CRecordset :: IsEOF](#iseof)|Retourne une valeur différente de zéro si le jeu d’enregistrements a été placé après le dernier enregistrement. Aucun enregistrement actif.|
...
|[CRecordset :: MoveFirst](#movefirst)|Positionne l’enregistrement actif sur le premier enregistrement du Recordset. Testez pour la `IsBOF` première fois.|
|[CRecordset :: MoveLast](#movelast)|Positionne l’enregistrement actif sur le dernier enregistrement ou sur le dernier ensemble de lignes. Testez pour la `IsEOF` première fois.|
|[CRecordset :: MoveNext](#movenext)|Positionne l’enregistrement actif sur l’enregistrement suivant ou sur l’ensemble de lignes suivant. Testez pour la `IsEOF` première fois.|
|[CRecordset :: MovePrev](#moveprev)|Positionne l’enregistrement actif sur l’enregistrement précédent ou sur l’ensemble de lignes précédent. Testez pour la `IsBOF` première fois.|
...
|[CRecordset :: Update](#update)|Termine une `AddNew` `Edit` opération ou en enregistrant les données nouvelles ou modifiées sur la source de données.|
...
|[CRecordset :: m_pDatabase](#m_pdatabase)|Contient un pointeur vers l' `CDatabase` objet par le biais duquel le Recordset est connecté à une source de données.|
|[CRecordset :: m_strFilter](#m_strfilter)|Contient un `CString` qui spécifie une clause langage SQL (SQL) `WHERE` . Utilisé comme filtre pour sélectionner uniquement les enregistrements qui répondent à certains critères.|
|[CRecordset :: m_strSort](#m_strsort)|Contient une `CString` qui spécifie une `ORDER BY` clause SQL. Utilisé pour contrôler la façon dont les enregistrements sont triés.|

## <a name="remarks"></a><a name="remarks"></a> Notes

Appelés « recordsets », `CRecordset` les objets sont généralement utilisés sous deux formes : les feuilles de réponse dynamiques et les instantanés. Une feuille de réponse dynamique reste synchronisée avec les mises à jour de données effectuées par d’autres utilisateurs. Un instantané est une vue statique des données. Chaque formulaire représente un ensemble d’enregistrements fixés au moment de l’ouverture du Recordset, mais lorsque vous faites défiler vers un enregistrement dans une feuille de réponse dynamique, il reflète les modifications apportées par la suite à l’enregistrement, soit par d’autres utilisateurs, soit par d’autres jeux d’enregistrements de votre application.
...
Pour travailler avec l’un ou l’autre type d’objet Recordset, vous dérivez généralement une classe Recordset spécifique à l’application à partir de `CRecordset` . Les jeux d’enregistrements sélectionnent les enregistrements d’une source de données, et vous pouvez ensuite :
...
Pour utiliser votre classe, ouvrez une base de données et construisez un objet Recordset, en passant au constructeur un pointeur vers votre `CDatabase` objet. Appelez ensuite la fonction membre du recordset `Open` , dans laquelle vous pouvez spécifier si l’objet est une feuille de réponse dynamique ou un instantané. `Open`L’appel de sélectionne des données à partir de la source de données. Une fois l’objet Recordset ouvert, utilisez ses fonctions membres et ses membres de données pour faire défiler les enregistrements et les utiliser. Les opérations disponibles varient selon que l’objet est une feuille de réponse dynamique ou un instantané, qu’il peut être mis à jour ou en lecture seule (cela dépend de la capacité de la source de données Open Database Connectivity (ODBC)) et si vous avez implémenté l’extraction de lignes en bloc. Pour actualiser les enregistrements qui ont peut-être été modifiés ou ajoutés depuis l' `Open` appel, appelez la fonction membre de l’objet `Requery` . Appelez la fonction membre de l’objet `Close` et détruisez l’objet une fois que vous l’avez terminé.

Dans une classe dérivée, l’échange de champs d’enregistrement `CRecordset` (RFX) ou l’échange de champs d’enregistrements en bloc (RFX en bloc) est utilisé pour prendre en charge la lecture et la mise à jour des champs d’enregistrement.
...
Il est interdit d’appeler `AddNew` pour un jeu d’enregistrements dont la `Open` fonction membre n’a pas été appelée. Une `CDBException` exception est levée si vous appelez `AddNew` pour un jeu d’enregistrements qui ne peut pas être ajouté à. Vous pouvez déterminer si le jeu d’enregistrements peut être mis à jour en appelant [CanAppend](#canappend).
...
Si la vérification automatique des champs de modification est activée, `CancelUpdate` restaure les variables membres avec les valeurs qu’elles avaient avant `Edit` ou `AddNew` a été appelée ; sinon, toute modification de valeur est conservée. Par défaut, la vérification automatique des champs est activée lors de l’ouverture du Recordset. Pour le désactiver, vous devez spécifier `CRecordset::noDirtyFieldCheck` dans le paramètre *dwOptions* de la fonction membre [Open](#open) .
...
|SQL_SUCCESS_WITH_INFO|La fonction s’est terminée avec succès, éventuellement avec une erreur récupérable. Des informations supplémentaires peuvent être obtenues en appelant `SQLError` .|
...
|SQL_STILL_EXECUTING|Une fonction qui a été démarrée de manière asynchrone est toujours en cours d’exécution. Notez que, par défaut, MFC ne passera jamais cette valeur à `CheckRowsetError` ; MFC continuera `SQLExtendedFetch` à appeler jusqu’à ce qu’il ne retourne plus SQL_STILL_EXECUTING.|
...
L’HSTMT ODBC et la mémoire de l’infrastructure allouée pour le Recordset sont désallouées. En général, après avoir appelé `Close` , vous supprimez l’objet Recordset C++ s’il a été alloué avec **`new`** .
...
> [!NOTE]
> Une classe dérivée *doit* fournir son propre constructeur. Dans le constructeur de votre classe dérivée, appelez le constructeur `CRecordset::CRecordset` , en lui passant les paramètres appropriés.

Transmettez la valeur NULL au constructeur de votre jeu d’enregistrements pour qu’un `CDatabase` objet soit construit et connecté automatiquement pour vous. Il s’agit d’un raccourci utile qui ne nécessite pas la construction et la connexion d’un `CDatabase` objet avant la construction de votre Recordset.
...
Après une suppression réussie, les membres de données de champ du Recordset sont définis sur une valeur null, et vous devez appeler explicitement l’une des `Move` fonctions pour déplacer l’enregistrement supprimé. Une fois que vous avez quitté l’enregistrement supprimé, il n’est pas possible de le retourner. Si la source de données prend en charge les transactions, vous pouvez faire en sorte que l' `Delete` appel fasse partie d’une transaction. Pour plus d’informations, consultez l’article [transaction (ODBC)](../../data/odbc/transaction-odbc.md).

> [!NOTE]
> Si vous avez implémenté l’extraction de lignes en bloc, vous ne pouvez pas appeler `Delete` . Cela entraînera l’échec d’une assertion. Bien que `CRecordset` la classe ne fournisse pas de mécanisme de mise à jour des lignes de données en bloc, vous pouvez écrire vos propres fonctions à l’aide de la fonction API ODBC `SQLSetPos` . Pour plus d’informations sur l’extraction de lignes en bloc, consultez l’article [Recordset : extraction globale d’enregistrements en bloc (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).

> [!CAUTION]
> Le Recordset doit pouvoir être mis à jour et un enregistrement valide doit être présent dans le recordset lorsque vous appelez `Delete` ; sinon, une erreur se produit. Par exemple, si vous supprimez un enregistrement mais que vous ne faites pas défiler jusqu’à un nouvel enregistrement avant d’appeler `Delete` à nouveau, `Delete` lève une [CDBException](../../mfc/reference/cdbexception-class.md).

Contrairement à [AddNew](#addnew) et [Edit](#edit), un appel à `Delete` n’est pas suivi d’un appel à [Update](#update). En cas d’échec d’un `Delete` appel, les données membres de champ restent inchangées.

### <a name="example"></a>Exemple

Cet exemple montre un jeu d’enregistrements créé sur le frame d’une fonction. L’exemple suppose l’existence de `m_dbCust` , une variable membre de type `CDatabase` déjà connectée à la source de données.
...
Si la source de données prend en charge les transactions, vous pouvez faire en sorte que l' `Edit` appel fasse partie d’une transaction. Notez que vous devez appeler [CDatabase :: BeginTrans](../../mfc/reference/cdatabase-class.md#begintrans) avant `Edit` d’appeler et après l’ouverture du Recordset. Notez également que l’appel de [CDatabase :: CommitTrans](../../mfc/reference/cdatabase-class.md#committrans) ne remplace pas `Update` l’appel à pour terminer l' `Edit` opération. Pour plus d’informations sur les transactions, consultez la classe [CDatabase](../../mfc/reference/cdatabase-class.md).

Selon le mode de verrouillage actuel, l’enregistrement en cours de mise à jour peut être verrouillé par `Edit` jusqu’à ce que vous appeliez `Update` ou défilez vers un autre enregistrement, ou il peut être verrouillé uniquement pendant l' `Edit` appel. Vous pouvez modifier le mode de verrouillage avec [SetLockingMode](#setlockingmode).

La valeur précédente de l’enregistrement en cours est restaurée si vous faites défiler jusqu’à un nouvel enregistrement avant d’appeler `Update` . Une `CDBException` exception est levée si vous appelez `Edit` pour un jeu d’enregistrements qui ne peut pas être mis à jour ou s’il n’existe aucun enregistrement en cours.
...
Vous devez appeler `FlushResultSet` uniquement lorsque vous avez terminé avec le curseur sur le jeu de résultats actuel. Notez que lorsque vous récupérez le jeu de résultats suivant en appelant `FlushResultSet` , votre curseur n’est pas valide sur ce jeu de résultats ; vous devez appeler la fonction membre [MoveNext](#movenext) après avoir appelé `FlushResultSet` .
...
Le code suivant suppose que `COutParamRecordset` est un `CRecordset` objet dérivé de basé sur une requête prédéfinie avec un paramètre d’entrée et un paramètre de sortie, et ayant plusieurs jeux de résultats. Notez la structure de la substitution [DoFieldExchange](#dofieldexchange) .
...
Pour déterminer si les signets sont pris en charge sur le Recordset, appelez [CanBookmark](#canbookmark). Pour rendre les signets disponibles s’ils sont pris en charge, vous devez définir l' `CRecordset::useBookmarks` option dans le paramètre *dwOptions* de la fonction membre [Open](#open) .
...
Si vous avez implémenté l’extraction de lignes en bloc, l’enregistrement actif est toujours positionné sur le premier enregistrement d’un ensemble de lignes. Pour utiliser `GetFieldValue` sur un enregistrement dans un ensemble de lignes donné, vous devez d’abord appeler la fonction membre [SetRowsetCursorPosition](#setrowsetcursorposition) pour déplacer le curseur vers la ligne souhaitée dans cet ensemble de lignes. Appelez ensuite `GetFieldValue` pour cette ligne. Pour implémenter l’extraction de lignes en bloc, vous devez spécifier l' `CRecordset::useMultiRowFetch` option du paramètre *dwOptions* dans la fonction membre [Open](#open) .

Vous pouvez utiliser `GetFieldValue` pour extraire dynamiquement des champs au moment de l’exécution plutôt que de les lier statiquement au moment de la conception. Par exemple, si vous avez déclaré un objet Recordset directement à partir de `CRecordset` , vous devez utiliser `GetFieldValue` pour extraire les données de champ ; l’échange de champs d’enregistrement (RFX) ou l’échange de champs d’enregistrements en bloc (RFX en bloc) n’est pas implémenté.
...
> [!NOTE]
> Contrairement à la classe DAO `CDaoRecordset` , `CRecordset` n’a pas de `SetFieldValue` fonction membre. Si vous créez un objet directement à partir de `CRecordset` , il est en lecture seule.
...
Pour implémenter l’extraction de lignes en bloc, vous devez spécifier l' `CRecordset::useMultiRowFetch` option dans le paramètre *dwOptions* de la fonction membre [Open](#open) . Pour modifier le paramètre de la taille de l’ensemble de lignes, appelez [SetRowsetSize](#setrowsetsize).
...
La chaîne retournée par `GetSQL` est généralement différente de toute chaîne que vous avez peut-être passée au recordset dans le paramètre *lpszSQL* de la `Open` fonction membre. Cela est dû au fait que le recordset construit une instruction SQL complète basée sur ce que vous avez passé à `Open` , ce que vous avez spécifié avec ClassWizard, ce que vous avez spécifié dans les `m_strFilter` membres de `m_strSort` données et, ainsi que tous les paramètres que vous avez spécifiés. Pour plus d’informations sur la façon dont le recordset construit cette instruction SQL, consultez l’article [Recordset : sélection d’enregistrements par les recordsets (ODBC)](../../data/odbc/recordset-how-recordsets-select-records-odbc.md).
...
Le résultat de `IsDeleted` dépend de nombreux facteurs, tels que le type de jeu d’enregistrements, de la possibilité de mettre à jour votre jeu d’enregistrements, si vous avez spécifié l' `CRecordset::skipDeletedRecords` option lorsque vous avez ouvert le Recordset, si votre pilote Pack a supprimé les enregistrements et s’il y a plusieurs utilisateurs.

Pour plus d’informations sur `CRecordset::skipDeletedRecords` et la compression des pilotes, consultez la fonction membre [Open](#open) .
...
Appelez cette fonction membre pour déterminer si le membre de données de champ spécifié d’un jeu d’enregistrements a été marqué comme null. (Dans la terminologie de base de données, null signifie « aucune valeur » et n’est pas identique à NULL en C++.) Si un membre de données de champ est marqué comme null, il est interprété comme une colonne de l’enregistrement actif pour lequel il n’y a aucune valeur.
...
> [!NOTE]
> Si vous avez implémenté l’extraction de lignes en bloc, vous ne pouvez pas appeler `IsFieldNullable` . Au lieu de cela, appelez la fonction membre [GetODBCFieldInfo](#getodbcfieldinfo) pour déterminer si un champ peut être défini sur une valeur null. Notez que vous pouvez toujours appeler `GetODBCFieldInfo` , même si vous avez implémenté l’extraction de lignes en bloc. Pour plus d’informations sur l’extraction de lignes en bloc, consultez l’article [Recordset : extraction globale d’enregistrements en bloc (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).
...
L’utilisation de NULL pour le premier argument de la fonction applique la fonction uniquement aux `outputColumn` champs, et non aux `param` champs. Par exemple, l’appel

[!code-cpp[NVC_MFCDatabase#26](../../mfc/codesnippet/cpp/crecordset-class_10.cpp)]

définit uniquement les `outputColumn` champs sur null ; les `param` champs ne seront pas affectés.

Pour travailler sur des `param` champs, vous devez fournir l’adresse réelle de l’individu `param` sur lequel vous souhaitez travailler, par exemple :
...
Le constructeur de la classe Recordset doit s’initialiser `m_nFields` avec le nombre correct. Si vous n’avez pas implémenté l’extraction de lignes en bloc, ClassWizard écrit cette initialisation pour vous quand vous l’utilisez pour déclarer votre classe de Recordset. Vous pouvez également l’écrire manuellement.
...
Vous pouvez lier des colonnes de manière dynamique, comme expliqué dans l’article « Recordset : liaison dynamique des colonnes de données ». Dans ce cas, vous devez augmenter le nombre dans `m_nFields` pour refléter le nombre d’appels de fonction RFX ou RFX en bloc dans `DoFieldExchange` votre `DoBulkFieldExchange` fonction membre ou pour les colonnes liées dynamiquement.
...
Si votre classe Recordset contient des membres de données de paramètre, le constructeur de la classe doit s’initialiser `m_nParams` avec le nombre correct. La valeur `m_nParams` par défaut est 0. Si vous ajoutez des membres de données de paramètre (que vous devez effectuer manuellement), vous devez également ajouter manuellement une initialisation dans le constructeur de classe pour refléter le nombre de paramètres (qui doit être au moins aussi grand que le nombre d’espaces réservés «» dans votre `m_strFilter` `m_strSort` chaîne ou).

L’infrastructure utilise ce nombre lorsqu’il paramètre la requête du Recordset.

> [!CAUTION]
> Ce nombre doit correspondre au nombre de « params » inscrits dans `DoFieldExchange` ou `DoBulkFieldExchange` après un appel à [SetFieldType](../../mfc/reference/cfieldexchange-class.md#setfieldtype) avec une valeur de paramètre de `CFieldExchange::inputParam` ,, `CFieldExchange::param` `CFieldExchange::outputParam` ou `CFieldExchange::inoutParam` .
...
Le recordset utilise cette chaîne pour contraindre (ou filtrer) les enregistrements qu’il sélectionne pendant `Open` l' `Requery` appel de ou. Cela est utile pour sélectionner un sous-ensemble d’enregistrements, par exemple « tous les commerciaux basés en Californie » (« State = CA »). La syntaxe SQL ODBC pour une clause **Where** est
...
Le recordset utilise cette chaîne pour trier les enregistrements qu’il sélectionne pendant `Open` l' `Requery` appel de ou. Vous pouvez utiliser cette fonctionnalité pour trier un jeu d’enregistrements sur une ou plusieurs colonnes. La syntaxe SQL ODBC pour une clause **order by** est
...
*wFetchType*<br/>
Détermine l’ensemble de lignes qui `Move` sera récupéré par. Pour plus d'informations, consultez Notes.

### <a name="remarks"></a>Notes

Si vous transmettez la valeur 0 à *nrows*, `Move` actualise l’enregistrement en cours ; `Move` met fin à tout en cours `AddNew` ou en `Edit` mode, et restaure la valeur de l’enregistrement actif avant `AddNew` ou `Edit` a été appelée.
...
Le tableau suivant répertorie les valeurs possibles pour *wFetchType*, l’ensemble de lignes qui `Move` effectue une extraction en fonction de *wFetchType* et de *nrows*, et toute fonction membre équivalente correspondant à *wFetchType*.
...
> [!CAUTION]
> `Move`L’appel de lève une exception si le Recordset n’a pas d’enregistrement. Pour déterminer si le Recordset contient des enregistrements, appelez [IsBOF](#isbof) et [IsEOF](#iseof).

> [!NOTE]
> Si vous avez défilé au-delà du début ou de la fin de l’ensemble d’enregistrements ( `IsBOF` ou que `IsEOF` retourne une valeur différente de zéro), l’appel d’une `Move` fonction lèvera peut-être une `CDBException` . Par exemple, si `IsEOF` retourne une valeur différente de zéro et `IsBOF` ne le fait pas, `MoveNext` lèvera une exception, mais `MovePrev` pas.

> [!NOTE]
> Si vous appelez `Move` pendant que l’enregistrement actif est mis à jour ou ajouté, les mises à jour sont perdues sans avertissement.
...
> [!CAUTION]
> L’appel de l’une des `Move` fonctions lève une exception si le Recordset n’a pas d’enregistrement. Pour déterminer si le Recordset contient des enregistrements, appelez `IsBOF` et `IsEOF` .
...
Si vous n’avez pas implémenté l’extraction de lignes en bloc, votre jeu d’enregistrements a une taille d’ensemble de lignes de 1. par conséquent, `MoveLast` il se déplace simplement vers le dernier enregistrement du Recordset.
...
Si vous n’avez pas implémenté l’extraction de lignes en bloc, votre jeu d’enregistrements a une taille d’ensemble de lignes de 1, par conséquent, `MoveNext` il passe simplement à l’enregistrement suivant.
...
> [!NOTE]
> Il est également recommandé d’appeler `IsEOF` avant d’appeler `MoveNext` . Par exemple, si vous avez défilé au-delà de la fin du Recordset, `IsEOF` retourne une valeur différente de zéro ; un appel ultérieur à `MoveNext` lève une exception.
...
Si vous n’avez pas implémenté l’extraction de lignes en bloc, votre jeu d’enregistrements a une taille d’ensemble de lignes de 1 `MovePrev` . par conséquent, il passe simplement à l’enregistrement précédent.
...
> [!NOTE]
> Il est également recommandé d’appeler `IsBOF` avant d’appeler `MovePrev` . Par exemple, si vous avez défilé avant le début de l’ensemble d’enregistrements, `IsBOF` retourne une valeur différente de zéro ; un appel ultérieur à `MovePrev` lève une exception.
...
Substituez `OnSetUpdateOptions` pour définir les options d’une instruction ODBC avant l’utilisation de cette instruction pour accéder à une base de données.
...
Vous devez appeler cette fonction membre pour exécuter la requête définie par le Recordset. Avant `Open` d’appeler, vous devez construire l’objet Recordset.

La connexion de cet ensemble d’enregistrements à la source de données dépend de la façon dont vous construisez le Recordset avant d’appeler `Open` . Si vous transmettez un objet [CDatabase](../../mfc/reference/cdatabase-class.md) au constructeur Recordset qui n’a pas été connecté à la source de données, cette fonction membre utilise [GetDefaultConnect](#getdefaultconnect) pour tenter d’ouvrir l’objet de base de données. Si vous transmettez la valeur NULL au constructeur Recordset, le constructeur construit un `CDatabase` objet pour vous et `Open` tente de connecter l’objet de base de données. Pour plus d’informations sur la fermeture de l’ensemble d’enregistrements et de la connexion dans ces différentes circonstances, consultez [Close](#close).

> [!NOTE]
> L’accès à une source de données via un `CRecordset` objet est toujours partagé. Contrairement à la `CDaoRecordset` classe, vous ne pouvez pas utiliser un `CRecordset` objet pour ouvrir une source de données avec un accès exclusif.
...
Les exemples de code suivants illustrent différentes formes de l' `Open` appel.
...
|---------------|-----------------|
|SQL_LOCK_NO_CHANGE (valeur par défaut)|Le pilote ou la source de données garantit que la ligne est dans le même état verrouillé ou déverrouillé qu’avant l' `RefreshRowset` appel de.|
...
> [!NOTE]
> Si les signets ne sont pas pris en charge ou ne sont pas disponibles, l’appel à entraîne la `SetBookmark` levée d’une exception. Les signets ne sont pas pris en charge sur les recordsets avant uniquement.

Pour récupérer d’abord le signet de l’enregistrement actif, appelez [GetBookmark](#getbookmark), qui enregistre la valeur de signet dans un `CDBVariant` objet. Plus tard, vous pouvez revenir à cet enregistrement en appelant `SetBookmark` à l’aide de la valeur de signet enregistrée.

> [!NOTE]
> Après certaines opérations sur les jeux d’enregistrements, vous devez vérifier la persistance des signets avant d’appeler `SetBookmark` . Par exemple, si vous récupérez un signet avec `GetBookmark` , puis appelez `Requery` , le signet n’est peut-être plus valide. Appelez [CDatabase :: GetBookmarkPersistence](../../mfc/reference/cdatabase-class.md#getbookmarkpersistence) pour vérifier si vous pouvez appeler en toute sécurité `SetBookmark` .
...
> [!NOTE]
> Cette fonction membre n’est pas applicable sur les recordsets qui utilisent l’extraction de lignes en bloc. Si vous avez implémenté l’extraction de lignes en bloc, vous `SetFieldDirty` obtiendrez un échec d’assertion. Pour plus d’informations sur l’extraction de lignes en bloc, consultez l’article [Recordset : extraction globale d’enregistrements en bloc (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).
...
> [!NOTE]
> N’appelez pas cette fonction membre sur les recordsets qui utilisent l’extraction de lignes en bloc. Si vous avez implémenté l’extraction de lignes en bloc, `SetFieldNull` l’appel de provoque un échec d’assertion. Pour plus d’informations sur l’extraction de lignes en bloc, consultez l’article [Recordset : extraction globale d’enregistrements en bloc (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).
...
|wLockType|Description|
|---------------|-----------------|
|SQL_LOCK_NO_CHANGE (valeur par défaut)|Le pilote ou la source de données garantit que la ligne est dans le même état verrouillé ou déverrouillé qu’avant l' `SetRowsetCursorPosition` appel de.|
|SQL_LOCK_EXCLUSIVE|Le pilote ou la source de données verrouille la ligne en mode exclusif. Toutes les sources de données ne prennent pas en charge ce type de verrou.|
|SQL_LOCK_UNLOCK|Le pilote ou la source de données déverrouille la ligne. Toutes les sources de données ne prennent pas en charge ce type de verrou.|
...
Cette fonction membre virtuelle spécifie le nombre de lignes que vous souhaitez récupérer lors d’une extraction unique lors de l’utilisation de l’extraction de lignes en bloc. Pour implémenter l’extraction de lignes en bloc, vous devez définir l' `CRecordset::useMultiRowFetch` option dans le paramètre *dwOptions* de la fonction membre [Open](#open) .

> [!NOTE]
> `SetRowsetSize`L’appel sans implémenter l’extraction de lignes en bloc entraînera l’échec d’une assertion.

Appelez `SetRowsetSize` avant `Open` d’appeler pour définir initialement la taille de l’ensemble de lignes pour le Recordset. La taille de l’ensemble de lignes par défaut lors de l’implémentation de l’extraction de lignes en bloc est 25.

> [!NOTE]
> Soyez prudent lors de l’appel de `SetRowsetSize` . Si vous allouez manuellement le stockage pour les données (comme spécifié par l' `CRecordset::userAllocMultiRowBuffers` option du paramètre dwOptions dans `Open` ), vous devez vérifier si vous devez réallouer ces mémoires tampons de stockage après avoir appelé `SetRowsetSize` , mais avant d’effectuer une opération de navigation de curseur.
...
Appelez cette fonction membre après un appel à la fonction membre [AddNew](#addnew) ou [Edit](#edit) . Cet appel est requis pour terminer l' `AddNew` `Edit` opération ou.
...
> [!CAUTION]
> Si vous appelez `Update` sans appeler d’abord `AddNew` ou `Edit` , `Update` lève une exception `CDBException` . Si vous appelez `AddNew` ou `Edit` , vous devez appeler `Update` avant d’appeler une `Move` opération ou avant de fermer le recordset ou la connexion à la source de données. Dans le cas contraire, vos modifications seront perdues sans notification.

Pour plus d’informations sur la gestion des `Update` échecs, consultez l’article [Recordset : comment les recordsets mettent à jour les enregistrements (ODBC)](../../data/odbc/recordset-how-recordsets-update-records-odbc.md).
