...
Pour plus d’informations sur ces services d’objets au moment de l’exécution et la sérialisation [, consultez l’article classe CObject: Accès aux informations](../../mfc/accessing-run-time-class-information.md)de classe au moment de l’exécution.
...
|[DECLARE_OLECTLTYPE](#declare_olectltype)|Déclare les `GetUserTypeNameID` fonctions membres `GetMiscStatus` et de votre classe de contrôle.|
...
|[IMPLEMENT_OLECTLTYPE](#implement_olectltype)|Implémente les `GetUserTypeNameID` fonctions `GetMiscStatus` membres et de votre classe de contrôle.|
...
Ajoute la possibilité d’accéder aux informations d’exécution relatives à la classe d’un objet lors de la dérivation `CObject`d’une classe de.
...
Si vous utilisez les macros dynamiques et IMPLEMENT_DYNAMIC DECLARE_ comme décrit, vous pouvez utiliser la macro RUNTIME_CLASS et la `CObject::IsKindOf` fonction pour déterminer la classe de vos objets au moment de l’exécution.
...
Consultez l’exemple pour [IMPLEMENT_DYNAMIC](#implement_dynamic).
...
Permet de créer `CObject`des objets de classes dérivées de manière dynamique au moment de l’exécution.
...
Consultez l’exemple pour [IMPLEMENT_DYNCREATE](#implement_dyncreate).
...
Déclare les `GetUserTypeNameID` fonctions membres `GetMiscStatus` et de votre classe de contrôle.
...
`GetUserTypeNameID`et `GetMiscStatus` sont des fonctions virtuelles pures, `COleControl`déclarées dans. Étant donné que ces fonctions sont virtuelles pures, elles doivent être remplacées dans votre classe de contrôle. En plus de DECLARE_OLECTLTYPE, vous devez ajouter la macro IMPLEMENT_OLECTLTYPE à votre déclaration de classe de contrôle.
...
Utilisez la `DECLARE_PROPPAGEIDS` macro à la fin de votre déclaration de classe. Ensuite, dans le fichier. cpp qui définit les fonctions membres de la classe, utilisez la `BEGIN_PROPPAGEIDS` macro, les entrées de macro pour chacune des pages de propriétés de votre contrôle `END_PROPPAGEIDS` et la macro pour déclarer la fin de la liste des pages de propriétés.

Pour plus d’informations sur les pages de propriétés, [consultez l’article contrôles ActiveX: Pages](../mfc-activex-controls-property-pages.md)de propriétés.
...
Génère le C++ code d’en-tête `CObject`nécessaire pour une classe dérivée de qui peut être sérialisée.
...
Vous pouvez utiliser la macro AFX_API pour exporter automatiquement l' `CArchive` opérateur d’extraction pour les classes qui utilisent les macros DECLARE_SERIAL et IMPLEMENT_SERIAL. Faites défiler les déclarations de classe (situées dans le fichier. h) avec le code suivant:
...
Génère le C++ code nécessaire pour une classe `CObject`dérivée dynamique avec un accès au moment de l’exécution au nom et à la position de la classe dans la hiérarchie.
...
Permet de créer `CObject`des objets de classes dérivées de manière dynamique au moment de l’exécution lorsqu’ils sont utilisés avec la macro DECLARE_DYNCREATE.
...
Si vous utilisez les macros DECLARE_DYNCREATE et IMPLEMENT_DYNCREATE, vous pouvez utiliser la macro RUNTIME_CLASS et la `CObject::IsKindOf` fonction membre pour déterminer la classe de vos objets au moment de l’exécution.
...
> [!NOTE]
>  Si vous utilisez IMPLEMENT_OLECREATE_FLAGS, vous pouvez spécifier le modèle de thread pris en charge par votre objet à l’aide du paramètre *nFlags* . Si vous souhaitez prendre en charge uniquement le modèle de simple-roulement, utilisez IMPLEMENT_OLECREATE.
...
Implémente les `GetUserTypeNameID` fonctions `GetMiscStatus` membres et de votre classe de contrôle.
...
> [!NOTE]
>  Les paramètres par défaut utilisés par le ControlWizard ActiveX sont les suivants: OLEMISC_ACTIVATEWHENVISIBLE, OLEMISC_SETCLIENTSITEFIRST, OLEMISC_INSIDEOUT, OLEMISC_CANTLINKINSIDE et OLEMISC_RECOMPOSEONRESIZE.
...
RUNTIME_CLASS retourne un pointeur vers une structure [CRuntimeClass](../../mfc/reference/cruntimeclass-structure.md) pour la classe spécifiée par *class_name*. Seules `CObject`les classes dérivées de déclarées avec DECLARE_DYNAMIC, DECLARE_DYNCREATE ou DECLARE_SERIAL retournent des `CRuntimeClass` pointeurs vers une structure.
...
Permet de créer `CCmdTarget`des objets de classes dérivées de à l’aide d’OLE Automation.
