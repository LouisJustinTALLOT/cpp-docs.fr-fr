...
|Nom|Description|
|----------|-----------------|
|[CFile :: m_pTM](#m_ptm)|Pointeur vers un `CAtlTransactionManager` objet.|

## <a name="remarks"></a>Remarques

Il fournit directement des services d’entrée/sortie de disque binaires non mis en mémoire tampon, et il prend indirectement en charge les fichiers texte et les fichiers de mémoire par le biais de ses classes dérivées. `CFile` fonctionne conjointement avec la `CArchive` classe pour prendre en charge la sérialisation d’objets Microsoft Foundation Class.

La relation hiérarchique entre cette classe et ses classes dérivées permet à votre programme de fonctionner sur tous les objets de fichier par le biais de l’interface polymorphe `CFile` . Un fichier de mémoire, par exemple, se comporte comme un fichier sur disque.
...
Normalement, un fichier disque est ouvert automatiquement en cours de `CFile` création et fermé lors de la destruction. Les fonctions membres statiques vous permettent d’interroger l’état d’un fichier sans ouvrir le fichier.
...
Lors de la gestion des exceptions, `CFile::Abort` diffère de `CFile::Close` de deux façons importantes. Tout d’abord, la `Abort` fonction ne lève pas d’exception en cas d’échec, car les échecs sont ignorés par `Abort` . Deuxièmement, `Abort` ne **déclare** pas si le fichier n’a pas été ouvert ou a été fermé précédemment.

Si vous avez utilisé **`new`** pour allouer l' `CFile` objet sur le tas, vous devez le supprimer après avoir fermé le fichier. `Abort` affecte `m_hFile` à la valeur `CFile::hFileNull` .
...
Le constructeur par défaut initialise des membres, mais n’attache pas de fichier à l' `CFile` objet. Après avoir utilisé ce constructeur, utilisez la méthode [CFile :: Open](#open) pour ouvrir un fichier et l’attacher à l' `CFile` objet.
...
Si un `CFile` objet ouvre correctement un fichier spécifié, il ferme automatiquement ce fichier quand l' `CFile` objet est détruit ; sinon, vous devez fermer explicitement le fichier une fois qu’il n’est plus attaché à l' `CFile` objet.
...
Si vous avez utilisé **`new`** pour allouer l' `CFile` objet sur le tas, vous devez le supprimer après avoir fermé le fichier. `Close` affecte `m_hFile` à la valeur `CFile::hFileNull` .

### <a name="example"></a>Exemple

Consultez l’exemple pour [CFile :: CFile](#cfile).

## <a name="cfileduplicate"></a><a name="duplicate"></a> CFile ::D quée
...
L’utilisation de `Flush` ne garantit pas le vidage des `CArchive` mémoires tampons. Si vous utilisez une archive, appelez [CArchive :: Flush](../../mfc/reference/carchive-class.md#flush) en premier.
...
Ce fragment de code ouvre le fichier SYSTEM.INI dans votre répertoire WINDOWS. S’il est trouvé, l’exemple affiche le nom et le chemin d’accès et le titre, comme le montre la sortie :
...
*rStatus*<br/>
Référence à une `CFileStatus` structure fournie par l’utilisateur qui recevra les informations d’État. La `CFileStatus` structure contient les champs suivants :
...
Détermine la présence d’un handle de fichier valide pour l' `CFile` objet.
...
Lorsque vous déverrouillez la région à l’aide de la `UnlockRange` fonction membre, la plage d’octets doit correspondre exactement à la région précédemment verrouillée. La `LockRange` fonction ne fusionne pas les zones adjacentes. Si deux régions verrouillées sont adjacentes, vous devez déverrouiller chaque région séparément.

> [!NOTE]
> Cette fonction n’est pas disponible pour la `CMemFile` classe dérivée de.
...
L’utilisation de `m_hFile` n’est pas recommandée, car la signification du membre dépend de la classe dérivée. `m_hFile` est devenu un membre public pour faciliter la prise en charge de l’utilisation des non polymorphes de la classe.
...
Tandis que le `CFile` constructeur lève une exception dans une condition d’erreur, `Open` retourne false pour les conditions d’erreur. `Open` Toutefois, il est toujours possible d’initialiser un objet [CFileException](../../mfc/reference/cfileexception-class.md) pour décrire l’erreur. Si vous ne fournissez pas le paramètre *perror* , ou si vous transmettez la valeur null pour *PERROR*, `Open` retourne false et ne lève pas d’exception `CFileException` . Si vous transmettez un pointeur à un existant `CFileException` et qu' `Open` il rencontre une erreur, la fonction le remplit avec les informations décrivant cette erreur. `Open` ne lève pas d’exception dans les deux cas.
...
## <a name="cfileoperator-handle"></a><a name="operator_handle"></a> HANDLE CFile :: Operator

Utilisez cet opérateur pour passer un handle à un `CFile` objet à des fonctions telles que [ReadFileEx](/windows/win32/api/fileapi/nf-fileapi-readfileex) et [GetFileTime](/windows/win32/api/fileapi/nf-fileapi-getfiletime) qui attendent un `HANDLE` .
...
Lit les données dans une mémoire tampon à partir du fichier associé à l' `CFile` objet.
...
Position du pointeur de fichier si la méthode a réussi ; dans le cas contraire, la valeur de retour n’est pas définie et un pointeur vers une `CFileException` exception est levée.
...
> [!NOTE]
> `SetFilePath` n’ouvre pas le fichier ou ne crée pas le fichier ; Il associe simplement l' `CFile` objet à un nom de chemin d’accès, qui peut ensuite être utilisé.
...
Écrit des données à partir d’une mémoire tampon dans le fichier associé à l' `CFile` objet.
