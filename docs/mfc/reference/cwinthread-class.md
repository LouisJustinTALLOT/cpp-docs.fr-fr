...
|Nom|Description|
|----------|-----------------|
|[CWinThread::CreateThread](#createthread)|Démarre l’exécution d' `CWinThread` un objet.|
...
|[CWinThread::PostThreadMessage](#postthreadmessage)|Publie un message dans un `CWinThread` autre objet.|
|[CWinThread::PreTranslateMessage](#pretranslatemessage)|Filtre les messages avant qu’ils ne soient distribués aux fonctions Windows [TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage) et [DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage).|
...
|[CWinThread::ProcessWndProcException](#processwndprocexception)|Intercepte toutes les exceptions non gérées levées par les gestionnaires de commandes et de messages du thread.|
...
Le thread d’exécution principal est généralement fourni par un objet dérivé de `CWinApp`; est dérivée `CWinThread`de. `CWinApp` Les `CWinThread` objets supplémentaires autorisent plusieurs threads dans une application donnée.

Il existe deux types généraux de threads `CWinThread` qui prennent en charge: les threads de travail et les threads d’interface utilisateur. Les threads de travail n’ont pas de pompe de messages: par exemple, un thread qui effectue des calculs en arrière-plan dans une application de feuille de calcul. Les threads d’interface utilisateur ont une pompe de messages et traitent les messages reçus du système. Les classes [CWinApp](../../mfc/reference/cwinapp-class.md) et dérivées de cette classe sont des exemples de threads de l’interface utilisateur. D’autres threads de l’interface utilisateur peuvent également être `CWinThread`dérivés directement à partir de.

Les objets de `CWinThread` la classe existent généralement pour la durée du thread. Si vous souhaitez modifier ce comportement, affectez à [m_bAutoDelete](#m_bautodelete) la valeur false.

La `CWinThread` classe est nécessaire pour rendre votre code et MFC entièrement thread-safe. Les données locales de thread utilisées par l’infrastructure pour conserver les informations spécifiques au thread sont `CWinThread` gérées par les objets. En raison de cette dépendance `CWinThread` à la gestion des données locales de thread, tout thread qui utilise MFC doit être créé par MFC. Par exemple, un thread créé par la fonction d’exécution _ [BeginThread, _ beginthreadex](../../c-runtime-library/reference/beginthread-beginthreadex.md) ne peut pas utiliser d’API MFC.

Pour créer un thread, appelez [AfxBeginThread](application-information-and-management.md#afxbeginthread). Il existe deux formes, selon que vous voulez un thread de travail ou un thread d’interface utilisateur. Si vous souhaitez un thread d’interface utilisateur, transmettez `AfxBeginThread` à un pointeur désignant `CWinThread`le `CRuntimeClass` de votre classe dérivée de. Si vous souhaitez créer un thread de travail, transmettez `AfxBeginThread` à un pointeur vers la fonction de contrôle et le paramètre à la fonction de contrôle. Pour les threads de travail et les threads d’interface utilisateur, vous pouvez spécifier des paramètres facultatifs qui modifient la priorité, la taille de la pile, les indicateurs de création et les attributs de sécurité. `AfxBeginThread`retourne un pointeur vers votre nouvel `CWinThread` objet.

Au lieu d' `AfxBeginThread`appeler, vous pouvez construire `CWinThread`un objet dérivé de, puis `CreateThread`appeler. Cette méthode de construction en deux étapes est utile si vous souhaitez réutiliser `CWinThread` l’objet entre une création et des arrêts successifs des exécutions de threads.

Pour plus d’informations `CWinThread`sur, consultez les articles sur le multithreading [ C++ avec et MFC](../../parallel/multithreading-with-cpp-and-mfc.md), [Multithreading: Création de threads](../../parallel/multithreading-creating-user-interface-threads.md)d’interface [utilisateur, Multithreading: Création de threads](../../parallel/multithreading-creating-worker-threads.md)de [travail et multithread: Comment utiliser les classes](../../parallel/multithreading-how-to-use-the-synchronization-classes.md)de synchronisation.
...
- CREATE_SUSPENDED démarre le thread avec un nombre de suspension de un. Utilisez CREATE_SUSPENDED si vous souhaitez initialiser toutes les données de membre de `CWinThread` l’objet, telles que [m_bAutoDelete](#m_bautodelete) ou les membres de votre classe dérivée, avant que le thread ne commence à s’exécuter. Une fois l’initialisation terminée, utilisez [CWinThread:: ResumeThread](#resumethread) pour démarrer le thread en cours d’exécution. Le thread ne s’exécute pas `CWinThread::ResumeThread` tant que n’est pas appelé.
...
*lpSecurityAttrs*<br/>
Pointe vers une structure [SECURITY_ATTRIBUTES](/previous-versions/windows/desktop/legacy/aa379560\(v=vs.85\)) qui spécifie les attributs de sécurité du thread.
...
Pour commencer l’exécution du thread, appelez la fonction membre [CreateThread](#createthread) . Vous allez généralement créer des threads en appelant [AfxBeginThread](application-information-and-management.md#afxbeginthread), qui appellera ce constructeur `CreateThread`et.
...
N’appelez pas cette fonction membre à partir de n’importe `Run` où, mais dans la fonction membre. Cette fonction membre est utilisée uniquement dans les threads d’interface utilisateur.
...
Si votre application est un serveur OLE, appelez cette fonction pour récupérer un pointeur vers la fenêtre principale active de l’application au lieu de faire directement référence `m_pMainWnd` au membre de l’objet d’application.
...
Si aucun objet n’est actif sur place dans un conteneur ou si votre application n’est pas un serveur OLE, cette fonction retourne le membre de données [m_pMainWnd](#m_pmainwnd) de votre objet thread.

### <a name="remarks"></a>Notes

Pour les threads d’interface utilisateur, cela équivaut à faire directement référence `m_pActiveWnd` au membre de votre objet application.

Si votre application n’est pas un serveur OLE, l’appel de cette fonction équivaut à faire directement référence `m_pMainWnd` au membre de votre objet application.
...
Substituez cette fonction pour empêcher `OnIdle` l’appel après la génération de messages spécifiques.
...
Différent de zéro `OnIdle` si doit être appelé après le traitement du message; sinon, 0.

### <a name="remarks"></a>Notes

L’implémentation par défaut n’appelle `OnIdle` pas après des messages de souris redondants et des messages générés par des signes de clignotement.

Si une application a créé un minuteur bref `OnIdle` , est appelé fréquemment, ce qui entraîne des problèmes de performances. Pour améliorer les performances d’une application de ce type `IsIdleMessage` , remplacez dans la `CWinApp`classe dérivée de l’application pour vérifier les messages WM_TIMER comme suit:
...
Spécifie si `CWinThread` l’objet doit être automatiquement supprimé au moment de l’arrêt du thread.
...
La valeur de `m_bAutoDelete` n’affecte pas la manière dont le handle de thread sous-jacent est fermé, mais elle affecte le minutage de la fermeture du handle. Le handle de thread est toujours fermé lorsque `CWinThread` l’objet est détruit.
...
Le destructeur CWinThread appelle CloseHandle sur `m_hThread`. Si [m_bAutoDelete](#m_bautodelete) a la valeur true lorsque le thread se termine, l’objet CWinThread est détruit, ce qui invalide tous les pointeurs vers l’objet CWinThread et ses variables membres. Vous aurez peut- `m_hThread` être besoin du membre pour vérifier la valeur de sortie du thread ou pour attendre un signal. Pour conserver l’objet CWinThread et son `m_hThread` membre pendant l’exécution du thread et après son arrêt, `m_bAutoDelete` affectez à la valeur false avant d’autoriser l’exécution du thread à continuer. Sinon, le thread peut se terminer, détruire l’objet CWinThread et fermer le descripteur avant d’essayer de l’utiliser. Si vous utilisez cette technique, vous êtes responsable de la suppression de l’objet CWinThread.
...
Consultez également remarques sur la durée de vie de [m_hThread](#m_hthread) .
...
Le bibliothèque MFC (Microsoft Foundation Class) met automatiquement fin à votre thread lorsque la fenêtre référencée `m_pActiveWnd` par est fermée. Si ce thread est le thread principal d’une application, l’application s’arrête également. Si ce membre de données a la valeur null, la fenêtre active de `CWinApp` l’objet de l’application est héritée. `m_pActiveWnd`variable publique de type `CWnd*`.
...
Le bibliothèque MFC (Microsoft Foundation Class) met automatiquement fin à votre thread lorsque la fenêtre référencée `m_pMainWnd` par est fermée. Si ce thread est le thread principal d’une application, l’application s’arrête également. Si ce membre de données a la valeur null, la fenêtre principale de `CWinApp` l’objet de l’application sera utilisée pour déterminer quand arrêter le thread. `m_pMainWnd`variable publique de type `CWnd*`.
...
Étant donné que l’application ne peut `OnIdle` pas traiter les messages jusqu’à ce qu’elle soit renvoyée, n’effectuez pas de tâches longues dans cette fonction.
...
Appelé pour envoyer un message défini par l’utilisateur à `CWinThread` un autre objet.
...
|-------------|------------|
|WM_CREATE|Incident.|
|WM_PAINT|Validez la fenêtre affectée, ce qui empêche la génération d’un autre message WM_PAINT.|
...
L' `PumpMessage` appel direct et la substitution de son comportement par défaut sont recommandés pour les utilisateurs expérimentés uniquement.
...
Valeur **int** qui est retournée par le thread. Cette valeur peut être récupérée en appelant [GetExitCodeThread](/windows/win32/api/processthreadsapi/nf-processthreadsapi-getexitcodethread).

### <a name="remarks"></a>Notes

`Run`acquiert et distribue des messages Windows jusqu’à ce que l’application reçoive un message [WM_QUIT](/windows/win32/winmsg/wm-quit) . Si la file d’attente de messages du thread ne contient `Run` actuellement `OnIdle` aucun message, les appels pour effectuer le traitement au moment de l’inactivité. Les messages entrants sont envoyés à la fonction membre [PreTranslateMessage](#pretranslatemessage) pour un traitement spécial, puis à la fonction Windows [TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage) pour la traduction du clavier standard. Enfin, la fonction Windows [DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage) est appelée.
