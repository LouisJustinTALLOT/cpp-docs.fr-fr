...
|Name|Description|
|----------|-----------------|
|[CWinThread :: CreateThread](#createthread)|Démarre l’exécution d’un objet `CWinThread`.|
...
|[CWinThread ::P ostThreadMessage](#postthreadmessage)|Publie un message à un autre objet `CWinThread`.|
...
Le thread d’exécution principal est généralement fourni par un objet dérivé de `CWinApp`; `CWinApp` est dérivée de `CWinThread`. Les objets `CWinThread` supplémentaires autorisent plusieurs threads dans une application donnée.

Il existe deux types généraux de threads que `CWinThread` prend en charge : les threads de travail et les threads d’interface utilisateur. Les threads de travail n’ont pas de pompe de messages : par exemple, un thread qui effectue des calculs en arrière-plan dans une application de feuille de calcul. Les threads d’interface utilisateur ont une pompe de messages et traitent les messages reçus du système. Les classes [CWinApp](../../mfc/reference/cwinapp-class.md) et dérivées de cette classe sont des exemples de threads de l’interface utilisateur. D’autres threads de l’interface utilisateur peuvent également être dérivés directement de `CWinThread`.
...
La classe `CWinThread` est nécessaire pour rendre votre code et MFC entièrement thread-safe. Les données locales de thread utilisées par l’infrastructure pour conserver les informations spécifiques au thread sont gérées par les objets `CWinThread`. En raison de cette dépendance vis-à-vis de `CWinThread` pour gérer des données locales de thread, tout thread qui utilise MFC doit être créé par MFC. Par exemple, un thread créé par la fonction runtime [_beginthread, _beginthreadex](../../c-runtime-library/reference/beginthread-beginthreadex.md) ne peut pas utiliser d’API MFC.

Pour créer un thread, appelez [AfxBeginThread](application-information-and-management.md#afxbeginthread). Il existe deux formes, selon que vous voulez un thread de travail ou un thread d’interface utilisateur. Si vous souhaitez un thread d’interface utilisateur, transmettez à `AfxBeginThread` un pointeur vers l' `CRuntimeClass` de votre classe dérivée de `CWinThread`. Si vous souhaitez créer un thread de travail, transmettez à `AfxBeginThread` un pointeur vers la fonction de contrôle et le paramètre vers la fonction de contrôle. Pour les threads de travail et les threads d’interface utilisateur, vous pouvez spécifier des paramètres facultatifs qui modifient la priorité, la taille de la pile, les indicateurs de création et les attributs de sécurité. `AfxBeginThread` retourne un pointeur vers votre nouvel objet `CWinThread`.

Au lieu d’appeler `AfxBeginThread`, vous pouvez construire un objet dérivé de `CWinThread`, puis appeler `CreateThread`. Cette méthode de construction en deux étapes est utile si vous souhaitez réutiliser l’objet `CWinThread` entre une création et des arrêts successifs des exécutions de threads.
...
- CREATE_SUSPENDED démarrer le thread avec un nombre de suspension d’un. Utilisez CREATE_SUSPENDED si vous souhaitez initialiser toutes les données de membre de l’objet `CWinThread`, telles que [m_bAutoDelete](#m_bautodelete) ou les membres de votre classe dérivée, avant que le thread ne commence à s’exécuter. Une fois l’initialisation terminée, utilisez [CWinThread :: ResumeThread](#resumethread) pour démarrer le thread en cours d’exécution. Le thread ne s’exécute pas tant que `CWinThread::ResumeThread` n’est pas appelé.
...
N’appelez pas cette fonction membre à partir de n’importe où, mais dans la fonction membre `Run`. Cette fonction membre est utilisée uniquement dans les threads d’interface utilisateur.
...
Cette fonction retourne un pointeur vers l’un des deux types de fenêtres. Si votre thread fait partie d’un serveur OLE et possède un objet qui est actif sur place à l’intérieur d’un conteneur actif, cette fonction retourne le membre de données [CWinApp :: m_pActiveWnd](../../mfc/reference/cwinapp-class.md#m_pactivewnd) de l’objet `CWinThread`.
...
Pour les threads d’interface utilisateur, cela équivaut à faire directement référence au membre `m_pActiveWnd` de votre objet application.
...
En général, vous substituez `InitInstance` pour effectuer des tâches qui doivent être terminées lors de la création d’un thread.
...
Substituez cette fonction pour empêcher l’appel de `OnIdle` après la génération de messages spécifiques.
...
Si une application a créé un minuteur bref, `OnIdle` est appelé fréquemment, ce qui entraîne des problèmes de performances. Pour améliorer les performances d’une application de ce type, remplacez `IsIdleMessage` dans la classe dérivée de `CWinApp`de l’application pour rechercher les messages WM_TIMER comme suit :
...
Spécifie si l’objet `CWinThread` doit être automatiquement supprimé au moment de l’arrêt du thread.
...
Le membre de données `m_bAutoDelete` est une variable publique de type BOOL.

La valeur de `m_bAutoDelete` n’affecte pas la manière dont le handle de thread sous-jacent est fermé, mais elle affecte le minutage de la fermeture du handle. Le handle de thread est toujours fermé lorsque l’objet `CWinThread` est détruit.
...
Le membre de données `m_hThread` est une variable publique de type HANDLE. Elle est valide uniquement si l’objet de thread de noyau sous-jacent existe actuellement, et si le handle n’a pas encore été fermé.

Le destructeur CWinThread appelle CloseHandle sur `m_hThread`. Si [m_bAutoDelete](#m_bautodelete) a la valeur true lorsque le thread se termine, l’objet CWinThread est détruit, ce qui invalide tous les pointeurs vers l’objet CWinThread et ses variables membres. Vous aurez peut-être besoin du membre `m_hThread` pour vérifier la valeur de sortie du thread ou pour attendre un signal. Pour conserver l’objet CWinThread et son membre `m_hThread` pendant l’exécution du thread et après son arrêt, affectez à `m_bAutoDelete` la valeur FALSe avant d’autoriser l’exécution du thread. Sinon, le thread peut se terminer, détruire l’objet CWinThread et fermer le descripteur avant d’essayer de l’utiliser. Si vous utilisez cette technique, vous êtes responsable de la suppression de l’objet CWinThread.
...
Le membre de données `m_nThreadID` est une variable publique de type DWORD. Elle est valide uniquement si l’objet de thread de noyau sous-jacent existe actuellement.
...
Le bibliothèque MFC (Microsoft Foundation Class) met automatiquement fin à votre thread lorsque la fenêtre référencée par `m_pActiveWnd` est fermée. Si ce thread est le thread principal d’une application, l’application s’arrête également. Si ce membre de données a la valeur NULL, la fenêtre active de l’objet `CWinApp` de l’application est héritée. `m_pActiveWnd` est une variable publique de type `CWnd*`.
...
Le bibliothèque MFC (Microsoft Foundation Class) met automatiquement fin à votre thread lorsque la fenêtre référencée par `m_pMainWnd` est fermée. Si ce thread est le thread principal d’une application, l’application s’arrête également. Si ce membre de données a la valeur NULL, la fenêtre principale de l’objet `CWinApp` de l’application sera utilisée pour déterminer quand arrêter le thread. `m_pMainWnd` est une variable publique de type `CWnd*`.
...
`OnIdle` doit retourner la valeur 0 pour indiquer qu’aucun temps de traitement inactif supplémentaire n’est nécessaire. Le paramètre *lCount* est incrémenté chaque fois que `OnIdle` est appelée lorsque la file d’attente de messages est vide et est réinitialisée à 0 à chaque fois qu’un nouveau message est traité. Vous pouvez appeler vos différentes routines inactives en fonction de ce nombre.
...
Étant donné que l’application ne peut pas traiter les messages tant que `OnIdle` n’est pas retourné, n’effectuez pas de tâches longues dans cette fonction.
...
Appelé pour envoyer un message défini par l’utilisateur à un autre objet `CWinThread`.
...
`PumpMessage` contient la boucle de message du thread. `PumpMessage` est appelée par `CWinThread` pour pomper les messages du thread. Vous pouvez appeler `PumpMessage` directement pour forcer le traitement des messages, ou vous pouvez remplacer `PumpMessage` pour modifier son comportement par défaut.

L’appel de `PumpMessage` directement et la substitution de son comportement par défaut est recommandé pour les utilisateurs expérimentés uniquement.
...
Nombre de suspensions précédentes du thread en cas de réussite ; Sinon, `0xFFFFFFFF`. Si la valeur de retour est égale à zéro, le thread actuel n’a pas été suspendu. Si la valeur de retour est 1, le thread a été suspendu, mais est maintenant redémarré. Toute valeur de retour supérieure à un signifie que le thread reste suspendu.
...
`Run` acquiert et distribue des messages Windows jusqu’à ce que l’application reçoive un message [WM_QUIT](/windows/win32/winmsg/wm-quit) . Si la file d’attente de messages du thread ne contient actuellement aucun message, `Run` appelle `OnIdle` pour effectuer un traitement en temps inactif. Les messages entrants sont envoyés à la fonction membre [PreTranslateMessage](#pretranslatemessage) pour un traitement spécial, puis à la fonction Windows [TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage) pour la traduction du clavier standard. Enfin, la fonction Windows [DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage) est appelée.

`Run` est rarement remplacée, mais vous pouvez la remplacer pour implémenter un comportement spécial.
...
Nombre de suspensions précédentes du thread en cas de réussite ; Sinon, `0xFFFFFFFF`.
