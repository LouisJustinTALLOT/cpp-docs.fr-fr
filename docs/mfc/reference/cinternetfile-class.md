...
|[CInternetFile :: Close](#close)|Ferme une `CInternetFile` et libère ses ressources.|
...
Fournit une classe de base pour les classes de fichier [CHttpFile](../../mfc/reference/chttpfile-class.md) et [CGopherFile](../../mfc/reference/cgopherfile-class.md) . Vous ne créez jamais un objet `CInternetFile` directement. Au lieu de cela, créez un objet de l’une de ses classes dérivées en appelant [CGopherConnection :: OpenFile](../../mfc/reference/cgopherconnection-class.md#openfile) ou [CHttpConnection :: OpenRequest](../../mfc/reference/chttpconnection-class.md#openrequest). Vous pouvez également créer un objet `CInternetFile` en appelant [CFtpConnection :: OpenFile](../../mfc/reference/cftpconnection-class.md#openfile).

Les fonctions membres de `CInternetFile` `Open`, `LockRange`, `UnlockRange`et `Duplicate` ne sont pas implémentées pour `CInternetFile`. Si vous appelez ces fonctions sur un objet `CInternetFile`, vous obtiendrez un [CNotSupportedException](../../mfc/reference/cnotsupportedexception-class.md).
...
Lors de la gestion des exceptions, `Abort` diffère de [près](#close) de deux façons importantes. Tout d’abord, la fonction `Abort` ne lève pas d’exception en cas d’échec, car elle ignore les échecs. Deuxièmement, `Abort` ne **déclare** pas si le fichier n’a pas été ouvert ou a été fermé précédemment.

##  <a name="cinternetfile"></a>CInternetFile :: CInternetFile

Cette fonction membre est appelée lorsqu’un objet `CInternetFile` est créé.
...
*dwContext*<br/>
Identificateur de contexte de l’objet `CInternetFile`. Pour plus d’informations sur l’identificateur de contexte, consultez [concepts de base de WinInet](../../mfc/wininet-basics.md) .

### <a name="remarks"></a>Notes

Vous ne créez jamais un objet `CInternetFile` directement. Au lieu de cela, créez un objet de l’une de ses classes dérivées en appelant [CGopherConnection :: OpenFile](../../mfc/reference/cgopherconnection-class.md#openfile) ou [CHttpConnection :: OpenRequest](../../mfc/reference/chttpconnection-class.md#openrequest). Vous pouvez également créer un objet `CInternetFile` en appelant [CFtpConnection :: OpenFile](../../mfc/reference/cftpconnection-class.md#openfile).

##  <a name="close"></a>CInternetFile :: Close

Ferme une `CInternetFile` et libère l’une de ses ressources.
...
Utilisez `Flush` pour vous assurer que toutes les données en mémoire ont été écrites sur l’ordinateur cible et pour vous assurer que votre transaction avec l’ordinateur hôte est terminée. `Flush` n’est effective que sur les objets `CInternetFile` ouverts en écriture.
...
Pour vous assurer que toutes les données sont récupérées, une application doit continuer à appeler la méthode `CInternetFile::Read` jusqu’à ce que la méthode retourne zéro.
...
Pointeur vers la mémoire tampon qui contient les données brutes récupérées de l’objet [CInternetFile](../../mfc/reference/cinternetfile-class.md) . Quel que soit le type de données de la mémoire tampon passé à cette méthode, il n’effectue aucune manipulation sur les données (par exemple, la conversion en Unicode). vous devez donc mapper les données retournées à la structure attendue, comme si le type de <strong>\*</strong> **void** était retourné.
...
Si vous appelez `ReadString` sans appeler d’abord [SetReadBufferSize](#setreadbuffersize), vous obtiendrez une mémoire tampon de 4096 octets.
...
- `CFile::begin` déplacer le pointeur de fichier *lOff* d’octets vers l’avant à partir du début du fichier.

- `CFile::current` déplacer le pointeur de fichier *lOff* octets à partir de la position actuelle dans le fichier.

- `CFile::end` déplacer le pointeur de fichier *lOff* octets à partir de la fin du fichier. *lOff* doit être négatif pour effectuer une recherche dans le fichier existant ; les valeurs positives recherchent au-delà de la fin du fichier.
...
La fonction `Seek` autorise un accès aléatoire au contenu d’un fichier en déplaçant le pointeur d’une quantité spécifiée, de manière absolue ou relativement. Aucune donnée n’est réellement lue pendant la recherche.

À ce stade, un appel à cette fonction membre est uniquement pris en charge pour les données associées aux objets `CHttpFile`. Elle n’est pas prise en charge pour les demandes FTP ou gopher. Si vous appelez `Seek` pour l’un de ces services non pris en charge, il vous renverra au code d’erreur Win32 ERROR_INTERNET_INVALID_OPERATION.

Lorsqu’un fichier est ouvert, le pointeur de fichier est au décalage 0, le début du fichier.

> [!NOTE]
>  L’utilisation de `Seek` peut provoquer le [vidage](#flush)d’un appel implicite.
...
Appelez cette fonction membre pour définir la taille de la mémoire tampon de lecture temporaire utilisée par un objet dérivé de `CInternetFile`.
...
Par défaut, un objet `CInternetFile` ne fournit pas de mise en mémoire tampon pour la lecture. Si vous appelez cette fonction membre, vous devez vous assurer que le fichier a été ouvert pour l’accès en lecture.

Vous pouvez augmenter la taille de la mémoire tampon à tout moment, mais la réduction de la mémoire tampon n’aura aucun effet. Si vous appelez [ReadString](#readstring) sans appeler d’abord `SetReadBufferSize`, vous obtiendrez une mémoire tampon de 4096 octets.

##  <a name="setwritebuffersize"></a>CInternetFile :: SetWriteBufferSize

Appelez cette fonction membre pour définir la taille de la mémoire tampon d’écriture temporaire utilisée par un objet dérivé de `CInternetFile`.
