---
description: 'En savoir plus sur : classe CMFCToolBar'
...
|[CMFCToolBar :: LoadToolBarEx](#loadtoolbarex)|Charge la barre d’outils à partir des ressources d’application à l’aide de la `CMFCToolBarInfo` classe d’assistance pour permettre à l’application d’utiliser des images de grande taille.|
...
La méthode [CMFCToolBar :: AdjustLayout](#adjustlayout) appelle cette méthode pour recalculer la taille si le parent de la barre d’outils n’est pas un `CMFCReBar` objet.
...
Cette méthode calcule la taille de la barre d’outils à l’aide de la `CMFCToolBar::CalcLayout` méthode. Elle passe l’indicateur LM_STRETCH pour le paramètre *dwMode* si *bStretch* a la valeur true. Elle passe l’indicateur LM_HORZ si *bHorz* a la valeur true.
...
Cette méthode étend l’implémentation de la classe de base, [CBasePane :: EnableDocking](../../mfc/reference/cbasepane-class.md#enabledocking), en affectant `CBasePane::m_dwControlBarStyle` à la donnée membre la valeur AFX_CBRS_FLOAT. Cette méthode passe ensuite *dwAlignment* à l’implémentation de la classe de base.
...
Cette méthode partagée examine chaque barre d’outils de l’application pour l' `CMFCToolBar` objet qui contient le handle de fenêtre donné.
...
Vous pouvez appeler la méthode [CMFCToolBar :: IsExistCustomizeButton](#isexistcustomizebutton) pour déterminer si la barre d’outils contient un `CMFCCustomizeButton` objet valide.
...
*lpRect*<br/>
à Pointeur vers un `CRect` objet qui reçoit les coordonnées du rectangle englobant de l’image.

### <a name="remarks"></a>Notes

`CRect`Objet auquel *lpRect* pointe la valeur 0 si un bouton à l’index spécifié n’existe pas.
...
`CSize`Structure qui spécifie la taille des images de barre d’outils verrouillées ou une `CSize` structure vide si la barre d’outils n’est pas verrouillée.
...
Substituez cette méthode dans une classe dérivée de `CMFCToolBar` et retournez true si votre barre peut afficher les boutons de barre d’outils avec la taille de bordure supplémentaire et retourner false dans le cas contraire. L’implémentation par défaut retourne la valeur TRUE.
...
Charge la barre d’outils à partir des ressources d’application à l’aide de la `CMFCToolBarInfo` classe d’assistance pour permettre à l’application d’utiliser des images de grande taille.
...
*strTTText*<br/>
à Référence à un `CString` objet qui reçoit le texte de l’info-bulle.
...
## <a name="cmfctoolbarsavestate"></a><a name="savestate"></a> CMFCToolBar :: saveste
...
Cette méthode génère un échec d’assertion dans les builds de débogage si l' `CMFCToolBarImages` objet spécifié n’est pas valide ou a une taille d’image qui diffère de la taille d’image par défaut de la barre d’outils.
