...
|[CWinApp ::D elRegTree](#delregtree)|Supprime une clé spécifiée et toutes ses sous-clés.|
|[CWinApp ::D oMessageBox](#domessagebox)|Implémente [AfxMessageBox](cstring-formatting-and-message-box-display.md#afxmessagebox) pour l’application.|
|[CWinApp ::D oWaitCursor](#dowaitcursor)|Active et désactive le curseur d’attente.|
...
|[CWinApp :: GetAppRegistryKey](#getappregistrykey)|Retourne la clé pour HKEY_CURRENT_USER\\« logiciel » \RegistryKey\ProfileName.|
...
|[CWinApp :: GetSectionKey](#getsectionkey)|Retourne la clé pour HKEY_CURRENT_USER\\« logiciel » \RegistryKey\AppName\lpszSection.|
|[CWinApp :: HideApplication](#hideapplication)|Masque l’application avant de fermer tous les documents.|
|[CWinApp :: HtmlHelp](#htmlhelp)|Appelle la fonction Windows `HTMLHelp`.|
...
|[CWinApp ::P arseCommandLine](#parsecommandline)|Analyse des paramètres et des indicateurs individuels dans la ligne de commande.|
|[CWinApp ::P reTranslateMessage](#pretranslatemessage)|Filtre les messages avant qu’ils ne soient distribués aux fonctions Windows [TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage) et [DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage).|
|[CWinApp ::P rocessMessageFilter](#processmessagefilter)|Intercepte certains messages avant qu’ils n’atteignent l’application.|
|[CWinApp ::P rocessShellCommand](#processshellcommand)|Gère les indicateurs et les arguments de ligne de commande.|
|[CWinApp ::P rocessWndProcException](#processwndprocexception)|Intercepte toutes les exceptions non gérées levées par les gestionnaires de commandes et de messages de l’application.|
...
|[CWinApp :: RunAutomated](#runautomated)|Teste la ligne de commande de l’application pour l’option **/Automation** . Obsolète. Utilisez plutôt la valeur dans [CCommandLineInfo :: m_bRunAutomated](../../mfc/reference/ccommandlineinfo-class.md#m_brunautomated) après avoir appelé [ParseCommandLine](#parsecommandline).|
|[CWinApp :: RunEmbedded](#runembedded)|Teste la ligne de commande de l’application pour l’option **/Embedding** . Obsolète. Utilisez plutôt la valeur dans [CCommandLineInfo :: m_bRunEmbedded](../../mfc/reference/ccommandlineinfo-class.md#m_brunembedded) après avoir appelé [ParseCommandLine](#parsecommandline).|
...
|[CWinApp :: Unregister](#unregister)|Annule l’enregistrement de tous les éléments connus pour être inscrits par l’objet `CWinApp`.|
|[CWinApp :: WinHelp](#winhelp)|Appelle la fonction Windows `WinHelp`.|
...
Chaque application qui utilise Microsoft Foundation classes ne peut contenir qu’un seul objet dérivé de `CWinApp`. Cet objet est construit lorsque d' C++ autres objets globaux sont construits et est déjà disponible lorsque Windows appelle la fonction `WinMain`, qui est fournie par le bibliothèque MFC (Microsoft Foundation Class). Déclarez votre objet `CWinApp` dérivé au niveau global.

Quand vous dérivez une classe d’application de `CWinApp`, substituez la fonction membre [InitInstance](#initinstance) pour créer l’objet de fenêtre principale de votre application.

Outre les fonctions membres `CWinApp`, le bibliothèque MFC (Microsoft Foundation Class) fournit les fonctions globales suivantes pour accéder à votre objet `CWinApp` et à d’autres informations globales :

- [AfxGetApp](application-information-and-management.md#afxgetapp) Obtient un pointeur vers l’objet `CWinApp`.
...
- [AfxGetAppName](application-information-and-management.md#afxgetappname) Obtient un pointeur vers une chaîne contenant le nom de l’application. Sinon, si vous avez un pointeur vers l’objet `CWinApp`, utilisez `m_pszExeName` pour obtenir le nom de l’application.

Consultez [CWinApp : la classe d’application](../../mfc/cwinapp-the-application-class.md) pour plus d’informations sur la classe `CWinApp`, y compris une vue d’ensemble des éléments suivants :

- Code dérivé de `CWinApp`écrit par l’Assistant Application.

- rôle de `CWinApp`dans la séquence d’exécution de votre application.

- implémentations de la fonction membre par défaut de `CWinApp`.

- remplaçable clé de l' `CWinApp`.
...
L’implémentation par défaut de `ApplicationRecoveryCallback` utilise la `CDataRecoveryHandler` pour enregistrer la liste des documents actuellement ouverts dans le registre. Cette méthode n’enregistre pas les fichiers de façon automatique.
...
`CreatePrinterDC` Initialise le contexte de périphérique que vous transmettez par référence, vous pouvez l’utiliser pour imprimer.
...
Construit un objet `CWinApp` et passe *lpszAppName* à stocker comme nom de l’application.
...
*lpszAppName*<br/>
Chaîne terminée par le caractère null qui contient le nom de l’application que Windows utilise. Si cet argument n’est pas fourni ou si a la valeur NULL, `CWinApp` utilise la chaîne de ressource AFX_IDS_APP_TITLE ou le nom du fichier exécutable.

### <a name="remarks"></a>Notes

Vous devez construire un objet global de votre classe dérivée de `CWinApp`. Vous ne pouvez avoir qu’un seul objet `CWinApp` dans votre application. Le constructeur stocke un pointeur vers l’objet `CWinApp` afin que `WinMain` puisse appeler les fonctions membres de l’objet pour initialiser et exécuter l’application.
...
N’appelez pas cette fonction membre pour ouvrir une boîte de message. Utilisez à la place `AfxMessageBox`.

Substituez cette fonction membre pour personnaliser le traitement de vos appels `AfxMessageBox` au niveau de l’application.
...
Bien que vous n’appeliez normalement pas `DoWaitCursor` directement, vous pouvez remplacer cette fonction membre pour modifier le curseur d’attente ou pour effectuer un traitement supplémentaire pendant que le curseur d’attente est affiché.

Pour une façon plus simple et rationalisée d’implémenter un curseur d’attente, utilisez `CWaitCursor`.
...
Appelez cette fonction membre à partir du constructeur de votre classe dérivée de `CWinApp`pour utiliser HTMLHelp pour l’aide de votre application.
...
Appelez cette fonction, en général à partir de votre remplacement `InitInstance`, pour permettre aux utilisateurs de votre application d’ouvrir des fichiers de données lorsqu’ils double-cliquent sur les fichiers à partir du gestionnaire de fichiers Windows.
...
Appelez la fonction membre `RegisterShellFileTypes` conjointement avec cette fonction ou fournissez un. Fichier REG avec votre application pour l’inscription manuelle des types de documents.
...
Appelée par l’infrastructure à partir de la fonction membre `Run` pour quitter cette instance de l’application.
...
N’appelez pas cette fonction membre à partir de n’importe où, mais dans la fonction membre `Run`.
...
Retourne la clé pour HKEY_CURRENT_USER\\« logiciel » \RegistryKey\ProfileName.
...
Vous pouvez utiliser `GetNextDocTemplate` dans une boucle d’itération directe si vous établissez la position initiale avec un appel à `GetFirstDocTemplatePosition`.
...
Retourne la clé pour HKEY_CURRENT_USER\\« logiciel » \RegistryKey\AppName\lpszSection.
...
*dwData*<br/>
Spécifie des données supplémentaires. La valeur utilisée dépend de la valeur du paramètre *nCmd* . La valeur par défaut `0x000F`, ce qui signifie [HH_HELP_CONTEXT](/previous-versions/windows/desktop/htmlhelp/hh-help-context-command).
...
L’initialisation de l’application est divisée de manière conceptuelle en deux sections : l’initialisation d’une application unique qui est effectuée lors de la première exécution du programme et l’initialisation de l’instance qui s’exécute chaque fois qu’une copie du programme s’exécute, y compris la première fois. L’implémentation de `WinMain` de l’infrastructure appelle cette fonction.

Remplacez `InitInstance` pour initialiser chaque nouvelle instance de votre application s’exécutant sous Windows. En règle générale, vous substituez `InitInstance` pour construire votre objet de fenêtre principale et définir le `CWinThread::m_pMainWnd` membre de données pour qu’il pointe vers cette fenêtre. Pour plus d’informations sur la substitution de cette fonction membre, consultez [CWinApp : la classe application](../../mfc/cwinapp-the-application-class.md).
...
*lpszResourceName*<br/>
Pointe vers une chaîne se terminant par un caractère null qui contient le nom de la ressource icône. Vous pouvez également utiliser une `CString` pour cet argument.
...
> [!NOTE]
> Cette fonction membre appelle la fonction d’API Win32 [LoadIcon](/windows/win32/api/winuser/nf-winuser-loadiconw), qui peut uniquement charger une icône dont la taille est conforme à la SM_CXICON et SM_CYICON valeurs de métriques système.
...
*nIDCursor*<br/>
Identificateur de constante de manifeste **OCR_** qui spécifie un curseur Windows prédéfini. Vous devez disposer d' `#define OEMRESOURCE` avant `#include \<afxwin.h>` pour accéder aux constantes **OCR_** dans Windows. Manutention.
...
Utilisez la fonction membre `LoadOEMCursor` ou [LoadStandardCursor](#loadstandardcursor) pour accéder aux curseurs Windows prédéfinis.
...
*nIDIcon*<br/>
Identificateur de constante de manifeste **OIC_** qui spécifie une icône Windows prédéfinie. Vous devez disposer d' `#define OEMRESOURCE` avant `#include \<afxwin.h>` pour accéder aux constantes **OIC_** dans Windows. Manutention.
...
Utilisez la fonction membre `LoadOEMIcon` ou [LoadStandardIcon](#loadstandardicon) pour accéder aux icônes prédéfinies de Windows.
...
- Curseur IDC_WAIT sablier utilisé lorsque Windows effectue une tâche qui prend du temps

- IDC_CROSS le curseur réticule pour la sélection

- IDC_UPARROW flèche pointant vers le haut

- IDC_SIZE obsolètes et non prises en charge ; utiliser IDC_SIZEALL
...
Utilisez la fonction membre `LoadStandardCursor` ou [LoadOEMCursor](#loadoemcursor) pour accéder aux curseurs Windows prédéfinis.
...
Utilisez la fonction membre `LoadStandardIcon` ou [LoadOEMIcon](#loadoemicon) pour accéder aux icônes prédéfinies de Windows.
...
En mode de contexte d’aide, le curseur prend la forme d’un point d’interrogation et l’utilisateur peut le déplacer à propos de l’écran. Examinez cet indicateur si vous souhaitez implémenter une gestion spéciale dans le mode d’aide. `m_bHelpMode` est une variable publique de type BOOL.
...
|-AFX_RESTART_MANAGER_RESTORE_AUTOSAVED_FILES|Le gestionnaire de redémarrage invite l’utilisateur à restaurer les fichiers enregistrés de façon automatique après le redémarrage de l’application. La classe `CDataRecoveryHandler` interroge l’utilisateur.|
...
|-AFX_RESTART_MANAGER_SUPPORT_RECOVERY_ASPECTS|OfAFX_RESTART_MANAGER_SUPPORT_RECOVERY d’Union, AFX_RESTART_MANAGER_AUTOSAVE_AT_INTERVAL, AFX_RESTART_MANAGER_REOPEN_PREVIOUS_FILES et AFX_RESTART_MANAGER_RESTORE_AUTOSAVED_FILES.|

##  <a name="cwinappm_ehelptype"></a><a name="m_ehelptype"></a>CWinApp :: m_eHelpType

Le type de ce membre de données est le type énuméré AFX_HELP_TYPE, qui est défini dans la classe `CWinApp`.
...
Le membre de données `m_hInstance` est un handle de l’instance actuelle de l’application s’exécutant sous Windows. Elle est retournée par la fonction globale [AfxGetInstanceHandle](application-information-and-management.md#afxgetinstancehandle). `m_hInstance` est une variable publique de type HINSTANCE.
...
Vous devez passer `m_nCmdShow` comme argument quand vous appelez [CWnd :: ShowWindow](../../mfc/reference/cwnd-class.md#showwindow) pour la fenêtre principale de votre application. `m_nCmdShow` est une variable publique de type **int**.
...
> [!NOTE]
> Si vous assignez une valeur à `m_pszAppName`, elle doit être allouée dynamiquement sur le tas. Le destructeur `CWinApp` appelle **Free**() avec ce pointeur. Vous souhaitez utiliser la fonction de bibliothèque Runtime `_tcsdup`() pour effectuer l’allocation. Libérez également la mémoire associée au pointeur actuel avant d’assigner une nouvelle valeur. Par exemple :
...
Contrairement à [m_pszAppName](#m_pszappname), ce nom ne peut pas contenir de valeurs vides. `m_pszExeName` est une variable publique de type **const char** <strong>\*</strong>.

> [!NOTE]
> Si vous assignez une valeur à `m_pszExeName`, elle doit être allouée dynamiquement sur le tas. Le destructeur `CWinApp` appelle **Free**() avec ce pointeur. Vous souhaitez utiliser la fonction de bibliothèque Runtime `_tcsdup`() pour effectuer l’allocation. Libérez également la mémoire associée au pointeur actuel avant d’assigner une nouvelle valeur. Par exemple :
...
Par défaut, le Framework Initialise `m_pszHelpFilePath` sur le nom de l’application avec «. HLP» ajouté. Pour modifier le nom du fichier d’aide, définissez `m_pszHelpFilePath` de façon à ce qu’il pointe vers une chaîne qui contient le nom complet du fichier d’aide souhaité. Pour ce faire, il est pratique d’utiliser la fonction [InitInstance](#initinstance) de l’application. `m_pszHelpFilePath` est une variable publique de type **const char** <strong>\*</strong>.

> [!NOTE]
> Si vous assignez une valeur à `m_pszHelpFilePath`, elle doit être allouée dynamiquement sur le tas. Le destructeur `CWinApp` appelle **Free**() avec ce pointeur. Vous souhaitez utiliser la fonction de bibliothèque Runtime `_tcsdup`() pour effectuer l’allocation. Libérez également la mémoire associée au pointeur actuel avant d’assigner une nouvelle valeur. Par exemple :
...
> [!NOTE]
> Si vous assignez une valeur à `m_pszProfileName`, elle doit être allouée dynamiquement sur le tas. Le destructeur `CWinApp` appelle **Free**() avec ce pointeur. Vous souhaitez utiliser la fonction de bibliothèque Runtime `_tcsdup`() pour effectuer l’allocation. Libérez également la mémoire associée au pointeur actuel avant d’assigner une nouvelle valeur. Par exemple :
...
Si vous assignez une valeur à `m_pszRegistryKey`, elle doit être allouée dynamiquement sur le tas. Le destructeur `CWinApp` appelle **Free**() avec ce pointeur. Vous souhaitez utiliser la fonction de bibliothèque Runtime `_tcsdup`() pour effectuer l’allocation. Libérez également la mémoire associée au pointeur actuel avant d’assigner une nouvelle valeur. Par exemple :
...
Vous devez ajouter une instruction `ON_COMMAND( ID_CONTEXT_HELP, OnContextHelp )` à votre table des messages de la classe `CWinApp` et également ajouter une entrée de table d’accélérateurs, en général MAJ + F1, pour activer cette fonction membre.
...
Vous devez ajouter une instruction `ON_COMMAND( ID_FILE_NEW, OnFileNew )` à votre table des messages de la classe `CWinApp` pour activer cette fonction membre. S’il est activé, cette fonction gère l’exécution de la commande fichier nouveau.
...
Vous devez ajouter une instruction `ON_COMMAND( ID_FILE_OPEN, OnFileOpen )` à votre table des messages de la classe `CWinApp` pour activer cette fonction membre. S’il est activé, cette fonction gère l’exécution de la commande fichier ouvrir.
...
Vous devez ajouter une instruction `ON_COMMAND( ID_FILE_PRINT_SETUP, OnFilePrintSetup )` à votre table des messages de la classe `CWinApp` pour activer cette fonction membre. S’il est activé, cette fonction gère l’exécution de la commande fichier print.
...
Vous devez ajouter une instruction `ON_COMMAND( ID_HELP, OnHelp )` à votre table des messages de la classe `CWinApp` pour activer cette fonction membre. S’il est activé, appelé par le Framework lorsque l’utilisateur appuie sur la touche F1.
...
Vous devez ajouter une instruction `ON_COMMAND( ID_HELP_FINDER, OnHelpFinder )` à votre table des messages de la classe `CWinApp` pour activer cette fonction membre. Si cette option est activée, l’infrastructure appelle cette fonction de gestionnaire de messages lorsque l’utilisateur de votre application sélectionne la commande de recherche d’aide pour appeler `WinHelp` avec la rubrique d' **HELP_FINDER** standard.
...
Vous devez ajouter une instruction `ON_COMMAND( ID_HELP_INDEX, OnHelpIndex )` à votre table des messages de la classe `CWinApp` pour activer cette fonction membre. Si cette option est activée, l’infrastructure appelle cette fonction de gestionnaire de messages lorsque l’utilisateur de votre application sélectionne la commande d’index de l’aide pour appeler `WinHelp` avec la rubrique d' **HELP_INDEX** standard.
...
Vous devez ajouter une instruction `ON_COMMAND( ID_HELP_USING, OnHelpUsing )` à votre table des messages de la classe `CWinApp` pour activer cette fonction membre. L’infrastructure appelle cette fonction de gestionnaire de messages lorsque l’utilisateur de votre application sélectionne la commande aide pour appeler l’application `WinHelp` avec la rubrique d' **HELP_HELPONHELP** standard.
...
`OnIdle` doit retourner la valeur 0 pour indiquer qu’aucun temps de traitement inactif n’est nécessaire. Le paramètre *lCount* est incrémenté chaque fois que `OnIdle` est appelé lorsque la file d’attente de messages est vide et rétablit la valeur 0 à chaque fois qu’un nouveau message est traité. Vous pouvez appeler vos différentes routines inactives en fonction de ce nombre.

Voici un résumé du traitement des boucles inactives :

1. Si la boucle de message dans le bibliothèque MFC (Microsoft Foundation Class) vérifie la file d’attente de messages et ne trouve aucun message en attente, elle appelle `OnIdle` pour l’objet d’application et fournit 0 comme argument *lCount* .
...
3. La boucle de message vérifie à nouveau la file d’attente de messages. Si aucun message n’est en attente, il appelle à nouveau `OnIdle`, en incrémentant l’argument *lCount* .

4. Finalement, `OnIdle` termine le traitement de toutes ses tâches inactives et retourne 0. Cela indique à la boucle de message d’arrêter l’appel de `OnIdle` jusqu’à ce que le message suivant soit reçu à partir de la file d’attente de messages, auquel moment le cycle d’inactivité redémarre avec l’argument défini sur 0.

N’effectuez pas de tâches longues pendant la `OnIdle`, car votre application ne peut pas traiter l’entrée utilisateur tant que `OnIdle` n’est pas retourné.

> [!NOTE]
> L’implémentation par défaut de `OnIdle` les objets d’interface utilisateur de commande de mise à jour, tels que les éléments de menu et les boutons de barre d’outils, et effectue le nettoyage de la structure de données interne. Par conséquent, si vous substituez `OnIdle`, vous devez appeler `CWinApp::OnIdle` avec la `lCount` dans votre version substituée. Tout d’abord, appelez tout le traitement inactif de la classe de base (autrement dit, jusqu’à ce que la classe de base `OnIdle` retourne 0). Si vous devez effectuer le travail avant la fin du traitement de la classe de base, examinez l’implémentation de la classe de base pour sélectionner le *lCount* approprié pour effectuer votre travail.

Si vous ne souhaitez pas que `OnIdle` soit appelée chaque fois qu’un message est extrait de la file d’attente de messages, vous pouvez remplacer [CWinThreadIsIdleMessage](../../mfc/reference/cwinthread-class.md#isidlemessage). Si une application a défini un minuteur très bref, ou si le système envoie le message WM_SYSTIMER, `OnIdle` sera appelée à plusieurs reprises et dégradera les performances.
...
Quand vous démarrez un nouveau projet MFC à l’aide de l’Assistant Application, l’Assistant application crée une instance locale de `CCommandLineInfo`, puis appelle `ProcessShellCommand` et `ParseCommandLine` dans la fonction membre [InitInstance](#initinstance) . Une ligne de commande suit l’itinéraire décrit ci-dessous :

1. Une fois créé dans `InitInstance`, l’objet `CCommandLineInfo` est passé à `ParseCommandLine`.
...
Cette fonction membre est appelée par [InitInstance](#initinstance) pour accepter les paramètres transmis à partir de l’objet `CCommandLineInfo` identifié par *rCmdInfo*et exécuter l’action indiquée.
...
Quand vous démarrez un nouveau projet MFC à l’aide de l’Assistant Application, l’Assistant application crée une instance locale de `CCommandLineInfo`, puis appelle `ProcessShellCommand` et [ParseCommandLine](#parsecommandline) dans la fonction membre `InitInstance`. Une ligne de commande suit l’itinéraire décrit ci-dessous :
...
Les données membres de l’objet `CCommandLineInfo`, identifié par [CCommandLineInfo :: m_nShellCommand](../../mfc/reference/ccommandlineinfo-class.md#m_nshellcommand), sont du type énuméré suivant, qui est défini dans la classe `CCommandLineInfo`.
...
Si la base de données d’inscription associe déjà une extension de nom de fichier donnée à un autre type de fichier, aucune nouvelle association n’est créée. Consultez la classe `CDocTemplate` pour connaître le format des chaînes nécessaires à l’enregistrement de ces informations.
...
|*dwRestartFlags*|dans Indicateurs facultatifs pour le gestionnaire de redémarrage. Pour plus d'informations, consultez la section Notes.|
|*pRecoveryCallback*|dans Fonction de rappel de récupération. Cette fonction doit accepter un paramètre LPVOID comme entrée et retourner une valeur DWORD. La fonction de rappel de récupération par défaut est `CWinApp::ApplicationRecoveryCallback`.|
...
|*dwCallbackFlags*|dans Indicateurs passés à la fonction de rappel de récupération. Réservé pour un usage futur.|
...
`Run` acquiert et distribue des messages Windows jusqu’à ce que l’application reçoive un message WM_QUIT. Si la file d’attente de messages de l’application ne contient actuellement aucun message, `Run` appelle [OnIdle](#onidle) pour effectuer un traitement en temps inactif. Les messages entrants sont dirigés vers la fonction membre [PreTranslateMessage](#pretranslatemessage) pour un traitement spécial, puis vers la fonction Windows `TranslateMessage` pour la traduction du clavier standard. Enfin, la fonction Windows `DispatchMessage` est appelée.

`Run` est rarement remplacée, mais vous pouvez la remplacer pour fournir un comportement spécial.
...
S’il est présent, l’option est supprimée de la ligne de commande. Pour plus d’informations sur l’incorporation, consultez l’article [serveurs : implémentation d’un serveur](../../mfc/servers-implementing-a-server.md).

##  <a name="cwinappsaveallmodified"></a><a name="saveallmodified"></a>CWinApp :: SaveAllModified

Appelé par le Framework pour enregistrer tous les documents lorsque la fenêtre frame principale de l’application doit être fermée ou via un message d’WM_QUERYENDSESSION.
...
Si *hDevMode* et *hDevNames* ont tous les deux la valeur null, `SelectPrinter` utilise l’imprimante par défaut actuelle.
...
Pour définir le type d’aide de votre application sur HTMLHelp, vous pouvez appeler [EnableHTMLHelp](#enablehtmlhelp). Une fois que vous avez appelé `EnableHTMLHelp`, votre application doit utiliser HTMLHelp comme application d’aide. Si vous souhaitez modifier pour utiliser WinHelp, vous pouvez appeler `SetHelpMode` et définir *eHelpType* sur `afxWinHelp`.
...
La fonction `Unregister` annule l’inscription effectuée par l’objet application et la fonction [Register](#register) . Normalement, les deux fonctions sont appelées implicitement par MFC et, par conséquent, n’apparaissent pas dans votre code.
