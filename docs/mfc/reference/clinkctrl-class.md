...
|[CLinkCtrl::CreateEx](#createex)|Crée un contrôle de lien avec des styles étendus et l’attache `CLinkCtrl` à un objet.|
...
Ce contrôle (et par conséquent `CLinkCtrl` la classe) est uniquement disponible pour les programmes qui s’exécutent sous Windows XP et versions ultérieures.
...
*dwStyle*<br/>
Spécifie le style du contrôle de lien. Appliquez une combinaison de styles de contrôle. Pour plus d’informations, consultez `Windows SDK` [styles de contrôles communs](/windows/win32/Controls/common-control-styles) dans le.
...
Vous construisez `CLinkCtrl` un objet en deux étapes. Tout d’abord, appelez le constructeur, `Create`puis appelez, qui crée le contrôle de lien et l’attache `CLinkCtrl` à l’objet. Si vous souhaitez utiliser des styles Windows étendus avec votre contrôle, appelez [CLinkCtrl :: CreateEx](#createex) à `Create`la place de.
...
L’exemple de code suivant définit deux variables, `m_Link1` nommées et `m_Link2`, qui sont utilisées pour accéder à deux contrôles de lien.
...
Crée un contrôle de lien avec des styles étendus et l’attache `CLinkCtrl` à un objet.
...
Cette fonction membre implémente le comportement du [LM_GETIDEALHEIGHT](/windows/win32/Controls/lm-getidealheight)de message Win32, comme décrit dans la SDK Windows.
...
|---------------|-----------------|
|*cxMaxWidth*|dans Largeur maximale, en pixels, du lien.|
|[out] \* *pSize*|Pointeur vers une structure de [taille](/windows/win32/api/windef/ns-windef-size) Windows. Lorsque cette méthode est retournée , le membre CY `SIZE` de la structure contient la hauteur de texte de lien idéale pour la largeur du texte de lien spécifiée par *cxMaxWidth*. Le membre *CX* de la structure contient la largeur du texte du lien qui est réellement nécessaire.|
...
Pour obtenir un exemple de `GetIdealSize` la méthode, consultez l’exemple dans [CLinkCtrl :: Create](#create).
...
Cette fonction membre implémente le comportement du [LM_GETITEM](/windows/win32/Controls/lm-getitem)de message Win32, comme décrit dans la SDK Windows.
...
Cette fonction membre implémente le comportement du [LM_HITTEST](/windows/win32/Controls/lm-hittest)de message Win32, comme décrit dans la SDK Windows.
...
Cette fonction membre implémente le comportement du [LM_SETITEM](/windows/win32/Controls/lm-setitem)de message Win32, comme décrit dans la SDK Windows.
