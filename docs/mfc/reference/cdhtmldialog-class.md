...
|Nom|Description|  
|----------|-----------------|  
|[CDHtmlDialog::CDHtmlDialog](#cdhtmldialog)|Construit un objet de CDHtmlDialog.|  
...
|Nom|Description|  
|----------|-----------------|  
|[CDHtmlDialog::CanAccessExternal](#canaccessexternal)|Substituable qui est appelé une vérification pour voir si les objets de script dans la page chargée peuvent accéder à la distribution externe de contrôle d’accès. Vérifie que la distribution est sécurisé pour le script ou la zone actuelle permet pour les objets qui ne sont pas sécurisés pour les scripts.|  
|[CDHtmlDialog::CreateControlSite](#createcontrolsite)|Overridable utilisé pour créer une instance de site de contrôle pour héberger le contrôle WebBrowser dans la boîte de dialogue.|  
...
|[CDHtmlDialog::DDX_DHtml_SelectString](#ddx_dhtml_selectstring)|Obtient ou définit le texte d’affichage d’une entrée de zone de liste (basé sur l’index en cours) sur une page HTML.|  
|[CDHtmlDialog::DDX_DHtml_SelectValue](#ddx_dhtml_selectvalue)|Obtient ou définit la valeur d’une entrée de zone de liste (basée sur l’index en cours) sur une page HTML.|  
...
|[CDHtmlDialog::GetCurrentUrl](#getcurrenturl)|Récupère le localisateur URL (Uniform Resource) associées au document en cours.|  
...
### <a name="remarks"></a>Notes  
 Ce message est envoyé à la boîte de dialogue lors de la `Create`, `CreateIndirect`, ou `DoModal` appels, qui se produisent immédiatement avant l’affichage de la boîte de dialogue.  
