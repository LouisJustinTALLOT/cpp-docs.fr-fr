...
3.  Si le volet de la cible est un volet d’ancrage régulière, l’infrastructure crée un volet à onglets.  
...
4.  Si le volet actif est dérivé de `CBaseTabbedPane`, tous ses onglets sont déplacés vers *pTabControlBarAttachTo* et le volet actif est détruit. Par conséquent, soyez prudent lorsque vous appelez cette méthode, car un pointeur vers le volet actif peut être non valide lorsque la méthode est retournée.  
