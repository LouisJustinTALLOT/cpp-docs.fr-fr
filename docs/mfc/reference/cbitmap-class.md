...
|[CBitmap :: CreateBitmapIndirect](#createbitmapindirect)|Initialise l’objet avec une bitmap avec la largeur, la hauteur et le modèle binaire (le cas échéant) spécifiés dans une structure `BITMAP`.|
...
|[CBitmap :: FromHandle](#fromhandle)|Retourne un pointeur vers un objet `CBitmap` lorsqu’il reçoit un handle vers une image bitmap Windows `HBITMAP`.|
|[CBitmap :: GetBitmap](#getbitmap)|Remplit une structure `BITMAP` avec des informations sur l’image bitmap.|
...
|Name|Description|
|----------|-----------------|
|[CBitmap :: Operator HBITMAP](#operator_hbitmap)|Retourne le handle Windows attaché à l’objet `CBitmap`.|

## <a name="remarks"></a>Notes

Pour utiliser un objet `CBitmap`, construisez l’objet, attachez-lui un handle de bitmap avec l’une des fonctions membres d’initialisation, puis appelez les fonctions membres de l’objet.
...
Pour plus d’informations, consultez la description du champ `bmBits` dans la structure `BITMAP`. La structure [BITMAP](/windows/win32/api/wingdi/ns-wingdi-bitmap) est décrite sous la fonction membre [CBitmap::CreateBitmapIndirect](#createbitmapindirect) .
...
Même si une image bitmap ne peut pas être sélectionnée directement pour un périphérique d’affichage, elle peut être sélectionnée en tant que bitmap actuelle pour un contexte de périphérique de mémoire à l’aide de [CDC :: SelectObject](../../mfc/reference/cdc-class.md#selectobject) et copiée dans n’importe quel contexte de périphérique compatible à l’aide de la fonction [CDC :: BitBlt](../../mfc/reference/cdc-class.md#bitblt) ou [CDC :: StretchBlt](../../mfc/reference/cdc-class.md#stretchblt) . (La fonction [CDC ::P atblt](../../mfc/reference/cdc-class.md#patblt) peut copier l’image bitmap du pinceau actuel directement dans le contexte de périphérique d’affichage.)

Si la `BITMAP` structure vers laquelle pointe le paramètre *lpBitmap* a été remplie à l’aide de la fonction `GetObject`, les bits de l’image bitmap ne sont pas spécifiés et l’image bitmap n’est pas initialisée. Pour initialiser la bitmap, une application peut utiliser une fonction telle que [CDC :: BitBlt](../../mfc/reference/cdc-class.md#bitblt) ou [SetDIBits](/windows/win32/api/wingdi/nf-wingdi-setdibits) pour copier les bits de la bitmap identifiée par le premier paramètre de `CGdiObject::GetObject` dans l’image bitmap créée par `CreateBitmapIndirect`.

Lorsque vous avez terminé avec l’objet `CBitmap` créé avec `CreateBitmapIndirect` fonction, commencez par sélectionner l’image bitmap hors du contexte de périphérique, puis supprimez l’objet `CBitmap`.
...
Étant donné qu’un contexte de périphérique de mémoire couleur peut être sélectionné pour des bitmaps couleur ou monochrome, le format de l’image bitmap retournée par la fonction `CreateCompatibleBitmap` n’est pas toujours le même ; Toutefois, le format d’une bitmap compatible pour un contexte de périphérique qui n’est pas en mémoire est toujours au format de l’appareil.

Lorsque vous avez terminé avec l’objet `CBitmap` créé avec la fonction `CreateCompatibleBitmap`, commencez par sélectionner l’image bitmap hors du contexte de périphérique, puis supprimez l’objet `CBitmap`.
...
Lorsque vous avez terminé avec l’objet `CBitmap` créé avec la fonction `CreateDiscardableBitmap`, commencez par sélectionner l’image bitmap hors du contexte de périphérique, puis supprimez l’objet `CBitmap`.

##  <a name="fromhandle"></a>CBitmap :: FromHandle

Retourne un pointeur vers un objet `CBitmap` lorsqu’il reçoit un handle vers une image bitmap Windows GDI.
...
La largeur et la hauteur de la bitmap, mesurées en unités de 0,1 mm. La hauteur se trouve dans le membre `cy` de l’objet `CSize` et la largeur est dans le membre `cx`. Si la largeur et la hauteur de la bitmap n’ont pas été définies à l’aide de `SetBitmapDimension`, la valeur de retour est 0.
...
Vous pouvez utiliser la fonction [CGdiObject ::D eleteobject](../../mfc/reference/cgdiobject-class.md#deleteobject) pour supprimer la bitmap chargée par la fonction `LoadBitmap`, ou le destructeur `CBitmap` supprimera l’objet pour vous.
...
*nFlags*<br/>
Indicateur pour une image bitmap. Peut être zéro ou CMB_MASKED.

*lpColorMap*<br/>
Pointeur vers une structure `COLORMAP` qui contient les informations de couleur nécessaires pour mapper les bitmaps. Si ce paramètre a la valeur NULL, la fonction utilise la carte de couleur par défaut.
...
Notez que la constante OEMRESOURCE doit être définie avant les fenêtres incluses. H pour pouvoir utiliser l’une des constantes de **OBM_** .

##  <a name="operator_hbitmap"></a>CBitmap :: Operator HBITMAP

Utilisez cet opérateur pour récupérer le handle Windows GDI attaché de l’objet `CBitmap`.
...
En cas de réussite, un handle vers l’objet Windows GDI représenté par l’objet `CBitmap` ; Sinon, NULL.

### <a name="remarks"></a>Notes

Cet opérateur est un opérateur de cast qui prend en charge l’utilisation directe d’un objet `HBITMAP`.
...
Dimensions précédentes de la bitmap. La hauteur est dans la variable membre `cy` de l’objet `CSize`, et la largeur est dans la variable de membre `cx`.
