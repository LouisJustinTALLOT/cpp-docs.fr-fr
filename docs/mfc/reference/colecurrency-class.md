...
|Nom|Description|
|----------|-----------------|
|[operator =](#operator_eq)|Copie un `COleCurrency` valeur.|
|[opérateur +, -](#operator_plus_minus)|Ajoute, soustrait et modifie le signe de `COleCurrency` valeurs.|
|[operator +=, -=](#operator_plus_minus_eq)|Ajoute et soustrait un `COleCurrency` valeur à partir de ce `COleCurrency` objet.|
|[opérateur * /](#operator_star)|Échelles un `COleCurrency` valeur d’une valeur entière.|
|[operator *=, /=](#operator_star_div_eq)|Cela met à l’échelle `COleCurrency` valeur d’une valeur entière.|
|[opérateur <<](#operator_stream)|Sorties un `COleCurrency` valeur `CArchive` ou `CDumpContext`.|
|[operator >>](#operator_stream)|Entrées un `COleCurrency` à partir de l’objet `CArchive`.|
...
*nUnits*, *nFractionalUnits* indiquer les unités et les fractions de seconde partie (en 1/10 000 s) de la valeur doit être copié dans le nouveau `COleCurrency` objet.
...
- COleCurrency (`cySrc`) construit un `COleCurrency` de l’objet à partir d’un [devise](/windows/desktop/api/wtypes/ns-wtypes-tagcy) valeur.
...
Sous-jacent [devise](/windows/desktop/api/wtypes/ns-wtypes-tagcy) structure pour ce `COleCurrency` objet.
...
Pour plus d’informations, consultez le [devise](/windows/desktop/api/wtypes/ns-wtypes-tagcy) entrée dans le SDK Windows.
...
*nUnits*, *nFractionalUnits* indiquer les unités et les fractions de seconde partie (en 1/10 000 s) de la valeur doit être copié dans ce `COleCurrency` objet.
