---
description: 'En savoir plus sur : classe CRichEditDoc'
...
*preo*<br/>
Pointeur vers une structure de [réobjet](/windows/win32/api/richole/ns-richole-reobject) qui décrit un élément OLE. Le nouvel `CRichEditCntrItem` objet est construit autour de cet élément OLE. Si *Preo* a la valeur null, le nouvel élément client est vide.
...
Appelez cette fonction pour accéder à l’objet [CRichEditView](../../mfc/reference/cricheditview-class.md) associé à cet `CRichEditDoc` objet.
...
Le texte et les informations de mise en forme sont contenus dans l' `CRichEditView` objet. L' `CRichEditDoc` objet gère les éléments OLE pour la sérialisation. Il ne doit y avoir qu’un seul `CRichEditView` pour chaque `CRichEditDoc` .
