...
##  <a name="_core_print.2d.time_pagination"></a> Pagination de délai d’impression  
 ...
 Le framework s’appuie sur la classe d’affichage [OnPrepareDC](../mfc/reference/cview-class.md#onpreparedc) fonction pour indiquer qu’il doit s’arrêter. Après chaque appel à `OnPrepareDC`, l’infrastructure vérifie un membre de la `CPrintInfo` structure appelée *membre m_bContinuePrinting*. Sa valeur par défaut est **TRUE.** Dès lors qu'elle reste inchangée, le framework poursuit la boucle d'impression. Si elle est définie sur **FALSE**, le framework s’arrête. Pour exécuter la pagination de délai d’impression, substituez `OnPrepareDC` pour vérifier si la fin du document a été atteinte et définir *membre m_bContinuePrinting* à **FALSE** lorsqu’il a.  
