...
-   [Pages d’impression et les pages de document](#_core_printer_pages_vs.._document_pages)  
...
##  <a name="_core_the_printing_protocol"></a> Le protocole d’impression  
 Pour imprimer un document multipage, le framework et l'affichage interagissent de la façon suivante. Le framework affiche tout d’abord le **impression** boîte de dialogue, crée un contexte de périphérique pour l’imprimante, puis appelle la [StartDoc](../mfc/reference/cdc-class.md#startdoc) fonction membre de la [CDC](../mfc/reference/cdc-class.md) objet. Puis, pour chaque page du document, le framework appelle la [StartPage](../mfc/reference/cdc-class.md#startpage) fonction membre de la `CDC` d’objet, indique à l’objet de vue pour imprimer la page, puis appelle la [EndPage](../mfc/reference/cdc-class.md#endpage) fonction membre. Si le mode d’impression doit être modifié avant de commencer une page particulière, la vue appelle [ResetDC](../mfc/reference/cdc-class.md#resetdc), les mises à jour le [DEVMODE](/windows/desktop/api/wingdi/ns-wingdi-_devicemodea) structure contenant les nouvelles informations de mode d’imprimante. Lorsque la totalité du document a été imprimé, le framework appelle la [EndDoc](../mfc/reference/cdc-class.md#enddoc) fonction membre.  
...
|`m_nCurPage`|Page actuellement imprimée|  
 ...
 Les numéros de page commence à 1, c'est-à-dire, la première page est numérotée à 1, et non à 0. Pour plus d’informations sur ces et d’autres membres de [CPrintInfo](../mfc/reference/cprintinfo-structure.md), consultez le *référence MFC*.  
...
##  <a name="_core_print.2d.time_pagination"></a> Pagination de délai d’impression  
 ...
 Le framework s’appuie sur votre classe d’affichage [OnPrepareDC](../mfc/reference/cview-class.md#onpreparedc) fonction pour indiquer qu’il doit s’arrêter. Après chaque appel à `OnPrepareDC`, l’infrastructure vérifie un membre de la `CPrintInfo` structure appelée *membre m_bContinuePrinting*. Sa valeur par défaut est **TRUE.** Dès lors qu'elle reste inchangée, le framework poursuit la boucle d'impression. Si elle est définie sur **FALSE**, le framework s’arrête. Pour exécuter la pagination de délai d’impression, substituez `OnPrepareDC` pour vérifier si la fin du document a été atteinte et définie *membre m_bContinuePrinting* à **FALSE** lorsqu’elle est.  
  
 L’implémentation par défaut de `OnPrepareDC` définit *membre m_bContinuePrinting* à **FALSE** si la page actuelle est supérieure à 1. Cela signifie que si la longueur du document n'est pas spécifiée, le framework suppose que le document ne comporte qu'une seule page. La conséquence de ce fait est que vous devez être prudent lorsque vous appelez la version de la classe de base de `OnPrepareDC`. Ne supposez pas que *membre m_bContinuePrinting* sera **TRUE** après l’appel de la version de la classe de base.  
