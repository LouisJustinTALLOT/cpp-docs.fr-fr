...
```  
 ...
 Extrait la macro autres, V_BSTRT, ANSI ou Unicode *bstrVal* membre `COleVariant` selon le type de build (ANSI ou Unicode). Le code suivant montre comment extraire le `BSTR` valeur un `COleVariant` dans un `CString`:  
