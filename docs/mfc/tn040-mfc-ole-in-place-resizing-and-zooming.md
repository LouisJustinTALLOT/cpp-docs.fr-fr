---
description: 'En savoir plus sur : TN040 : MFC/OLE In-Place le redimensionnement et le zoom'
title: 'TN040 : MFC-OLE In-Place le redimensionnement et le zoom'
ms.date: 11/04/2016
helpviewer_keywords:
- resizing in-place
- TN040
...
- Le conteneur et le serveur doivent afficher des objets de modification utilisant les mêmes mesures. Cela implique l’utilisation d’un mode de mappage basé sur le nombre de pixels *logiques* par pouce, et non sur les pixels physiques par pouce, lors du rendu sur le périphérique d’affichage.

> [!NOTE]
> Étant donné que l'activation en place s'applique uniquement aux éléments qui sont incorporés (non liés), le zoom s'applique uniquement aux objets incorporés. Vous pouvez voir des API dans `COleServerDoc` et `COleServerItem` permettant de zoomer. La raison de cet dichotomie est que seules les fonctions qui sont valides pour les éléments liés et incorporés sont dans `COleServerItem` (cela vous permet d’avoir une implémentation commune) et les fonctions qui sont valides uniquement pour les objets incorporés se trouvent dans la `COleServerDoc` classe (du point de vue du serveur, il s’agit du **document** qui est incorporé).

La majorité de la charge est placée sur l'implémenteur du serveur, car le serveur doit avoir connaissance du facteur de zoom du conteneur et modifier son interface de modification comme il convient. Mais comment le serveur détermine-t-il le facteur de zoom utilisé par le conteneur ?
...
Pour obtenir un exemple de zoom correct, consultez l’exemple [HIERSVR](../overview/visual-cpp-samples.md)MFC OLE. Le zoom dans HIERSVR est compliqué du fait qu'il affiche le texte, et le texte, en général, n'évolue pas de façon linéaire (conseils, conventions typographiques, largeurs et hauteurs de conception compliquent la question). Toutefois, HIERSVR est une référence raisonnable pour l’implémentation correcte du zoom. c’est donc le didacticiel MFC [Scribble](../overview/visual-cpp-samples.md) (étape 7).
...
MFC implémente entièrement l'interface de redimensionnement en place comme décrit dans la spécification OLE 2. L’interface utilisateur est prise en charge par la `COleResizeBar` classe, un message personnalisé WM_SIZECHILD et une gestion spéciale de ce message dans `COleIPFrameWnd` .
...
Vous pouvez implémenter le redimensionnement personnalisé tout en tirant parti de l’interface utilisateur fournie par `COleResizeBar` en remplaçant le message d’WM_SIZECHILD dans votre `COleIPFrameWnd` classe. Pour plus d’informations sur les spécificités de WM_SIZECHILD, consultez [Technical note 24](../mfc/tn024-mfc-defined-messages-and-resources.md).
