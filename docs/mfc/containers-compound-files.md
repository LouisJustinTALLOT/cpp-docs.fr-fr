...
- `LockBytes`représentent l’interface entre les objets de stockage et le matériel physique. Ils déterminent comment les octets réels `LockBytes` sont écrits à n’importe quel périphérique de stockage de l’objet est l’accès, comme un disque dur ou une zone de mémoire globale. Pour plus `LockBytes` d’informations `ILockBytes` sur les objets et l’interface, consultez la *Référence du programmeur OLE*.
...
### <a name="incremental-access-to-files"></a><a name="_core_incremental_access_to_files"></a>Accès incrémental aux fichiers
...
### <a name="standardization"></a><a name="_core_standardization"></a>Normalisation
...
### <a name="size-and-performance-considerations"></a><a name="_core_size_and_performance_considerations"></a>Considérations de taille et de performance
...
Dans la *référence du programmeur OLE*, voir [IStream](/windows/win32/api/objidl/nn-objidl-istream), [IStorage](/windows/win32/api/objidl/nn-objidl-istorage), et [ILockBytes](/windows/win32/api/objidl/nn-objidl-ilockbytes).
