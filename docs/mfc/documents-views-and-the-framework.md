...
- Les objets thread
   ...
   Si votre application crée des threads d’exécution distincts, par exemple pour effectuer des calculs en arrière-plan, vous utiliserez les classes dérivées de [CWinThread](reference/cwinthread-class.md). [CWinApp](reference/cwinapp-class.md) est dérivée de `CWinThread` et représente le thread principal d’exécution (ou le processus principal) dans votre application. Vous pouvez également utiliser MFC dans les threads secondaires.
