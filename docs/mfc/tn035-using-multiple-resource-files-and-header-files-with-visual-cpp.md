---
title: 'TN035 : Utilisation de plusieurs fichiers de ressources et les fichiers d’en-tête avec Visual C++ | Microsoft Docs'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
- Lorsque pourrez vous souhaitez fractionner un projet dans plusieurs fichiers de ressources et/ou fichiers d’en-tête, et comment procéder
...
- Comment vous (et les outils) gérer les dépendances de génération entre. RC. CPP, et. Fichiers H
...
- **Analyse de créée par AppWizard. RC et. Fichiers de H** examine les différents fichiers de ressources et d’en-tête qui sont utilisés par une application créée par AppWizard. Ces fichiers constituent un bon modèle pour les fichiers de ressources et les fichiers d'en-tête supplémentaires que vous pourriez souhaiter ajouter à votre projet.

- **Y compris les fichiers d’en-tête supplémentaires** décrit où vous voudriez inclure plusieurs fichiers d’en-tête et fournit des détails comment faire.
...
- **À l’aide de plusieurs fichiers de ressources dans le même projet** décrit l’endroit où vous souhaitez diviser votre projet en plusieurs. RC de fichiers et fournit des détails comment faire.

- **Mise en œuvre de fichiers de Visual C++ Non modifiables** décrit comment vous pouvez vous assurer que Visual C++ ne pas modifier et reformater involontairement une ressource personnalisée.

- **Gestion des symboles partagés par plusieurs Visual C++ modifié. Fichiers RC** explique comment partager les mêmes symboles entre plusieurs. Fichiers RC et la manière d’éviter d’attribuer des valeurs numériques d’ID en double.

- **Gestion des dépendances entre. RC. CPP, et. Fichiers de H** décrit comment Visual C++ évite de recompiler inutiles. Fichiers CPP qui dépendent de fichiers de symboles des ressources.

- **Comment Visual C++ gère ensemble contient des informations** fournit des détails techniques sur la façon dont Visual C++ assure le suivi de plusieurs (imbriqué). Fichiers RC et plusieurs fichiers d’en-tête sont # inclus par un. Fichier RC.
...
MYAPP. RC le fichier de ressources d’application que vous modifiez en Visual C++.
...
Le RES\MYAPP créée par AppWizard. RC2 est un exemple d’un fichier qui contient les ressources que vous ne *pas* souhaitez lire par erreur dans Visual C++, puis les écrire avec une perte de mise en forme des informations. Pour vous protéger contre cela, insérez les lignes suivantes au début du fichier RES\MYAPP.RC2 :
...
Lorsque Visual C++ le compile le. Fichier RC, elle définit `APSTUDIO_INVOKED` ainsi que `RC_INVOKED`. Si la structure de fichiers créée par AppWizard est endommagée et que Visual C++ lit la ligne #error ci-dessus, il signale une erreur irrécupérable et abandonne la lecture du fichier .RC.
...
`_APS_NEXT_RESOURCE_VALUE` est la valeur de symbole suivante qui sera utilisée pour une ressource de boîte de dialogue, ressource de menu et ainsi de suite. La plage valide des valeurs de symboles de ressources est comprise entre 1 et 0x6FFF.
...
Maintenant chaque fois que vous créez un fichier de ressources, même dans le même projet, Visual C++ définit les mêmes `_APS_NEXT_` valeurs. Cela signifie que si vous ajoutez par exemple plusieurs boîtes de dialogue dans deux fichiers .RC différents, il est très probable que la même valeur #define soit assignée à différentes boîtes de dialogue. Par exemple, le même nombre, 101, peut être assigné à IDD_MY_DLG1 dans le premier fichier .RC et à IDD_MY_DLG2 dans un deuxième fichier .RC.

Pour éviter cela, vous devez réserver une plage numérique distincte pour chacun des quatre domaines d'ID dans les fichiers .RC respectifs. Cela en mettant à jour manuellement le `_APS_NEXT` dans chacune des valeurs de la. Fichiers RC **avant** vous commencez à ajouter des ressources. Par exemple, si le premier. Fichier RC utilise la valeur par défaut `_APS_NEXT` des valeurs, vous pouvez ensuite affecter les éléments suivants `_APS_NEXT` valeurs à la seconde. Fichier RC :
...
**Gestion des dépendances entre. RC. CPP, et. Fichiers H**
...
Visual C++ ajoute toujours la ligne de commentaire //{{NO_DEPENDENCIES}} à un fichier .RC lorsqu'il enregistre le fichier. Dans certains cas, le contournement de la dépendance de build vis-à-vis de RESOURCE.H peut provoquer des erreurs d'exécution non détectées au moment de la liaison. Par exemple, si vous utilisez le navigateur de symbole pour modifier la valeur numérique assignée à un symbole pour une ressource, celle-ci ne sera pas correctement détectée et chargée lors de l'exécution de l'application si le fichier .CPP faisant référence à la ressource n'est pas recompilé. Dans ce cas, vous devez explicitement recompilé aucun. Les fichiers CPP que vous savez sont affectées par les modifications de symbole dans la ressource. H ou sélectionnez **Rebuild All**. Si vous avez la nécessité de modifier fréquemment les valeurs de symboles pour un certain groupe de ressources, vous constaterez probablement qu’il plus pratique et plus sécurisé de consigner ces symboles dans un fichier d’en-tête en lecture seule distinct, comme décrit dans la section ci-dessus [, y compris Fichiers d’en-tête supplémentaires](#_mfcnotes_tn035_including).
