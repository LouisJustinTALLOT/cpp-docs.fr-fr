...
Pour obtenir des informations récentes sur l’optimisation de vos contrôles ActiveX, consultez [contrôles ActiveX MFC : optimisation](mfc-activex-controls-optimization.md).
...
Lorsqu’un [contrôle sans fenêtre](providing-windowless-activation.md) devient actif, il appelle la capture de la souris, le focus clavier, le défilement et d’autres services de fenêtre à partir de son conteneur. Vous pouvez également [fournir une interaction souris pour les contrôles inactifs](providing-mouse-interaction-while-inactive.md), ainsi que créer des contrôles qui [attendent jusqu’à ce qu’ils soient activés pour créer une fenêtre](turning-off-the-activate-when-visible-option.md).
