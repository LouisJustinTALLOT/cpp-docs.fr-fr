...
7. Ensuite, créez un menu élément et gestionnaires d’événements pour le stylet épais.

   1. Dans le **affichage des ressources** fenêtre, ouvrez la ressource de menu IDR_SCRIBBTYPE.

   2. Cliquez sur **stylet** pour ouvrir le menu de stylet. Puis cliquez sur **Type ici** et type `Thic&k Pen`.
   ...
   4. Cliquez sur le **stylet épais** élément de menu que vous venez de créer, puis sur **ajouter un gestionnaire d’événements**. Le **Assistant Gestionnaire d’événements** s’affiche.

   5. Dans le **liste de classes** zone de l’Assistant, sélectionnez **CScribbleDoc** puis cliquez sur **ajouter et modifier des**. Cette opération crée un gestionnaire d’événements nommé `CScribbleDoc::OnPenThickWidth`.
...
6. Définissez la couleur de stylet par défaut pour la classe de document. Dans scribdoc.cpp, ajoutez les lignes suivantes à `CScribbleDoc::InitDocument`, après la `m_nThickWidth = 5;` instruction.
...
2. Enregistrer une couleur de dessin à un fichier. Ajoutez l’instruction suivante pour scribdoc.cpp, dans le `CStroke::Serialize` (méthode), après la `ar << (WORD)m_nPenWidth;` instruction.

   ```cpp
   ar << (COLORREF)m_penColor;
    ```

3. Enfin, de charger une couleur de dessin à partir d’un fichier. Ajoutez la ligne suivante de code, dans le `CStroke::Serialize` (méthode), après la `m_nPenWidth = w;` instruction.

   ```cpp
   ar >> m_penColor;
   ```

4. Maintenant, dessin à main levée dans la couleur et enregistrez votre dessin dans un fichier.
