...
Ces étapes varient en fonction de la version de Visual Studio que vous utilisez. Vérifiez que vous avez sélectionné la bonne version dans le sélecteur de version situé en haut à gauche de la page.
...
1. Dans la boîte de dialogue **nouveau projet** , développez le nœud **visuel C++**  dans le volet **types de projets** , puis sélectionnez **MFC**. Ensuite, dans le volet **modèles** , sélectionnez **application MFC**. Tapez un nom pour le projet, par exemple `MFCShellControls` , puis cliquez sur **OK**.
...
1. Dans cette section, vous allez ajouter une instance de `CMFCShellListCtrl` à la vue créée par l’Assistant. Ouvrez le fichier d’en-tête de vue en double-cliquant sur **MFCShellControlsView. h** dans le **Explorateur de solutions**.
   ...
   Localisez `#pragma once` la directive près du haut du fichier d’en-tête. Immédiatement en dessous, ajoutez ce code pour inclure le fichier d' `CMFCShellListCtrl`en-tête pour :

   ```cpp
   #include <afxShellListCtrl.h>
   ```

   À présent, ajoutez une variable membre `CMFCShellListCtrl`de type. Tout d’abord, recherchez le commentaire suivant dans le fichier d’en-tête :
...
1. L' **Assistant Application MFC** a déjà créé `CMFCShellTreeCtrl` un objet dans `CMainFrame` la classe, mais il s’agit d’un membre protégé. Nous allons accéder ultérieurement à l’objet. par conséquent, créez un accesseur pour celui-ci. Ouvrez le fichier d’en-tête MainFrm. h en double-cliquant dessus dans la **Explorateur de solutions**. Recherchez le commentaire suivant :
...
1. À présent, nous `CMFCShellControlsView` mettons à jour la `WM_CREATE` classe pour gérer le message Windows. Ouvrez la fenêtre de **affichage de classes** et sélectionnez `CMFCShellControlsView` la classe. Cliquez avec le bouton droit et sélectionnez **Propriétés**.

   Ensuite, dans l' [Assistant classe](reference/mfc-class-wizard.md), cliquez sur l’onglet **messages** . Faites défiler jusqu’à `WM_CREATE` ce que vous trouviez le message. Dans la liste déroulante en regard `WM_CREATE`de, sélectionnez  **\<Ajouter > OnCreate**. La commande crée un gestionnaire de messages pour nous et met automatiquement à jour la table des messages MFC.
...
1. Répétez l’étape précédente, mais `WM_SIZE` pour le message. Cela entraîne le redessin de la vue des applications chaque fois qu’un utilisateur modifie la taille de la fenêtre d’application. Remplacez la définition de la `OnSize` méthode par le code suivant :
...
1. La dernière étape consiste à connecter les `CMFCShellTreeCtrl` objets `CMFCShellListCtrl` et à l’aide de la méthode [CMFCShellTreeCtrl :: SetRelatedList](../mfc/reference/cmfcshelltreectrl-class.md#setrelatedlist) . Une fois que `CMFCShellTreeCtrl::SetRelatedList`vous avez `CMFCShellListCtrl` appelé, le affiche automatiquement le contenu de l’élément sélectionné `CMFCShellTreeCtrl`dans le. Nous connectons les objets dans `OnActivateView` la méthode, qui est remplacée par [CView :: OnActivateView](../mfc/reference/cview-class.md#onactivateview).

   Dans le fichier d’en-tête MFCShellControlsView. h `CMFCShellControlsView` , à l’intérieur de la déclaration de classe, ajoutez la déclaration de méthode suivante :
   ...
   Étant donné que nous appelons des méthodes `CMainFrame` à partir de la classe, `#include` nous devons ajouter une directive en haut du fichier source MFCShellControlsView. cpp :
...
1. Vérifiez que l’application a été créée avec succès en la générant et en l’exécutant. Pour générer l’application, dans le menu **générer** , sélectionnez **générer la solution**. Si l’application est générée avec succès, exécutez-la en sélectionnant **Démarrer le débogage** dans le menu **Déboguer** .
   ...
   Vous devez maintenant voir les détails de l’élément sélectionné dans le `CMFCShellTreeCtrl` volet vue. Lorsque vous cliquez sur un nœud dans `CMFCShellTreeCtrl`le, `CMFCShellListCtrl` le est automatiquement mis à jour. De même, si vous double-cliquez sur un dossier `CMFCShellListCtrl`dans le `CMFCShellTreeCtrl` , le doit être mis à jour automatiquement.
...
- Le `CMFCShellListCtrl` prend en charge l’affichage des fichiers dans différents modes, tels que les **grandes icônes**, les **petites icônes**, les **listes**et les **Détails**. Mettez à jour votre application pour implémenter cette fonctionnalité. Conseil : consultez [les C++ exemples visuels](../overview/visual-cpp-samples.md).
