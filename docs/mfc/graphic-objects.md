...
1. Définissez un objet graphique sur le frame de pile. Initialisez l'objet avec la fonction de création propre au type, par exemple, `CreatePen`. Vous pouvez aussi initialiser l'objet dans le constructeur. Consultez la discussion sur la [création d’une étape et de deux étapes](../mfc/one-stage-and-two-stage-construction-of-objects.md), qui fournit un exemple de code.

1. [Sélectionnez l’objet dans le contexte de périphérique actuel](../mfc/selecting-a-graphic-object-into-a-device-context.md), en enregistrant l’ancien objet graphique qui a été sélectionné précédemment.
...
- [Exemple de construction d’un stylet en une et en deux étapes](../mfc/one-stage-and-two-stage-construction-of-objects.md)
