...
## <a name="binding-a-long-data-column-to-a-clongbinary"></a>Liaison d’une colonne de données de type long à CLongBinary  
 ...
 Pour mettre à jour la durée pendant laquelle les valeurs de colonne de données, assurez-vous d’abord alloué `HGLOBAL` est suffisamment grande pour contenir les nouvelles données en appelant **:: GlobalSize** sur la *m_hData* membre de la `CLongBinary`. Si elle est trop petite, libérez `HGLOBAL` et allouez une mémoire de la taille appropriée. Définissez ensuite *m_dwDataLength* afin de refléter la nouvelle taille.  
  
 Sinon, si *m_dwDataLength* est supérieure à la taille des données que vous remplacez, vous pouvez libérer et réallouer la `HGLOBAL`, ou la laisser allouée. Veillez à indiquer le nombre d’octets réellement utilisés dans *m_dwDataLength*.  
