...
ms.locfileid: "81353572"
---
# <a name="control-bars"></a>Barres de contrôles

"La barre de contrôle" est le nom général pour les barres d'outils, les barres d'état et les barres de boîte de dialogue. MFC `CToolBar`classes `CStatusBar` `CDialogBar`, `COleResizeBar`, `CReBar` , et dérivent de la classe [CControlBar](../mfc/reference/ccontrolbar-class.md), qui met en œuvre leur fonctionnalité commune.
...
- Barres d’outils, en classe [CToolBar](../mfc/reference/ctoolbar-class.md)

- Barres de statut, en classe [CStatusBar](../mfc/reference/cstatusbar-class.md)

- Barres de dialogue, en classe [CDialogBar](../mfc/reference/cdialogbar-class.md)

- Barres d’armature, en classe [CReBar](../mfc/reference/crebar-class.md)

> [!IMPORTANT]
> À partir de la version MFC 4.0, les barres d’outils, les barres d’état et les conseils d’outil sont implémenté à l’aide de fonctionnalités du système implémentées dans le *comctl32.dll* au lieu de la mise en œuvre précédente spécifique à MFC. Dans la version MFC 6.0, , `CReBar`qui enveloppe également comctl32.dll fonctionnalité, a été ajouté.
...
> [!NOTE]
> Si un bouton sur la barre de contrôle n’a pas de **gestionnaire COMMAND** ou **UPDATE_COMMAND_UI,** le cadre désactive automatiquement le bouton.
...
> [!NOTE]
> À partir de la version MFC 4.0, la classe [CToolBar](../mfc/reference/ctoolbar-class.md) utilise le contrôle commun de la barre d’outils Windows. A `CToolBar` contient un [CToolBarCtrl](../mfc/reference/ctoolbarctrl-class.md). Néanmoins, les anciennes barres d'outils sont toujours prises en charge. Voir l’article [ToolBars](../mfc/mfc-toolbar-implementation.md).

## <a name="status-bars"></a>Barres d'état

Une barre d'état est une barre de contrôle qui contient les volets de sortie de texte ou "indicateurs". Les volets de sortie sont fréquemment utilisés comme des lignes de message et comme indicateurs d'états. Les exemples de lignes de message incluent les lignes de message d'aide de commande qui expliquent brièvement le menu ou la commande de barre d'outils sélectionné(e) dans le volet situé à l'extrême gauche de la barre d'état par défaut créée par l'Assistant Application MFC. Les exemples d'indicateur d'état sont notamment ARRÊT DÉFIL, VERR. NUM. Les barres d'état sont généralement alignées en bas de la fenêtre frame. Voir la classe [CStatusBar](../mfc/reference/cstatusbar-class.md) et la classe [CStatusBarCtrl](../mfc/reference/cstatusbarctrl-class.md).

## <a name="dialog-bars"></a>Barres de boîte de dialogue

Une barre de boîte de dialogue est une barre de contrôle, basée sur une ressource de modèle de boîte de boîte de dialogue, avec la fonctionnalité d'une boîte de dialogue non modale. Les barres de boîte de dialogue peuvent contenir des contrôles Windows, des contrôles personnalisés ou des contrôles ActiveX. Comme dans une boîte de dialogue, l'utilisateur peut se déplacer dans les contrôles. Les barres de boîte de dialogue peuvent être alignées dans la partie supérieure, inférieure, gauche, ou le côté droit d'une fenêtre frame, et elles peuvent également flotter dans leur propre fenêtre frame. Voir la classe [CDialogBar](../mfc/reference/cdialogbar-class.md).

## <a name="rebars"></a>Rebars

Une [barre d’armature](../mfc/using-crebarctrl.md) est une barre de contrôle qui fournit des informations d’amarrage, de disposition, d’état et de persistance pour les commandes de barres d’armature. Un objet rebar peut contenir plusieurs fenêtres enfants, généralement d'autres contrôles, notamment des zones d'édition, des barres d'outils et des zones de liste. Un objet rebar peut afficher les fenêtres enfants sur une image bitmap spécifiée. Il peut être redimensionné automatiquement ou manuellement en cliquant ou en faisant glisser sa barre de pinces. Voir la classe [CReBar](../mfc/reference/crebar-class.md).
