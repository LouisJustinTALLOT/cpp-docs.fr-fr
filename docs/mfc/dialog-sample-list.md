---
title: Exemple de boîte de dialogue liste | Documents Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
ms.topic: conceptual
dev_langs:
- C++
helpviewer_keywords:
- sample applications [MFC], dialog boxes
ms.assetid: 3fc7dd7c-d758-4c43-96bb-0ea638ca1ad7
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
ms.openlocfilehash: 0edef5dc0c97985ba81b28c9df5233a8a2da6933
ms.sourcegitcommit: 76b7653ae443a2b8eb1186b789f8503609d6453e
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 05/04/2018
ms.locfileid: "33343606"
---
# <a name="dialog-sample-list"></a>Exemple de liste de boîte de dialogue
Consultez les exemples de programmes suivants qui illustrent des boîtes de dialogue et les feuilles de propriétés :  
  
 *Exemple d’Application MDI avec les boîtes de dialogue*  
  
-   [DESSIN À MAIN LEVÉE](../visual-cpp-samples.md)  
  
 *Boîte de dialogue non modales*  
  
-   [NON MODALES](../visual-cpp-samples.md)  
  
 *Boîte de dialogue de feuille de propriétés (onglet boîte de dialogue)*  
  
-   [PROPDLG](../visual-cpp-samples.md)  
  
-   [CMNCTRL1](../visual-cpp-samples.md)  
  
-   [CMNCTRL2](../visual-cpp-samples.md)  
  
 *Application basée sur une boîte de dialogue*  
  
-   [CMNCTRL1](../visual-cpp-samples.md)  
  
-   [CMNCTRL2](../visual-cpp-samples.md)  
  
 *Contrôles de boîte de dialogue*  
  
-   [CMNCTRL1](../visual-cpp-samples.md)  
  
-   [CMNCTRL2](../visual-cpp-samples.md)  
  
-   [CTRLTEST](../visual-cpp-samples.md)  
  
 *Vues de la forme de boîtes de dialogue*  
  
-   [VIEWEX](../visual-cpp-samples.md)  
  
 *Modèle de boîte de dialogue en mémoire*  
  
-   [DLGTEMPL](../visual-cpp-samples.md)  
  
## <a name="see-also"></a>Voir aussi  
 [Boîtes de dialogue](../mfc/dialog-boxes.md)

