...
5. Faites des essais avec d'autres valeurs qui pourraient convenir pour vos contrôles. Une boîte de dialogue avec une seule ligne d’une zone de texte peut avoir le **Type de redimensionnement** définie sur **Horizontal** uniquement, par exemple.
...
9. La prochaine fois que l’utilisateur redimensionne la boîte de dialogue, le [CMFCDynamicLayout::Adjust](../mfc/reference/cmfcdynamiclayout-class.md#adjust) méthode est appelée, et les paramètres.

10. Si vous souhaitez désactiver la disposition dynamique, appelez [CWnd::EnableDynamicLayout](../mfc/reference/cwnd-class.md#enabledynamiclayout) avec **FALSE** que pour le *case d’option bActivé* paramètre.
