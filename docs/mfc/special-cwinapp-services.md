---
description: 'En savoir plus sur : services CWinApp spéciaux'
...
## <a name="shell-registration"></a><a name="_core_shell_registration"></a> Inscription de l’interpréteur de commandes
...
Pour ce faire, vous pouvez affecter la `SuppressBackgroundThread` **valeur true** au membre de la structure [GdiplusStartupInput](/windows/win32/api/gdiplusinit/ns-gdiplusinit-gdiplusstartupinput) . Lors de la suppression du thread d’arrière-plan GDI+, les `NotificationHook` `NotificationUnhook` appels et doivent être effectués juste avant l’entrée et la sortie de la boucle de message de l’application. Pour plus d’informations sur ces appels, consultez [GdiplusStartupOutput](/windows/win32/api/gdiplusinit/ns-gdiplusinit-gdiplusstartupoutput). Par conséquent, un bon emplacement à appeler `GdiplusStartup` et les fonctions de notification de raccordement seraient dans une substitution de la fonction virtuelle [CWinApp :: Run](../mfc/reference/cwinapp-class.md#run), comme indiqué ci-dessous :
...
## <a name="file-manager-drag-and-drop"></a><a name="_core_file_manager_drag_and_drop"></a> Glisser-déplacer du gestionnaire de fichiers
