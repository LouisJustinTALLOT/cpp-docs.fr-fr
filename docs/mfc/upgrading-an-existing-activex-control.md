...
Pour plus d’informations sur ce sujet, consultez empaquetage de [contrôles ActiveX](https://docs.microsoft.com//previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa751974%28v%3dvs.85%29).

### <a name="the-codebase-tag"></a>La balise de code base

Les contrôles ActiveX sont incorporés dans des pages `<OBJECT>` Web à l’aide de la balise. Le `CODEBASE` paramètre de la `<OBJECT>` balise spécifie l’emplacement à partir duquel le contrôle doit être téléchargé. `CODEBASE`peut pointer vers un certain nombre de types de fichiers différents.
...
Cette solution ne télécharge que le fichier. ocx du contrôle et nécessite que toutes les dll de prise en charge soient déjà installées sur l’ordinateur client. Cela fonctionne pour Internet Explorer et les contrôles ActiveX MFC créés avec Visual C++, car Internet Explorer est fourni avec les dll de prise C++ en charge pour les contrôles visuels. Si un autre navigateur Internet prenant en charge les contrôles ActiveX est utilisé pour afficher ce contrôle, cette solution ne fonctionnera pas.
...
L’exemple suivant, spindial. inf, répertorie les fichiers de prise en charge et les informations de version nécessaires pour le contrôle MFC spindial. Notez que l’emplacement des DLL MFC est un site Web Microsoft. Le fichier Mfc42. cab est fourni et signé par Microsoft.
...
### <a name="the-object-tag"></a>La \<balise d' > d’objet

L’exemple suivant illustre l’utilisation de `<OBJECT>` la balise pour empaqueter l’exemple de contrôle MFC spindial.
...
Selon la version spécifiée, vous pouvez forcer le téléchargement de votre contrôle. Pour obtenir les spécifications complètes `OBJECT` de la balise, y compris le paramètre *code base* , consultez la référence du W3C.

##  <a name="_core_marking_a_control_safe_for_scripting_and_initializing"></a>Marquage d’un contrôle comme sécurisé pour l’écriture de scripts et l’initialisation
...
Ajoutez des entrées similaires à ce qui suit (en remplaçant l’ID de classe de votre `{06889605-B8D0-101A-91F1-00608CEAD5B3}`contrôle à la place de) pour associer vos clés à l’ID de classe suivant:
...
##  <a name="_core_licensing_issues"></a>Problèmes liés aux licences
...
Un contrôle ActiveX sous licence ne se chargera pas correctement dans une page HTML si l’ordinateur exécutant Internet Explorer n’a pas de licence d’utilisation du contrôle. Par exemple, si un contrôle sous licence a été créé C++à l’aide de Visual, la page HTML qui utilise le contrôle se chargera correctement sur l’ordinateur sur lequel le contrôle a été créé, mais il ne se chargera pas sur un autre ordinateur, sauf si les informations de licence sont incluses.
...
1. Insérez la \<balise d' > d’objet pour votre contrôle sous licence après la balise du gestionnaire de licences.
...
1. Insérez l' \<attribut > Embed pour votre fichier LPK, si vous utilisez le plug-in ActiveX NCompass.

   Si votre contrôle peut être affiché sur d’autres navigateurs activés (par exemple, Netscape à l’aide du plug-in ActiveX NCompass), \<vous devez ajouter la syntaxe d' > Embed comme indiqué ci-dessous.
...
Pour plus d’informations sur le contrôle des [licences, consultez contrôles ActiveX: Gestion des licences d'](../mfc/mfc-activex-controls-licensing-an-activex-control.md)un contrôle ActiveX.

##  <a name="_core_signing_code"></a>Code de signature
...
##  <a name="_core_internet_explorer_browser_safety_levels_and_control_behavior"></a>Niveau de sécurité du navigateur Internet Explorer et comportement de contrôle
...
[Contrôles ActiveX MFC : Licence d’un contrôle ActiveX](../mfc/mfc-activex-controls-licensing-an-activex-control.md)
