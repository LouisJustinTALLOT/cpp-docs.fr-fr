---
description: 'En savoir plus sur : TN053 : routines DFX personnalisées pour les classes de base de données DAO'
...
## <a name="example-1--use-of-dao-record-field-exchange-only"></a><a name="_mfcnotes_tn053_examples"></a> Exemple 1 : utilisation de l’échange de champs d’enregistrements DAO uniquement

(part du principe que la `CDaoRecordset` classe dérivée est `CMySet` déjà ouverte)
...
(suppose l’utilisation de la `CDaoRecordset` classe, `rs` , et elle est déjà ouverte)
...
(suppose l’exploration des données des employés avec la `CDaoRecordset` classe dérivée de `emp` )
...
- DFX construit la structure de liaison utilisée par la fonction de DAO `GetRows` (plus d’informations sur ce point plus tard).
...
La fonctionnalité la plus importante à comprendre sur le processus d’échange de champs d’enregistrements DAO est qu’elle utilise la `GetRows` fonction de l' `CDaoRecordset` objet. La `GetRows` fonction DAO peut fonctionner de plusieurs façons. Cette note technique décrira brièvement `GetRows` le fait qu’elle ne se trouve pas dans le cadre de cette note technique.
...
- Une autre solution `GetRows` consiste à autoriser les programmeurs à spécifier des adresses de liaison pour les données récupérées de chaque champ pour un enregistrement de données.

- DAO effectue également un « rappel » dans l’appelant pour les colonnes de longueur variable afin de permettre à l’appelant d’allouer de la mémoire. Cette deuxième fonctionnalité présente l’avantage de réduire le nombre de copies de données et d’autoriser le stockage direct des données dans les membres d’une classe ( `CDaoRecordset` classe dérivée). Ce deuxième mécanisme est la méthode utilisée par MFC pour établir une liaison avec les membres de données dans les `CDaoRecordset` classes dérivées.

## <a name="what-your-custom-dfx-routine-does"></a><a name="_mfcnotes_tn053_what_your_custom_dfx_routine_does"></a> Ce que fait votre routine DFX personnalisée

Dans cette discussion, il est évident que l’opération la plus importante implémentée dans une fonction DFX doit être la possibilité de configurer les structures de données requises pour appeler avec succès `GetRows` . Une fonction DFX doit également prendre en charge un certain nombre d’autres opérations, mais aucune d’elles n’est aussi importante ou complexe que de se préparer correctement pour l' `GetRows` appel.

L’utilisation de DFX est décrite dans la documentation en ligne. Pour l’essentiel, il existe deux exigences. Tout d’abord, les membres doivent être ajoutés à la `CDaoRecordset` classe dérivée pour chaque champ et paramètre liés. Ce qui suit `CDaoRecordset::DoFieldExchange` doit être remplacé. Notez que le type de données du membre est important. Il doit correspondre aux données du champ de la base de données ou au moins être convertibles en ce type. Par exemple, un champ numérique dans une base de données, tel qu’un entier long, peut toujours être converti en texte et lié à un `CString` membre, mais un champ de texte dans une base de données ne peut pas nécessairement être converti en une représentation numérique, telle qu’un entier long et lié à un membre long Integer. DAO et le moteur de base de données Microsoft Jet sont responsables de la conversion (plutôt que de MFC).
