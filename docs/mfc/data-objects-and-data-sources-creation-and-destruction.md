...
Une autre situation dans laquelle vous pouvez créer un objet `COleDataSource` se produit si vous modifiez le comportement par défaut d'une opération Glisser-déplacer. Pour plus d’informations, consultez le [glisser- déposer : personnalisation](../mfc/drag-and-drop-customizing.md) article.
