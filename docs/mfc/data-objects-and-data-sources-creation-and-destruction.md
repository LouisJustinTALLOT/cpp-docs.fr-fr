...
Comme expliqué dans l’article [objets de données et sources de données (OLE)](../mfc/data-objects-and-data-sources-ole.md), les objets de données et les sources de données représentent les deux côtés d’un transfert de données. Cet article explique la création et la destruction des objets et sources pour effectuer les transferts de données correctement, notamment :
...
##  <a name="_core_destroying_data_objects"></a>Destruction d’objets de données
...
Une autre situation dans laquelle vous pouvez créer un objet `COleDataSource` se produit si vous modifiez le comportement par défaut d’une opération Glisser-déposer. Pour plus d’informations, consultez l’article [glisser-déplacer OLE : personnaliser l’article glisser-déplacer](../mfc/drag-and-drop-ole.md#customize-drag-and-drop) .

##  <a name="_core_destroying_data_sources"></a>Destruction de sources de données

Les sources de données doivent être détruites par l’application actuellement chargée de ces derniers. Dans les situations où vous confiez la source de données à OLE, par exemple en appelant [COleDataSource ::D odragdrop](../mfc/reference/coledatasource-class.md#dodragdrop), vous devez appeler `pDataSrc->InternalRelease`. Par exemple :
...
Pour plus d’informations, consultez [glisser-déplacer](../mfc/drag-and-drop-ole.md), [presse-papiers](../mfc/clipboard.md)et manipulation d' [objets de données et de sources de données](../mfc/data-objects-and-data-sources-manipulation.md).
