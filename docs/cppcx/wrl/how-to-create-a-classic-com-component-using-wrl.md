...
1. Inscrivez le composant COM dans le Registre Windows. Pour ce faire, créez un fichier des entrées d’inscription, nommez-le `RegScript.reg`et ajoutez le texte suivant. Remplacez *\<dll-path >* par le chemin d’accès de votre dll, par exemple, `C:\temp\WRLClassicCOM\Debug\CalculatorComponent.dll`.
