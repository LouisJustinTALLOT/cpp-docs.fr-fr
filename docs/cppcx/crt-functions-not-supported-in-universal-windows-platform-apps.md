---
title: Fonctions CRT non prises en charge dans les applications de la plateforme Windows universelle
description: Guide de référence des fonctions CRT non pris en charge dans les applications Universal Windows Platform.
ms.date: 04/16/2020
...
De nombreuses fonctions C runtime (CRT) ne sont pas disponibles lorsque vous créez des applications Universal Windows Platform (UWP). Parfois, des solutions de contournement sont disponibles, par exemple, vous pouvez utiliser Windows Runtime ou Win32 API. Dans d’autres cas, les fonctions CRT ont été interdites parce que les fonctionnalités correspondantes ou les API de soutien ne sont pas applicables aux applications UWP. Pour rechercher une méthode alternative qui est pris en charge pour le Windows Runtime, voir [Alternatives aux API Windows dans les applications UWP](/uwp/win32-and-com/alternatives-to-windows-apis-uwp).

Le tableau suivant répertorie les fonctions CRT qui ne sont pas disponibles lorsque vous créez des applications UWP. Il indique toutes les solution de contournement qui s’appliquent.
...
|_loaddll _getdllprocaddr _unloaddll|Il s’agit de fonctions obsolètes dans les versions précédentes de CRT. En outre, un utilisateur ne peut pas charger DLLs sauf ceux dans le même paquet d’application.|Utilisez les API Win32 `LoadPackagedLibrary`, `GetProcAddress`et `FreeLibrary` pour charger et utiliser les DLL packagées.|
...
Les fonctions suivantes sont disponibles dans le CRT pour les applications UWP. Toutefois, ne les utilisez que lorsque vous ne pouvez pas utiliser les API Win32 ou Windows Runtime correspondantes, par exemple lorsque vous transférez de grandes bases de code :

|||
|-|-|
|Fonctions de chaîne simple octet : par exemple, `strcat`, `strcpy`, `strlwr`, et ainsi de suite.|Faites de vos applications UWP strictement Unicode parce que toutes les API Win32 et Windows Runtime API qui sont exposés utilisent des ensembles de caractères Unicode uniquement.  Des fonctions d’un seul-byte ont été laissées pour le portage de grandes bases de code, mais devraient autrement être évitées. Les fonctions d’omble large correspondante doivent être utilisées à la place lorsque c’est possible.|
...
Les API précédemment mentionnées et les API suivantes ne sont pas disponibles dans les applications Windows 8.x Store et les applications Windows Phone 8.x.
