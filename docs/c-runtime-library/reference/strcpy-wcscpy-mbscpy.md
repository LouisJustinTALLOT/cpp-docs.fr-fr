...
La fonction **strcpy** copie *strSource*, y compris le caractère nul de fin, à l’emplacement qui est spécifié par *strDestination*. Le comportement de **la strcpy** est indéfini si les chaînes de la source et de la destination se chevauchent.

> [!IMPORTANT]
> Parce que **la strcpy** ne vérifie pas suffisamment d’espace dans *la strDestination* avant qu’il copie *strSource*, il est une cause potentielle de dépassements de mémoire tampon. Par conséquent, nous vous recommandons d’utiliser plutôt [strcpy_s](strcpy-s-wcscpy-s-mbscpy-s.md).

**wcscpy** et **_mbscpy** sont, respectivement, des versions à caractère large et multioctets de **strcpy**. Les arguments et la valeur de retour de **wcscpy** sont des chaînes de caractère large; ceux de **_mbscpy** sont des cordes multioctets-caractères. Ces trois fonctions se comportent sinon de façon identique.
