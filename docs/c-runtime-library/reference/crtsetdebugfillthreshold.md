...
description: Utilisez la fonction _CrtSetDebugFillThreshold pour définir la quantité maximale de mémoire tampon à remplir dans les fonctions CRT sécurisées.
ms.date: 10/31/2019
api_name:
...
Les versions Debug de certaines fonctions CRT de sécurité améliorées remplissent la mémoire tampon qui leur est transmise avec un caractère spécial (0xFE). Ce caractère de remplissage permet de trouver les cas où la taille incorrecte a été passée à la fonction. Malheureusement, cela réduit aussi les performances. Pour améliorer les performances, utilisez **_CrtSetDebugFillThreshold** pour désactiver le remplissage de mémoire tampon pour les mémoires tampons supérieures au seuil *newThreshold* . Une valeur *newThreshold* égale à 0 la désactive pour toutes les mémoires tampons.
...
Cette fonction est spécifique à Microsoft. Pour plus d’informations sur la compatibilité, voir [Compatibilité](../../c-runtime-library/compatibility.md).
