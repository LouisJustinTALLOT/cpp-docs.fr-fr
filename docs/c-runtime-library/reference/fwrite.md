...
Lorsque *le flux* est ouvert en mode de traduction Unicode, par exemple, si le *flux* est ouvert en appelant **fopen** et en utilisant un paramètre de mode qui inclut **CCS = Unicode**, **CCS = UTF-16LE**ou **CCS = UTF-8**, ou si le mode est modifié en mode de traduction Unicode à l’aide de **_setmode** et d’un paramètre de mode qui inclut **_O_WTEXT**, **_O_U16TEXT**ou **_O_U8TEXT**, la*mémoire tampon* est interprétée comme un pointeur vers un tableau de **wchar_t** qui contient des Toute tentative d'écriture d'une quantité impaire d'octets dans ce mode provoque une erreur de validation de paramètre.
