...
|---------------|-------------|-----------------|
|**_CRTDBG_ALLOC_MEM_DF**|ON|SUR Activez les allocations de tas de débogage et l’utilisation des identificateurs de type de bloc de mémoire, tels que _ **client_block**. PRÉFÉRABLE Ajoutez de nouvelles allocations à la liste liée du tas, mais affectez à type de bloc la valeur **_IGNORE_BLOCK**.<br /><br /> Peuvent aussi être combinés avec n'importe quelle macro de contrôle de fréquence sur le tas.|
|**_CRTDBG_CHECK_ALWAYS_DF**|OFF|SUR Appelez [_CrtCheckMemory](crtcheckmemory.md) à chaque demande d’allocation et de désallocation. OFF : **_CrtCheckMemory** doit être appelé explicitement.<br /><br /> Les macros de contrôle de fréquence sur le tas n'ont aucun effet quand cet indicateur est défini.|
|**_CRTDBG_CHECK_CRT_DF**|OFF|SUR Incluez les types _ **crt_block** dans les opérations de détection des fuites et de différence d’état de la mémoire. PRÉFÉRABLE La mémoire utilisée en interne par la bibliothèque Runtime est ignorée par ces opérations.<br /><br /> Peuvent aussi être combinés avec n'importe quelle macro de contrôle de fréquence sur le tas.|
|**_CRTDBG_DELAY_FREE_MEM_DF**|OFF|SUR Conservez les blocs de mémoire libérés dans la liste liée du tas, assignez-leur le type **_FREE_BLOCK** et remplissez-les avec la valeur d’octet 0xDD. PRÉFÉRABLE Ne pas conserver les blocs libérés dans la liste liée du tas.<br /><br /> Peuvent aussi être combinés avec n'importe quelle macro de contrôle de fréquence sur le tas.|
|**_CRTDBG_LEAK_CHECK_DF**|OFF|SUR Effectuer une vérification des fuites automatique à la sortie du programme via un appel à _ [crtdumpmemoryleaks](crtdumpmemoryleaks.md) et générer un rapport d’erreurs si l’application n’a pas pu libérer toute la mémoire qu’elle a allouée. PRÉFÉRABLE N’effectuez pas automatiquement la vérification des fuites à la sortie du programme.<br /><br /> Peuvent aussi être combinés avec n'importe quelle macro de contrôle de fréquence sur le tas.|
...
_ **Crtsetdbgflag** inspecte ensuite les 16 bits supérieurs du paramètre *newFlag* pour une valeur. La valeur spécifiée est le nombre d’appels **malloc**, **realloc**, **Free**et **_msize** entre les appels **_CrtCheckMemory** . Quatre macros prédéfinies sont fournies à cet effet.
...
Par exemple, vous pouvez spécifier une vérification de segment de mémoire toutes les 16 opérations **malloc**, **réallocation**, **Free**et **_msize** avec le code suivant :
...
Les 16 bits supérieurs du paramètre *newFlag* sont ignorés lorsque _CRTDBG_CHECK_ALWAYS_DF est spécifié. Dans ce cas, **_CrtCheckMemory** est appelé chaque fois que vous appelez **malloc**, **realloc**, **Free**et **_msize**.

*newFlag* est le nouvel État à appliquer à _ **crtdbgflag** et est une combinaison des valeurs pour chacun des champs de bits.
...
1. Appelez _ **crtsetdbgflag** avec *NewFlag* égal à **_CRTDBG_REPORT_FLAG** pour obtenir l’État _ **crtdbgflag** actuel et stocker la valeur retournée dans une variable temporaire.
...
1. Appelez _ **crtsetdbgflag** avec *newFlag* égal à la valeur stockée dans la variable temporaire pour définir le nouvel État pour _ **crtdbgflag**.
