...
*destination*\
Pointeur vers l' **`char16_t`** **`char32_t`** équivalent ou du caractère multioctet UTF-8 à convertir. Si la valeur est null, la fonction ne stocke pas de valeur.
...
Si *source* a la valeur null, ces fonctions retournent l’équivalent d’un appel effectué à l’aide des arguments **null** pour la *destination*, `""` (une chaîne vide se terminant par null) pour la *source*, et 1 pour *max_bytes*. Les valeurs passées de *destination* et *max_bytes* sont ignorées.
