...
**_vprintf_p** et **_vwprintf_p** retourner le nombre de caractères écrits, non compris le caractère null de fin, ou une valeur négative si une erreur se produit.

## <a name="remarks"></a>Notes

Chacune de ces fonctions prend un pointeur vers une liste d’arguments, met en forme et écrit les données spécifiées vers **stdout**. Ces fonctions diffèrent **vprintf_s** et **vwprintf_s** uniquement car ils prennent en charge la possibilité de spécifier l’ordre dans lequel les arguments sont utilisés. Pour plus d’informations, consultez [Paramètres positionnels printf_p](../../c-runtime-library/printf-p-positional-parameters.md).

**_vwprintf_p** est la version à caractères larges de **_vprintf_p**; les deux fonctions se comportent de façon identique si le flux est ouvert en mode ANSI. **_vprintf_p** ne prend actuellement en charge la sortie dans un flux de données UNICODE.
