---
description: 'En savoir plus sur les éléments suivants : _snscanf, _snscanf_l, _snwscanf, _snwscanf_l'
...
*...*<br/>
Variables facultatives qui seront utilisées pour stocker les valeurs extraites de la chaîne d’entrée par les spécificateurs de format au *format*.
...
Si l' *entrée* ou le *format* est un pointeur **null** , ou si la *longueur* est inférieure ou égale à zéro, le gestionnaire de paramètres non valides est appelé, comme décrit dans [validation de paramètre](../../c-runtime-library/parameter-validation.md). Si l’exécution est autorisée à se poursuivre, ces fonctions retournent **EOF** et attribuent à **errno** la valeur **EINVAL**.
...
Cette fonction est similaire à **sscanf** , à ceci près qu’elle permet de spécifier un nombre fixe de caractères à examiner à partir de la chaîne d’entrée. Pour plus d’informations, consultez [sscanf](sscanf-sscanf-l-swscanf-swscanf-l.md).
