---
description: 'En savoir plus sur : _aligned_realloc_dbg'
...
**_aligned_realloc_dbg** est une version de débogage de la fonction [_aligned_realloc](aligned-realloc.md) . Lorsque [_DEBUG](../../c-runtime-library/debug.md) n’est pas défini, chaque appel à **_aligned_realloc_dbg** est réduit à un appel à **_aligned_realloc**. **_Aligned_realloc** et **_aligned_realloc_dbg** réallouent un bloc de mémoire dans le tas de base, mais **_aligned_realloc_dbg** prend en charge plusieurs fonctionnalités de débogage : les mémoires tampons de chaque côté de la partie utilisateur du bloc pour tester les fuites, et les informations de *nom de fichier* / *LineNumber* pour déterminer l’origine des demandes d’allocation. Le suivi de types d’allocation spécifiques avec un paramètre de type de bloc n’est pas une fonctionnalité de débogage prise en charge pour les allocations alignées. Les allocations alignées s’affichent sous la forme d’un type de bloc _NORMAL_BLOCK.
