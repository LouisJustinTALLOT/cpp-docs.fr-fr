---
description: 'En savoir plus sur : _aligned_recalloc_dbg'
...
**_aligned_recalloc_dbg** est une version de débogage de la fonction [_aligned_recalloc](aligned-recalloc.md) . Lorsque [_DEBUG](../../c-runtime-library/debug.md) n’est pas défini, chaque appel à **_aligned_recalloc_dbg** est réduit à un appel à **_aligned_recalloc**. **_Aligned_recalloc** et **_aligned_recalloc_dbg** réallouent un bloc de mémoire dans le tas de base, mais **_aligned_recalloc_dbg** prend en charge plusieurs fonctionnalités de débogage : les mémoires tampons de chaque côté de la partie utilisateur du bloc pour tester les fuites, et les informations de *nom de fichier* / *LineNumber* pour déterminer l’origine des demandes d’allocation. Le suivi de types d’allocation spécifiques avec un paramètre de type de bloc n’est pas une fonctionnalité de débogage prise en charge pour les allocations alignées. Les allocations alignées s’affichent sous la forme d’un type de bloc _NORMAL_BLOCK.
