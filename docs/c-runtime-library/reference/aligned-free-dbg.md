...
La fonction **_aligned_free_dbg** est une version de débogage de la fonction [_aligned_free](aligned-free.md) . Lorsque [_ DEBUG](../../c-runtime-library/debug.md) n’est pas défini, chaque appel à **_aligned_free_dbg** est réduit à un `_aligned_free`appel à. Et `_aligned_free` **_aligned_free_dbg** libèrent tous deux un bloc de mémoire dans le tas de base, mais **_aligned_free_dbg** prend en charge une fonctionnalité de débogage : la capacité à conserver les blocs libérés dans la liste liée du tas pour simuler des conditions de mémoire insuffisante.
