...
Cette fonction valide ses paramètres. Si *strDest*, *format*ou *timeptr* est un pointeur null, ou si la structure de données **TM** adressée par *timeptr* n’est pas valide (par exemple, si elle contient des valeurs hors limites pour l’heure ou la date) ou si la chaîne de *format* contient un code de mise en forme non valide, le gestionnaire de paramètres non valides est appelé, comme décrit dans [validation de paramètre](../../c-runtime-library/parameter-validation.md). Si l’exécution est autorisée à se poursuivre, la fonction retourne 0 et définit **errno** sur **EINVAL**.
