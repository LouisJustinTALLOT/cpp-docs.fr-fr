...
Le `is_invocable` prédicat de type a la valeur true si le type *Callable* pouvant être appelé peut être appelé à l’aide des arguments *args* dans un contexte non évalué.

Le `is_invocable_r` prédicat de type a la valeur true si le type *Callable* pouvant être appelé peut être appelé à l’aide des arguments *args* dans un contexte non évalué pour produire un type de résultat convertible en *convertible*.
...
Le `is_nothrow_invocable_r` prédicat de type a la valeur true si le type *Callable* pouvant être appelé peut être appelé à l’aide des arguments *args* dans un contexte non évalué pour produire un type de résultat convertible en *convertible*, et qu’un tel appel est connu pour ne pas lever d’exception.
