...
|[do_encoding](#do_encoding)|Fonction virtuelle qui vérifie si l’encodage du flux de `Byte` est dépendant de l’État, si le rapport entre les valeurs de `Byte` utilisées et les valeurs de `CharType` produites est constant et, le cas échéant, détermine la valeur de ce ratio.|
|[do_in](#do_in)|Fonction virtuelle appelée pour convertir une séquence de valeurs `Byte` internes en une séquence de valeurs `CharType` externes.|
|[do_length](#do_length)|Fonction virtuelle qui détermine le nombre de valeurs de `Byte` d’une séquence donnée de valeurs de `Byte` externes qui ne produisent pas plus d’un nombre donné de valeurs de `CharType` internes et retourne ce nombre de valeurs `Byte`.|
...
|[do_out](#do_out)|Fonction virtuelle appelée pour convertir une séquence de valeurs `CharType` internes en une séquence d’octets externes.|
|[do_unshift](#do_unshift)|Fonction virtuelle appelée pour fournir les valeurs `Byte` nécessaires dans une conversion dépendante de l’État pour terminer le dernier caractère d’une séquence de valeurs `Byte`.|
|[encoding](#encoding)|Teste si l’encodage du flux de `Byte` est dépendant de l’État, si le rapport entre les valeurs de `Byte` utilisées et les valeurs de `CharType` produites est constant et, le cas échéant, détermine la valeur de ce ratio.|
|[in](#in)|Convertit une représentation externe d’une séquence de valeurs `Byte` en représentation interne d’une séquence de valeurs `CharType`.|
|[length](#length)|Détermine le nombre de valeurs de `Byte` d’une séquence donnée de valeurs de `Byte` externes qui ne produisent pas plus d’un nombre donné de valeurs de `CharType` internes et retourne ce nombre de valeurs de `Byte`.|
|[max_length](#max_length)|Retourne le nombre maximal de valeurs de `Byte` externes nécessaires pour produire un `CharType`interne.|
|[out](#out)|Convertit une séquence de valeurs `CharType` internes en une séquence de valeurs `Byte` externes.|
|[unshift](#unshift)|Fournit les valeurs de `Byte` externe nécessaires dans une conversion dépendante de l’État pour terminer le dernier caractère de la séquence de valeurs `Byte`.|
...
Valeur booléenne qui est **true** si aucune conversion ne doit être effectuée ; **false** si au moins un doit être effectué.
...
Le constructeur initialise son objet de base `locale::facet` avec [locale :: facette](../standard-library/locale-class.md#facet_class)`(refs)`.
...
Fonction virtuelle qui vérifie si l’encodage du flux de `Byte` est dépendant de l’État, si le rapport entre les valeurs de `Byte` utilisées et les valeurs de `CharType` produit est constant et, le cas échéant, détermine la valeur de ce ratio.
...
Fonction virtuelle appelée pour convertir une séquence de valeurs de `Byte` externes en une séquence de valeurs `CharType` internes.
...
l' *État* doit représenter l’état de conversion initial au début d’une nouvelle séquence source. La fonction modifie sa valeur stockée selon les besoins pour refléter l’état actuel d’une conversion réussie. Sinon, sa valeur stockée n’est pas spécifiée.
...
Fonction virtuelle qui détermine le nombre de valeurs de `Byte` d’une séquence donnée de valeurs de `Byte` externes qui ne produisent pas plus d’un nombre donné de valeurs de `CharType` internes et retourne ce nombre de valeurs `Byte`.
...
*len2*\
Nombre maximal de valeurs `Byte` qui peuvent être retournées par la fonction membre.
...
La fonction membre virtuelle protégée appelle efficacement `do_in( state, first1, last1, next1, buf, buf + len2, next2)` pour l' *État* (une copie de l’État), certaines `buf`de mémoire tampon et les pointeurs `next1` et `next2`.

Elle retourne ensuite `next2`  -  `buf`. Par conséquent, il compte le nombre maximal de conversions, non supérieur à *len2*, défini par la séquence source à l’adresse [`first1`, `last1`).

La version de modèle retourne toujours la valeur la plus petite de *last1* - *First1* et *len2*.
...
Fonction virtuelle qui retourne le nombre maximal de valeurs `Byte` externes nécessaires pour produire un `CharType`interne.
...
Nombre maximal de valeurs `Byte` nécessaires pour produire un `CharType`.
...
Fonction virtuelle appelée pour convertir une séquence de valeurs `CharType` internes en une séquence de valeurs `Byte` externes.
...
Fonction virtuelle appelée pour fournir les valeurs `Byte` nécessaires dans une conversion dépendante de l’État pour terminer le dernier caractère d’une séquence de valeurs `Byte`.
...
Teste si l’encodage du flux de `Byte` est dépendant de l’État, si le rapport entre les valeurs de `Byte` utilisées et les valeurs de `CharType` produites est constant et, le cas échéant, détermine la valeur de ce ratio.
...
Convertit une représentation externe d’une séquence de valeurs `Byte` en représentation interne d’une séquence de valeurs `CharType`.
...
l' *État* doit représenter l’état de conversion initial au début d’une nouvelle séquence source. La fonction modifie sa valeur stockée selon les besoins pour refléter l’état actuel d’une conversion réussie. Après une conversion partielle, l' *État* doit être défini pour permettre à la conversion de reprendre à l’arrivée de nouveaux caractères.
...
Détermine le nombre de valeurs de `Byte` d’une séquence donnée de valeurs de `Byte` externes qui ne produisent pas plus d’un nombre donné de valeurs de `CharType` internes et retourne ce nombre de valeurs de `Byte`.
...
Retourne le nombre maximal de valeurs de `Byte` externes nécessaires pour produire un `CharType`interne.
...
Convertit une séquence de valeurs `CharType` internes en une séquence de valeurs `Byte` externes.
...
Fournit les valeurs de `Byte` nécessaires dans une conversion dépendante de l’État pour terminer le dernier caractère d’une séquence de valeurs `Byte`.
