...
Les conteneurs à base de nœuds de la Bibliothèque standard de CMD (std:list, std::set, std::multiset, std::map, and std::multimap) rangent leurs éléments dans des nœuds individuels. Tous les nœuds d’un type de conteneur particulier ont la même taille, il n’est donc pas nécessaire d’avoir recourt à la flexibilité d’un gestionnaire de mémoire à usage général. Comme la taille de chaque bloc de mémoire est connue au moment de la compilation, le gestionnaire de mémoire peut être beaucoup plus simple et plus rapide.

Lorsqu’il est utilisé avec des contenants qui ne sont pas basés sur les nœuds (tels que les conteneurs de la Bibliothèque standard de Cmd std:::vector std::deque, et std::basic_string), les modèles d’alloueur fonctionneront correctement, mais ne sont pas susceptibles de fournir une amélioration des performances par rapport à l’allocateur par défaut.

Un allocateur est un modèle de classe qui décrit un objet qui gère l’allocation de stockage et la libération d’objets et de tableaux d’objets d’un type désigné. Les objets d’alloueur sont utilisés par plusieurs modèles de classe de conteneurs dans la bibliothèque standard de C.
...
où l’argument de modèle `Type` est le type géré par l’instance d’allocateur. La Bibliothèque standard de CMD fournit un allocateur par défaut, [un alloueur](../standard-library/allocator-class.md)de modèle de classe, qui est défini dans [ \<la mémoire>](../standard-library/memory.md). L’en-tête \<allocators> fournit les allocateurs suivants :
...
Un allocateur de blocs est un cache ou un filtre. Un cache est un modèle de classe qui prend un argument de type std::size_t. Il définit un allocateur de blocs qui alloue et désalloue des blocs de mémoire de taille unique. Il doit obtenir la mémoire en utilisant l’opérateur **nouveau**, mais il n’a pas besoin de faire un appel séparé à l’opérateur **nouveau** pour chaque bloc. Il peut, par exemple, sous-allouer la mémoire d’un bloc plus grand ou mettre en cache des blocs désalloués en vue d’une réallocation ultérieure.

Avec un compilateur qui ne peut pas reliser la valeur de la std::size_t argument utilisé lorsque le modèle a été instantané n’est pas nécessairement la valeur de l’argument _Sz passé aux fonctions membres d’un cache allouer et traiter.
...
Un filtre est un allocateur de blocs qui implémente ses fonctions membres à l’aide d’un autre allocateur de bloc, qui lui est transmis comme un argument de modèle. La forme de filtre la plus courante est le filtre de synchronisation, qui applique une stratégie de synchronisation pour contrôler l’accès aux fonctions membres d’une instance d’un autre allocateur de blocs. \<allocators> fournit les filtres de synchronisation suivants :
...
Le modèle `cache_freelist` de cache prend un argument de classe max, qui détermine le nombre maximum d’éléments à stocker dans la liste libre.
...
|Macro|Description|
|-|-|
|[ALLOCATOR_DECL](../standard-library/allocators-functions.md#allocator_decl)|Donne un modèle de classe d’allocateur.|
...
|-|-|
|[opérateur!MD\<(allocateurs>)](../standard-library/allocators-operators.md#op_neq)|Vérifie l'inégalité entre les objets allocateurs d'une classe spécifiée.|
...
|[allocator_newdel](../standard-library/allocator-newdel-class.md)|Implémente un allocateur qui utilise **la suppression de l’opérateur** pour traiter un bloc de mémoire et **l’opérateur nouveau** pour allouer un bloc de mémoire.|
...
|[rts_alloc](../standard-library/rts-alloc-class.md)|Le modèle de classe rts_alloc décrit un [filtre](../standard-library/allocators-header.md) qui contient un éventail d’instances de cache et détermine l’instance à utiliser pour l’allocation et la transaction au moment de la transaction au lieu de le moment de la compilation.|
