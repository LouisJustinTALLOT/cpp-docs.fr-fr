---
description: 'En savoir plus sur : classe unary_negate'
...
Le modèle de classe stocke une copie d’un objet de fonction unaire *\_ Func*. Elle définit sa fonction membre `operator()` comme retournant `!_Func(left)` .
