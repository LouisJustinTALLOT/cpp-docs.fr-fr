...
Le modèle de classe décrit différentes caractéristiques d’expression régulière pour le type *elem*. La [classe basic_regex](../standard-library/basic-regex-class.md) du modèle de classe utilise ces informations pour manipuler les éléments de type *elem*.
...
La fonction membre copie *loc* dans l’objet `locale` stocké et retourne une copie de la valeur précédente de l’objet `locale` stocké.
