...
`left` Un objet de type **paire.**

`right` Un objet de type `pair`.
...
`left` Un objet de type `pair` sur le côté gauche de l’opérateur.

`right` Un objet de type `pair` sur le côté droit de l’opérateur.
