...
Le champ d’entrée d’entier est converti par les mêmes règles que celles utilisées par les fonctions d’analyse pour la correspondance et la conversion d’une série d' **`char`** éléments à partir d’un fichier. (Chaque **`char`** élément de ce type est supposé être mappé à un élément équivalent de type `Elem` par un simple mappage un-à-un.) La spécification de conversion d’analyse équivalente est déterminée comme suit :
...
`fac.`[numpunct :: regroupement](../standard-library/numpunct-class.md#grouping) `()` détermine la façon dont les chiffres sont regroupés à gauche de la virgule décimale
...
Se comporte comme la première, sauf qu’elle essaie de mettre en correspondance un champ d’entrée à virgule flottante complet et non vide. `fac.`[numpunct ::d ecimal_point](../standard-library/numpunct-class.md#decimal_point) `()` détermine la séquence qui sépare les chiffres entiers des chiffres de fraction. Le spécificateur de conversion d’analyse équivalent est `lf`.
...
La première fonction membre protégée virtuelle tente de faire correspondre des éléments séquentiels en commençant au premier dans la séquence [ `first`, `last`) jusqu’à ce qu’elle ait reconnu un champ d’entrée d’entier complet et non vide. En cas de réussite, elle convertit ce champ en sa valeur équivalente en tant que type **`long`** et stocke le résultat dans *Val*. Elle retourne un itérateur désignant le premier élément au-delà du champ d’entrée numérique. Dans le cas contraire, la fonction ne stocke rien dans *Val* et définit `ios_base::failbit` l' *État*. Elle retourne un itérateur désignant le premier élément au-delà de tout préfixe d’un champ d’entrée d’entier valide. Dans les deux cas, si la valeur de retour est égale à *Last*, la fonction définit `ios_base::eofbit` dans *State*.

Le champ d’entrée d’entier est converti par les mêmes règles que celles utilisées par les fonctions d’analyse pour la correspondance et la conversion d’une série d' **`char`** éléments à partir d’un fichier. Chacun **`char`** de ces éléments est supposé être mappé à un élément équivalent de type `CharType` par un simple mappage un-à-un. La spécification de conversion d’analyse équivalente est déterminée comme suit :

- Si `iosbase.` la [balise](../standard-library/ios-base-class.md#flags) `& ios_base::basefield == ios_base::` [Oct](../standard-library/ios-functions.md#oct)est spécifiée, la spécification de conversion est `lo` .
...
Si aucune instance de `fac.thousands_sep` ne se produit dans le champ d’entrée numérique, aucune contrainte de regroupement n’est imposée. Sinon, toutes les contraintes de regroupement imposées par `fac.grouping` sont appliquées et les séparateurs sont supprimés avant que la conversion d’analyse ne se produise.
...
Le constructeur initialise son objet de base avec la `locale::` [facette](../standard-library/locale-class.md#facet_class) `(refs)` .
