---
title: unordered_multimap, classe
ms.date: 11/04/2016
f1_keywords:
...
|[Vide](#empty)|Vérifie l'absence d'éléments.|
...
L’objet trie la séquence qu’il contrôle en appelant deux objets stockés, un objet de fonction de comparaison de type [unordered_multimap::key_equal](#key_equal) et un objet de fonction de hachage de type [unordered_multimap::hasher](#hasher). Vous accédez au premier objet stocké en appelant la fonction membre [unordered_multimap ::key_eq](#key_eq)`()`; et vous accédez au deuxième objet stocké en appelant la fonction membre [unordered_multimap::hash_function](#hash)`()`. Pour toutes les valeurs `X` et `Y` de type `Key`, l'appel `key_eq()(X, Y)` retourne true uniquement si les valeurs des deux arguments ont un classement équivalent. L'appel `hash_function()(keyval)` génère une distribution des valeurs de type `size_t`. Contrairement au modèle de classe [unordered_map](../standard-library/unordered-map-class.md)Classe `unordered_multimap` , un `key_eq()(X, Y)` objet de type ne garantit pas que c’est toujours faux pour deux éléments de la séquence contrôlée. Il n'est pas nécessaire que les clés soient uniques.

L'objet stocke également un facteur de charge maximale, qui spécifie le nombre moyen maximal d'éléments souhaité par compartiment. Si l’insertion d’un élément provoque [unordered_multimap : : load_factor](#load_factor) `()` dépasser le facteur de charge maximum, le conteneur augmente le nombre de seaux et reconstruit la table de hachage au besoin.
...
## <a name="unordered_multimapbegin"></a><a name="begin"></a>unordered_multimap::commencer
...
## <a name="unordered_multimapbucket"></a><a name="bucket"></a>unordered_multimap::seau
...
## <a name="unordered_multimapclear"></a><a name="clear"></a>unordered_multimap::clair
...
## <a name="unordered_multimapcount"></a><a name="count"></a>unordered_multimap::compte
...
La fonction membre renvoie le nombre d’éléments de la plage délimité par [unordered_multimap::equal_range](#equal_range)`(keyval)`.
...
## <a name="unordered_multimapdifference_type"></a><a name="difference_type"></a>unordered_multimap::difference-type
...
## <a name="unordered_multimapempty"></a><a name="empty"></a>unordered_multimap::vide
...
## <a name="unordered_multimaperase"></a><a name="erase"></a>unordered_multimap::effacer
...
## <a name="unordered_multimapfind"></a><a name="find"></a>unordered_multimap::trouver
...
La fonction membre revient [unordered_multimap::equal_range](#equal_range)`(keyval).first`.
...
|*ValTy (ValTy)*|Paramètre de modèle qui spécifie le type d’argument que le unordered_multimap peut utiliser pour construire un élément de [value_type](../standard-library/map-class.md#value_type), et parfait-avant *Val* comme argument.|
...
La fonction de membre de gamme (5) insère la séquence des valeurs d’élément dans un `[First, Last)`unordered_multimap qui correspond à chaque élément adressé par un itérateur dans la gamme ; par conséquent, *Last* ne s’insérer pas. La fonction membre de conteneur `end()` fait référence à la position qui suit le dernier élément du conteneur. Par exemple, l'instruction `m.insert(v.begin(), v.end());` insère tous les éléments de `v` dans `m`.
...
La fonction `(float)`membre revient [unordered_multimap: ::taille](#size)`() / (float)`[unordered_multimap::bucket_count](#bucket_count)`()`, le nombre moyen d’éléments par seau.
...
## <a name="unordered_multimapoperator"></a><a name="op_eq"></a>unordered_multimap::opérateur
...
|Paramètre|Description|
|-|-|
|*Oui*|unordered_multimap qui est copié dans le unordered_multimap.|

### <a name="remarks"></a>Notes

Après avoir effacé les éléments existants dans `operator=` un unordered_multimap, soit des copies ou déplace le contenu de *la droite* dans le unordered_multimap.
...
## <a name="unordered_multimapreference"></a><a name="reference"></a>unordered_multimap::référence
...
## <a name="unordered_multimapsize"></a><a name="size"></a>unordered_multimap::taille
...
|Paramètre|Description|
|-|-|
|*InputIterator (en)*|Type d'itérateur.|
...
Le premier constructeur spécifie une copie de la séquence contrôlée par *Right*. Le deuxième constructeur spécifie une séquence vide contrôlée. Le troisième constructeur spécifie une copie de la séquence en se déplaçant *à droite*. Le quatrième, le cinquième, le sixième, le septième et le huitième constructeurs utilisent une initializer_list pour les membres. Le neuvième constructeur insère la séquence de valeurs d'éléments `[First, Last)`.
...
Le nombre minimum de seaux est l’argument *Bucket_count*, si présent; sinon il s’agit d’une valeur `N0`par défaut décrite ici comme la valeur définie par la mise en œuvre .
