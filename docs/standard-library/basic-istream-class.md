...
Objet de magasins de classes `basic_istream<Char_T, Tr>` :

- Objet de base public virtuel de classe [`basic_ios`](../standard-library/basic-ios-class.md) `<Char_T, Tr>` .
...
|[tenir](#ignore)|Ignore un certain nombre d'éléments à partir de la position de lecture actuelle.|
|[furtive](#peek)|Retourne le caractère suivant à lire.|
...
La première fonction d’entrée non mise en forme extrait un élément, si possible, comme s’il avait retourné `rdbuf->sbumpc` . Sinon, elle retourne `traits_type::` [`eof`](../standard-library/char-traits-struct.md#eof) . Si la fonction n’extrait aucun élément, elle appelle [`setstate`](../standard-library/basic-ios-class.md#setstate) `(failbit)` .

La deuxième fonction extrait l’élément [int_type](../standard-library/basic-ios-class.md#int_type)`meta` de la même manière. Si `meta` la comparaison est égale à `traits_type::eof` , la fonction appelle `setstate(failbit)` . Dans le cas contraire, elle est stockée `traits_type::` [`to_char_type`](../standard-library/char-traits-struct.md#to_char_type) `(meta)` dans *ch*. La fonction retourne __* This__.
...
- Une fois que la fonction a extrait des `count - 1` éléments.
...
La deuxième fonction extrait jusqu’à `count - 1` éléments et les stocke dans le tableau à partir de *Str*. Elle stocke toujours le caractère de fin de chaîne après tous les éléments extraits qu’elle stocke. Pour les besoins du test, l’extraction s’arrête :
...
La fonction d’entrée non mise en forme extrait jusqu’à *Count* éléments et les ignore. Toutefois, si *Count* est égal `numeric_limits<int>::max` à, il est considéré comme arbitrairement grand. L’extraction s’arrête tôt à la fin du fichier ou sur un élément de `Ch` façon à ce qu’elle soit `traits_type::` [`to_int_type`](../standard-library/char-traits-struct.md#to_int_type) `(Ch)` égale au *délimiteur* (qui est également extrait). La fonction retourne __* This__.
...
## <a name="basic_istreamoperator"></a><a name="op_gt_gt"></a>\_IStream :: operator>> de base
...
extrait un champ et le convertit en valeur booléenne en appelant [`use_facet`](../standard-library/basic-filebuf-class.md#open) `< num_get<Char_T, InIt>(` [`getloc`](../standard-library/ios-base-class.md#getloc) `).` [`get`](../standard-library/ios-base-class.md#getloc) `( InIt(` [`rdbuf`](../standard-library/basic-ios-class.md#rdbuf) `), Init(0), *this, getloc, val)` . Ici, `InIt` est défini comme [`istreambuf_iterator`](../standard-library/istreambuf-iterator-class.md) `<Char_T, Tr>` . La fonction retourne __* This__.
...
Extrayez un champ et convertissez-le en une valeur numérique en appelant `use_facet<num_get<Char_T, InIt>(getloc).` [`get`](#get) `(InIt(rdbuf), Init(0), *this, getloc, val)` . Ici, `InIt` est défini comme `istreambuf_iterator<Char_T, Tr>` et *Val* a le type **`long`** , **`unsigned long`** ou, **`void`** <strong>\*</strong> si nécessaire.

Si la valeur convertie ne peut pas être représentée en tant que type de *Val*, la fonction appelle [`setstate`](../standard-library/basic-ios-class.md#setstate) `(failbit)` . Dans tous les cas, la fonction retourne __* This__.
...
Extrayez un champ et convertissez-le en une valeur numérique en appelant `use_facet<num_get<Char_T, InIt>(getloc).get(InIt(rdbuf), Init(0), *this, getloc, val)` . Ici, `InIt` est défini comme `istreambuf_iterator<Char_T, Tr>` , et *Val* a un type **`double`** ou **`long double`** si nécessaire.
...
La fonction d’entrée non mise en forme extrait un élément, si possible, comme s’il avait retourné `rdbuf->` [`sgetc`](../standard-library/basic-streambuf-class.md#sgetc) . Sinon, elle retourne `traits_type::` [`eof`](../standard-library/char-traits-struct.md#eof) .
...
La [fonction d’entrée non mise en forme](../standard-library/basic-istream-class.md) remet le *ch*, si possible, comme en appelant [`rdbuf`](../standard-library/basic-ios-class.md#rdbuf) `->` [`sputbackc`](../standard-library/basic-streambuf-class.md#sputbackc) . Si `rdbuf` est un pointeur null ou si l’appel à `sputbackc` retourne `traits_type::` [`eof`](../standard-library/char-traits-struct.md#eof) , la fonction appelle [`setstate`](../standard-library/basic-ios-class.md#setstate) `(badbit)` . Dans tous les cas, elle retourne __* This__.
...
La fonction d’entrée non mise en forme extrait jusqu’à *Count* éléments et les stocke dans le tableau en commençant à *Str*. L’extraction s’arrête tôt à la fin du fichier, auquel cas la fonction appelle [`setstate`](../standard-library/basic-ios-class.md#setstate) `(failbit)` . Dans tous les cas, elle retourne __* This__.
...
- Appelle efficacement [`ws`](../standard-library/istream-functions.md#ws) `(_Istr)` si `_Istr.` [`flags`](../standard-library/ios-base-class.md#flags) `&` [`skipws`](../standard-library/ios-functions.md#skipws) est différent de zéro.
...
Si [`fail`](../standard-library/basic-ios-class.md#fail) a la valeur false, la fonction membre retourne [`rdbuf`](../standard-library/basic-ios-class.md#rdbuf) `->` [`pubseekoff`](../standard-library/basic-streambuf-class.md#pubseekoff) `(0, cur, in)` . Sinon, `pos_type(-1)`est retourné.
