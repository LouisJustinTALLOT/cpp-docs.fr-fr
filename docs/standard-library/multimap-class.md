...
|[const_reference](#const_reference)|Type qui fournit une référence à un élément **const** stocké dans une `multimap` pour la lecture et l’exécution d’opérations **const** .|
...
La `const_reverse_iterator` définie par Multimap pointe vers les objets de [Value_type](#value_type), qui sont de type `pair<const Key, Type>`. La valeur de la clé est disponible par l’intermédiaire du premier membre de la paire, et la valeur de l’élément mappé est disponible par l’intermédiaire du deuxième membre de la paire.

Pour déréférencer un *`const_reverse_iterator` plus* d’une expression pointant vers un élément d’un multimap, utilisez l’opérateur **->** .

Pour accéder à la valeur de la clé pour l’élément, utilisez `crIter->first`, ce qui équivaut à `(*crIter).first`. Pour accéder à la valeur de la référence mappée de l’élément, utilisez `crIter->second`, qui est équivalente à `(*crIter).first`.
...
|-|-|
|*attend*|Arguments transférés pour construire un élément à insérer dans le multimap.|
...
Si la valeur de retour de `find` est affectée à un `const_iterator`, l’objet multimap ne peut pas être changé. Si la valeur de retour de `find` est assignée à un `iterator`, l’objet Multimap peut être modifié.
...
La fonction membre de plage (5) insère la séquence de valeurs d’éléments dans un Multimap qui correspond à chaque élément traité par un itérateur dans la plage `[First, Last)` ; par conséquent, la *dernière* n’est pas insérée. La fonction membre de conteneur `end()` fait référence à la position qui suit le dernier élément du conteneur. Par exemple, l'instruction `m.insert(v.begin(), v.end());` insère tous les éléments de `v` dans `m`.
...
La `iterator` définie par Multimap pointe vers les objets de [Value_type](#value_type), qui sont de type `pair<const Key, Type>`. La valeur de la clé est disponible par l’intermédiaire du premier membre de la paire, et la valeur de l’élément mappé est disponible par l’intermédiaire du deuxième membre de la paire.
...
|*Droite*|Mappage dont l’ensemble construit doit être une copie.|
...
La `reverse_iterator` définie par Multimap pointe vers les objets de [Value_type](#value_type), qui sont de type `pair<const Key, Type>`. La valeur de la clé est disponible par l’intermédiaire du premier membre de la paire, et la valeur de l’élément mappé est disponible par l’intermédiaire du deuxième membre de la paire.

Pour déréférencer un opérateur *de `reverse_iterator` pointant vers un* élément d’un multimap, utilisez l’opérateur **->** .

Pour accéder à la valeur de la clé pour l’élément, utilisez `rIter->first`, ce qui équivaut à `(*rIter).first`. Pour accéder à la valeur de la référence mappée de l’élément, utilisez `rIter->second`, qui est équivalente à `(*rIter).second`.
...
Si la valeur de retour est affectée à un `const_iterator`, l’objet multimap ne peut pas être changé. Si la valeur de retour est assignée à un `iterator`, l’objet Multimap peut être modifié.
...
Pour un Multimap *m*, si deux éléments *E1*(*K1*, *D1*) et *E2*(*K2*, *D2*) sont des objets de type `value_type`, où *K1* et *K2* sont leurs clés de type 0 et *D1* et *D2* sont leurs données de type 3, 4 équivaut à 5.
