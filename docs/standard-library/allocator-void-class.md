...
La classe spécialise explicitement l' [allocateur](allocator-class.md) de modèle de classe pour le type **void**. Ses constructeurs et son opérateur d’assignation se comportent de la même façon que pour le modèle de classe, mais il définit uniquement les types suivants :
