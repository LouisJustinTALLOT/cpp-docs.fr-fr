...
- **Analyse de virgule flottante de valeurs hexadécimales et infinies/NaN** Les algorithmes d’analyse de virgule flottante analyseront maintenant les chaînes hexadécimales à virgule flottante (telles que celles générées par les spécificateurs de format printf %a et %A) ainsi que toutes les chaînes infinies et NaN générées par les fonctions `printf`, comme décrit ci-dessus.  
...
- **smallheap** L’option de lien `smalheap` a été supprimée. Consultez [Options de lien](../c-runtime-library/link-options.md).  
...
- **strftime et wcsftime** Les fonctions `strftime` et `wcsftime` prennent désormais en charge les spécificateurs de format %C, %D, %e, %F, %g, %G, %h, %n, %r, %R, %t, %T, %u et %V. En outre, les modificateurs E et O sont analysés mais ignorés.  
...
- **Stratégies launch::any et launch::sync** Les stratégies non standard `launch::any` et `launch::sync` ont été supprimées. Pour `launch::any`, utilisez `launch:async | launch:deferred` à la place. Pour `launch::sync`, utilisez `launch::deferred`. Consultez [launch, énumération](../standard-library/future-enums.md#launch).  
