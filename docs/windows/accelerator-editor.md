...
Pour une liste des touches accélérateur prédéfinies, consultez [touches d’accès rapide prédéfinies](../windows/predefined-accelerator-keys.md).
...
1. Sélectionnez un **ID** dans la liste déroulante dans l’ID de zone ou tapez un nouvel ID dans le **ID** boîte.

1. Type de la **clé** vous souhaitez utiliser comme accélérateur, ou avec le bouton droit et choisissez **enfoncée suivante** dans le menu contextuel pour définir une combinaison de touches (la **enfoncée suivante** commande est également disponible à partir de la **modifier** menu).

1. Modifier le **modificateur** et **Type**, si nécessaire.
...
> [!TIP]
> Un raccourci pour la suppression consiste à appuyer sur la **supprimer** clé.
