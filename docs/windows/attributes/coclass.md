---
description: 'En savoir plus sur : coclass'
title: coclasse (attribut COM C++)
ms.date: 10/02/2018
f1_keywords:
- vc-attr.coclass
helpviewer_keywords:
...
Si deux fichiers d’en-tête contiennent des classes avec l’attribut **coclass** et ne spécifient pas de GUID, le compilateur utilisera le même GUID pour les deux classes et cela entraînera une erreur MIDL.  Par conséquent, vous devez utiliser l' `uuid` attribut lorsque vous utilisez **coclass**.
...
- Injecte du code ou des données pour implémenter `IUnknown` et transformer l’objet en objet com.
...
- La [classe CComObjectRootEx](../../atl/reference/ccomobjectrootex-class.md) a un modèle basé sur la classe de modèle de thread spécifiée par l’attribut de [thread](threading-cpp.md) . Si l' `threading` attribut n’est pas spécifié, le modèle de thread par défaut est Apartment.
