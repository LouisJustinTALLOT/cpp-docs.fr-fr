...
-   `__has_finalizer(` `type` `)`  
     ...
     (Non pris en charge dans [!INCLUDE[cppwrt](../build/reference/includes/cppwrt_md.md)].) Retourne **true** si le type CLR a un finaliseur. Consultez [destructeurs et finaliseurs dans Comment : définir et consommer des classes et structs (C++ / c++ / CLI)](../dotnet/how-to-define-and-consume-classes-and-structs-cpp-cli.md#BKMK_Destructors_and_finalizers) pour plus d’informations.  
...
-   `__is_base_of(` `base` `,` `derived` `)`  
     ...
     Retourne **true** si le premier type est une classe de base du second type, of si les deux types sont les mêmes.  
  
     `__is_base_of` fonctionne également sur les types de plateforme. Par exemple, elle retournera **true** si le premier type est un [classe d’interface](../windows/interface-class-cpp-component-extensions.md) et le second type implémente l’interface.  
...
-   `__is_convertible_to(` `from` `,`  `to` `)`  
     ...
     Retourne **true** si le premier type peut être converti en second type.  
 ...
 **Exemple**  
 ...
 L'exemple de code suivant montre comment utiliser un modèle de classe pour exposer un trait de type de compilateur pour une compilation `/clr`. Pour plus d’informations, consultez [Windows Runtime et modèles gérés](../windows/windows-runtime-and-managed-templates-cpp-component-extensions.md).  
