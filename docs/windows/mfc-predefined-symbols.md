...
Les projets MFC incluent toujours plusieurs fichiers d’en-tête qui prennent en charge Windows. Celles-ci sont ajoutées via `#include` des instructions dans le `StdAfx.h` fichier :
...
Les fichiers d’en-tête incluent des valeurs d’ID de symbole pour les valeurs communes MFC. Ces symboles sont disponibles uniquement lorsque vous travaillez dans un projet MFC. Le `AFX_` préfixe est suivi des préfixes de noms de symboles standard.
