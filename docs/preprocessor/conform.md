---
title: se conformer | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-tools
...
*afficher* (facultatif)  
Provoque le paramètre actuel de *nom* (true ou false) à afficher au moyen d’un message d’avertissement pendant la compilation. Par exemple, `#pragma conform(forScope, show)`.  
  
*activé, désactivé*(facultatif)  
...
*push* (facultatif)  
Exécute un push de la valeur actuelle de *nom* dans la pile interne du compilateur. Si vous spécifiez *identificateur*, vous pouvez spécifier le *sur* ou *hors* valeur pour *nom* à la pile. Par exemple, `#pragma conform(forScope, push, myname, on)`.  
...
Peut être inclus avec un *push* ou *pop* commande. Si *identificateur* est utilisé, puis un *sur* ou *hors* spécificateur peut également être utilisé.  
