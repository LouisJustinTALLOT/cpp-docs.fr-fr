...
*push* (facultatif)  
Valeur de l’alignement de compression actuel de notifications push sur la pile interne du compilateur et la valeur de l’alignement de compression actuel *n*. Si *n* n’est pas spécifié, en cours valeur d’alignement de compression est envoyée.  
...
*n* (facultatif)  
Spécifie la valeur, en octets, à utiliser pour la compression. Si l’option de compilateur [/Zp](../build/reference/zp-struct-member-alignment.md) n’est pas définie pour le module, la valeur par défaut *n* est 8. Les valeurs valides sont 1, 2, 4, 8 et 16. L’alignement d’un membre sera sur une limite qui est soit un multiple de *n* ou un multiple de la taille du membre, plus petite étant retenue.  
