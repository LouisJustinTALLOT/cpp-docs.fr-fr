---
description: 'En savoir plus sur : opérateur de chaîne (#)'
...
En outre, si un caractère contenu dans l’argument requiert généralement une séquence d’échappement lorsqu’il est utilisé dans un littéral de chaîne, par exemple, un guillemet ( `"` ) ou une barre oblique inverse ( `\` ), la barre oblique inverse nécessaire est automatiquement insérée avant le caractère.

L’opérateur de chaîne Microsoft C++ ne se comporte pas correctement lorsqu’il est utilisé avec des chaînes qui incluent des séquences d’échappement. Dans ce cas, le compilateur génère une [Erreur du compilateur C2017](../error-messages/compiler-errors-1/compiler-error-c2017.md).
