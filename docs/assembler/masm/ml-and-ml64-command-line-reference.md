...
|Option|Action|
|------------|------------|
|**/AT**|Active la prise en charge des modèles de mémoire minuscules. Active les messages d’erreur pour les constructions de code qui enfreignent les spécifications pour les fichiers de format. com. Notez que cela n’est pas équivalent à [. Directive de modèle](dot-model.md) **Tiny** .<br /><br /> Non disponible dans ml64. exe.|
...
|**/Sl** *width*|Définit la largeur de ligne de la liste de sources en caractères par ligne. La plage est comprise entre 60 et 255. La valeur par défaut est 0. Identique à la largeur de la [page](page.md) .|
