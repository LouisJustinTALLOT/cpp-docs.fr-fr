...
Le déplacement de vos fichiers de code existants dans un projet C++ permet d’utiliser les fonctionnalités de gestion de projets MSBuild natives intégrées à l’IDE. Si vous préférez utiliser votre système de génération existant, comme des makefiles nmake, CMake ou des alternatives, vous pouvez utiliser l’option Ouvrir un dossier ou CMake à la place. Pour plus d’informations, consultez [Projets Ouvrir un dossier dans Visual C++](../ide/non-msbuild-projects.md) ou [Projets CMake dans Visual Studio](../ide/cmake-tools-for-visual-cpp.md). Les deux options vous permettent d’utiliser les fonctionnalités de l’IDE comme [IntelliSense](/visualstudio/ide/using-intellisense) et [Propriétés du projet](../ide/working-with-project-properties.md).
