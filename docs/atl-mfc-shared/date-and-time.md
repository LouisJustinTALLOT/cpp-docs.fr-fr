...
- Classes de temps à usage général. Les classes [ctime](../atl-mfc-shared/reference/ctime-class.md) et [CTimeSpan](../atl-mfc-shared/reference/ctimespan-class.md) encapsulent la plupart des fonctionnalités associées à la bibliothèque de temps ANSI-standard, qui est déclarée dans le temps. Manutention.

- Prise en charge de l’horloge système. Avec la version 3,0 de MFC, la prise `CTime` en charge a `SYSTEMTIME` été `FILETIME` ajoutée à pour les types de données Win32 et.
...
Les `COleDateTime` classes `COleDateTimeSpan` et sont conçues pour être utilisées avec `COleVariant` la classe. `COleDateTime`et `COleDateTimeSpan` sont également utiles dans la programmation de bases de données MFC, mais elles peuvent être utilisées chaque fois que vous souhaitez manipuler des valeurs de date et d’heure. Bien que `COleDateTime` la classe ait une plus grande plage de valeurs et une granularité `CTime` plus fine que la classe, elle nécessite davantage `CTime`de stockage par objet que. Il y a également quelques considérations particulières à prendre en compte lors de l’utilisation du type de DATE sous-jacent. Pour plus d’informations sur l’implémentation de DATE, consultez [le type date](../atl-mfc-shared/date-type.md).
...
`COleDateTime`ignore l’heure d’été (DST). L’exemple de code suivant compare deux méthodes de calcul d’un intervalle de temps qui dépasse la date de basculement de l’heure d’été: `COleDateTime`une utilisant la bibliothèque CRT et l’autre à l’aide de.

La première méthode définit deux `CTime` objets, *time1* et *TIME2?* , respectivement les 5 avril et 6 avril, à l’aide des structures `tm` de type C standard et. `time_t` Le code affiche *time1* et *TIME2?* et l’intervalle de temps entre eux.
...
1. Affectez à l' `COleDateTime` un des objets l’heure actuelle.

1. Effectuer une tâche longue.

1. Affectez à `COleDateTime` l’autre objet l’heure actuelle.
...
La structure `FILETIME` Win32 représente l’heure sous la forme d’une valeur 64 bits. Il s’agit d’un format plus commode pour le stockage `SYSTEMTIME` interne qu’une structure, et le format utilisé par Win32 pour représenter l’heure de création du fichier. Pour plus d’informations sur la structure SYSTEMTIME, consultez [SystemTime](/windows/desktop/api/minwinbase/ns-minwinbase-systemtime). Pour plus d’informations sur la structure FILETIME, consultez [fileTime](/windows/desktop/api/minwinbase/ns-minwinbase-filetime).
